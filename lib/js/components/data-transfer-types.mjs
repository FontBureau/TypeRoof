import { deCamelize } from '../util.mjs';

// NOTE: this is a newer aproach to the older registry below, it wont be
// a good fit for all cases, but should be a great simplification for most
// cases. Especially since type-driven starts to automatically build drag-
// and-drop interfaces by model type, this ensures we can match the model
// to it's transfer-type-slug
export function getTransferTypesForModel(Model) {
    const slug = deCamelize(Model.name);
    return Object.freeze({
        PATH: `application/x.typeroof-${slug}-path`
      , CREATE: `application/x.typeroof-${slug}-create`
    });
}
...
export const DATA_TRANSFER_TYPES = Object.freeze({
  ...
  , PROSEMIRROR_NODE_SPEC_PATH: 'application/x.typeroof-prosemirror-node-spec-path'
  , PROSEMIRROR_MARK_SPEC_PATH: 'application/x.typeroof-prosemirror-mark-spec-path'
});
