...
class UIDialogOpeners extends _BaseComponent {
    ...
    async _openDialogHandler(index, event) {
        ...
        if(Constructor === UIDialogManageState) {
            ...
            const setAppStateFN = this._changeStateHandler((newState)=>{
                // We don't have a replace functionality yet, hence,
                // copy stuff...
                // a special method to replace could be an option here as
                // well.
                // Also, currentStateDraft could be injected into the
                // deserializeItem method directly, maybe.
                const currentStateDraft = this.getEntry('/');
                for(const [key, value] of newState)
                    currentStateDraft.set(key, value);
            });
