...
import { logicalLevelsToStorageParts } from './type-spec-paths.mjs';
...
// paths (e.g. "/quote/bq"), converts to the storage form that the
// model tree needs. Legacy storage-form values ("children/……") are
// rejected as custom — they never match, so stored values surface as
// 'custom' correctly.
function typeSpecHas(typeSpec, path) {
    ...
    const storagePath =
        path === ''
            ? ''
            : `/${logicalLevelsToStorageParts(instance.parts).join('/')}`;
    return !!getEntry(typeSpec, storagePath, false);
}
    ...
    update(changedMap) {
        if(changedMap.has('data') || changedMap.has('sourceMap')) {
            const edge = (changedMap.has('data')
              ...
              // has no parent-node context. The storage conversion is
              ...
              , storagePath = isAbsolutePath
                    ? (path === ''
                        ? ''
                        : `/${logicalLevelsToStorageParts(instance.parts).join('/')}`)
                    : null
              , typeSpec = isAbsolutePath
                        ? getEntry(rootTypeSpec, storagePath, null)
                        : null
