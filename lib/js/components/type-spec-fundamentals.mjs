    ...
    constructor(widgetBus, _zones, optionGetLabel=null, optionsGen=null) {
        ...
        const widgets = [
          ...
          , [
            ...
            // The edge's optional link target (sibling of stylePatch, like
            // the label input in UINodeToTypeSpecLinksValue): either a
            // generic tag applied to intent at render time, or a
            // full-featured schema mark. The activationTest settings create/
            // destroy the inputs; they are never active concurrently.
          , [
                { zone: 'local'
                , activationTest: ()=>this.getEntry('linkType').value === 'generic-tag'
                }
              , [
          ...
          , [
                { zone: 'local'
                , activationTest: ()=>this.getEntry('linkType').value === 'mark'
                }
              , [
                    [Path.fromString(sourceKeyPath).append('..', 'mark').toString(), 'value']
                ]
              , UILineOfTextOrEmptyInput
              , () => '' // getDefault
              , null // requireUpdateDefaults
          ...
          , [
                { zone: 'local' }
              , [
                    [Path.fromString(sourceKeyPath).append('..', 'type').toString(), 'value']
                ]
              , UISelectInput
              , 'Apply as' // label
              , [ // items ([value, label] pairs)
                    ['generic-tag', 'apply as tag']
                  , ['mark', 'link mark']
                ]
            ]
        ];
...
class UIInheritedStyleLinksList extends _BaseComponent {
    ...
    update(changedMap) {
        const typeSpecProperties = changedMap.has("typeSpecProperties@")
            ...
            localStylePatches = changedMap.has("localStylePatches")
                ? changedMap.get("localStylePatches")
                : this.getEntry("localStylePatches"),
            effectiveLinks =
        ...
        for (const [key, edge] of effectiveLinks) {
            ...
            if (localStylePatches.has(key)) continue;
            // NULL-STYLE is "" in link mode; unlinked edges are excluded
            const description =
                edge.get("stylePatch").value || EMPTY_STYLE_LINK_LABEL,
                linkTarget =
                    edge.get("type").value === "generic-tag"
                        ? edge.get("tag")
                        : edge.get("mark"),
                targetLabel =
                    !linkTarget.isEmpty && linkTarget.value !== ""
                        ? ` → ${linkTarget.value}`
                        : "";
            items.push(
...
export class UIStylePatchesLinksMapCollapsible extends CollapsibleContainer {
    constructor(widgetBus, _zones, togglerLabel, isOpened = false, scroll = false) {
        const classNameParticle = "ui_links_map",
            flavor = "minimal",
            // childrenOrderedMap is <typeSpecPath>/stylePatches
            typeSpecPath = Path.fromString(
                widgetBus.getExternalName("childrenOrderedMap"),
            ).parent;
        const widgets = [
            [
                { zone: "main" },
            ...
            [
                {zone: 'main'},
                [
                    [widgetBus.getExternalName('childrenOrderedMap'), 'childrenOrderedMap'], //  require('settings:internalPropertyName', 'childrenOrderedMap')
                    [widgetBus.getExternalName('sourceMap'), 'sourceMap'], // require('settings:dependencyMapping', ['./stylePatchesSource', 'sourceMap'])
                ],
