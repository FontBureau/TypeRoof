    ...
    update(changedMap) {
        if(changedMap.has('data') || changedMap.has('sourceMap')) {
            const edge = (changedMap.has('data')
              ...
              // A relative link (bare name, "./…", "../…") is anchored
              // at the parent node's resolved spec — this label has
              // no parent-node context, so it cannot resolve it here.
              // Absolute links ("/…") are resolvable against the root;
              // relative ones read distinctly. Stored links are logical
              // exclusively.
              , typeSpecLabel = typeSpec === null
                        ? (Path.fromString(path).isExplicitlyAbsolute
...
// Select options for typeSpec links use logical paths exclusively
// (bare level names); storage segments ("children") are an implementation
// detail of the tree model, not part of the key users write.
function* typeSpecOptionsGen(typeSpec, path=Path.fromParts(Path.RELATIVE)) {
