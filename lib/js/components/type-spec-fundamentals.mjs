    ...
    update(changedMap) {
        if(changedMap.has('data') || changedMap.has('sourceMap')) {
            const edge = (changedMap.has('data')
              ...
              // at the parent node's resolved spec — this label has no
              // parent-node context, so it cannot resolve it here.
              // Display the raw link distinctly rather than as NULL:
              // the link is meaningful, just unresolvable at this level.
              , isRelativeLink = path !== '' && !Path.fromString(path).isExplicitlyAbsolute
