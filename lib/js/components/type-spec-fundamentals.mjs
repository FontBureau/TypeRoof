...
class LinksMapTargetKeyCreateSelect extends GenericSelect {
    ...
    _updateOptions(availableOptions) {
        ...
        // The below is probably n longoer required in here!
        // If selected value is no longer in options.
        if(availableOptions !== null && this._select.selectedIndex === -1) {
            // select first option
            this._select.options[0].selected = true;
    ...
    update(changed) {
        ...
        if(changed.has('restrictions'))
            this._updateRestrictions(changed.get('restrictions'));
    }
    ...
    get _initialWidgets() {
        const selectState = {lastSelectedValue: null}
          ...
          , selectNodeSpec = [
              ...
              , [
                    // TODO: I'm not sure, is there more elegant way to
                    // map these names? ['targetMap', 'options'] did
                    // not work.
                    [this.widgetBus.wrapper.dependencyReverseMapping.get('targetMap'), 'options']// => the keys are the options
                    // we can only create an option if it's not in
                    // childrenOrderedMap, so if it is in there, the options
                    // should be disabled.
                  , [this.widgetBus.wrapper.dependencyReverseMapping.get('childrenOrderedMap'), 'restrictions']// => the keys are the restrictions

                ]
              , LinksMapTargetKeyCreateSelect
              , 'ui_links_map-key-create_select' // baseClass
              , 'Link a NodeSpec' // labelContent
              , null // optionGetLabel=null
              , [] // allowNull: [allowNull=false, allowNullLabel, nullModelValue]
              , this._keyCreateInputHandler.bind(this) //onChangeFn => this validates
              , null // optionGetGroup=null
              , function* optionsGen(availableOptionsMap) { //optionsGen=null
