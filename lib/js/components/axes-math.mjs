 ...
 */
class UIAxesMathLocation extends _UIBaseAxesMap {
...
/**
 *  TODO: This uses it's own implementation of _createWrapper but
 * it would be nice to use the default _UIBaseList.UIItem as a base
 * and build on that. UIStylePatch._createWrapperForType(typeKey)is
 * a nice implementation example where a switch creates different widgets
 * by type.
 * However, the widgets created in here must follow that approach and
...
class UIAxesMathLocationsSum extends _UIBaseList {
    ...
    static BASE_CLASSES = [...super.BASE_CLASSES, super.ROOT_CLASS];
    static TYPE_CLASS_PART = null;
    static UIItem = null; // SEE FIXME comment above.
    ...
    constructor(widgetBus, _zones, childrenSettings, eventHandlers, label) {
        const labelElement = widgetBus.domTool.createElement('span', {'class': 'typeroof-ui-label'}, label)
        ...
        this.element.append(dragHandleElement, labelElement, childrensMainZoneElement);
