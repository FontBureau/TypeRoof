...
export function* applyAxesMathLocations(videoproofActor, axesMath, installedFonts
    ...
    if(videoproofActor.hasOwn('activeActors')) {
        ...
        for(const k of activeActors.ownKeys()) {
            ...
            // NOTE: the GENERATED_DATA marker excludes these keyMoments from
            // serialization. When copy/paste of actors between layouts is
            // implemented (e.g. to motion-stage, where keyMoments are
            // user-editable), the marker must be stripped when the actor
            // leaves the videoproof layout, otherwise the other layout
            // would silently not serialize this data. See copyToDraft in
            // presets.mjs for the stripping pattern.
            keyMoments[GENERATED_DATA] = 'axesMath';
        }
    }
