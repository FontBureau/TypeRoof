...
export const NUMERIC = 'numericProperties/'
  , COLOR = 'colors/'
  , LANGUAGE = 'language/' // very similar to color, it also has multiple "channels"
  , GENERIC = 'generic/'
  ...
  , SPECIFIC = 'specific/'
  , LEADING = 'leading/'
    // LAYOUT is a naming convention only (deliberately not in
    // SYMBOLIC_TO_PREFIXES and not in the registered-properties
    // registry): keys for the nodeProperties@ channel — per document
    // node computed facts (geometry/constraints), parallel to the
    // typeSpecnion's style properties.
  , LAYOUT = 'layout/'
  , DIMENSION = 'dimension/'
  , THREE_D = '3dProperties/'
  , OPENTYPE_FEATURES = 'opentype-features/'
  , METAMODEL = 'metamodel/'
    // Not a registered property (deliberately not in SYMBOLIC_TO_PREFIXES):
    // style-link edges travel the properties stream as whole structs (see
    // styleLinksGen) and inherit by default, as isInheritingPropertyFn
    // returns true for unregistered properties. Use getStyleLinks to
    // extract them from a typeSpecnion properties map.
  , INTENT_STYLE_LINKS = 'intentStyleLinks/'
  , MARK_STYLE_LINKS = 'markStyleLinks/'
  , SYMBOLIC_TO_PREFIXES =  Object.freeze(new FreezableMap(Object.entries({NUMERIC, COLOR, GENERIC, SPECIFIC, LEADING, DIMENSION, THREE_D, OPENTYPE_FEATURES})))
  , PREFIXES_TO_SYMBOLIC =  Object.freeze(new FreezableMap(Array.from(SYMBOLIC_TO_PREFIXES).map(entry=>entry.reverse())))
  ;

/**
 * The effective, available style-links of a typeSpecnion properties map
 * (tombstoned keys are excluded). Used by StyleLinksMeta, the ProseMirror
 * resolvers and the UI inherited-links list.
 */
export function getStyleLinks(propertyValuesMap, prefix = INTENT_STYLE_LINKS) {
    const result = new Map();
    for(const [name, value] of propertyValuesMap)
        if(name.startsWith(prefix) && value !== null)
            result.set(name.slice(prefix.length), value);
    return result;
}
...
export class ProcessedPropertiesSystemRecord {
    constructor(props) {
        ...
        Object.assign(this, props, {
              registryKey: (props.registryKey === null
                              ? props.modelFieldName
                              : props.registryKey)
        });
        if(this.prefix === undefined || this.prefix.startsWith('undefined'))
            throw new Error(`${this} "undefined" is not a propper prefix-prefix...`);
    }
    ...
    get registryCredentials() {
        return [this.prefix, this.registryKey];
    }
    get registryFullKey() {
      return this.registryCredentials.join('');
    }
}
...
export class ProcessedPropertiesSystemMap extends FreezableMap {
    ...
    static createSimpleRecord(propertyRoot, fieldName, fullKey=null, registryKey=null) {
        return new ProcessedPropertiesSystemRecord({
                ...
                // FIXME: should maybe called propertyKey???
...
export function getPropertiesBroomWagonGen(prefix, fieldsSet) {
    return function* propertiesBroomWagonGen(_nionAPI/*outerTypespecnionAPI*/, hostInstance) {
...
/* FIXME: this is not in production, it's more an exploration of future
 * possible variants.
 *
 * In the old world, properties are flat paths in a map, there's no
 * depth, instead, each key is the full path, representing depth. so
 * a structured/complex item can be rebuild from reading the right paths.
 *
 * childrenPropertiesBroomWagonGen is "flattening" the items by `
 * path.concat(fieldName)` and then yielding that.
 *
 * In the new world, we will yield a deep object.
 *
 *
 * I guess it would be nice to yield immutable objects, but that's only
 * really cool if it helps to quickly detect change. We have a similar
 * layer within metamodel upstream from here, so the impact here may
 * be limited. producing a mutable PropertiesMap which is just a Map
 * desendant, as a marker with a new type.
 */
export class PropertiesMap extends Map{}
export function* structuredPropertiesGen(prefix, path, item) {

    // empty is the marker that allows the slot to be filled by a
    // ancestor scope (inheritance)
    // i.e. when the slot is not set it can be set otherwise
    // TODO: we should have a NULL value, to explicitly erase an
    // entry and ensure it's not set by inheritance, but maybe
    // there will be another mechanism eventually
    // if(item instanceof NULL) return NULL ... we would need a
    // marker n
    if(item instanceof _AbstractSimpleOrEmptyModel && item.isEmpty)
        return;
    ...
    if(item instanceof _BaseSimpleModel) {
        yield [path.at(-1), item.value];
        return;
    }
    if(item instanceof _BaseContainerModel) {
        const resultMap = new PropertiesMap();
        for(const [fieldName, childItem] of item) {
            const childPath = Object.freeze(path.concat(fieldName));
            resultMap.set(...structuredPropertiesGen(prefix, childPath, childItem));
        }
        yield [path.at(-1), resultMap];
    }
}
