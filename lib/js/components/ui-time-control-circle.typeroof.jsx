...
export class _UITimeCircleBase extends _BaseComponent {
    constructor(widgetBus) {
        ...
        this._rotateStart = 0.25;
        [this.element, this._track, this._hand, this._trackBack] =
            this._initTemplate();
    }
    ...
    _getTemplate() {
        ...
        return (
            ...
            >
                <circle
                    class="ui_time_control_circle-track_back"
                    pointer-event="all"
                    cx="0"
                    cy="0"
                    fill="black"
                />
                <path class="ui_time_control_circle-track" d="" fill="#333" />
                <circle
                    class="ui_time_control_circle-hand"
                    cx="0"
                    cy="0"
                    r="13"
                    fill="white"
    ...
    _initTemplate() {
        const container = this._getTemplate(),
            ...
            trackBack = container.querySelector(
    ...
    _setT(t) {
        const radius = this._trackRadius,
            ...
            dCommands = [
                `M ${mx} ${my}`,
                `A ${radius} ${radius} 0 ${t < 0.5 ? 0 : 1} 1 ${dx.toFixed(5)} ${dy.toFixed(5)}`,
                ...(t === 1
...
export class UITimeControlCircle extends _UITimeCircleBase {
    constructor(widgetBus) {
        ...
        this.element.addEventListener("pointerdown", (event) =>
            this._onPointerDownHandler(event),
        );
    }
    ...
    _onPointerDownHandler(event) {
        ...
        this._removeEventListeners();
        this._windowEventListeners.push([
            "pointermove",
            this._onPointerMoveHandler.bind(this, tOffset),
        ]);
        this._windowEventListeners.push([
            "pointerup",
            this._onPointerUpHandler.bind(this),
        ]);
        for (const eventListener of this._windowEventListeners)
...
const timeCircleKeyMomentMixin = (TimeCircleBase) =>
    class TimeCircleKeyMomentMixin extends TimeCircleBase {
        constructor(...args) {
            ...
            this._keyMoments = this._domTool.document.createElementNS(
                SVGNS,
                "g",
            );
            this._keyMoments.classList.add(
        ...
        _updateKeyMoments(fullDuration, tToKeyMoments) {
            ...
            for (const [momentT, keyMoments] of tToKeyMoments) {
                ...
                for (const [i, [key /*, keyMoment*/]] of keyMoments.entries()) {
                    const marker = this._domTool.document.createElementNS(
                            SVGNS,
                            "circle",
                        ),
                        r = 3,
...
const timeCircleActiveKeyMomentMixin = (TimeCircleBase) =>
    class TimeCircleAvtiveKeyMomentMixin extends timeCircleKeyMomentMixin(
        TimeCircleBase,
    ) {
        constructor(...args) {
            super(...args);
        ...
        _toggleKeyMomentActive(key) {
            throw new Error(
                `NOT IMPLEMENTED ${this}._toggleKeyMomentActive for key "${key}".`,
            );
        }
        ...
        _activateMarkers() {
            for (const [key, marker] of this._keyMomentMarkers.entries()) {
                ...
                if (this._activeKeys.has(key)) marker.classList.add("active");
            }
        }
...
export class UITimeControlKeyMomentSelectCircle extends timeCircleActiveKeyMomentMixin(
    UITimeControlCircle,
) {
    constructor(widgetBus) {
        super(widgetBus);
    ...
    _toggleKeyMomentActive(key) {
        ...
        this._changeState(() => {
            ...
            if (remove.length) {
                remove.reverse();
                for (const i of remove) activeKeyMoments.delete(i);
                return;
            }
            // It was not selected: add to selection
            const activeKeyMoment =
                activeKeyMoments.constructor.Model.createPrimalState(
                    activeKeyMoments.dependencies,
                ).getDraft();
    ...
    _updateActiveKeyMoments(activeKeyMoments) {
        this._activeKeys = new Set(
            activeKeyMomentsSortedKeys(activeKeyMoments),
        );
        for (const [markerKey, marker] of this._keyMomentMarkers)
            marker.classList[
                this._activeKeys.has(markerKey) ? "add" : "remove"
            ]("active");
    }
...
export class UIActorTimeControlKeyMomentSelectCircle extends timeCircleActiveKeyMomentMixin(
    UIActorTimeControlCircle,
) {
    constructor(widgetBus) {
        super(widgetBus);
        this.element.classList.add(
            "ui_actor_time_control_key_moment_select_circle",
        );
    }
    ...
    _toggleKeyMomentActive(key) {
        this._changeState(() => {
            const isActive = this.getEntry(
                Path.fromParts(
                    this.widgetBus.getExternalName("keyMoments"),
                    key,
                    "isActive",
                ),
            );
    ...
    _updateActiveKeyMoments(keyMoments) {
        ...
        for (const [key, keyMoment] of keyMoments.entries()) {
            if (keyMoment.get("isActive").value) this._activeKeys.add(key);
        }
        for (const [markerKey, marker] of this._keyMomentMarkers)
            marker.classList[
                this._activeKeys.has(markerKey) ? "add" : "remove"
