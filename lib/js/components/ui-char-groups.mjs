...
export class UISelectCharGroups extends _BaseComponent {
     //jshint ignore:start
    static TEMPLATE = `<label class="ui_select-char-groups"
    ><span><!-- insert: label --></span>
    <select class="ui_-select"></select>
</label>`;
    //jshint ignore:end
    constructor (parentAPI
        ...
        [this.element, this._select] = this._initTemplate(charGroups, label);
    }
    ...
    _initTemplate(charGroups, label, defaultVal) {
        const container = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
          , select = container.querySelector('.ui_-select')
          ;
    ...
    update(changedMap) {
        this._select.value = changedMap.get('value').value;
    }
}
