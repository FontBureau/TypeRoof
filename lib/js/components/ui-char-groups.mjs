...
export class UICharGroupContainer extends _BaseTypeDrivenContainerComponent {
    ...
    _onChange() {
        ...
        if(summaryWidget && summaryWidget._ui && summaryWidget._ui._togglerElement) {
            let summary = `${charGroup || '(empty)'}`;
            if(charGroup === 'custom') {
                const customText = this._getDefaults('customText', '')
                  , customSeparator = this._getDefaults('customSeparator', '')
                  , preview = customText.length > 24
                        ? `${customText.slice(0, 23)}…`
                        : customText
                  ;
                summary = `custom: "${preview}"${customSeparator ? ` sep="${customSeparator}"` : ''}`;
            }
            const showExtended = this._getDefaults('extended', false);
            if(showExtended)
                summary += ' +ext';
