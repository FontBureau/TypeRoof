...
export function getExtendedChars(charGroups, chars) {
    ...
    for(const c of chars) {
        if(fullExtendedMap.has(c)) continue;
    ...
    for(const c of chars) {
        if(!fullExtendedMap.has(c)) continue;
        extendedMap.set(c, fullExtendedMap.get(c));
    }
    return extendedMap;
}
...
export function getCharsFromCharGroups(charGroups, keyPath) {
    ...
    if(typeof target === 'object' && '_default' in target
        ...
        chars = [...target];
        extendedChars = getExtendedChars(charGroups, target);
    }
    else
...
export function getCharGroupSummary(options, showExtended=false, customText='', customSeparator='') {
    const option = options || '(empty)';
    if(option === 'custom') {
        const preview = _truncateSummaryText(customText, 24);
        return `custom: "${preview}"${customSeparator ? ` sep="${customSeparator}"` : ''}${showExtended ? ' +ext' : ''}`;
    }
    return `${option}${showExtended ? ' +ext' : ''}`;
}
...
export function getCharGroupSummaryFromModel(charGroupModel) {
    const optionsModel = charGroupModel.get('options')
      ...
      ;
    if(options === 'custom') {
        const customTextModel = charGroupModel.get('customText')
          ...
          ;
        return getCharGroupSummary(options, showExtended, customText, customSeparator);
    }
    return getCharGroupSummary(options, showExtended, '', '');
}
...
export class UICharGroupContainer extends _BaseTypeDrivenContainerComponent {
    ...
    _onChange() {
        ...
        const summaryWidget = this.getWidgetById('summary');
        if(summaryWidget && summaryWidget._ui && summaryWidget._ui._togglerElement) {
            const customText = this._getDefaults('customText', '')
              , customSeparator = this._getDefaults('customSeparator', '')
              , showExtended = this._getDefaults('extended', false)
              , summary = getCharGroupSummary(charGroup, showExtended, customText, customSeparator)
              ;
            summaryWidget._ui._togglerElement.textContent = summary;
        }
    }

    // TODO: seems unnecessary, the this._activationTestCache pattern, can
    // it be cleaned up?i
...
export class UICharGroupsList extends UITypeDrivenListWithAddButton {
