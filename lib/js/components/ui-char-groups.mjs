...
export class UICharGroupContainer extends _BaseTypeDrivenContainerComponent {
    ...
    _defineGenericWidget(injectable, generalSettings, propertyRoot, fieldName, FieldType) {
        ...
        if(fieldName === 'customText'||  fieldName === 'customSeparator') {
            const [,,parameters] = elementTypeConfig;
            // 'options' is read in the activationTest; the injected getEntry
            // requires it to be a declared dependency (see
            // ComponentWrapper._activationTestGetEntry). It's a sibling
            // field of customText/customSeparator (these widgets are rooted
            // at the char group itself).
            parameters.push(require('settings:dependencyMapping', ['options', 'options']));
    ...
    _activateCustom(getEntry) {
        // customText and customSeparator are only visible if the options
        // value is "custom".
        const options = getEntry('options');
        return (options.isEmpty ? null : options.value) === 'custom';
    }
    _onChange() {
