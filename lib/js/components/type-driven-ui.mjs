    ...
    constructor(widgetBus, _zones, injectable, propertyRoot, label) {
        ...
        addButton.addEventListener('click', ()=>{
            this._changeState(()=>{
                ...
                rules.push(rules.constructor.Model.createPrimalDraft(rules.dependencies));
            });
        });
...
class UICharsSelectorContainer
    ...
    constructor(widgetBus, _zones, injectable) {
        const localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_chars_selector_container'})
          , zones = new Map([..._zones, ['local', localZoneElement]])
          ;
        widgetBus.insertElement(localZoneElement);
        super(widgetBus, zones);
        ...
        if(FieldType === null) {
            ...
            const instancePath = this.widgetBus.rootPath.append('instance')
              ...
              , widgetDefinitions = this._defineGenericWidgets(
                  ...
                  , {zone: 'local', rootPath: instancePath}
                  , ppsMap
                  , this._injectable
                )
              ;
...
// UICharGroupsList: wraps UITypeDrivenList with an "add" button.
class UICharGroupsList extends UITypeDrivenList {
    constructor(widgetBus, _zones, injectable, label=null) {
        super(widgetBus, _zones, injectable, label);
        const addButton = widgetBus.domTool.createElement('button'
                , {'class': 'ui_char_groups_list-add_button'}
                , '+ add char group');
        addButton.addEventListener('click', ()=>{
            this._changeState(()=>{
                const charGroups = this.getEntry(this.widgetBus.rootPath);
                charGroups.push(charGroups.constructor.Model.createPrimalDraft(charGroups.dependencies));
            });
        });
...
export const uiElementsMap = new Map([
          ...
          , [TemplateRulesModel, [UITypeDrivenList
                , require('settings:internalPropertyName', 'collection')
                , require('zones')
                , require('injectable')
                , require('label')
          ...
          , [CharsSelectorItemsModel, [UITypeDrivenList
                , require('settings:internalPropertyName', 'collection')
                , require('zones')
                , require('injectable')
                , require('label')
