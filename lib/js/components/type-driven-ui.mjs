...
function _getCharGroupSummary(charGroupModel) {
    const optionsModel = charGroupModel.get('options')
      ...
      , extended = !extendedModel.isEmpty && !!extendedModel.value
      ;
    if(options === 'custom') {
        const customTextModel = charGroupModel.get('customText')
          ...
          , customSummary = _truncateSummary(customText, 24)
          ;
        return `custom: "${customSummary}"${customSeparator ? ` sep="${customSeparator}"` : ''}${extended ? ' +ext' : ''}`;
    }
    return `${options}${extended ? ' +ext' : ''}`;
}
...
function _getSelectorSummary(selectorModel) {
    ...
    if(typeKey === 'Simple') {
        ...
        for(const [, charGroup] of charGroups)
            groups.push(_getCharGroupSummary(charGroup));
