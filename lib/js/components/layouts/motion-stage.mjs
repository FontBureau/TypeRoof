...
class KeyMomentController extends _BaseTypeDrivenContainerComponent {
    ...
    _defineWidgets(TypeClass, typeKey, localZoneElement
        ...
        const typographySpecificWidgets = []
            // CSS `perspective` applies to the children of the element it
            // is set on, hence it is only offered for the containers: the
            // stage and the layer actor. The stage in turn is not rotatable
            // itself, it only establishes the perspective for its children.
          , isStage = typeKey === '[STAGE:null]'
          , isLayer = typeKey === 'LayerActorModel'
          , hasPerspective = isStage || isLayer
