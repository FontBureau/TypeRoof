...
} from '../dev-layouts/example-key-moments.mjs';

import {
    UIManualAxesLocationsCollapsible
  , AxesLocationsModel
} from '../ui-manual-axis-locations.mjs';

import {
    UIOTFeaturesChooserCollapsible as UIOTFeaturesChooser
} from '../ui-opentype-features.typeroof.jsx';

import {
    UILanguageTagCollapsible as UILanguageTag
...
const StageSizeNumberModel = _AbstractNumberModel.createClass(
      ...
      , {
            ...
            defaultValue: 720 // includes start and end
          , min: 0
  ...
  , StageZoomTypeModel = _AbstractEnumModel.createClass('StageZoomTypeModel'
        , ['app-default', 'custom', '25%', '50%', '75%' , '100%', '150%', '200%', 'automatic']
        , 'app-default'
    )
    // Default is 100%
    // We can zoom bigger than 100%
  , StageZoomScalingModel = _AbstractNumberModel.createClass('StageZoomScalingModel', {min:0, max:Number.MAX_SAFE_INTEGER, defaultValue: 100, toFixedDigits: 5})
  , StageZoomScalingOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(StageZoomScalingModel)
  , StageZoomLevelModel = _AbstractStructModel.createClass(
        'StageZoomLevelModel',
        ['type', StageZoomTypeModel]
      , ['scaling', StageZoomScalingOrEmptyModel]
      , CoherenceFunction.create(
            ['type', 'scaling']
          , function checkValues({type, scaling}) {
                // If type is not automatic we can always read a value from scaling
                if(type.value.endsWith('%')) {
                    scaling.value = parseFloat(type.value.slice(0, -1));
                }
                else if(type.value === 'custom') {
                    if(scaling.isEmpty)
                        scaling.value = scaling.constructor.Model.defaultValue;
                }
                else if(type.value === 'app-default')
                    // the app can choose to ignore this value
                    // but one reasonable default is 'automatic' and the
                    // other is '100%', if the app does not come with
                    // it's own behavior, it can still use this scaling.value
                    scaling.value = 100;
                else if(type.value === 'automatic')
                    scaling.clear();
                else
                    throw new Error(`NOT IMPLEMENTED StageZoomTypeModel type: ${type.value}.`);
            }
        )
    )
  , MotionStageModel = _BaseLayoutModel.createClass(
      ...
      , ['width', StageSizeNumberModel]
      , ['height', StageSizeNumberModel]
      , ['zoomLevel', StageZoomLevelModel]
...
/**
 * yield [propertyName, propertyValue]
 * like numericPropertiesGen, but for the 3d-transform properties.
 */
function* threeDPropertiesGen(outerAnimanionAPI, keyMoment/*, momentT*/) {
    const threeDProperties = keyMoment.get('3dProperties');
    for(const [key, property] of threeDProperties)
        yield [`${THREE_D}${key}`, property.value];
}
...
export function* openTypeFeaturesGen(outerAnimanionAPI, keyMoment/*, momentT*/) {
  const openTypeFeatures = keyMoment.get("openTypeFeatures");
  for (const [featureTag, featureValue] of openTypeFeatures) {
    yield [`${OPENTYPE_FEATURES}${featureTag}`, featureValue.value];
  }
}

// While in type-spec-ramp we can use SyntheticValue to calculate
// ${LANGUAGE}lang we don't have that facility here, so far, hence,
// we will use createLanguageTag in the update functions via
// a version of setLanguageTag.
// This makes the languageTagGen much simpler, but removes the nice
// property of setting the lang attribute only at the level where it
// was changed, which is not too bad in a context where document structure
// and semantics are not crucial so far.
function* languageTagGen(
    outerAnimanionAPI,
    keyMoment /* here a TypeSpecModel */,
) {
    const languageTag = keyMoment.get('languageTag');
    for (const [subTag, subTagValue] of languageTag) {
        if(subTagValue.isEmpty) continue;
...
/**
 * A subset of REGISTERED_GENERIC_KEYMOMENT_FIELDS: these belong next to
 * "Font Size" in the UI, hence they are pulled out of the generic widgets
 * block at the bottom and placed within the typography specific widgets.
 *
 * NOTE: they must stay in REGISTERED_GENERIC_KEYMOMENT_FIELDS, as that set
 * is not only used to define widgets, it is also what makes
 * genericPropertiesBroomWagonGen yield them into the animanion, without
 * which they never reach the actor renderers.
 */
const TYPOGRAPHY_SPACING_KEYMOMENT_FIELDS = Object.freeze(new FreezableSet([
    'letterSpacing', 'wordSpacing'
]));

/**
 * The actor types whose renderers apply TYPOGRAPHY_SPACING_KEYMOMENT_FIELDS:
 * LineOfTextActorRenderer sets them as CSS `letter-spacing`/`word-spacing`
 * and the containers -- stage and layer -- propagate them to their children,
 * as these are inheriting properties (REGISTERED_PROPERTIES[GENERIC]).
 * The videoproof-array/-contextual actors are typographic as well, but
 * their renderers don't implement these properties (just like they don't
 * implement e.g. `generic/textAlign`), hence they don't get the widgets.
 *
 * FIXME: hard coded, just like isTypographicActorTypeKey. Ideally an actor
 * type would announce which of the registered properties its renderer
 * implements, so the UI could be derived from that.
 */
const TYPOGRAPHY_SPACING_ACTOR_TYPE_KEYS = Object.freeze(new FreezableSet([
    '[STAGE:null]', 'LayerActorModel', 'LineOfTextActorModel'
]));

/**
 * The actor types whose renderers apply THREE_D_PROPERTIES_DATA, i.e. the
 * `rotationX/Y/Z` => CSS `transform` setter. Same reasoning as for
 * TYPOGRAPHY_SPACING_ACTOR_TYPE_KEYS: without the renderer support the
 * widgets would be there, but without any effect.
 * The stage is deliberately not in here, it is not rotatable itself, it
 * only establishes the perspective for its children.
 */
const THREE_D_ROTATION_ACTOR_TYPE_KEYS = Object.freeze(new FreezableSet([
    'LayerActorModel', 'LineOfTextActorModel'
  , 'CircleActorModel', 'EllipseActorModel', 'RectangleActorModel'
]));

/**
 * The actor types whose renderers apply THREE_D_PERSPECTIVE_PROPERTY_DATA,
 * see THREE_D_ROTATION_ACTOR_TYPE_KEYS.
 * CSS `perspective` applies to the children of the element it is set on,
 * not to the element itself, hence it is only offered for the containers:
 * StageDOMNode and LayerDOMNode. The stage in turn is not in the rotations
 * set, it is not rotatable itself, it only establishes the perspective for
 * its children.
 */
const THREE_D_PERSPECTIVE_ACTOR_TYPE_KEYS = Object.freeze(new FreezableSet([
    '[STAGE:null]', 'LayerActorModel'
]));

const REGISTERED_METAMODEL_FIELDS = Object.freeze(new FreezableMap([
    ['template', function templateIsEmptyTest(templateModel/* , fieldName*/) {
        return templateModel.get('defaultPattern').value === '' && templateModel.get('rules').size === 0;
    }]
]));

function* metamodelGen(outerAnimanionAPI, keyMoment/*, momentT*/) {
    for(const fieldName of keyMoment.keys()) {
        if(!REGISTERED_METAMODEL_FIELDS.has(fieldName))
            continue;
        const isEmptyTest=REGISTERED_METAMODEL_FIELDS.get(fieldName) || false;
        const field =  keyMoment.get(fieldName);
        if(isEmptyTest && isEmptyTest(field, fieldName))
            continue;
        yield [`${METAMODEL}${fieldName}`, field];
    }
}
...
export function initAnimanion(keyMoments, isLoop) {
    const propertyGenerators = [numericPropertiesGen, threeDPropertiesGen, colorsGen];
    if(_keyMomentsImplementsTypography(keyMoments))
        // add typography
        propertyGenerators.push(fontSizeGen, axisLocationsGen, languageTagGen, openTypeFeaturesGen);
    propertyGenerators.push(metamodelGen, genericPropertiesBroomWagonGen);
...
class StageHTML extends _BaseContainerComponent {
    constructor(widgetBus, _zones, baseCLass='Stage_HTML', automaticZoomDefault=false) {
        ...
        const widgets = [
            [
              ...
              , [
                  ...
                  , ['t', 'globalT']
                  , 'animationProperties@'
                    // host box for the automatic-zoom fit (replacing a
                    // private ResizeObserver on wrapper.host)
                  , 'environment@layout'
                ]
              , StageDOMNode
              , containerElement, topLayerElement
              , [baseCLass, `${baseCLass}-wrapper`]
              , automaticZoomDefault
            ]
...
export class _BaseSelectAndDrag extends _BaseComponent {
    ...
    <select></select><span class="select_and_drag-drag_handle drag_handle">
    ...
    _renderLayer(path, activeActors, state=null) {
        ...
        for(const [key, actor] of activeActors) {
            const itemElement = this._domTool.createElement('li')
              ...
              , dragHandleElement = this._domTool.createElement('span', {'class': 'drag_handle'}, createIcon('drag_pan'))
...
class KeyMomentsControls extends _BaseComponent {
    ...
    static TEMPLATE = `<div class="key_moments_controls">
        ...
        <menu>
            <button class="key_moments_controls-add_moment" title="Add Moment">
                <span class="material-symbols-outlined">add</span> add
            </button>
            <button class="key_moments_controls-remove_moment" title="Remove Active Moment">
                <span class="material-symbols-outlined">remove</span> remove
            </button>
            <button class="key_moments_controls-insert_moment" title="Insert Moment at t">
                <span class="material-symbols-outlined">place_item</span> insert
            </button>
        </menu>
        <menu>
            <button class="key_moments_controls-select_previous" title="Select Previous">
                <span class="material-symbols-outlined">keyboard_double_arrow_left</span> select previous
            </button>
            <button class="key_moments_controls-select_next" title="Select Next">
                next <span class="material-symbols-outlined">keyboard_double_arrow_right</span>
            </button>
        </menu>
</div>`;
    ...
    initTemplate() {
        ...
        insertButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            ...
            for(const [path_, value] of newMomentProperties) {
                ...
                else if(path_.startsWith(THREE_D)) {
                    if(!Object.hasOwn(models ,'3dProperties'))
                        models['3dProperties'] = getEntry(newMoment, '3dProperties');
                    const propertyName = Path.fromString(path_).parts.at(-1);
                    models['3dProperties'].setSimpleValue(propertyName, value);
                }
...
export class UINumericProperties extends _BaseComponent {
    // The key moment model field this UI reads from and writes to, an
    // _AbstractOrderedMapModel of NumberModel. Subclasses drive another
    // field, see e.g. UI3DProperties.
    static ENTRY_NAME = 'numericProperties';
    static CONTAINER_CLASS = 'numeric_properties';
    static DEFAULT_LABEL = 'Numeric Properties';
    constructor (widgetBus, ppsMap, getDefaults=null, requireUpdateDefaults=()=>false, label=new.target.DEFAULT_LABEL) {
        super(widgetBus);
        this._ppsMap = ppsMap;
        ...
        this.element = this._domTool.createElement('div',{class: new.target.CONTAINER_CLASS});
        if(label)
            this.element.append(this._domTool.createElement('h3', {}, label));
    ...
    __propertiesChangeHandler(key, value) {
        const propertiesEntry = this.getEntry(this.constructor.ENTRY_NAME);
        this._setOrReset(propertiesEntry, key, value);
    }
        ...
        for(const key of this.propertiesKeys()) {
            ...
            const input = new PlainNumberAndRangeOrEmptyInput(
              ...
              , value=>this._propertiesChangeHandler(key, value)
                // toggleChangeHandler
              , ()=>{
                    const value = this._propertiesEntry.has(key)
    ...
    *propertiesEntries() {
    ...
    _getValueForProperty(key) {
        return this._propertiesEntry.has(key)
                    ? [true, this._propertiesEntry.get(key).value]
    ...
    update (changedMap) {
        ...
        const entryName = this.constructor.ENTRY_NAME;
        if(changedMap.has(entryName) || requireUpdateDefaults) {
            this._propertiesEntry = changedMap.has(entryName)
                                        ? changedMap.get(entryName)
                                        : this.getEntry(entryName)
                                        ;
            for(const key of this.propertiesKeys()) {
...
export class UI3DProperties extends UINumericProperties {
    static ENTRY_NAME = '3dProperties';
    static CONTAINER_CLASS = 'three_d_properties';
    static DEFAULT_LABEL = '3D Properties';
}
...
class KeyMomentController extends _BaseTypeDrivenContainerComponent {
    constructor(widgetBus, _zones, typeKey) {
        ...
        // getEntry is injected by ComponentWrapper and only serves
        // declared dependencies; 'isActive' is declared in _defineWidgets.
        const activationTest = (getEntry)=>{
            if(this._activationTestCache === null) {
                // see trace to figure when to reset the cache!
                this._activationTestCache = getEntry('isActive').value;
            }
    ...
    _defineWidgets(TypeClass, typeKey, localZoneElement
        ...
        const typographySpecificWidgets = []
          , hasPerspective = THREE_D_PERSPECTIVE_ACTOR_TYPE_KEYS.has(typeKey)
          , hasRotation = THREE_D_ROTATION_ACTOR_TYPE_KEYS.has(typeKey)
          , threeDPropertiesKeys = Array.from(REGISTERED_PROPERTIES[THREE_D].keys())
                .filter(key=>key === 'perspective' ? hasPerspective : hasRotation)
          , hasTypographySpacing = TYPOGRAPHY_SPACING_ACTOR_TYPE_KEYS.has(typeKey)
          ;
        if(typeKey === '[STAGE:null]' || isTypographicActorTypeKey(typeKey)) {
            ...
            // "Letter Spacing" and "Word Spacing", directly below "Font Size".
            // These are generic fields, but they are defined here, and
            // excluded from the generic widgets block below, to control
            // their position in the UI.
          , ...(hasTypographySpacing
            ? [[
                {zone: 'local', activationTest}
              , []
              , CollapsibleContainer
              , this._zones
              , 'Spacing' // togglerLabel
              , 'minimal' // flavor
              , 'spacing_properties' // classNameParticle
              , [//widgets
                    ...this._defineGenericWidgets(
                            TypeClass
                          , fieldName=>TYPOGRAPHY_SPACING_KEYMOMENT_FIELDS.has(fieldName)
                          , {zone: 'main'}
                          , GENERIC
                          , { // injectable
                                updateDefaultsDependencies
          ...
          , [
                {
                    zone: 'local', activationTest
                  , relativeRootPath: Path.fromParts('.', 'languageTag')
                }
              , [
                    [this._animationPropertiesKey, 'properties@']
                ]
              , UILanguageTag
              , this._zones
            ]
          , [
              ...
              , (ppsRecord, modelFieldName/*, defaultVal=_NOTDEF*/)=>{
                    if (ppsRecord.fullKey === `${SPECIFIC}font`) {
                      const path = typeKey === '[STAGE:null]'
                        ? Path.fromString('/font')
                        : this.widgetBus.rootPath.append('../../font');
                      return this.getEntry(path).value;
                    }
                    throw new Error(`getDefault for UIOTFeaturesChooser don't know how to get ppsRecord: "${ppsRecord}" modelFieldName: "${modelFieldName}".`)
                }//getDefaults
        ...
        const widgets = [
          ...
          , [
              ...
              , CollapsibleContainer
              , this._zones
              , 'Numeric Properties' // togglerLabel
              , 'minimal' // flavor
              , 'numeric_propertuies' // classNameParticle
              , [//widgets
                    [
                        {zone: 'main'}
                      , [
                      ...
                      , this._getDefaults.bind(this)
                      , requireUpdateDefaults
                      , null // label="Numeric Properties"
                    ]
                ]
                , false // isOpen
                , false // scroll
            ]
            // Empty for the actor types that implement neither the
            // rotations nor the perspective, these don't get the container.
          , ...(threeDPropertiesKeys.length
            ? [[
                {zone: 'local', activationTest}
              , []
              , CollapsibleContainer
              , this._zones
              , '3D Properties' // togglerLabel
              , 'minimal' // flavor
              , 'three_d_properties' // classNameParticle
              , [//widgets
                    [
                        {zone: 'main'}
                      , [
                      ...
                      , UI3DProperties
                      , ProcessedPropertiesSystemMap.fromPrefix(THREE_D, threeDPropertiesKeys)
                      , this._getDefaults.bind(this)
                      , requireUpdateDefaults
                      , null // label="Numeric Properties"
                    ]
          ...
          , ...this._defineGenericWidgets(
                  ...
                  , fieldName=>(REGISTERED_GENERIC_KEYMOMENT_FIELDS.has(fieldName)
                            && !TYPOGRAPHY_SPACING_KEYMOMENT_FIELDS.has(fieldName))
                        || REGISTERED_METAMODEL_FIELDS.has(fieldName)
                  , {zone: 'local', activationTest}
                  , GENERIC
                  , { // injectable
                        updateDefaultsDependencies
        ...
        // The shared activationTest reads 'isActive' via the injected
        // getEntry, which requires it to be a declared dependency (see
        // ComponentWrapper._activationTestGetEntry). Declare it on every
        // widget that uses the test — as an absolute path, because the
        // widgets have differing rootPaths (generic widgets are rooted at
        // their field name).
        const isActiveDependency = [this.widgetBus.rootPath.append('isActive').toString(), 'isActive'];
        for(const widgetDefinition of widgets)
            if(widgetDefinition[0].activationTest === activationTest)
                widgetDefinition[1].push(isActiveDependency);
        return widgets;
    }
    _update(...args) {
        try {
...
class ZoomLevelController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        const zones = new Map(_zones)
            , mainContainer = widgetBus.domTool.createElement('div', {'class': 'ui_zoom_level'})
            ;
        zones.set('main', mainContainer);
        super(widgetBus, zones);
        this._insertElement(mainContainer);
        const widgets = [
            [
                { zone: 'main'
                }
              ...
              , UISelectInput
              , 'Zoom Level'
              , new Map(StageZoomTypeModel.enumItems.map(value=>[value, value]))// items
            ]
            // if type is custom, display a text field!
          , [
                { zone: 'main'
                  // getEntry is injected by ComponentWrapper and only
                  // serves declared dependencies ('type' is declared below).
                , activationTest: function(getEntry) {
                        const type = getEntry('type');
...
class PropertiesManager extends _CommonContainerComponent {
    ...
    _createStageWrappers() {
        ...
        const widgets = [
          ...
          , [
                {
                    rootPath: widgetAbsRootPath
                  , relativeRootPath: Path.fromParts('.', 'zoomLevel')
                  , zone: 'main'
                }
              , []
              , ZoomLevelController
    ...
    _provisionWidgets(compareResult) {
        ...
        if(actor !== null && rebuild) {
            ...
            // The MotionStageModel actually functions as the root
...
export class UITimeControl extends _BaseContainerComponent{
    constructor(widgetBus, _zones) {
         const localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_time_control'})
        ...
        const widgets = [
          ...
          , ...Object.entries(getBasicPlayerControlWidgets({zone: 'contents'}))
...
class MotionStageController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        widgetBus.wrapper.setProtocolHandlerImplementation(
            ...SimpleProtocolHandler.create('animationProperties@'));
        const stageManagerContainer = widgetBus.domTool.createElement('div', {'class': 'stage-manager'})
          , propertiesManagerContainer = widgetBus.domTool.createElement('div', {'class': 'properties-manager'})
        ...
        this._classesAndStylesManager.setClass('typeroof-layout--motion-stage');
        this._initWidgets(widgets);
    }
    destroy() {
...
class MotionStagePlayerController extends _BaseContainerComponent {
    constructor(widgetBus, zones) {
        widgetBus.wrapper.setProtocolHandlerImplementation(
            ...SimpleProtocolHandler.create('animationProperties@'));
        super(widgetBus, zones);
...
export const PlayerLayout = {
    Controller: MotionStagePlayerController
  , Model: MotionStageModel
};

export {
    MotionStageModel as Model
  , MotionStageController as Controller
