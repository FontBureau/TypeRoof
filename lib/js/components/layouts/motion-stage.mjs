...
export class UINumericProperties extends _BaseComponent {
    constructor (widgetBus, ppsMap, getDefaults=null, requireUpdateDefaults=()=>false, label='Numeric Properties') {
        super(widgetBus);
        this._ppsMap = ppsMap;
        ...
        this.element = this._domTool.createElement('div',{class: 'numeric_properties'});
        if(label)
...
export class UI3DProperties extends _BaseComponent {
    constructor (widgetBus, ppsMap, getDefaults=null, requireUpdateDefaults=()=>false, label='3D Properties') {
        super(widgetBus);
        this._ppsMap = ppsMap;
        this._getDefaults = getDefaults;
        this._requireUpdateDefaults = requireUpdateDefaults;
        ...
        this.element = this._domTool.createElement('div',{class: '3d-properties'});
        if(label)
            this.element.append(this._domTool.createElement('h3', {}, label));
    ...
    _setOrReset(mapLike, key, value) {
     ...
     /* Run within transaction context */
    __propertiesChangeHandler(key, value) {
        const threeDProperties = this.getEntry('3dProperties');
        this._setOrReset(threeDProperties, key, value);
    }
        ...
        for(const key of this.propertiesKeys()) {
            ...
            const input = new PlainNumberAndRangeOrEmptyInput(
              ...
              , value=>this._propertiesChangeHandler(key, value)
                // toggleChangeHandler
              , ()=>{
                    const value = this._threeDProperties.has(key)
                        ? null // if the property is defined delete
                        // if the property is not defined set default ...
                        : (this._getDefaults
                                    ? this._getDefaults(ppsRecord, key, defaultVal)
    ...
    *propertiesEntries() {
    ...
    _getValueForProperty(key) {
        return this._threeDProperties.has(key)
                    ? [true, this._threeDProperties.get(key).value]
                    : [false, (this._getDefaults !== null
                            ? this._getDefaults(
                                      this._ppsMap.get(key)
                                    , key
    ...
    update (changedMap) {
        ...
        if(changedMap.has('3dProperties') || requireUpdateDefaults) {
            const threeDProperties = changedMap.has('3dProperties')
                                        ? changedMap.get('3dProperties')
                                        : this.getEntry('3dProperties')
                                        ;
            this._threeDProperties = threeDProperties;
            for(const key of this.propertiesKeys()) {
                const [active, value] = this._getValueForProperty(key);
                // It's interesting: in a way, the sub-ui's could listen
                // directly to their entry at axesLocations/{axisTag}
