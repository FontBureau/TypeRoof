...
class ProseMirror extends _BaseComponent {
    ...
    _rawCreateProseMirrorNode(cacheMap/* null or a map*/, metamodelNode, schema) {
        ...
        for(const [, mmMark] of metamodelNode.get('marks')) {
            ...
            // TODO: add mark.attrs!
            const mark = schema.mark(mmMark.get('typeKey').value);
            marks.push(mark);
        }
        ...
        if(type === 'text') {
            ...
            // TODO: add node.attrs
            newNode = schema.node(type, null, content, marks);
        }
        return newNode;
    }
    ...
    update (changedMap) {
        ...
        if(changedMap.has('document')) {
            ...
            const { state } = this._view
              , newDoc = this._createProseMirrorNode(this._nodesCache, document, state.schema)
              ;
            if(newDoc === state.doc) {
                // nothing to do;
