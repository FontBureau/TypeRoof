...
import {schema as proseMirrorTestingSchema} from "../dev-layouts/prosemirror-testing-schema"
...
const
  ...
  ,  TypeSpecRampModel = _BaseLayoutModel.createClass(
      ...
      , CoherenceFunction.create(
            ['proseMirrorSchema']
          , function initNodeSpecMap({proseMirrorSchema}) {
                // This is so far just to make development simpler,
                // eventually the data should be loaded from presets or
                // originally created.
                const nodeSpecMap = proseMirrorSchema.get('nodes');
                if(nodeSpecMap.size === 0) {
                    // See prosemirror-testing-schema. I rather duplicate
                    // h1 to h6 as there's no structure (yet?) to link to
                    // different typeSpecs within a single NodeSpec.
                    // So, in this case going for flat is better than nested,
                    // as it is simpler for now.
                    // Also, I'm not going to create the "text" node in
                    // here as that should be present automatically,
                    // depending on the document purpose, other types
                    // like hard_break or horizontal_rule could also
                    // be automated. Maybe, we need reserved names in
                    // validateNodeSpecName, e.g. for text.
                    for(const nodeName of ['doc', 'paragraph-1', 'paragraph-2' ,'blockquote', 'heading-1', 'heading-2', 'heading-3']) {
                        const node = nodeSpecMap.constructor.Model.createPrimalDraft(nodeSpecMap.dependencies);
                        nodeSpecMap.set(nodeName, node);
                    }
                }
            }
        )
...
class ProseMirror extends _BaseComponent {
    ...
    constructor(widgetBus) {
        super(widgetBus);
        // The cache will be bi-directional meaning that both
        // mappings will be set proseMirrorNode -> metamodelNode
        // and metamodelNode -> proseMirrorNode.
        this._nodesCache = new WeakMap();
        [this.element, this._view] = this.initTemplate();
    }

    _createMetamodelNode(dependencies, pmNode, cacheMap=null) {
        if(cacheMap !== null && cacheMap.has(pmNode))
            return cacheMap.get(pmNode);

        const draft = NodeModel.createPrimalDraft(dependencies);

        draft.get('typeKey').value = pmNode.type.name;
        if(pmNode.type.name === 'text')
            ...
            for (let i=0,l=pmNode.content.childCount; i<l; i++) {
                ...
                contentDraft.push(this._createMetamodelNode(dependencies, pmChildNode, cacheMap));
            }
        }
    ...
    /**
     * If caching is to be used inject cache here, e.g. use this._pmNodesCache
     ...
     */
    _createProseMirrorNode(schema, metamodelNode, cacheMap=null) {
        ...
        if(type === 'text') {
            ...
            newNode = schema.text(text.value)
        }
        else {
            ...
            for(const [/*index*/, mmChildNode] of mmContent) {
                const child = this._createProseMirrorNode(schema, mmChildNode, cacheMap);
                content.push(child);
            }
            ...
            newNode = schema.node(type, null, content);
        }
    ...
    _prosemirrorDispatchTranscation(transaction) {
        console.log( "DISPATCH_TRANSACTION size went from", transaction.before.content.size,
        ...
        // console.log('newState.doc', newState.doc);
        const document = this.getEntry('document');
        // console.log(`document ${document}`, document);
        this._view.updateState(newState);
        const pmDocNode = this._nodesCache.get(document);
        if(pmDocNode === this._view.state.doc) {
            // nothing to do
            ...
            this._changeState(()=>{
                const documentDraft = this.getEntry('document')
                  , pmDoc = this._view.state.doc
                  ;
                // I'm not so happy rebuilding this cache on each transaction :-/
                // maybe better to keep a reverse weakmap around ???
                const immutableDoc = this._createMetamodelNode(documentDraft.oldState.dependencies, pmDoc, this._nodesCache)
                ...
                this._nodesCache.set(immutableDoc, this._view.state.doc);
                mapSetBiDirectional(this._nodesCache, immutableDoc, this._view.state.doc);
            });
        }
    }
    ...
    _initProseMirrorView(element) {
        const state = EditorState.create({
            schema:proseMirrorTestingSchema
          , plugins: [
                history()
          ...
          , doc: proseMirrorTestingSchema.topNodeType.createAndFill()
        })
    ...
    update (changedMap) {
        ...
        // Do I have to change the editorState(`.doc` etc)...
        if(changedMap.has('document')) {
            // it looks like document has to change...
            const document = changedMap.get('document');
            // use document to get it's according editor state
            // NOTE: this could go both ways...
            // NOTE: it's important, a pm-node as well as a typeRoof node
            // can be used multiple times. Hence, the position of the node
            // in the document can't be stored this way. It simply can
            // have multiple adresses.
            const { state } = this._view
              , newDoc = this._createProseMirrorNode(state.schema, document, this._nodesCache)
              ;
            if(newDoc === state.doc) {
                // nothing to do;
                // this happens when document was changed via dispatchTransaction
                // there, the correct reference was already created.

                // This should actually not happen, as changedMap.has('document')
                // and that means it changed...
                 console.error(`${this}UPDATE: nothing to do`);
                ...
                // update view.state.doc with  metamodel document
                // ideally, create a transaction from document/view.state.document

                // TODO
                // NOTE: this could itself detect a cache hit and return
                // view.state.doc === newDoc thus the logic in here may
                // be moved there...
                const { tr } = state; // tr: Transaction

                tr.replace(0, state.doc.content.size, newDoc.slice(0));
                // hmm, ideally this will keep dispatchTransaction from
                // updating document.
                // FIXME: I wonder if const newState = view.state.apply(transaction)
                // would be actually the better way to do this, i.e. not
                // going via dispatchTransaction
                // this._nodesCache.set(document, tr.doc);
                // mapSetBiDirectional(this._nodesCache, document, tr.doc);
                // view.dispatch(tr);
                const newState = state.apply(tr);
                mapSetBiDirectional(this._nodesCache, document, newState.doc);
                // eventually update view.state
                this._view.updateState(newState);
            }
        }
    }
