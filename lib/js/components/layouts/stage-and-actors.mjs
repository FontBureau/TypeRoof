...
class StageManager extends _BaseComponent {
    ...
    _dragstartHandler(path, event) {
        ...
        event.dataTransfer.setData('application/x.typeroof-actor-path', `${path}`);
    ...
    _takeDragEventOrLeaveIt(event) {
        const isActorPath = event.dataTransfer.types.includes('application/x.typeroof-actor-path');
        if(!isActorPath)
            return false;
    ...
    _dragoverHandler(path, event) {
        if(!this._takeDragEventOrLeaveIt(event))
            return;
        // Don't use event.dataTransfer.getData('application/x.typeroof-actor-path');
        ...
        //      still be enumerated.
        // const data = event.dataTransfer.getData('application/x.typeroof-actor-path');
        ...
        event.dataTransfer.dropEffect = path.toString().indexOf('./0') !== -1 ? 'move' : 'copy';
    ...
    _setDropTargetIndicator(element, insertPosition=null) {
        ...
        for(const elem of [element.previousSibling, element]) {
            if(!elem) continue; /*there may be no previousSibling*/
    ...
    _dragleaveHandler(path, event) {
        if(!this._takeDragEventOrLeaveIt(event))
            return;
        this._setDropTargetIndicator(event.currentTarget);
    }

    _dropHandler(path, event) {
        if(!this._takeDragEventOrLeaveIt(event))
            return;
        const rootPath = Path.fromString(this.parentAPI.getExternalName('activeActors'))
          , sourcePath = rootPath.appendString(event.dataTransfer.getData('application/x.typeroof-actor-path'))
          , targetPath = rootPath.append(...path);
          ;
        this._setDropTargetIndicator(event.currentTarget);

        console.log(`${this}._dropHandler rootPath ${rootPath} targetPath ${targetPath} sourcePath: ${sourcePath}`, event, path);


        const canMove = !sourcePath.isRootOf(targetPath);
        if(!canMove){
            console.warn(`${this} can't move source into target as source path "${sourcePath}" is root of target path "${targetPath}".`);
            return;
        }
        const insertPosition = this._getDropTargetInsertPosition(event);
        this._changeState(()=>{
            ...
            // TODO: can't drop actors into empty layer.
            console.log('sourcePath.parent: ' + sourcePath.parent, sourcePath.parent);
            const sourceParent = this.getEntry(sourcePath.parent)
              ...
              , targetKey = path.parts.at(-1)
              , targetIndex = parseInt(targetKey, 10)
              , sourceIndex = parseInt(sourceKey, 10)
              , source = sourceParent.get(sourceKey)
              ;
            if(targetParent === sourceParent) {
                console.warn(`${this} targetParent equals sourceParent ${sourcePath.parent} ${targetPath.parent}`, targetParent);

                if(sourceIndex === targetIndex)
                    return;// nothing to do

                let insertIndex = insertPosition === 'after'
                        ? targetIndex + 1
                        : targetIndex
                        ;
                if(sourceIndex < targetIndex)
    ...
    _renderLayer(path, activeActors) {
        ...
        for(const [key, actor] of activeActors) {
            const itemElement = this._domTool.createElement('li')
              ...
              , dragHandleElement = this._domTool.createElement('span', {'class': 'drag-handle'}, '✥')
              ;

            dragHandleElement.setAttribute('draggable', 'true');
            dragHandleElement.addEventListener('dragstart', this._dragstartHandler.bind(this, itemPath));
            dragHandleElement.addEventListener('dragend', this._dragendHandler.bind(this, itemPath));


            itemElement.addEventListener('dragenter', this._dragenterHandler.bind(this, itemPath));
            itemElement.addEventListener('dragleave', this._dragleaveHandler.bind(this, itemPath));
            itemElement.addEventListener('dragover', this._dragoverHandler.bind(this, itemPath));
            itemElement.addEventListener('drop', this._dropHandler.bind(this, itemPath));
