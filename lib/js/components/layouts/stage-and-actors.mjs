...
export class UIGenericStructContainer extends _BaseContainerComponent {
    ...
    _defineGenericWidgets(generalSettings, propertyRoot, injectable) {
        const resultWidgets = [];
        const TypeClass =  this.parentAPI.getEntry(this.parentAPI.rootPath).constructor;
        // caution REGISTERED_GENERIC_KEYMOMENT_FIELDS is not usefule in
        // the context of
        for(const [fieldName, FieldType] of TypeClass.fields) {
            // if(!TypeClass.fields.has(fieldName)){
            //     console.log(`SKIPPING`, propertyRoot, fieldName);
            //     continue;
            // }
            const fullKey = `${propertyRoot}${fieldName}`
...
function genericTypeToUIElement(ModelType, defaultVal=_NOTDEF) {
    ...
    const valueUIElements = new Map([
          ...
          , [CharGroupModel, [UIGenericStructContainer
...
class KeyMomentController extends _BaseContainerComponent {
        ...
        for(const fieldName of getColorModelFields(TypeClass)) {
            ...
            resultWidgets.push([
              ...
              , new Map([...this._zones.entries(), ['main', this._zones.get(settings.zone)]])
                // ... label, getDefaults, updateDefaultsDependencies, requireUpdateDefaults
    ...
    _defineGenericWidgets(generalSettings, propertyRoot, injectable/*{updateDefaultsDependencies, requireUpdateDefaults}*/) {
        const resultWidgets = [];
        const TypeClass =  this.parentAPI.getEntry(this.parentAPI.rootPath).constructor;
        for(const fieldName of REGISTERED_GENERIC_KEYMOMENT_FIELDS) {
            if(!TypeClass.fields.has(fieldName))
            ...
            const FieldType = TypeClass.fields.get(fieldName)
              ...
              , uiElementArgs = []
              ;
            // FIXME: this is the wrong conditon!!!
            // but it seems to be coincident with the settings:rootPath
            // dependency (compare: UIColorChooser)
            //maybe the condition for both is: UIElement.prototype instanceof _BaseContainerComponent
            if(UIElement !== UIGenericStructContainer)
                dependencyMappings.push([fieldName, 'value']);
            for(const arg of args) {
                if(!(arg instanceof InjectDependency)) {
                    uiElementArgs.push(arg);
                    continue;
                }
                // resolve
                switch(arg.name) {
                    ...
                    case "getDefault":
                        ...
                        // by all UIElements.
                        // getDefault: ()=>this._getDefaults.bind(this, `${propertyRoot}/`)// , fieldName, BaseModelType.defaultValue));
                        uiElementArgs.push(injectable.getDefault.bind(null, fieldName, BaseModelType.defaultValue));
                        break;
                    case "requireUpdateDefaults":
                        // this is only required if requireUpdateDefaults is
                        // an argument. it basically injects @animationProperties
                        // which is an (the!) requirement of requireUpdateDefaults
                        dependencyMappings.push(...injectable.updateDefaultsDependencies);
                        uiElementArgs.push(injectable.requireUpdateDefaults);
                        break;
                    case "parentInjectable":
                        uiElementArgs.push(injectable);
                         break;
                    case "fieldName":
                        uiElementArgs.push(fieldName);
                        break;
                    case "fullKey":
                         uiElementArgs.push(fullKey);
                         break;
                    case "propertyRoot":
                        uiElementArgs.push(`${fullKey}/`);
                        break;
                    case "label":
                        uiElementArgs.push(getRegisteredPropertySetup(fullKey, {label:fieldName}).label || fieldName);
                        break;
                    case "items":
                        // TODO: No nice labels via _AbstractEnumModel so far! Maybe a
                        // general internationalization and localization layer could provide
                        // that?
                        uiElementArgs.push(new Map(BaseModelType.enumItems.map(value=>[value, value])));
                        break;
                    default:
                        throw new Error(`KEY ERROR ${this}._defineGenericWidgets `
                            + `for Type ${TypeClass.name}.${fieldName} don't know `
                            + `how to provide parameter "${arg.name}".`);
                }
            }
            resultWidgets.push([
                settings
    ...
    _getDefaultColor(fullKey, defaultVal=_NOTDEF) {
        ...
        // this will raise.
        const fallback = getRegisteredPropertySetup(fullKey, defaultVal === _NOTDEF ? getRegisteredPropertySetup.NOTDEF : defaultVal);
