...
class LayerDOMNode extends StaticNode {
    ...
    update(changedMap){
        const setXYZ = (property, value)=>{
            const [cssProperty, unit] = {
                      'numericProperties/x': ['left', 'px']
                    , 'numericProperties/y': ['top', 'px']
                    , 'numericProperties/z-index': ['z-index', '']
            }[property];
            this.node.style.setProperty(cssProperty, `${value}${unit}`);
        };
...
class SelectAndDrag extends _BaseComponent {
    ...
    <span class="select_and_drag-drag_handle drag_handle">✥</span>
    <label><span class="select_and_drag-label"></span> <select><select></label>
</div>`;
    // jshint ignore:end
    constructor(parentAPI, label, setDragDataFN) {
        super(parentAPI);
        this._setDragDataFN = setDragDataFN;
        [this.element, this._selectElement] = this.initTemplate(label);
    }
    initTemplate(label) {
        const frag = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE)
          , element = frag.firstElementChild
          , selectElement = frag.querySelector('select')
          , labelContainer = frag.querySelector('.select_and_drag-label')
    ...
    _dragstartHandler(event) {
        ...
        //
        // FIXME: I don't see how dropEffect has any efect in FireFox or Chrome...
        // this must fit to the drop target => same effect
        // If the effects don't match, _dropHandler won't be called
        // create is more like a copy than link or move IMO.
...
class StageManager extends _BaseComponent {
    ...
    _dragendHandler({path}, event) {
        console.log(`${this}._dragendHandler ${path}`, event, path);
        event.currentTarget.classList.remove('dragging');
    }
    ...
    _dragoverHandler(item, event) {
        ...
        // FIXME: can't see a different effect!?
        event.dataTransfer.dropEffect = item.path.toString().indexOf('./0') !== -1 ? 'move' : 'copy';
    ...
    _getDropTargetInsertPosition(item, event) {
        const {height, top} = event.currentTarget.getBoundingClientRect()
          ...
          , relativeY = elementY/height
            // The isNotLast is important, because otherwise we can't
            // drop an element "after" an empty layer.
            // FIXME: There's still an issue in that case, that we can't
            // directly "insert" into an empty layer that is last, but at
            // least, FOR NOW, we can insert "after" and then move that
            // element to "insert".
          , isLayer = item.actor.get('actorTypeKey').value === 'LayerActorModel'
          , isEmptyLayer = isLayer && item.actor.get('instance').get('activeActors').size === 0
          , isNotLastEmptyLayer = isEmptyLayer && !item.isLast
          , insert = relativeY < 0.5
                ? 'before'
                : (isNotLastEmptyLayer ? 'insert' : 'after')
          ;
        return insert;
    }
    ...
    _create(typeKey, targetPath, insertPosition) {
        ...
        return this._changeState(()=>{
            const targetParent =  this.getEntry(targetPath.parent) // is an 'ActorsModel' ('activeActors')
              , targetKey = targetPath.parts.at(-1)
                // Ensure we take the dependencies for the create from the
                // correct element, even though, at the moment, they are
                // all identical.
              , activeActors = insertPosition === 'insert'
                    ? getEntry(targetParent, Path.fromParts(targetKey, 'instance', 'activeActors'))
                    : targetParent
            ...
            if(insertPosition === 'insert') {
                ...
                targetParent.splice(targetIndex + 1, 0, newActor);
            else if(insertPosition === 'before')
                targetParent.splice(targetIndex, 0, newActor);
            else
    ...
    _move(sourcePath, targetPath, insertPosition) {
        ...
        return this._changeState(()=>{
            ...
            //
            // TODO: add copy, create and delete actions
            // TODO: can't drop actors into empty layer that are also last elements.

            const sourceParent = this.getEntry(sourcePath.parent)
              , targetParent =  this.getEntry(targetPath.parent)
            ...
            if(insertPosition === 'insert') {
                ...
                // targetParent === sourceParent and the targetKey could
                // change is circumvented.
                getEntry(targetParent, Path.fromParts(targetKey, 'instance', 'activeActors')).push(source);
            ...
            if(targetParent === sourceParent) {
                ...
                sourceParent.delete(sourceKey);
                targetParent.splice(insertIndex, 0, source);
                return;
            }
            sourceParent.delete(sourceKey);
            if(insertPosition === 'after')
    ...
    _renderLayer(path, activeActors) {
        const container = this._domTool.createElement('ol')
          , lastIndex = activeActors.size - 1
          ;
        for(const [index, [key, actor]] of enumerate(activeActors)) {
            const itemElement = this._domTool.createElement('li')
              ...
              , item = {path: itemPath, actor, isLast}
              ;
            ...
            itemElement.append(dragHandleElement, ...this._renderActor(itemPath, actor));
