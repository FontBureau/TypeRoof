...
const ColorTypeModel = _AbstractGenericModel.createClass('ColorTypeModel')// => .value will be a concrete _BaseColorModel
  ...
  , ColorModel = _AbstractStructModel.createClass(
    ...
    , PercentNumberModel = _AbstractNumberModel.createClass('PercentNumberModel', {min:0, max:100, defaultValue: 0})
    , PercentNumberOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(PercentNumberModel)
    , PercentWithNegativeNumberModel = _AbstractNumberModel.createClass('PercentWithNegativeNumberModel', {min:-100, max:100, defaultValue: 0})
    ...
    , HueNumberModel = _AbstractNumberModel.createClass('HueNumberModel', {min: 0, max:1, defaultValue: 0})
    ...
    update(changedMap){
        ...
        if(changedMap.has('@animationProperties') || changedMap.has('globalT')) {
            const animationProperties = changedMap.has('@animationProperties')
              ...
              , getDefault = property => {
                    const numericPropertiesPrefix = 'numericProperties/';
                    if(property.startsWith(numericPropertiesPrefix)) {
                        const name = property.slice(numericPropertiesPrefix.length);
                        if(REGISTERED_NUMERIC_PROPERTIES.has(name))
                            return REGISTERED_NUMERIC_PROPERTIES.get(name).default;
                    }
                    throw new Error(`KEY ERROR don't know how to get default value for property "${property}".`);
                }
              ;
        ...
        if(changedMap.has('@animationProperties') || changedMap.has('globalT')) {
            ...
            for(const [key, property] of [
                ...
                if(!propertyValuesMap.has(key))
                    continue;
                const color = propertyValuesMap.get(key)
                  , cssColor = culoriFormatCss(color)
                  ;
                this._layerNode.style.setProperty(property, cssColor);
            }
        }
    }
}
...
function* colorsGen(outerAnimanionAPI, keyMoment/*, momentT*/) {
    for(const fieldName of getColorModelFields(keyMoment.constructor)) {
        const colorModel = keyMoment.get(fieldName)
          , instance = colorModel.get('instance')
          ;
        if(!instance.hasWrapped)
            continue;
        ...
        //}
        yield [`colors/${fieldName}`, colorToCulorijs(instance.wrapped)];
    }
}
...
function createActor(typeKey, availableActorTypes, dependencies) {
    const getTypeFor=key=>availableActorTypes.get(key).value.get('typeClass').value
      , getDraftFor=(name, deps)=>getTypeFor(name).createPrimalDraft(deps)
      , draft = getDraftFor(typeKey, dependencies)
      , actorDraft = ActorModel.createPrimalDraft(dependencies)
      ;
    actorDraft.get('actorTypeKey').value = typeKey;
    actorDraft.get('instance').wrapped = draft;
    return actorDraft;
}
    ...
    _create(typeKey, targetPath, insertPosition) {
        ...
        return this._changeState(()=>{
            const activeActors = insertPosition === 'insert'
              ...
              , sourceTypes = this.getEntry('sourceTypes')
              ...
              , newActor = createActor(typeKey, sourceTypes, activeActors.dependencies)
              ;
...
class CommonActorProperties extends _BaseComponent {
    ...
    _createColorValueElement(key, value) {
        console.log(`${this}._createColorValueElement ${key}`, value);

        const valueContainer = this._domTool.createElement('span')
          , colorCss = culoriFormatCss(value)
          , colorPatch = this._domTool.createElement('span', {'class': `${this._baseClass}-value-color_patch ui_color_patch ui_color_patch-has_color`})
          ;
        colorPatch.style.setProperty(`--color`, colorCss);
        valueContainer.append(colorPatch,' ',colorCss);
        return valueContainer;
    }

    // NOTE: copied from AnimationInfo
    _createBasicDisplayElement(key, value, label=null) {
        const [type, valueContainer] = key.startsWith('colors/')
                    ? ['color', this._createColorValueElement(key, value)]
                    : ['text', this._createTextValueElement(key, value)]
          , labelContainer = this._domTool.createElement('span', {'class': `${this._baseClass}-label`}, label !== null ? label : key)
          , container = this._domTool.createElement('li', {'class': `${this._baseClass}-item ${this._baseClass}-item-type_${type}`}, [
...
// FIXME: not all types require all REGISTERED_NUMERIC_PROPERTIES
...
function _mapComponentToCuloriRange(component, culoriRange) {
    const ComponentType = component instanceof _AbstractSimpleOrEmptyModel
                    ? component.constructor.Model
                    : component.constructor
      ,  {minVal, maxVal} = ComponentType
        // If it's an _AbstractSimpleOrEmptyModel and it's empty, it will
        // raise here!
      , value = component.value
        // normalize between 0 and 1
      , t = (value - minVal) / (maxVal - minVal)
      , [culoriMin, culoriMax] = culoriRange
      , culoriValue = interpolateNumber(t, culoriMin, culoriMax)
      ;
    return culoriValue;
}

function colorToCulorijs(color) {
    if(!COLOR_TYPE_TO_CULORI_MODE.has(color.constructor))
        throw new Error(`VALUE ERROR can't determine culori-js color mode from color ${color}.`);
    const mode = COLOR_TYPE_TO_CULORI_MODE.get(color.constructor)
      , culoriModeRanges = culoriGetMode(mode).ranges
      ;

    const culorijsColor = Object.fromEntries(Array.from(color.entries())
        .filter(([/*component*/, component])=>!(component instanceof _AbstractSimpleOrEmptyModel && component.isEmpty))
        .map(([componentKey, component])=>{
            const culoriComponent = componentKey.toLowerCase()
              , culoriValue = _mapComponentToCuloriRange(component, culoriModeRanges[culoriComponent])
              ;
...
export class PlainColorComponentOrEmptyInput {
    ...
    constructor(domTool, baseClass='ui_color_component') {
        ...
        this._uiNumber = new PlainNumberAndRangeInput(this._domTool
        ...
        this.element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild;
    ...
    update(componentKey, color, changeHandler, toggleHandler) {
        ...
        const min = ValueModel.minVal
          ...
          ;
        this._uiNumber.updateTemplateVars(componentKey, unit, {min, max, 'default': defaultValue});
...
class UIColorComponentRanges extends _BaseComponent {
    ...
    _componentToggleHandler(componentKey) {
        console.log(`${this}._componentToggleHandler(${componentKey})`);
        this._changeState(()=>{
            const color = this.getEntry('color')
              , component = color.get(componentKey)
              ;
            if(!component.isEmpty){
                console.log(`!component.isEmpty`, component);
                component.clear();
            }
            else {
                console.log(`component.isEmpty set value ...`, component);
    ...
    _componentChangeHandler(componentKey, newValue, ...args) {
        console.log(`${this}._componentChangeHandler(${componentKey}, newValue: ${newValue}) args:`, ...args);
        this._changeState(()=>{
            const color = this.getEntry('color')
              , component = color.get(componentKey)
              ;
    ...
    _updateComponent(i, componentKey, color) {
    ...
    update (changedMap) {
        const requireUpdateDefaults = this._requireUpdateDefaults(changedMap);
        console.log(`${this}.update requireUpdateDefaults ${requireUpdateDefaults}`, 'changedMap:', ...changedMap);
        if(changedMap.has('color')) {
            const color = changedMap.get('color').wrapped;
            let size = 0;
            for(const [i, componentKey] of enumerate(color.keys())) {
                size += 1;
                if(!this._components[i])
                    this._components.push(this._createComponent());
                this._updateComponent(i, componentKey, color);
            }
...
class UIColorPatch extends _BaseComponent{
    ...
    update (changedMap) {
        ...
        if(changedMap.has('color')) {
            console.log(`${this}.update  changedMap.get('color'):`, changedMap.get('color'));
            // .hasWrapped === true:
            const color = changedMap.get('color');
            if(color.hasWrapped) {
                this.element.classList.add(`ui_color_patch-has_color`);
                this.element.classList.remove(`ui_color_patch-no_color`);
                this.element.style.setProperty('--color', colorToCss(color.wrapped));
            }
            else {
...
class UIColorChooser extends _BaseContainerComponent {
    constructor(parentAPI, zones, label, getDefaults, requireUpdateDefaults) {
        console.log(`UIColorChooser constructor`, parentAPI.rootPath.toString(), parentAPI.getEntry(parentAPI.rootPath));
        ...
        const activationTest = ()=>{
          ...
          , [
              ...
              , label// labelContent
              , (key, availableColorTypes)=>{ return availableColorTypes.get('label').value; } // optionGetLabel
              , [true, '(undefined)', ForeignKey.NULL] // [allowNull, allowNullLabel]
                // changing this should convert the previous color to the
                // new color, using colori!
           ]
          ,[
...
class KeyMomentController extends _BaseContainerComponent {
    ...
    _defineColorWidgets(settings, requireUpdateDefaults) {
        const resultWidgets = [];
        ...
        for(const fieldName of getColorModelFields(TypeClass)) {
            ...
            resultWidgets.push([
              ...
              , fieldName
              , this._getDefaults.bind(this, `colors/${fieldName}/`)
              , requireUpdateDefaults
            ]);
    ...
    constructor(parentAPI, zones) {
        ...
        setTimeout(this._changeStateHandler(()=>{
            ...
            const availableActorTypes = state.get('availableActorTypes')
              ...
              , layerActorDraft = createActor(layerTypeKey, availableActorTypes, activeActors.dependencies)
            ...
            for(let i=0;i<2;i++) {
                const circleTypeKey = 'CircleActorModel'
                  , circleActorDraft = createActor(circleTypeKey, availableActorTypes, activeActors.dependencies)
            ...
            const LinoOfTextTypeKey = 'LineOfTextActorModel'
              , lotActorDraft = createActor(LinoOfTextTypeKey, availableActorTypes, activeActors.dependencies)
