 ...
 * `parentAPI`. Then each  _CommonContainerComponent requires additionally
...
function genericTypeToUIElement(ModelType, defaultVal=_NOTDEF) {
    const valueUIElements = new Map([
          ...
          // to be able to ma multiple UIs to one Type, like e.g. different
          // views.
...
class _BaseGenericKeyMomentWidgetsContainerComponent extends _BaseContainerComponent {
    _getArgumentConfig(injectable, zone, propertyRoot, fieldName, BaseModelType, parameter) {
        ...
        switch(parameter.name) {
            case "settings:rootPath":
                // settings.set('rootPath', this.parentAPI.rootPath.append(fieldName));
    ...
    //         widgetWrapper.updateRootPath(this.parentAPI.rootPath);
    // }
}
...
export class UICharGroupContainer extends _BaseGenericKeyMomentWidgetsContainerComponent {
    constructor(parentAPI, _zones, injectable, propertyRoot, label) {
        // run super first, so we can use `this` in the widgets definition.
        const localZoneElement = parentAPI.domTool.createElement('div', {'class': 'ui_char_group_container'})
          , zones = new Map([..._zones, ['local', localZoneElement]])
          ;
        super(parentAPI, zones);
    ...
    _defineWidgets(TypeClass, localZoneElement
        ...
        return [
          ...
          , this._getWidgetConfig(injectable, generalSettings, propertyRoot, 'irrelevant'// 'generic/', 'charGroup',
              , [ UIUpdateListener
    ...
    _onChange() {
        ...
        const {defaultValue, charGroupsData} = this._CharGroupOptionsBaseModel
          ...
          ;
        const data = {chars: [], extendedChars: []};
        if(charGroup === 'custom') {
            ...
            const font = this.getEntry(this.parentAPI.rootPath.append('../../../font')).value;
            [, data.extendedChars] = getCharsForSelectUI(charGroupsData , font, charGroup);
        }
        const extendedWidget = this.getWidgetById('extended');
        extendedWidget._ui.element.classList[(data.extendedChars.length ? 'remove' : 'add')]('not-available');
    }
...
class KeyMomentsController extends _BaseDynamicCollectionContainerComponent {
    constructor(parentAPI, zones, ...customArgs) {
        super(parentAPI, zones);
...
class PropertiesManager extends _CommonContainerComponent {
    ...
    _createStageWrappers() {
        const widgetAbsRootPath = Path.fromParts(this.parentAPI.getExternalName('stage'))
          , widgetRootPath = widgetAbsRootPath.toRelative(this.parentAPI.rootPath)
    ...
    _createActorWrappers(actorPath, actor) {
        const typeKey = actor.get('actorTypeKey').value
          , actorsAbsPath = Path.fromString(this.parentAPI.getExternalName('actors'))
          , actorsPath = actorsAbsPath.toRelative(this.parentAPI.rootPath)
