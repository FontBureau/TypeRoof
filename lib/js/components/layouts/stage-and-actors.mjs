...
class LayerDOMNode extends StaticNode {
    ...
    update(changedMap){
        const liveProperties = {
          ...
          , get:key=>{
                if(liveProperties._instance === null)
                    // FIXME: The t issue is not sufficiently resolved.
        ...
        const setXY = (property, value)=>{
            const cssProperty = {x: 'left', y: 'top'}[property];
...
class CircleActorRenderer extends _BaseComponent {
    ...
    update(changedMap) {
        ...
        // OLD: should disappear, but it's now used in building the initial
        // testing fixture.
        // if(changedMap.has('radius'))
        //     setRadius(changedMap.get('radius').value);
        //
        // for(const property of ['x', 'y']) {
        //     if(!changedMap.has(property))
        //         continue;
        //     setXY(property, changedMap.get(property).value);
        // }
    }
}
...
class ActorRendererContainer extends _BaseContainerComponent {
    constructor(parentAPI, zones, _widgets) {
        console.warn(`LayerActorRenderer: rp: ${parentAPI.rootPath} animationProperties@${parentAPI.rootPath.append('..','..', '..')}`);
        const widgets = [
            [
                {
              ...
              , [// 't', 'keyMoments', 'isLoop']
                    'keyMoments'
                    // FIXME: isLoop should probably always be next to keyMoments
                    // not global, next to global t
                  , ['/activeState/isLoop', 'isLoop']
                    // TODO: there's likely going to be a local t, that is a animated property, not part of the model
                  , ['/activeState/t', 't']
              ...
              , function initAnimanion(keyMoments, isLoop) {
                    // FIXME: it's not exactly elegant!
                    const parent = this.parentAPI.grandParentAPI.grandParentAPI.getWidgetById('AnimationLiveProperties');
                    // Creates e.g.:
                    // > AnimationLiveProperties initAnimanion must have a parent animanion
                    //   /activeState/activeActors/1/instance/activeActors/0/instance/activeActors/1/instance
                    //   PARENT: /activeState/activeActors/1/instance/activeActors/0/instance
                    console.warn(this.constructor.name, 'initAnimanion must have a parent animanion',
                            this.parentAPI.rootPath.toString(),
                            'PARENT:', parent.parentAPI.rootPath.toString());
                    // also, this should be reqistered in a common place at its root path
                    // AND removed on destroy!
                    // maybe it could do that by itself!
                    ...
                    const animanion = new Animanion(parent.animanion, numericPropertiesGen, keyMoments, isLoop)
                      , registry = this.parentAPI.getWidgetById('GlobalAnimationLivePropertiesRegistry')
                      ;
                    console.warn(`id@${this.parentAPI.rootPath.toString()}`);
                    registry.set(this.parentAPI.rootPath.toString(), animanion);
                    return animanion;
                }
            ]
          , ..._widgets
        ...
        if(typeClass === CircleActorModel) {
            const widgets = [
                [
                    { zone: 'layer' }
                  , [//  'x', 'y', 'radius'
                        // This is wrong, but at least we get some
                        // movement out of it
                        ['/activeState/t', 't']
                      , ['/activeState/isLoop', 'isLoop']
                      , 'keyMoments'
...
class LayerActorRenderer extends _BaseContainerComponent {
    constructor(parentAPI, _zones, baseCLass=[]) {
        ...
        console.warn(`LayerActorRenderer: animationProperties@${parentAPI.rootPath.append('..', '..')}`);
        const widgets = [
            [
                {
                    id: 'AnimationLiveProperties'
              ...
              , function initAnimanion(keyMoments, isLoop) {
                    ...
                    // WE will receive this via the local `@parentProperties`
                    // probably overloading this.getEntry for it.
                    // `@parentProperties` will resolve to `animationProperties@${this.parentAPI.rootPath.append('..', '..')}`
                    // const parent = this.getEntry(`@parentProperties`);
                    const parent = this.parentAPI.grandParentAPI.grandParentAPI.getWidgetById('AnimationLiveProperties');
                    // Creates e.g.:
                    // > AnimationLiveProperties initAnimanion must have a parent animanion
                    //   /activeState/activeActors/1/instance/activeActors/0/instance/activeActors/1/instance
                    ...
                    const animanion = new Animanion(parent.animanion, numericPropertiesGen, keyMoments, isLoop)
                      , registry = this.parentAPI.getWidgetById('GlobalAnimationLivePropertiesRegistry')
                      ;
                    // works in this context as a global id, but maybe
                    // should be made formal in a way ...
                    // e.g. like this.globalID = `id@${this.parentAPI.rootPath.toString()}`
                    // especially with a special prefix, we should be able
                    // to define that string somewhere else as a dependency,
                    // and to distinguish it from a regulat path
                    console.warn(`id@${this.parentAPI.rootPath.toString()}`);
                    // Will get registerd
                    registry.set(this.parentAPI.rootPath.toString(), animanion);
                    return animanion;
    ...
    constructor(parentAPI, _zones, baseCLass=[]) {
        ...
        const widgets = [
          ...
          , [
              ...
              , AnimationLiveProperties
              , function initAnimanion(keyMoments, isLoop) {
                    console.warn(`This is the root animanion:`, this.parentAPI.rootPath.toString());
                    // jshint validthis:true
                    // This makes it possible for fontSize to be a dependency of
                    // the axisLocations. Required for opsz with autoOPSZ = true and
                    // no explicitly set fontSize on the same keyMoment.
                    ...
                    // const outerAnimanion = new Animanion(null, fontSizeGen, keyMoments, isLoop);
                    // return new Animanion(outerAnimanion, axisLocationsGen, keyMoments, isLoop);
                    const animanion = new Animanion(null, numericPropertiesGen, keyMoments, isLoop)
                      , registry = this.parentAPI.getWidgetById('GlobalAnimationLivePropertiesRegistry')
                      ;
                    console.warn(`id@${this.parentAPI.rootPath.toString()}`);
                    registry.set(this.parentAPI.rootPath.toString(), animanion);
...
class CommonActorProperties extends _BaseComponent {
    ...
    update(changedMap) {
        console.log(`${this.constructor.name}.update #${this._typeKey}:`, ...changedMap.keys(), ...changedMap);
        if(changedMap.has('...')) {
...
export class UINumericProperties extends _BaseComponent {
    static REGISTERED_PROPERTIES = ['x', 'y', 'r']; //jshint ignore:line
    ...
    _initUI() {
        ...
        for(const key of this.propertiesKeys()) {
            ...
            const {name, min, max, 'default':defaultVal} = this.propertiesGet(key);
    ...
    propertiesHas(key) {
        return this.constructor.REGISTERED_PROPERTIES.indexOf(key) !== -1;
    }
    propertiesGet(key) {
        if(!this.propertiesHas(key))
            throw new Error(`KEY ERROR ${key} not found in REGISTERED_PROPERTIES.`);
        return {name: key, min: -1000, max: 1000, 'default': 123};
    }
    propertiesKeys() {
        return [...this.constructor.REGISTERED_PROPERTIES];
    }
...
class KeyMomentController extends _BaseContainerComponent {
    ...
    constructor(parentAPI, _zones) {
        ...
        const updateDefaultsDependencies = [
                ['../../keyMoments', 'keyMoments'] //??
                // isLoop is a property of the activeState, via timeControlModelMixin
              , ['/activeState/t', 't']
              , ['/activeState/isLoop', 'isLoop']
            ]
    ...
    _getDefaults(pefix, key, defaultVal=super._NOTDEF) {
        ...
        // actor: activeState
        const registryMyActorPath = this.parentAPI.rootPath.append('..', '..').toString();

        // console.log(`${this.constructor.name}_getDefaults`, pefix, key,
        //         `in rootPath ${this.parentAPI.rootPath}`
        //         , 'registryMyActorPath', registryMyActorPath);

        const fullKey = `${pefix}${key}`
           , registry = this.parentAPI.getWidgetById('GlobalAnimationLivePropertiesRegistry')
           , liveProperties = registry.get(registryMyActorPath)
    ...
    _createActorWrappers(actorPath, actor) {
        ...
        const widgets = [
          ...
          , [
            ...
            // Uncaught (in promise) Error: KEY ERROR id "AnimationLiveProperties" not found in a MainUIController.!
            //, [
            //    {zone: 'after-layout'}
            //  , [
            //        ['/activeState/t', 't']
            //      , ['/activeState/isLoop', 'isLoop'] // never stop playback
            //      , ['keyMoments', 'keyMoments']
            //    ]
            //  , AnimationInfo
            //  , 'AnimationLiveProperties'
...
class Registry extends _BaseComponent {
    constructor(parentAPI) {
        super(parentAPI);
    ...
    get (key) {
        if(!this._values.has(key))
            throw new Error(`KEY ERROR Registry doesn't have key "${key}".`);
        return this._values.get(key);
    }
}

class AnimationPropertiesProtocolHandler extends _ProtocolHandler {
    constructor(...args) {
        super(...args);
    ...
    register(identifier, component) {
        if(this._registeredComponents.has(identifier))
            throw new Error(`VALUE ERROR identifier "${identifier}" already exists in ${this}.`);
        console.log(`*******${this} register: ${identifier} component: ${component} `);
        this._registeredComponents.set(identifier, component);
        return this._unregister.bind(this, identifier);
    }
    ...
    _unregister(identifier) {
        this._registeredComponents.delete(identifier);
    }

    getRegisered(identifier) {
        if(!this._registeredComponents.has(identifier))
            throw new Error(`KEY ERROR not found identifier "${identifier}" in ${this}.`);
        return  this._registeredComponents.get(identifier);
    }

    getUpdated(identifier) {
        const value = this.getRegisered(identifier);
        return [true, value];
    }
}
...
class StageAndActorsController extends _BaseContainerComponent {
    constructor(parentAPI, _zones) {
        const zones = new Map([..._zones, ['layer', _zones.get('layout')]])
            // FIXME: review where this is actually required!!!
          , protocolHandlers = [
                AnimationPropertiesProtocolHandler.create('animationProperties@')
            ]
          , widgets = [
            [
                {id: 'GlobalAnimationLivePropertiesRegistry'}
              , []
              , Registry
            ]
          , [
