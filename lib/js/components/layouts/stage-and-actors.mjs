...
function _actorApplyCSSColors(element, propertyValuesMap, colorPropertiesMap, getDefault) {
...
function _actorApplyCssProperty (propertyValuesMap, getDefault, propertyData, element) {
    const [fullKey, cssProperty, unit, cleanFn] = propertyData
      , value = propertyValuesMap.has(fullKey)
            ? propertyValuesMap.get(fullKey)
            : getDefault(fullKey)
      ;
    if(typeof cssProperty === 'function')
        // setter function
        cssProperty(element, value);
    else
        element.style.setProperty(cssProperty, `${cleanFn ? cleanFn(value) : value}${unit}`);
}

function _actorApplyCssProperties(propertyValuesMap, getDefault, propertiesData, element) {
    for(const propertyData of propertiesData)
        _actorApplyCssProperty(propertyValuesMap, getDefault, propertyData, element);
}
    ...
    update(changedMap) {
        ...
        if(changedMap.has('@animationProperties') || changedMap.has('globalT')) {
            const animationProperties = changedMap.has('@animationProperties')
              ...
              ;
            _actorApplyCSSColors(this.node, propertyValuesMap, colorPropertiesMap, getDefault);
            ...
            // the example
            _actorApplyCssProperties(propertyValuesMap, getDefault, propertiesData, this.node);
        }
    }
}
    ...
    update(changedMap) {
        ...
        if(changedMap.has('@animationProperties') || changedMap.has('globalT')) {
            const animationProperties = changedMap.has('@animationProperties')
              ...
              ;
            _actorApplyCSSColors(this._layerNode, propertyValuesMap, colorPropertiesMap, getDefault);
        }
    }
}
    ...
    update(changedMap) {
        ...
        if(changedMap.has('@animationProperties') || changedMap.has('globalT')) {
            const animationProperties = changedMap.has('@animationProperties')
              ...
              ;
            _actorApplyCSSColors(this.element, propertyValuesMap, colorPropertiesMap, getDefault);
            // set properties to sample...
            // console.log(`${this.constructor.name}.update propertyValuesMap:`, ...propertyValuesMap);
            ...
            // should have a value!
            _actorApplyCssProperties(propertyValuesMap, getDefault, propertiesData, this.element);
        }
    }
}
...
const REGISTERED_PROPERTIES = Object.freeze({
    NUMERIC: _frozenMapFromObject({
      ...
      , width: {'default': 0, inherit: false}
      , height: {'default': 0, inherit: false}
    })
...
`;
    ...
    async show(parentAPI, ...args) {
        ...
        if(result !== 'create')
            ...
            return this._createInchworm(parentAPI, {
                 text: 'Inchworm'
               , x: 300
               , y: 300
               , fontSize: 21
               ...
               , advancement: {type: 'repetition', value: 5}// {type:'min-distance', value:25}
               , widthParams: [
                    ['wdth', [25, 100]]
                  , ['GRAD', [150, -200]]
                  , ['YTLC', [570, 514]]
     ...
     */
    async _createInchworm(parentApi, options/*, ...args*/) {
        // create container
        // create line of text
        //     KeyFrame 0 has textContent
        // insert lineOfText into container
        // insert container into parent...
        const [containerPath, lineofTextPath] = await parentApi.changeState(()=>{
            const activeActors = parentApi.getEntry(parentApi.rootPath.append('activeActors'))
              ...
              , newMoment = keyMoments.constructor.Model.createPrimalState(keyMoments.dependencies).getDraft()
              , numericProperties = getEntry(newMoment, 'numericProperties')
              , axesLocations = getEntry(newMoment, 'axesLocations')
              ;
            newMoment.get('label').value = '0: Initialization';
            newMoment.get('textRun').value = options.text;
            newMoment.get('fontSize').value = options.fontSize;
            newMoment.get('textAlign').value = 'right';
            numericProperties.setSimpleValue('x', 0);
            numericProperties.setSimpleValue('y', 0);
            ...
            keyMoments.push(newMoment);
            const containerDraftActiveActors = containerDraft.get('activeActors')
              , lineOfTextIndex = containerDraftActiveActors.size
              ;
            containerDraftActiveActors.push(lineOfTextActorDraft);
            ...
            const containerNewPath = parentApi.rootPath.append('activeActors', containerNewIndex, 'instance')
              , lineofTextNewPath = containerNewPath.append('activeActors', lineOfTextIndex, 'instance')
              ;
            return [containerNewPath, lineofTextNewPath];
        });
        ...
        const sample = this._domTool.createElement('span', {'class': 'measure_box-sample'})
          , measureBox = this._domTool.createElement('div', {'class': 'measure_box'}, sample)
          ;
        {
            const animationProperties = parentApi.getEntry(`animationProperties@${lineofTextPath.toString()}`)
              , propertyValuesMap = animationProperties.animanion.getPropertiesFromGlobalT(0)
        ...
        // const animationProperties = parentApi.getEntry('@animationProperties')
        // animationProperties.animanion.getPropertiesFromGlobalT(globalT)
        await parentApi.changeState(()=>{
            const axesLocations = parentApi.getEntry(lineofTextPath.append('keyMoments', 0, 'axesLocations'));
            // FIXME: should be possible to use fontSize
            for(const [property, [contracted/*, expanded*/]] of options.widthParams)
        ...
        const  {width:contractedWidth} = sample.getBoundingClientRect()
          ...
          ;
        // All measuremnets done, time to finalize the worm.
        measureBox.remove();
        return await parentApi.changeState(()=>{
            const advancementSetup = options.advancement
              ...
              , lineOfTextKeyMoments = parentApi.getEntry(lineofTextPath.append('keyMoments'))
              , containerKeyMoments = parentApi.getEntry(containerPath.append('keyMoments'))
              ;
            {
                /**
                 * 0
            ...
            for(let i=0;i<iterations;i++) {
                ...
                if(i===0 && Object.hasOwn(options, 'y'))
                    // set once for all
                ...
                numericPropertiesB.setSimpleValue('t', direction * 1);
            }
        });
    }
}
