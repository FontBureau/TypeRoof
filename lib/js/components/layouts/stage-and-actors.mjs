...
function colorToCss(color) {
    ...
    if(color instanceof RGBColorModel)
        formatter = (R, G, B, A)=>`rgb(${R} ${G} ${B} / ${A})`;
    else if(color instanceof OKLABColorModel)
        formatter = (L, a, b, A)=>`oklab(${L} ${a} ${b} / ${A})`;
    else if(color instanceof OKLCHColorModel)
        formatter = (L, C, H, A)=>`oklch(${L} ${C} ${H}/ ${A})`;
    else
    ...
    for(const [componentKey, component] of color) {
        if(component.isEmpty)
            ...
            componentStrings.push(componentKey === 'A' ? '100%': 'none');
...
class KeyMomentController extends _BaseContainerComponent {
    ...
    _defineColorWidgets(settings, requireUpdateDefaults) {
        ...
        for(const [fieldName, Type] of TypeClass.fields) {
            if(Type !== ColorModel)
                continue;
            resultWidgets.push([
                {
