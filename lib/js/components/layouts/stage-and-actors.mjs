 ...
 */
class Parameter {
    constructor(name, typeHint=_NOTDEF) {
        this.name = name;
        // TODO: the same name can mean a totally different thing.
        // We don't have real typing in here anyways, but this coul be
        ...
        if(typeHint !== _NOTDEF)
            throw new Error(`NOT IMPLEMENTED Parameter "typeHint" argument `
...
function genericTypeToUIElement(ModelType, defaultVal=_NOTDEF) {
    ...
    const valueUIElements = new Map([
            [StringModel, [UILineOfTextInput, param('label')]]
          , [_AbstractEnumModel, [UISelectInput, param('label'), param('items')]]
          , [CharGroupModel, [UISelectCharGroups, CharGroupModel.charGroupsData, param('label')]]
        ])
      , valueOrEmptyUIElements = new Map([
            [StringModel, [UILineOfTextOrEmptyInput, param('getDefault'), param('requireUpdateDefaults'), param('label')]]
          , [_AbstractEnumModel, [UISelectOrEmptyInput, param('getDefault'), param('requireUpdateDefaults'), param('label'), param('items')]]
        ])
    ...
    if(!setupMap.has(SetupKey)) {
        if(defaultVal !== _NOTDEF)
            return defaultVal;
        throw new Error(`KEY ERROR can't find generic UI-Element for ModelType ${ModelType.name} `
...
class KeyMomentController extends _BaseContainerComponent {
    ...
    _defineGenericWidgets(generalSettings, updateDefaultsDependencies, requireUpdateDefaults) {
        const resultWidgets = [];
        const TypeClass =  this.parentAPI.getEntry(this.parentAPI.rootPath).constructor;
        for(const fieldName of REGISTERED_GENERIC_KEYMOMENT_FIELDS) {
            ...
            for(const arg of args) {
                ...
                switch(arg.name) {
                    ...
                    case "getDefault":
                        ...
                        // by all UIElements.
                        uiElementArgs.push(()=>this._getDefaults('', fullKey, BaseModelType.defaultValue));
                        break;
                    case "requireUpdateDefaults":
                        ...
                        dependencyMappings.push(...updateDefaultsDependencies);
                        uiElementArgs.push(requireUpdateDefaults);
                        break;
                    case "label":
