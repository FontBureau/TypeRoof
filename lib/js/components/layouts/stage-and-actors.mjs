...
const NumericPropertiesModel = _AbstractOrderedMapModel.createClass('NumericPropertiesModel', NumberModel)
  ...
  , KeyMomentsModel = _AbstractListModel.createClass('KeyMomentsModel', KeyMomentModel)
    // FIXME: copied from example-key-moments, but that has another
    // KeyMomentModel/KeyMomentsModel definiton, could be generated by
    // e.g. a function getActiveKeyMomentModel(KeyMomentsModel)
...
export class _BaseActorModel extends _AbstractStructModel {
    static createClass(className, ...definitions) {
        return super.createClass(
          ...
          // NOTE: could be nice to have these maybe just once on the stage
          // bound to the editingActor, there's rather no need to keep
          // the selected key moments for each actor, DESPITE: it's probably
          // **really** nice for editing, when jumping between actors and the
