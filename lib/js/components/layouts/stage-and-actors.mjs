...
function _keyMomentsImplementsTypography(keyMoments) {
    ...
    return (
        ...
        && Model.fields.has('axisLocations')
        && Model.fields.get('axisLocations') === AxesLocationsModel
    );
}
...
function genericTypeToUIElement(Type, defaultVal=_NOTDEF) {
    const valueUIElements = new Map([
      ...
      ;
    if(!lookupMap.has(KeyType)) {
        if(defaultVal !== _NOTDEF)
            return defaultVal;
        throw new Error(`KEY ERROR can't find generic UI-Element for Type ${Type.name} (isOrEmpty: ${isOrEmpty}).`);
    }
    return lookupMap.get(KeyType);
}
    ...
    _defineGenericWidgets(settings, updateDefaultsDependencies, requireUpdateDefaults) {
        ...
        for(const [fieldName] of REGISTERED_GENERIC_KEYMOMENT_FIELDS) {
            ...
            const FieldType = TypeClass.fields.get(fieldName)
              ...
              , UIElement = genericTypeToUIElement(FieldType)
              , ValueType = FieldType.prototype instanceof _AbstractSimpleOrEmptyModel
                    ? FieldType.Model
                    : FieldType
              ;
            resultWidgets.push([
              ...
              , UIElement
              , ()=>this._getDefaults('', fullKey, ValueType.defaultValue)
              , requireUpdateDefaults
              , _getRegisteredPropertySetup(fullKey, {label:fieldName}).label || fieldName
            ]);
        }
