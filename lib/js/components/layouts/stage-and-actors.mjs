...
class InchwormTaskAutomationDialog extends _DialogBase {
    // jshint ignore:start
    static TEMPLATE = `<dialog class="ui_dialog_inchworm">
    <form method="dialog">
      <fieldset>
        ...
        <menu>
          <div><span>Font: </span><em class="ui_dialog_inchworm-selected_font"></em></div>
          <label>Text <input
                  class="ui_dialog_inchworm-text"
                  type="text"
                  placeholder="Inchworm"
                  value="" />
          </label>
          <label>x <input
                  class="ui_dialog_inchworm-x"
                  type="number"
                  value="0" />px
          </label>
          <label>y <input
                  class="ui_dialog_inchworm-y"
                  type="number"
                  value="0" />px
          </label>
          <input type="hidden" class="ui_dialog_inchworm-font" value="(not initialized)" />
          <!-- This is a bit more complicated, but the urgency is big
          <label> Font <select class="ui_dialog_inchworm-font">
          </select></label>
          -->
          <label>Font-Size <input
                  class="ui_dialog_inchworm-font_size"
                  type="number"
                  min="8"
                  value="{set default/current}" />pt
          </label>
          <label> Advancement By <select class="ui_dialog_inchworm-advancement_type">
          ...
          <label>Advancement Value <input
                  class="ui_dialog_inchworm-advancement_value"
                  type="number"
                  value="1" />
          </label>
          <!--
...
`;
    ...
    _initTemplate() {
        ...
        for(const entry of [
            ...
            const [key, selectorPart] = typeof entry === 'string' ? [entry, entry] : entry
              , selector = `.ui_dialog_inchworm-${selectorPart !== undefined ? selectorPart : key}`
    ...
    _getFont(parentAPI) {
        const currentRoot = parentAPI.getEntry(parentAPI.rootPath)
          , font = currentRoot.has('font')
                ...
                // FIXME: stage doesn't have it, but it probably should have its.
    ...
    async show(parentAPI, ...args) {
        ...
        if(result !== 'create')
            ...
            return this._createInchworm(parentAPI, {
                 text: this._formElements.get('text').value || 'Inchworm'
    ...
    async _createInchworm(parentAPI, options/*, ...args*/) {
        // console.log(`${this}._createInchworm options:`, options);
        // create container
        // create line of text
