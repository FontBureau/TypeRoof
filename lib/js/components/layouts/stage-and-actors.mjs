...
class StageManager extends _BaseComponent {
    ...
    _renderLayer(path, activeActors, state=null) {
        ...
        if(activeActors.size === 0) {
            // empty container
            const item = {path, isEmptyLayer: true};
        ...
        for(const [key, actor] of activeActors) {
            const itemElement = this._domTool.createElement('li', {'class': (state?.counter % 2) ? 'even-row' : 'odd-row'})
              , itemPath = path.append(key)
              , dragHandleElement = this._domTool.createElement('span', {'class': 'drag_handle'}, '✥')
              , item = {path: itemPath, isEmptyLayer: false}
              ;
    ...
    update(changedMap) {
        ...
        if(changedMap.has('activeActors')) {
            ...
            this._actorsElement.append(...this._renderLayer(basePath, activeActors, {counter: 0}));
