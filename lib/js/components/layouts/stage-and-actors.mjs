...
function* colorsGen(outerAnimanionAPI, keyMoment/*, momentT*/) {
    for(const fieldName of getColorModelFields(keyMoment.constructor)) {
        ...
        //}
        // yield [`${COLOR}${fieldName}`, colorToCulori(color)];
    }
}
    ...
    update(changedMap) {
        ...
        if(changedMap.has('animationProperties@') || changedMap.has('globalT')) {
            ...
            for(const [property, value] of propertyValuesMap) {
                ...
                if(consumed)
                    continue;
                if(property.startsWith('color/')) // colors/ is the key
...
export class UINumericProperties extends _BaseComponent {
    constructor (widgetBus, REGISTERED_PROPERTIES, getDefaults=null, requireUpdateDefaults=()=>false) {
        super(widgetBus);
        this._REGISTERED_PROPERTIES = REGISTERED_PROPERTIES;
    ...
    propertiesHas(key) {
        return this._REGISTERED_PROPERTIES.has(key);
    }
    propertiesGet(key) {
        if(!this.propertiesHas(key))
            throw new Error(`KEY ERROR ${key} not found in REGISTERED_PROPERTIES[NUMERIC].`);
        return this._REGISTERED_PROPERTIES.get(key);
    }

    propertiesKeys() {
        return this._REGISTERED_PROPERTIES.keys();
    }
    ...
    _defineWidgets(TypeClass, typeKey, localZoneElement
        ...
        return [
          ...
          , [
              ...
              , UINumericProperties
              , REGISTERED_PROPERTIES[NUMERIC]
              , this._getDefaults.bind(this, NUMERIC)
              , requireUpdateDefaults
            ]
