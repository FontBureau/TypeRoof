...
class ActorRendererContainer extends _BaseContainerComponent {
    constructor(parentAPI, zones, ActorModel) {
        const commonDependencyMapping = [// 't', 'keyMoments', 'isLoop']
                'keyMoments'
          ...
          , widgets = [
            [
              ...
              , [
                    ...commonDependencyMapping
                  , [`animationProperties@${parentAPI.rootPath.append('..' ,'..', '..')}`, '@parentProperties']
                ]
              , AnimationLiveProperties
              , function initAnimanion(keyMoments, isLoop, parentProperties) {
                    return new Animanion(parentProperties.animanion, numericPropertiesGen, keyMoments, isLoop);
                }
            ]
          , [
                { zone: 'layer' }
        ...
        const widgets = [
          ...
          , [
              ...
              , function initAnimanion(keyMoments, isLoop) {
                    return new Animanion(null, numericPropertiesGen, keyMoments, isLoop);
                }
            ]
    ...
    update(changedMap) {
        if(changedMap.has('keyMoments')) {
            ...
            for(const [, keyMoment] of keyMoments) {
                for(const propertiesKey of ['numericProperties']) {
    ...
    _createStageWrappers() {
        ...
        const widgets = [
            [
              ...
              , [
                    'keyMoments'
                  , [`animationProperties@${widgetRootPath}`, '@animationProperties']
                ]
...
class StageAndActorsController extends _BaseContainerComponent {
    constructor(parentAPI, _zones) {
        ...
        super(parentAPI, zones, widgets);
        this._protocolHandlers = Object.fromEntries(protocolHandlers);
    ...
    update(...args) {
        this._protocolHandlers['animationProperties@'].resetUpdatedLog();
        super.update(...args);
    }
    initialUpdate(...args) {
        this._protocolHandlers['animationProperties@'].resetUpdatedLog();
