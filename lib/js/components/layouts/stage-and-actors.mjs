...
function initAnimanion(keyMoments, isLoop, parentProperties=null) {
    const localScopeAnimanion = (()=>{
        // NOTE: since all the keyMomentsAnalysis is cached in these
        // animanions, it may be smart to only update that when the
        // keyMomennts/isLoop have changed. Local cache would also
        // benefit from that. We really don't want to calculate all of
        // these all the time. Also, we could pass these generators
        // in a row and have them internally stiched together, there's
        // no need to crate a new keyMomentsAnalysis for each cascading
        // generator, that operates on the exact same data.
        // i.e. eventually:
        const propertyGenerators = [numericPropertiesGen, fontSizeGen, axisLocationsGen];
        return new LocalScopeAnimanion(propertyGenerators, keyMoments, isLoop);
        // const numericAnimanion = new LocalScopeAnimanion(null, numericPropertiesGen, keyMoments, isLoop);
        // // add typography?
        // if(!(keyMoments instanceof TypographyKeyMomentsModel))
        //     return numericAnimanion;
        // const fontSizeAnimanion = new LocalScopeAnimanion(numericAnimanion, fontSizeGen, keyMoments, isLoop);
        // return new LocalScopeAnimanion(fontSizeAnimanion, axisLocationsGen, keyMoments, isLoop);
    })();

    return localScopeAnimanion;
    // return new HierarchicalScopeAnimanion(
    //         localScopeAnimanion
...
class CommonActorProperties extends _BaseComponent {
    ...
    update(changedMap) {
        if(changedMap.has('keyMoments')) {
            ...
            this._isTypographic = keyMoments instanceof TypographyKeyMomentsModel;
            const propertyContainerKeys = this._isTypographic
                        ? ['numericProperties', 'axesLocations']
                        : ['numericProperties']
                        ;
            this.ownProperties.clear();
            // TODO: this could be maybe done in @animationProperties as
            // well, as it has all the tools to iterate the properties.
            // Would probably have to be implemented there, as it is not.
            for(const [, keyMoment] of keyMoments) {
                if(this._isTypographic && !this.ownProperties.has('fontSize')) {
                    const fontSize = keyMoment.get('fontSize');
                    if(!fontSize.isEmpty)
                        this.ownProperties.add('fontSize');
                }
                for(const propertiesKey of propertyContainerKeys) {
                    const propertyContainer = keyMoment.get(propertiesKey);
                    for(const property of propertyContainer.keys()) {
                        this.ownProperties.add(`${propertiesKey}/${property}`);
                    }
                }
            }
        }
        ...
        if(changedMap.has('@animationProperties')) {
            const animationProperties = changedMap.get('@animationProperties')
              ...
              ;
            for(const [property, value] of animationProperties.propertyValuesMap) {
