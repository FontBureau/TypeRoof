...
class SimpleClockTaskAutomationDialog extends _DialogBase {
    // jshint ignore:start
...
`;
    ...
    async show(parentAPI, ...args) {
        ...
        if(result !== 'create')
            ...
            return this._create(parentAPI, {}, ...args);
    }
    ...
    async _create(parentAPI, /*options, ...args*/) {
        ...
        return await parentAPI.changeState(()=>{
            const activeActors = parentAPI.getEntry(parentAPI.rootPath.append('activeActors'))
              ...
              , root = parentAPI.getEntry(parentAPI.rootPath)
            ...
            // insert : separators
            for(const [i, separatorActiveActors] of enumerate([hoursActiveActors, minutesActiveActors])) {
