...
`;
    ...
    async show(...args) {
        ...
        if(result !== 'create')
            ...
            return this._createInchworm({}, ...args);
    }

    _createInchworm(options, ...args) {
        console.log(`${this}._createInchworm options:`, options);
    }
}


class UISelectTaskAutomation extends _BaseContainerComponent {
    //jshint ignore:start
    ...
    async _executeItem(key) {
        ...
        const [/*label*/, TaskAutomationDialog] = this._items.get(key)
          , dialog = new TaskAutomationDialog(this._domTool)
          ;

        try {
          result = await dialog.show(/** TODO: requires context! **/);
        }
        catch(error) {
            console.error(`TASK_AUTOMATIOND_IALOG ERROR ${this}._executeItem with "${key}"`, error);
        }
        finally {
            dialog.destroy();
        }
