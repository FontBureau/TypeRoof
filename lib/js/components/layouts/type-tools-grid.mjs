...
function getTypeToolsGridPPSMap(parentPPSRecord, TypeSpecType) {
    const entries = [];
...
const TYPETOOLSGRID_PPS_MAP = getTypeToolsGridPPSMap(null, TypeToolsGridModel);
console.log('TYPETOOLSGRID_PPS_MAP ...')
for(const [key, value] of TYPETOOLSGRID_PPS_MAP)
...
class TypeTensorController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        ...
        const dimensionsHostPath = Path.fromString(this.widgetBus.getExternalName('dimensionsHost'))
          , typeSpecPath = dimensionsHostPath
          ;
        ...
        const widgets = [
          ...
          , [
           ...
           , ... ['textColor', 'backgroundColor'].map(fieldName=>{
                    const ppsRecord = TYPETOOLSGRID_PPS_MAP.get(fieldName)
                    return [
                        {
          ...
          , [
              ...
              , getDefaults
              , TYPETOOLSGRID_PPS_MAP.get('axesLocations')
...
// NOTE!!! the pathSpec thingy within type-spec-ramp is similar!...
function* dimensionSequencePropertyGen(fieldName, dimensionSequence) {
    console.log(`XXXXX>dimensionSequencePropertyGen ${fieldName} dimensionSequence:${dimensionSequence}`);
    for( const [propertyName, value] of Object.entries(dimensionSequence)) {
        // Will also yield [`${COLOR}${fieldName}/mode`, colorMode] as
        // that's an entry in targetCuloriColor.
        console.log(`XXXXXXX>dimensionSequencePropertyGen`, `${DIMENSION}${fieldName}/${propertyName}`, value);
        yield [`${DIMENSION}${fieldName}/${propertyName}`, value];
    }
}

// This will produce:
//  * {prefix to leading/} axis
//  * {prefix to leading/} start
//  * {prefix to leading/} end
//
//  * {prefix to leading/} stepping/type
//  * {prefix to leading/} stepping/amountValue (MAYBE if stepping/type==='amount')
//  * {prefix to leading/} stepping/stepSizeValue (MAYBE if stepping/type==='stepSize')
const PATH_SPEC_AXIS_SELECTION = [['type', 'fontAxisTagValue', 'exoAxisTagValue']] // GridAxisSelectionModel
  , PATH_SPEC_AXES_MATH_AXIS_LOCATION_VALUE = [['logicalValue', 'numericValue']]  // AxesMathAxisLocationValueModel
  , PATH_SPEC_STEPPING = [['type', 'amountValue', 'stepSizeValue']] // SteppingModel
  , PATH_SPEC_RANGE_DIMENSION = [
        [['axis'], ...PATH_SPEC_AXIS_SELECTION]
      , [['start', 'end'], ...PATH_SPEC_AXES_MATH_AXIS_LOCATION_VALUE]
      , [['stepping'], ...PATH_SPEC_STEPPING]
    ]
  , PATH_SPEC_EXPLICIT_DIMENSION = [
        [['axis'], ...PATH_SPEC_AXIS_SELECTION]
      , [['values'], function*(items){for(let i=0, l=items.size;i<l;i++) yield i;}, ...PATH_SPEC_AXES_MATH_AXIS_LOCATION_VALUE] //AxesMathAxisLocationValuesModel: how to specify a list of PATH_SPEC_AXES_MATH_AXIS_LOCATION_VALUE?
    ]
  ;

function* dimensionsGen(_outerAPI, hostInstance) {
     console.log(`>>>>>dimensionsGen _outerAPI:${_outerAPI} hostInstance:${hostInstance}`);
    for(const fieldName of getDimensionModelFields(hostInstance.constructor)) {
        console.log(`>>>>>dimensionsGen ${fieldName}`);
       const PREFIX = `${DIMENSION}${fieldName}`
          ...
          ;
        console.log(`>>>>> dimensionSequence:${dimensionSequence} algorithm:`, algorithm);
        dimensionSequencePropertyGen(fieldName, dimensionSequence)
        ...
        // NULL Range Explicit
        console.log('dimensionSequence', dimensionSequence);
        ...
        if(algorithm === 'Range') {
            yield* pathSpecValuesGen(PATH_SPEC_RANGE_DIMENSION, PREFIX, data);

            // That should likeley become the actual sequence value as an Array!
            //yield [SEQUENCE, new AutoLinearLeadingSyntethicValue(PREFIX)];
        }
        else if('Explicit') {
            yield* pathSpecValuesGen(PATH_SPEC_EXPLICIT_DIMENSION, PREFIX, data);
        }
...
function _getTypeToolsGridDefaultsMap(dependencies) {
    console.log('TYPETOOLSGRID_PPS_MAP', TYPETOOLSGRID_PPS_MAP);
    const defaultTypeToolsGrid = (()=>{
            const draft = TypeToolsGridModel.createPrimalDraft(dependencies);
            for(const [fieldName, ppsRecord] of TYPETOOLSGRID_PPS_MAP) {
                if(_skipPrefix.has(ppsRecord.prefix)) {
                    console.log(`_getTypeToolsGridDefaultsMap SKIP ${fieldName} :::: ${ppsRecord}`);
                    continue;
                }
                console.log(`_getTypeToolsGridDefaultsMap TAKE ${fieldName} :::: ${ppsRecord}`);
                if(ppsRecord.prefix == COLOR) {
                    ...
                    console.error(`_getTypeToolsGridDefaultsMap GOT LEADING ${fieldName} ${ppsRecord}`);
                    continue;
                ...
                // to check here! However, if the pps
                    ...
                    // >> _getTypeToolsGridDefaultsMap FALLBACK dimensionX <ProcessedPropertiesSystemRecord dimension/dimensionX dimension/,dimensionX>
                    ...
                    const defaultValue = typeToolsGridGetDefaults(()=>null, ppsRecord, fieldName);
                    console.error(`_getTypeToolsGridDefaultsMap FALLBACK ${fieldName} ${ppsRecord} defaultValue:`, defaultValue);
...
class TypeToolsGridController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        const gridManagerContainer = widgetBus.domTool.createElement('fieldset', {'class': 'grid-manager'})
          ...
          , gridRelativePath = Path.fromParts('.')
          , originTypeToolsGridPath = widgetBus.rootPath.append(...gridRelativePath);
          ;
        ...
        const typeToolsGridDefaultsMap = _getTypeToolsGridDefaultsMap(widgetBus.getEntry(originTypeToolsGridPath).dependencies);
        const widgets = [
