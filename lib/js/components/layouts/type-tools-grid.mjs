...
} from './type-stage/type-specnion.mjs';
import {
    axisLocationsGen
  , openTypeFeaturesGen
  , languageTagGen
...
} from './type-stage/live-properties.typeroof.jsx';
import {
    SyntheticValue
} from './type-stage/synthetic-values.mjs';
...
function getGridPropertiesPPSMap(parentPPSRecord, Type) {
    ...
    for(const [modelFieldName, modelFieldType] of Type.fields.entries()) {
        ...
        else if(modelFieldName === 'languageTag')
            prefix = LANGUAGE
 ...
 */
class UIDimensionSequence extends _BaseTypeDrivenContainerComponentMixin(
                                        _BaseDynamicCollectionContainerComponent) {
...
export class AddOrAndDragButton extends _BaseComponent {
    ...
    initTemplate(label) {
        const frag = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE)
          ...
          ;
        this._domTool.clear(element);
        this._domTool.insert(element, 'append', createIconAndLabel('drag_pan', label));
...
const uiElementMap = new Map([
  ...
  , orEmptyUIElementMap = new Map([
        [FontAxisTagModel, [UILineOfTextInput
          ...
          , require('settings:@set', ['activationTest', function(getEntry){
                    // getEntry is injected by ComponentWrapper and only
                    // serves declared dependencies ('value' is declared
                    // via settings:internalPropertyName above).
                const value = getEntry('value');
...
class TypeTensorController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        ...
        this._availableDimensions = this._determineAvailableDimensions(dimensionsHostPath);
            ...
            // it's an artifact of motion-stage, but maybe it's also
        ...
        const widgets = [
          ...
          , [
           ...
           , ... ['textColor', 'backgroundColor'].map(fieldName=>{
            ...
            // FIXME: see comment as below for UIManualAxesLocations
          , [
                {
                    zone: 'main'
          ...
          , [
              ...
              , [
                  ...
                  , ['autoOPSZ', 'autoOPSZ']
                  // no need for this, as we don't have inheritance
                  // at this level.
                    // , ...updateDefaultsDependencies <- empty
                ]
              , UIManualAxesLocationsCollapsible // (widgetBus, getDefaults=null, ppsRecord=null, requireUpdateDefaults=()=>false
              , zones
              , getDefaults
              , GRID_PROPERTIES_PPS_MAP.get('axesLocations')
              , requireUpdateDefaults // always true
            ]
          , [
                {
...
const REGISTERED_GENERIC_TYPE_TOOLS_GRID_FIELDS = Object.freeze(new FreezableSet([
        // script+language!
        'textAlign', 'direction', 'lineLength'
    ]))
...
class UIGridScaffold extends _BaseComponent {
    static TEMPLATE = `<div class="ui_type_tools_grid"></div>`;
    static TEMPLATE_ROW = `<div class="ui_type_tools_grid-row"></div>`;
    static TEMPLATE_CELL = `<div class="ui_type_tools_grid-cell">
        <div class="ui_type_tools_grid-cell-content"></div>
    </div>`
    constructor(widgetBus, cellContents, styleBackgroundElsewhereFN=null) {
        super(widgetBus);
        this._cellContents = cellContents;
        this._cellElements = [];
        [this.element] = this._initTemplate();
        // A rough hack, to be able to use the grid background as a general
        // background color of the layout. When used as an actor, this
        // should not rather not be used. It's also not a good solution
        // for all if the cases, just quick and dirty for now.
        this._styleBackgroundElsewhereFN = styleBackgroundElsewhereFN;
    }
    _initTemplate() {
    ...
    update(changedMap) {
        ...
        const _dimensionsCache = []
          ...
          , getDimensions=(propertyValuesMap)=>{
                if(!_dimensionsCache.length) {
                    const dimensions = [DIMENSION_X, DIMENSION_Y].map(
                        dimensionKey=>getDimensionFromPropertyValuesMap(
                            propertyValuesMap, `${DIMENSION}${dimensionKey}`));
                    _dimensionsCache.push(...dimensions)
                }
                return _dimensionsCache;
            }
          ;

        if(changedMap.has('@properties') || changedMap.has('showCaptions')
                        || changedMap.has('verboseFontVariationSettings')) {
            cellsChanged = true;
            const dimensionLabel = dimension=>`${dimension.axisTag}`// [${dimension.axisType}, ${dimension.algorithm}]`
              , compactCaptionData = data=> (typeof data !== 'number' || data % 1 === 0)
                            ? `${data}`
                            : data.toFixed(3)
              , properties = changedMap.has('@properties')
                        ? changedMap.get('@properties')
                        : this.getEntry('@properties')
              , propertyValuesMap = properties.typeSpecnion.getProperties()
              , showCaptions = (changedMap.has('showCaptions')
                        ? changedMap.get('showCaptions')
                        : this.getEntry('showCaptions')
                ).value
              ;

            if(this._styleBackgroundElsewhereFN)
                this._styleBackgroundElsewhereFN(propertyValuesMap);
            ...
            // dimension/dimensionX/axis/type, dimension/dimensionX/axis/tag, dimension/dimensionX/sequence
            const [dimensionX, dimensionY] = getDimensions(propertyValuesMap);
            const rows = []

            const xLabels = dimensionX.algorithm !== 'NULL' && showCaptions
              , yLabels = dimensionY.algorithm !== 'NULL' && showCaptions
              ;
            if(yLabels) {
                const captionColumn = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_ROW).firstElementChild
                , label = this._domTool.createElement('div', {class: 'ui_type_tools_grid-labels-caption-cell'}, `${dimensionLabel(dimensionY)}`)
                ;
                captionColumn.classList.add('ui_type_tools_grid-labels-caption');
                captionColumn.classList.add('ui_type_tools_grid-labels-caption-y');
                captionColumn.append(label);
                rows.push(captionColumn);
            }
            if(xLabels) {
                const captionRow = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_ROW).firstElementChild
                  , label = this._domTool.createElement('div', {class: 'ui_type_tools_grid-labels-caption-cell '}, `${dimensionLabel(dimensionX)}`)
                  , dataRow = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_ROW).firstElementChild
                  ;
                captionRow.classList.add('ui_type_tools_grid-labels-caption');
                captionRow.classList.add('ui_type_tools_grid-labels-caption-x');
                dataRow.classList.add('ui_type_tools_grid-labels-data')
                captionRow.append(label);
                if(yLabels) {
                    const emptyCell = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_CELL).firstElementChild;
                    emptyCell.classList.add(('ui_type_tools_grid-labels-empty'));
                ...
                for(let x=0, l=dimensionX.sequence.length; x<l; x++) {
                    ...
                    cell.classList.add(('ui_type_tools_grid-labels-data'))
                    cell.textContent = `${compactCaptionData(dimensionX.sequence[x])}`;
                    dataRow.append(cell);
                }
                rows.push(captionRow, dataRow);
            }
            for(let y=0, l=dimensionY.sequence.length; y<l; y++) {
                ...
                if(yLabels) {
                    const cell = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_CELL).firstElementChild;
                    cell.classList.add(('ui_type_tools_grid-labels-data'))
                    cell.textContent = `${compactCaptionData(dimensionY.sequence[y])}`;
                    row.append(cell);
                }
                const initialCellIndex = dimensionX.sequence.length * y;
                for(let x=0, l=dimensionX.sequence.length; x<l; x++) {
                    const cellIndex = initialCellIndex + x;
                    // FIXME: newComponentData is not required as it! But,
                    // we could register [dimensionX, x], [dimensionY, y]
                    // with it, and that way store the information required
                    // to update the styling, i.e. to update the localPropertyValuesMap
                    // So, it's left here as a stub.
                    const newComponentData = {x, y}
                      , identifier = this._cellContents.getId(cellIndex)
                      ;
                    if(this._cellContents.hasRegistered(identifier)) {
                        const component = this._cellContents.getRegistered(identifier)
                        Object.assign(component, newComponentData);
                    }
                    else {
                        const cell = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE_CELL).firstElementChild
                          , cellContentElement = cell.querySelector('.ui_type_tools_grid-cell-content')
                          , component = {element:cellContentElement, ...newComponentData}
                          ;
                        const unregister = this._cellContents.register(identifier, component);
                        this._cellElements[cellIndex] = [cell, unregister];
                    }

                    this._cellContents.setUpdated(identifier);
                    const [cell] = this._cellElements[cellIndex];
                    row.append(cell);
                ...
                for(const stop=dimensionX.sequence.length * dimensionY.sequence.length; stop<this._cellElements.length;) {
                    const [cell, unregister] = this._cellElements.pop();
                    cell.remove();
                    unregister();
                }
            }
        ...
        if(changedMap.has('showParameters') || cellsChanged) {
            const PARAMETERS_ELEMENT_CLASS = 'ui_type_tools_grid-cell-parameters'
               , showParameters = (changedMap.has('showParameters')
                    ? changedMap.get('showParameters')
                    : this.getEntry('showParameters')
                ).value;
            if(!showParameters) {
                // changed and remove
                for(const [cell] of this._cellElements) {
                    cell.querySelector(`.${PARAMETERS_ELEMENT_CLASS}`)?.remove();
                }
            }
                ...
                const properties = changedMap.has('@properties')
                        ...
                        : this.getEntry('@properties')
                  , propertyValuesMap = properties.typeSpecnion.getProperties()
                  , [dimensionX, dimensionY] = getDimensions(propertyValuesMap);
                  ;
                // add or update
                ...
                for(let y=0, l=dimensionY.sequence.length; y<l; y++) {
                    ...
                    for(let x=0, l= dimensionX.sequence.length; x<l; x++) {
                        const cellIndex = initialCellIndex + x
                            ;
                        const [cell] = this._cellElements[cellIndex]
                          , localPropertyValuesMap = _applyDimensions(propertyValuesMap, [dimensionX, x], [dimensionY, y])
                          ;
                        let parametersElement = cell.querySelector(`.${PARAMETERS_ELEMENT_CLASS}`);
                        if(!parametersElement) {
                            parametersElement = this._domTool.createElement(`div`, {class: PARAMETERS_ELEMENT_CLASS});
                            cell.append(parametersElement);
                        }
                        renderAxesParameterDisplay(parametersElement, localPropertyValuesMap, {
                            verboseFontVariationSettings: this.getEntry('verboseFontVariationSettings').value
                          , font: localPropertyValuesMap.get(
                                ProcessedPropertiesSystemMap.createSimpleRecord(SPECIFIC, 'font').fullKey)
                        });
                    }
                }
            }
        ...
        // when backgroundColor is changed or showParameters is turned on
        // We could cache backgroundColor instead of redoing this whenever
        // @properties have changed.
        if(changedMap.has('@properties') || changedMap.has('showParameters')
                                    && changedMap.get('showParameters').value ) {
            const properties = changedMap.has('@properties')
                        ? changedMap.get('@properties')
                        : this.getEntry('@properties')
              , propertyValuesMap = properties.typeSpecnion.getProperties()
                //extract `${COLOR}backgroundColor from propertyValuesMap
              , [backgroundColor, ] = getColorFromPropertyValuesMap(`${COLOR}backgroundColor`, propertyValuesMap, [null])
              ...
              , computedStyle = this._domTool.constructor.getComputedStyle(this.element)
            ...
            if(enhancedContrastColor)
                this.element.style.setProperty('--parameters-label-high-contrast-text-color', culoriFormatCss(enhancedContrastColor));
            else
                // unset, will fall-back to whatever CSS defines
                this.element.style.removeProperty('--parameters-label-high-contrast-text-color');
        }
    }
    ...
    _styleCell(element, propertyValuesMap) {
        ...
        const colorPropertiesMap = [
          ...
          ;
        if(!this._styleBackgroundElsewhereFN)
            colorPropertiesMap.push([`${COLOR}backgroundColor`, 'background-color']);

        actorApplyCSSColors(element, propertyValuesMap, getDefault, colorPropertiesMap);
        setTypographicPropertiesToSample(element, propertyValuesMap, false, {
            verboseFontVariationSettings: this.getEntry('verboseFontVariationSettings').value
          , font
        });
...
class UIGridCellsProvider extends _BaseContainerComponent {
    constructor(widgetBus, _zones, cellContents) {
        const zones = new Map(_zones);
        super(widgetBus, zones);
        this._cellContents = cellContents;
        this._widgetsByIdentifiers = new Map();
    }

    _createWrapperKey(identifier) {
        const childWidgetBus = this._childrenWidgetBus
          , settings = {
                zone: identifier
            }
          , dependencyMappings = [
                // Declare the cellContent@ protocol dependency, so the
                // update-relevance filter can see when this cell was
                // marked updated via cellContent@.setUpdated — with an
                // empty protocol mapping the widget is judged irrelevant
                // and skipped, which also keeps UIGridCellsProvider (its
                // relevance comes from descendants) from provisioning
                // newly registered cells.
                [identifier, 'cellContent@']
              , ['document', 'document']
          ]
          , Constructor = ProseMirror
          , args = [proseMirrorDefaultSchema, {} /* idMap */,null /* originTypeSpecPath */ , ['editor-simple']]
          ;
    ...
    _provisionWidgets() {
        ...
        const toDelete = new Set(this._widgetsByIdentifiers.keys());

        for(const [identifier, {element}] of this._cellContents) {
            toDelete.delete(identifier);
            if(this._widgetsByIdentifiers.has(identifier))
                // no need to init
                continue;
            // init new widget
            this._zones.set(identifier, element);
            const widgetWrapper = this._createWrapperKey(identifier);
            this._createWidget(widgetWrapper);
            this._widgetsByIdentifiers.set(identifier, widgetWrapper);
            requiresFullInitialUpdate.add(widgetWrapper);
        }

        // delete deletedWidgets
        for(const identifier of toDelete) {
            const widgetWrapper = this._widgetsByIdentifiers.get(identifier);
            this._widgetsByIdentifiers.delete(identifier);
            this._zones.delete(identifier);
            this._destroyWidget(widgetWrapper);
        }
        this._widgets.splice(0, Infinity, ...this._widgetsByIdentifiers.values());
        return requiresFullInitialUpdate;
    }
}

class UIGrid extends _BaseContainerComponent {
    constructor(widgetBus, zones, styleBackgroundElsewhereFN) {
        const [
            protocolHandler, cellContentsProtocolHandlerImp
        ] = SimpleProtocolHandler.create('cellContent@', {treatAdressAsRootPath: false});
        widgetBus.wrapper.setProtocolHandlerImplementation(protocolHandler, cellContentsProtocolHandlerImp);
        super(widgetBus, zones);
        this._initWidgets([
            [
                {
              ...
              , UIGridScaffold
              , cellContentsProtocolHandlerImp
              , styleBackgroundElsewhereFN
            ]
          , [
                {}
              , [
                     // <= DO i even have to? seems like UIGridCellsProvider
                     // will run provisionWidgets in any case!
                    // '@cells'
                ]
                // initialize content handler per cell
                // NOTE: we will repeat the DOM content a lot, however we
                // need to do this still, because we're going to change the
                // styling per cell. Also,
              , UIGridCellsProvider
              , zones
              , cellContentsProtocolHandlerImp
            ]
...
class TypeToolsGridController extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        const gridManagerContainer = widgetBus.domTool.createElement('div', {'class': 'grid-manager'})
          , generalControlsContainer = widgetBus.domTool.createElement('div', {class: 'sidebar-group'})
          , gridLayoutContainer = widgetBus.domTool.createElement('div', {'class': 'ui_type_tools_grid_layout'})
          , zones = new Map([..._zones
                , ['grid-manager', gridManagerContainer]
                , ['general', generalControlsContainer]
                , ['grid-layout', gridLayoutContainer]
                ])
        ...
        const widgets = [
            ...
            [
              ...
              , [inheritancePolicyGen] // consistent with
                // type-stage/index.typeroof.jsx
          ...
          , [
              ...
              , UICheckboxInput
              , 'show-captions'// classToken
              , getRegisteredPropertySetup('generic/showCaptions').label //label
            ]
          , [
                {
                    zone: 'general'
          ...
          , [
              ...
              , getRegisteredPropertySetup('generic/verboseFontVariationSettings').label //label
            ]
          , [
                {
                    zone: 'grid-manager'
          ...
          , [
              ...
              , UIGrid
              , zones
                // use the cell background as general layout background
              , function styleBackgroundElsewhereFN(propertyValuesMap) {
                    const colorPropertiesMap = [
