...
export function validateColumnConfig(raw) {
    const errors = [],
        warnings = [];
    // Unset minLineLength is fine — normalize defaults it to 1 (a
    // no-op constraint, friendly for exploration).
    if (raw.minLineLength !== undefined && !(raw.minLineLength > 0))
        errors.push("minLineLength must be > 0.");
    if (raw.minPadding !== undefined && !(raw.minPadding >= 0))
        errors.push("minPadding must be >= 0.");
    // columns may be EMPTY: with growth (Infinity or >= 1) it's pure
    // min-driven growth; with growColumns false it's an explicit
    // single-column, no-growth layout (full line, minPadding margins).
    ...
    const gap = createColumnGap(raw.columnGap),
        // Unset ≡ 1, same default as normalize — raw.minLineLength may
        // be undefined here, and gap.fn(undefined) produces NaN noise.
        minLineLength = raw.minLineLength ?? 1;
    for (const [i, maxLine] of raw.columns.entries()) {
        const n = i + 1;
        if (n === raw.columns.length) break;
        const maxNaturalEn =
                n * maxLine + (n - 1) * (n > 1 ? gap.fn(maxLine) : 0),
            minEntryEn = (n + 1) * minLineLength + n * gap.fn(minLineLength);
...
// columnConfig: output of normalizeColumnConfig. Never returns null:
// degenerate budgets (non-positive width, minPadding reserve eats the
// width) render the overflow layout — the minimum line plus reserve,
// wider than the available width (the host scrolls).
export function calculateHorizontalRunion(availableWidthEn, columnConfig) {
    const reserve = columnConfig.minPadding ?? 0,
        budget = availableWidthEn - reserve;
    const { minLineLength, columns, growColumns, gap, paddingRatio } =
        ...
        maxColumns =
            growColumns === false
                ? // Empty list + no growth: exactly one column.
                  Math.max(1, columns.length)
                : growColumns === Infinity
                  ? Math.floor(budget / Math.min(minLineLength, ...columns)) + 1 // bounded
        ...
        minLineLengthFor = (count) =>
            // count is a 1-based column count; min(min, maxFor(count)):
            // config maxes below the general minimum waive min — the
            // config is stronger (drives both the reject-check here
            // and the overflow fallback below).
            Math.min(minLineLength, maxLineLengthFor(count));
    // Degenerate budget (non-positive width, or the minPadding reserve
    // eats it all): not "no layout" — the minimum still renders, as
    // overflow, the same as the fallback after the passes.
    if (!(budget > 0))
        return compose(1, minLineLengthFor(1), 0, reserve, paddingRatio);
    ...
    if (columns.length)
        for (let count = 1; count <= maxColumns; count++) {
            const [lineLengthEn, gapEn] = gap.solve(budget, count);
            if (
                lineLengthEn >= minLineLengthFor(count) &&
                lineLengthEn <= maxLineLengthFor(count)
            )
                // The reserve is padding here, not zero.
                return compose(
                    count,
                    lineLengthEn,
                    gapEn,
    ...
    // padding, until minPadding).
    for (let count = maxColumns; count >= 1; count--) {
        const [naturalLineEn] = gap.solve(budget, count);
        // Min inclusive, same boundary as pass 1.
        ...
        const lineLengthEn = Math.min(naturalLineEn, maxLineLengthFor(count)),
            ...
            // Remainder against the ORIGINAL width: reserve is
            // re-added, then split by paddingRatio.
            paddingEn =
                availableWidthEn - count * lineLengthEn - (count - 1) * gapEn;
        return compose(count, lineLengthEn, gapEn, paddingEn, paddingRatio);
    }
    // OVERFLOW fallback: nothing fits — not even one column at the
    // minimal line length. Never shrink below the (config-aware)
    // minimum; render one column wider than the available width and
    // let the host scroll (the stage CSS has overflow-tolerant widths).
    const overflowLineEn = minLineLengthFor(1);
    return compose(1, overflowLineEn, 0, reserve, paddingRatio);
}
