...
export function validateColumnConfig(raw) {
    ...
    if (
        ...
        !(
            Number.isInteger(raw.growColumns) &&
            raw.growColumns >= raw.columns.length
        )
    )
        errors.push(
            "growColumns must be false, Infinity, or an integer >= columns.length.",
        );
    if (errors.length)
        throw new Error(
...
export function normalizeColumnConfig(raw) {
    ...
    return Object.freeze({
        ...
        paddingRatio: Object.freeze(
            Array.from(raw.paddingRatio ?? [3 / 5, 2 / 5]),
        ),
    });
}
...
function compose(columns, lineLengthEn, columnGapEn, paddingEn, [left, right]) {
    return {
        ...
        lineLengthEn,
        columnGapEn,
        marginLeftEn: paddingEn * left,
        marginRightEn: paddingEn * right,
    };
}
