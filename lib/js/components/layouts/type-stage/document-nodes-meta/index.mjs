...
import { HierarchicalScopeNodeProperties } from "../node-properties.mjs";
...
class DocumentNodesMetaNode extends _BaseContainerComponent {
    ...
    // The nearest node-properties scope above this dispatcher: the
    // child container passes it down (see
    // DocumentNodesMetaNodes._createWrapper). null only for the
    // top-level nodes (their container is provisioned alongside the
    // document root and passes no arg) — they fall back to the root
    // scope via the "rootNodeProperties@" mapping.
    ...
    // The dispatcher's parent answer as a scope-like object (a
    // component answering .getProperties()): _parentNodeProperties may
    // answer a plain effective map (the meta root's lazy getter
    // resolves the root scope's map directly for the settled-chain);
    // the payload contract is scope-like, so the root registration's
    // component answers instead.
    get _parentNodePropertiesPayload() {
        const parent = this._parentNodeProperties;
        if (parent === null) return null;
        if (parent instanceof DocumentNodeProperties) return parent;
        // The root registration's *scope* (not the registering
        // TypeSpecLiveProperties component): the payload contract is
        // scope-like — an object answering .getProperties().
        return this.getEntry("rootNodeProperties@")?.nodeProperties ?? null;
    }

    // The registration payload: a scope-like object answering
    // .getProperties() with the node's effective map — this node's own
    // scope component (element nodes), the parent element's scope
    // component (text runs), or the root registration's component (the
    // end of every chain).
    ...
    // The dispatcher's parent scope-like object for the scope build,
    // with the guaranteed root endpoint: the threaded chain arg can
    // unwind to null transiently (before the root registered), but the
    // dispatcher's own "rootNodeProperties@" mapping always resolves
    // once the root registration exists. Returns a scope-like object
    // (a component answering .getProperties()), never a raw scope —
    // the scope build calls .getProperties() on it.
    // The nearest *settled effective map* at or above this dispatcher,
    // for the scope builds of this node's descendants: this node's own
    // scope's settled map (element nodes), else the threaded chain
    // arg's answer (text runs: the parent element's scope; top-level
    // nodes: the meta root's lazy root getter). Never a fallback map —
    // a child's cascade must see the parent's own effective map (the
    // G8 contract). null while no scope in the chain is built yet; the
    // scope build defers and retries when the parent scope's identity
    // change re-triggers it.
    get parentSettledProperties() {
        const child = this._widgets[0]?.widget;
        if (child instanceof DocumentNodesMetaElement) {
            const scopeComponent = child.nodePropertiesComponent;
            if (scopeComponent !== null && scopeComponent.hasScope)
                return scopeComponent.settledProperties;
        }
        const chained = this._parentNodeProperties;
        // The chain arg answers scope components (nested nodes), the
        // root scope instance, or — from the meta root's lazy getter —
        // the root scope's effective map directly (already a settled
        // map).
    ...
    // The dispatcher's own payload getter, exposed for the sibling
    // element's scope component: consumers attaching mid-cycle (before
    // the scope exists) read the same fallback chain as consumers
    // reading the dispatcher directly.
    // The changed map of the dispatcher's current update cycle is not
    // needed by the scope component: its own mappings (the resolved
    // typeSpecProperties@ id and the "@parentNodeProperties" trigger)
    // produce its changed entries directly.
    ...
    // effective map (5b). Always the scope-like payload (the own scope
    // component for element nodes) — an unbuilt scope component
    // delegates its .getProperties() to the parent payload until its
    // first update builds the scope, so consumers never see a
    // transient raw map or null.
    get nodeProperties() {
        return this._scopeLikePayload;
    }

    // The nearest scope-like payload above this dispatcher (its own
    // payload minus the own-scope case): used by the element's scope
    // component for consumers attaching before the scope exists.
    get parentPayload() {
        return this._parentNodePropertiesPayload;
    }
...
// Provisioned (and rebuilt on resolved-id change) by
// DocumentNodesMetaElement._provisionWidgets, mirroring the viewer's
// _provisionTypeSpecStyler: the host typeSpecnion arrives as the
// "properties@" mapping (resolved id, typeSpecProperties@<path>), the
// parent scope as a plain constructor arg (the nearest scope up the
// meta tree, threaded through the containers — the root scope's
// live-getter at the top level).
class DocumentNodeProperties extends _BaseComponent {
    constructor(
        ...
        // above this node: walks dispatcher ancestors via
        // parentSettledProperties (own scope's settled map for element
        // nodes, the root scope's effective map at the top level).
        this._parentScopeChainGetter = parentScopeChainGetter;
        // Zero-arg fn answering the dispatcher's current payload (its
        // fallback chain) — for consumers reading this component before
        // its first update built the scope.
        this._payloadFallback = payloadFallback;
        // The owning dispatcher: its update cycle (not this
        // component's) observes the document node and the typeSpec
        // registry; the rebuild checks below compare against the
        // dispatcher's changed map, which the element stashes here
    ...
    getProperties() {
        if (this._nodeProperties === null) {
            ...
            // The fallback chain ends at the root registration's
            // component (scope-like); a transient null (root not yet
            // registered) or a non-scope-like value answers an empty
            // map — consumers treat it as "no node properties yet",
            // the same as the notFoundFallbackValue path.
    ...
    update(changedMap) {
        ...
        if (parentNodeProperties === null || parentNodeProperties === undefined) {
            this._nodeProperties = null;
            return;
        }
        this._nodeProperties = new HierarchicalScopeNodeProperties(
            ...
            // The parent node's settled effective map: passed as the
            // "defaults map" argument (a plain Map/CascadingMap, not a
            // scope instance) so the cascade's "parent" layer IS that
            // map — the G8 contract.
        ...
        if (this.widgetBus.wrapper.hasProtocolHandlerRegistration("nodeProperties@")) {
...
class DocumentNodesMetaElement extends _DocumentNodesMetaDispatchedNode {
    ...
    // Provision (or rebuild) the node's scope component. The rebuild
    // test mirrors the viewer's _provisionTypeSpecStyler: a widget
    // cannot change its own external dependency mappings, so a changed
    // resolved typeSpecProperties@ id (typeSpec relinking) replaces the
    // wrapper; document-path-keyed ids (the registration, the parent
    // scope) are stable for the element's lifetime and need no
    // comparison.
    _provisionNodeProperties() {
        ...
        const widgetWrapper = this._initWrapper(
            ...
            // scope's identity change re-triggers it.
            () => this._dispatcher.parentSettledProperties,
