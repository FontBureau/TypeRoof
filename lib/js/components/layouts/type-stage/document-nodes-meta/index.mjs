...
class DocumentNodesMetaNode extends _BaseContainerComponent {
    ...
    // The node's key in its parent's children collection (its index);
    // null before the first wrapper resolution.
    get nodeKey() {
        return this.widgetBus.rootPath?.parts.at(-1) ?? null;
    }
    ...
    // The nearest *settled, control-projected map* strictly ABOVE this
    // dispatcher — the scope-build parent for this node's own scope
    // component (the G8 contract: a node's cascade sees the parent's
    // settled map, never a fallback map; "control-projected" = the
    // parent's inheritance controls applied, so DEMARCATION_INHERITANCE
    // governs what flows down). Resolved from the ancestor chain
    ...
    get ancestorSettledProperties() {
        // Children cascade onto the ancestor's CONTROL-PROJECTED map
        // (inheritance controls applied), not its raw effective map —
        // this is what makes DEMARCATION_INHERITANCE live in the
        // document-node tree.
        const chained = this._parentScopeChain;
        if (chained === null || chained === undefined) return null;
        if (chained instanceof DocumentNodeProperties)
            return chained.inheritableSettledProperties;
        if (chained instanceof HierarchicalScopeNodeProperties)
            return chained.getInheritableProperties();
        // The meta root's lazy getter answers the root scope's
        // projected map directly (already a settled map).
        return chained;
    }
...
class DocumentNodeProperties extends _BaseComponent {
    constructor(
        ...
        // Zero-arg fn answering the document-node context
        // ({node, nodeSpec, metaInfo, index, isLast}) for generators'
        // hostContext; null getter means no context (root).
        this._hostContextGetter = hostContextGetter;
    ...
    // The settled map as seen by DESCENDANTS: the effective map with
    // this scope's inheritance controls applied (tombstones withhold,
    // re-routes resolve). This is the map children cascade onto; the
    // node's own consumers (stylers) keep reading the effective map via
    // getProperties().
    get inheritableSettledProperties() {
        return this._nodeProperties === null
            ? null
            : this._nodeProperties.getInheritableProperties();
    }
    ...
    update(changedMap) {
        ...
        this._nodeProperties = HierarchicalScopeNodeProperties.createFromParentMap(
            ...
            NODE_PROPERTIES_INHERITANCE_POLICY,
            // Document-node context for the generators (null at the
            // meta root / while the getter is unset).
            this._hostContextGetter === null
                ? null
                : this._hostContextGetter(),
        );
...
class DocumentNodesMetaElement extends _DocumentNodesMetaDispatchedNode {
    ...
    // The document-node context handed to the node-properties
    // generators (hostContext): the metamodel node + its schema spec +
    // the attachment-facing metaInfo + sibling position. NOT the
    // ProseMirror runtime node — the meta tree works from the document
    // model, PM runtime only exists in editor mode.
    get _hostContext() {
        const dispatcher = this._dispatcher,
            siblings = dispatcher.getEntry("collection"),
            index = dispatcher.nodeKey;
        return {
            node: this.getEntry("."),
            nodeSpec: this.getEntry("nodeSpec"),
            metaInfo: this._metaInfo,
            index,
            isLast: index === siblings.size - 1,
        };
    }
        ...
        const documentNode = widgetBus.getEntry("."),
            ...
            widgets = [
                ...
                [
                    ...
                    // declare their own).
                    // The control-projected map (inheritance controls
                    // applied): top-level document nodes cascade onto
                    // the root's projection, like any other child.
                    () =>
                        this.getEntry(
                            `nodeProperties@${originTypeSpecPath.toString()}`,
                        )?.nodeProperties?.getInheritableProperties() ??
                        null,
