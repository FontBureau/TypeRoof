...
class DocumentNodesMetaNode extends _BaseContainerComponent {
    ...
    // the scope-build input for this node's descendants: this node's
    // own scope's settled map (element nodes), else the chain's answer
    // unwrapped to a map. Never a fallback map: a child's cascade must
    // see the parent's own effective map (the G8 contract). null while
    // no scope in the chain is built yet; the scope build defers and
    // retries when the parent scope's identity change re-triggers it.
...
class DocumentNodeProperties extends _BaseComponent {
    constructor(
        ...
        // above this node (the dispatcher's parentSettledProperties).
