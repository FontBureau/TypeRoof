...
// Compute the next sibling's nodeProperties@ registration id for
// resolving lineHeightAfter/emAfter margin units — the same channel
// the editor wires as nextNodeProperties@. The sibling's style facts
// (fontSize via its typeSpec layer, leading via the layout keys) live
// in its per-document-node node-properties scope; the typeSpec-based
// lookup this replaces could not resolve the layout keys.
export function resolveNextNodePropertiesId(
    parentCollection,
    currentKey,
    documentNodePath,
) {
    ...
    // nodeProperties@ ids are keyed by document-node path; the sibling
    // shares the parent path, only the node key (the collection index)
    // differs.
    return `nodeProperties@${documentNodePath.parent.append(nextIndex).toString()}`;
}
...
export function typeSpecStylerDependencyMappings(
    ...
    // nodeProperties@ ids are per-document-node, always distinct — no
    // dedup against nodePropertiesId needed (unlike the former
    // typeSpec-based nextProperties@, where siblings could share a
    // typeSpec registration).
    if (nextNodePropertiesId !== null)
        dependencyMappings.push([nextNodePropertiesId, "nextNodeProperties@"]);
