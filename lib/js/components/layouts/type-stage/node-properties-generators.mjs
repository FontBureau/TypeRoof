...
export function* horizontalLayoutRunionGen(inputCascade /*, hostContext ,*/) {
    ...
    if (
        ...
        // runion). The gutter is OPTIONAL (missing resorts to a
        // zero constant).
        columns === undefined ||
        // An empty columns list is only complete when growth is
        // actually possible (Infinity or a number >= 1; UNSET counts
        // as numeric 1 — the exploration default) — pure min-driven
        // growth (mirrors the dispatcher's check).
        (!columns.size &&
            growColumns !== undefined &&
    ...
    const rawConfig = {
        ...
        // undefined ≡ numeric 1 (the exploration default);
        // normalizeColumnConfig defaults paddingRatio to [3/5, 2/5].
        growColumns: growColumns === undefined ? 1 : growColumns,
        columnGap,
...
export function* horizontalLayoutNodePropertiesGen(inputCascade, hostContext) {
    ...
    const columnsValue = inputCascade.get(`${HORIZONTAL_LAYOUT}columns`),
        ...
        growthPossible =
            // UNSET counts as numeric 1 — the exploration default.
            growColumnsValue === undefined ||
