...
export class LocalScopeProperties {
    ...
    static resolveSyntheticProperties(
        ...
        const dependantsMap = new Map(),
            ...
            // A name that is a local synthetic must NOT enter the graph
            // as "already resolved": its dependants would consume the
            // edge on the parent's (already settled) value while the
            // local computation is still pending — the dependant then
            // reads the raw SyntheticValue from the result map
            // (observed live as SYNTH-INJECTION in the node-properties
            // channel, where a synthetic's only deps are parent-cascade
            // keys and its name collides with a parent key; the
            // typeSpecnion masked this because its synthetics always
            // had local non-synthetic deps anchoring the order).
            noDepsSet = new Set(
                Array.from(parentPropertyValuesMap.keys()).filter(
                    (key) => !syntheticProperties.has(key),
                ),
            );
