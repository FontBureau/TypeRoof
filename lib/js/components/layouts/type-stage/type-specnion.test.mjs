...
describe("inheritance controls (DEMARCATION_INHERITANCE)", () => {
    const identity = (x) => x,
        ...
        // "lineLength" from its own "width" and re-routes it to become
        // the children's "width", withholding "lineLength" itself.
        columnLayoutGen = function* () {
            ...
            yield [
                "width",
                new SyntheticValue(identity, ["lineLength"]),
                DEMARCATION_INHERITANCE,
            ];
            yield ["lineLength", TOMBSTONE, DEMARCATION_INHERITANCE];
        },
        emptyGen = function* () {},
        rootWith = (gen) =>
            new HierarchicalScopeTypeSpecnion([gen], {}, new Map()),
        childOf = (parent, gen = emptyGen) =>
            new HierarchicalScopeTypeSpecnion([gen], {}, parent);
    ...
    it("re-routes an own value to a different name for children", () => {
        ...
        expect(root.getProperties().get("lineLength")).toBe(67);
        // the child receives width = parent's lineLength
    ...
    it("withholds a tombstoned property from children", () => {
        const root = rootWith(columnLayoutGen),
            child = childOf(root);
        expect(child.getProperties().has("lineLength")).toBe(false);
        // the tombstone does not propagate further than the property:
        // a grandchild doesn't inherit lineLength either (it's simply
        // absent from the child's scope)
        const grandChild = childOf(child);
        expect(grandChild.getProperties().has("lineLength")).toBe(false);
    ...
    it("drops an optional control whose synthetic dependencies can't be resolved", () => {
        const root = rootWith(function* () {
                yield ["width", 134];
                yield [
                    "width",
            ...
            child = childOf(root);
        // no lineLength in the parent's scope: the optional control is
        // dropped and the original width is inherited unchanged
        expect(child.getProperties().get("width")).toBe(134);
    });

    it("throws on a required synthetic with unresolvable dependencies", () => {
        const root = rootWith(function* () {
            yield ["secret", "hidden"];
            yield ["secret", TOMBSTONE, DEMARCATION_INHERITANCE];
        });
        expect(root.getProperties().get("secret")).toBe("hidden");
        // the dependency can't be resolved against the projected parent
        // scope and the synthetic is required (default): loud failure,
        // the tombstoned property can't be resurrected silently
        expect(() =>
            childOf(root, function* () {
                yield ["parentValue", new SyntheticValue(identity, ["secret"])];
            }),
        ).toThrow(UnresolvableDependenciesError);
        expect(() =>
            childOf(root, function* () {
                yield ["parentValue", new SyntheticValue(identity, ["secret"])];
            }),
        ).toThrow(/"parentValue" can't resolve: missing "secret"/);
    });

    it("drops an optional synthetic whose dependency was tombstoned by the parent", () => {
        const root = rootWith(function* () {
            yield ["secret", "hidden"];
            yield ["secret", TOMBSTONE, DEMARCATION_INHERITANCE];
        });
        const child = childOf(root, function* () {
            yield [
                "parentValue",
                new SyntheticValue(identity, ["secret"], { optional: true }),
            ];
        });
    ...
    it("controls don't leak into the local scope of their own layer", () => {
        ...
        expect(root.localPropertyNames).toEqual(["width", "lineLength"]);
        expect(root.getPropertyValuesMap().has("lineLength")).toBe(true);
    });
});
...
const edgeStub = (mode, tag = null, stylePatch = "some-patch") => {
    ...
    typeSpecnionWith = (edges, parentOrDefaults, markEdges = []) =>
...
describe("style-link inheritance (styleLinksGen + HierarchicalScopeTypeSpecnion)", () => {
    ...
    it("overrides an inherited edge wholesale (whole-edge override)", () => {
        const parentEdge = edgeStub("link", "q"),
            ...
            child = typeSpecnionWith([["quote", childEdge]], root);
    ...
    it("tombstone (unlinked) removes the edge for child and descendants", () => {
        const root = typeSpecnionWith(
            ...
            child = typeSpecnionWith([["quote", edgeStub("unlinked")]], root),
            grandChild = typeSpecnionWith([], child);
    ...
    it("re-links a tombstoned edge in a descendant", () => {
        const root = typeSpecnionWith(
            ...
            grandChild = typeSpecnionWith([["quote", newEdge]], child);
...
describe("markStyleLinks: the second style-link family", () => {
    it("inherits, overrides and tombstones like the intent family", () => {
        const edge = edgeStub("link"),
            root = typeSpecnionWith([], new Map(), [["strong", edge]]);
        // inherit
        const child = typeSpecnionWith([], root);
        expect(
        ...
        const childEdge = edgeStub("link"),
            overridingChild = typeSpecnionWith([], root, [
                ["strong", childEdge],
            ]);
        expect(
            getStyleLinks(
        ...
        const tombChild = typeSpecnionWith([], root, [
                ["strong", edgeStub("unlinked")],
            ]),
            tombGrandChild = typeSpecnionWith([], tombChild);
        expect(
    ...
    it("the two maps are independent, even for the same key", () => {
        const intentEdge = edgeStub("link", "q", "intent-patch"),
            ...
            root = typeSpecnionWith([["strong", intentEdge]], new Map(), [
                ["strong", markEdge],
            ]),
    ...
    it("resolution is structurally separated: no cross-matching", () => {
        ...
        // and the reverse
        const markOnly = typeSpecnionWith([], new Map(), [
                ["strong", edgeStub("link")],
            ]),
...
describe("createPatched (style patches on the cascade)", () => {
    const emptyGen = function* () {},
        rootWith = (gen, defaults = new Map()) =>
            new HierarchicalScopeTypeSpecnion([gen], {}, defaults),
        childOf = (parent, gen = emptyGen) =>
            new HierarchicalScopeTypeSpecnion([gen], {}, parent);

    it("a style patch overrides the origin's own and inherited properties", () => {
        const root = rootWith(function* () {
                yield ["width", 100];
                yield ["color", "black"];
            }),
            child = childOf(root),
            patched = child.createPatched(new Map([["color", "red"]]));
        // the patch wins over the inherited value
        expect(patched.getProperties().get("color")).toBe("red");
        // unpatched inherited properties still resolve
        expect(patched.getProperties().get("width")).toBe(100);
        // the origin scope is unchanged
        expect(child.getProperties().get("color")).toBe("black");
    });

    it("a style patch overrides an origin-local property", () => {
        const root = rootWith(function* () {
                yield ["width", 100];
            }),
            child = childOf(root, function* () {
            ...
            patched = child.createPatched(new Map([["width", 300]]));
        expect(patched.getProperties().get("width")).toBe(300);
        expect(child.getProperties().get("width")).toBe(200);
    });

    it("patched synthetics resolve against raw-patch and origin-local values", () => {
        // resolveSyntheticProperties drops synthetics whose dependencies
        // are all parent-only unless allowParentOnlyResolution is opted
        // into (re-resolving them per layer would recompute inherited
        // values). A patch synthetic can therefore depend on raw-patch
        // values (and origin-local values made raw by createPatched),
        // not solely on inherited ones.
        const root = rootWith(function* () {
                yield ["width", 120];
            }),
            child = childOf(root, function* () {
        ...
        expect(patched.getProperties().get("scaledHeight")).toBe(80);
        // the inherited width still resolves through the cascade
        expect(patched.getProperties().get("width")).toBe(120);
    });

    it("root defaults remain readable through a patched scope", () => {
        const root = rootWith(emptyGen, new Map([["baseFontSize", 12]])),
            child = childOf(root),
            patched = child.createPatched(new Map([["color", "red"]]));
        expect(patched.getProperties().get("baseFontSize")).toBe(12);
        expect(patched.getProperties().get("color")).toBe("red");
    });

    it("a patch can't resurrect a property the grand-parent tombstoned", () => {
        // The origin's own controls are bypassed for the patch itself,
        // but the grand-parent's controls are honored via the consumed
        // parent maps (see the NOTE in PatchedScopeProperties).
        const root = rootWith(function* () {
                yield ["secret", "hidden"];
                yield ["secret", TOMBSTONE, DEMARCATION_INHERITANCE];
            }),
            child = childOf(root),
            patched = child.createPatched(new Map([["color", "red"]]));
        expect(patched.getProperties().has("secret")).toBe(false);
        expect(patched.getProperties().get("color")).toBe("red");
    });
});
