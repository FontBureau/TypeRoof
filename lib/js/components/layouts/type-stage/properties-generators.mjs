        ...
        if (algorithm === ForeignKey.NULL) {
            if (outerTypespecnionAPI.hasParentProperty(ALGORITHM_TYPE)) {
                ...
                if (algorithm === "AutoLinearLeading") {
                    // The computed line-height lives in the node
                    // channel (leadingNodePropertiesGen, from the
                    // actual column width); the typeSpecnion carries
                    // only the algorithm configuration.
                }
            }
            continue;
        }
...
export function* horizontalLayoutRunion(outerTypespecnionAPI, hostInstance) {
    ...
    */
    ...
    // Phase 5b teardown: the geometry re-routes left the typeSpecnion —
    // per-node geometry (availableWidth/width) is computed by the
    // nodeProperties@ channel now (horizontalLayoutNodePropertiesGen in
    // node-properties-generators.mjs, layout/* keys). Commented out,
    // not deleted (plan decision 6). The style-side yields above
    // (lineLength, columnCount, …) stay — the node channel reads them
    // via its hostMap.
