...
import { TOMBSTONE, DEMARCATION_INHERITANCE, DEMARCATION_PROPERTY } from "./type-specnion.mjs";
...
import { lengthToCSSUnit, cssUnitConvert } from "../../length-models.mjs";
function* fontGen(
...
function AutoLinearLeadingSyntheticValue(prefix) {
    ...
    function calculate(...args) {
        ...
        console.log(`AutoLinearLeadingSyntheticValue calculate valuesMap`, valuesMap)
...
*/
    ...
    for (const propertyName of horizontalLayoutProperties) {
        if(!hostInstance.has(propertyName)){
            console.error(`NOT FOUND ERROR "${propertyName}" in ${hostInstance} (horizontalLayoutRunion).`);
            continue;
        }
        const property = hostInstance.get(propertyName);
        if (property.isEmpty) continue;
        yield [`${GENERIC}${propertyName}`, property.value];
    }
    ...
    // yield [
    //     `${GENERIC}width`,
    //     new SyntheticValue(horizontalWidthSum, horizontalLayoutProperties),
    //     DEMARCATION_PROPERTY,
    // ];
...
export function* inheritancePolicyGen(propertyNames) {
    ...
    yield ['hello/test', 123, DEMARCATION_PROPERTY];
}
