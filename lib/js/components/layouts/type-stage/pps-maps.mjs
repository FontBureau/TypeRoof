...
const _excludesTypeSpecPPSMap = new Set([
    ...
    // Resolution/provisioning flags are not processed-properties:
    // they gate typeSpec resolution (excludeFromFallback) and styler
    // provisioning (noStyler) only, and must not flow through the
    // inheriting typeSpecnion property stream (round-2 decision #4).
    "excludeFromFallback",
    "noStyler",
]);
...
function getTypeSpecPPSMap(parentPPSRecord, Model) {
    ...
    for (const [modelFieldName, modelFieldType] of Model.fields.entries()) {
        ...
        else if (modelFieldName === "intentStyleLinks")
            prefix = "intentStyleLinks/";
        else if (modelFieldName === "markStyleLinks")
            prefix = "markStyleLinks/";
...
/**
 * Note how most of the Type -> prefix mappings don't apply to
...
// Same field-to-UI mapping, MarkSpecModel shares the relevant generic
// field types (and the "attrs" exclusion) with NodeSpecModel.
export const MARKSPEC_PPS_MAP = getNodeSpecPPSMap(null, MarkSpecModel);
