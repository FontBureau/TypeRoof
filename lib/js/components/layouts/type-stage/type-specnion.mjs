...
export function collectPropertyGeneratorEntries(propertiesGen, defaultDemarcation) {
    ...
    for (const [
        ...
        if (demarcation === DEMARCATION_PROPERTY) {
            if(propertyValue === TOMBSTONE)
                throw new Error(`VALUE ERROR propertyName "${propertyName}"`
...
export class HierarchicalScopeTypeSpecnion extends _BaseTypeSpecnion {
    ...
    getInheritableProperties() {
        ...
        if(rawProperties.size)
            console.error(`VALUE ERROR inheritance policy generators must `
    ...
    _getParentMaps() {
        ...
        const parentPropertyValuesMap =
            ...
            // Policy tombstones are applied by the parent's
            // getInheritableProperties, so no further filtering here.
            filteredParentPropertyValuesMap = new Map(parentPropertyValuesMap);
    ...
    _initPropertyValuesMaps() {
        const [parentPropertyValuesMap, filteredParentPropertyValuesMap] =
            ...
            [rawProperties, inheritanceControls] =
                collectPropertyGeneratorEntries(propertiesGen, DEMARCATION_PROPERTY);
