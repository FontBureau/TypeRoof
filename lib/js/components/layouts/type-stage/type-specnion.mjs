...
export class HierarchicalScopeTypeSpecnion extends _BaseTypeSpecnion {
    ...
    getInheritableProperties() {
        const ownProperties = this.getProperties(),
            ...
            [, policyControls] = collectPropertyGeneratorEntries(
                flattenGenerators(
                    ...this._inheritancePolicyGenerators.map(
                        (gen) => gen(ownPropertyKeys),
                    ),
                ),
            ),
            // explicit controls win over policy on same name (resurrection)
            allControls = mapSetProperties(new Map(), policyControls, this._inheritanceControls);
