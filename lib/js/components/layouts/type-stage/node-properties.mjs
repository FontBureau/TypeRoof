...
export class HierarchicalScopeNodeProperties {
    ...
    getInheritableProperties() {
        // Memoized: the scope is immutable after construction, and
        // consumers compare the projection by identity across update
        // cycles (the document-node scope build's change guard) — a
        // fresh map per call would read as perpetual change once a
        // policy yields controls.
        if (this._inheritableProperties === undefined)
            this._inheritableProperties = resolveInheritableProperties(this);
