...
// Production guarantee: the typeSpecnion's defaults layer resolves ALL
// registered style keys (registered-properties.mjs), so the node-
// properties generators' synthetic deps always resolve. Sparse stub
// maps must mirror that: fill the style defaults, override per test.
const _STYLE_DEFAULTS = new Map([
    [`${GENERIC}fontSize`, 12],
...
function _completeStyleMap(overrides = []) {
    return new Map([..._STYLE_DEFAULTS, ...overrides]);
}
...
describe("horizontalLayoutNodePropertiesGen width semantics (handoff 2026-09-08)", () => {
            ...
            HierarchicalScopeNodeProperties.createRoot(
                ...
                _completeStyleMap(
                    new Map([
                        [
                            `${LAYOUT}width`,
                            _lengthModelStub(100, "percent-layout"),
                        ],
    ...
    it("F: child of an overflowing parent fills the overflowed budget", () => {
        const overflowing = childScope(
            ...
            child = childScope(
                _completeStyleMap(new Map([[`${GENERIC}fontSize`, 10]])),
                overflowing,
            ),
            props = child.getProperties();
        expect(props.get(`${LAYOUT}width`)).toBeCloseTo(700, 6);
