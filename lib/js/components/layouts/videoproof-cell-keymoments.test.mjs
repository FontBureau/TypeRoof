...
describe("VideoproofModel keyMoments ownership (per-entry)", () => {
    ...
    it("cell keyMoments are user data: edits serialize and round-trip", async () => {
        ...
        const dependencies = await makeDependencies(),
            ...
            draft = primalState.getDraft();
        getDraftEntry(draft, CELL_KEYMOMENTS_PATH + "/0/showCellBoxes").value =
            true;
        const editedState = draft.metamorphose(dependencies),
            [errors, serialized] = serialize(editedState, SERIALIZE_OPTIONS);
        expect(errors).toEqual([]);
        const cellKeyMoments =
            JSON.parse(serialized).activeActors[0].instance.activeActors[0]
                .instance.activeActors[0].instance.keyMoments;
...
describe("VideoproofModel more axes quick dial keyMoments", () => {
    const LAYER_KEYMOMENTS_PATH = "activeActors/0/instance/keyMoments";

    async function createStateWithMoreAxis(dependencies) {
        const primalState = await createPrimalState(dependencies),
            draft = primalState.getDraft();
        draft.getDraftFor("moreAxisPausedT").value = 0.25;
        draft.getDraftFor("moreAxisTag").value = "GRAD";
        return draft.metamorphose(dependencies);
    }

    it("are marked GENERATED_DATA, not serialized and re-created on load", async () => {
        const dependencies = await makeDependencies(),
            state = await createStateWithMoreAxis(dependencies),
            keyMoments = getEntry(state, LAYER_KEYMOMENTS_PATH);
        // GRAD: default 0, min -200, max 150
        expect(keyMoments.size).toBe(3);
        // for (const [, keyMoment] of keyMoments)
        //     expect(Object.hasOwn(keyMoment, GENERATED_DATA)).toBe(true);

        const [errors, serialized] = serialize(state, SERIALIZE_OPTIONS);
        expect(errors).toEqual([]);
        const payload = JSON.parse(serialized);
        expect(payload.moreAxisTag).toBe("GRAD");
        // no keyMoment content of the layer is serialized
        const layerKeyMoments = payload.activeActors[0].instance.keyMoments;
        expect((layerKeyMoments || []).filter((item) => item !== null)).toEqual(
            [],
        );

        const loadedState = await loadFromSerialized(payload, dependencies),
            loadedKeyMoments = getEntry(loadedState, LAYER_KEYMOMENTS_PATH);
        expect(loadedKeyMoments.size).toBe(3);
        expect(
            Array.from(
                loadedKeyMoments,
                ([, keyMoment]) =>
                    getEntry(keyMoment, "axesLocations/GRAD").value,
            ),
        ).toEqual([0, -200, 150]);
        expect(getEntry(loadedKeyMoments, "0/numericProperties/t").value).toBe(
            0.25,
        );
        for (const [, keyMoment] of loadedKeyMoments)
            expect(Object.hasOwn(keyMoment, GENERATED_DATA)).toBe(true);
    }, 30000);

    it("are removed when a rap keyMoment is navigated to (legacy behavior)", async () => {
        const dependencies = await makeDependencies(),
            primalState = await createPrimalState(dependencies),
            rapDuration = primalState.get("duration").value,
            state = await createStateWithMoreAxis(dependencies);
        // GRAD has 3 keyMoments, the rap 27
        expect(state.get("duration").value).not.toBe(rapDuration);
        // UIVideoproofKeyMomentsLinkNavigation does this
        const draft = state.getDraft();
        draft.getDraftFor("moreAxisTag").value = "";
        draft.getDraftFor("t").value = 10 / 27;
        const navigated = draft.metamorphose(dependencies);
        expect(getEntry(navigated, LAYER_KEYMOMENTS_PATH).size).toBe(0);
        expect(navigated.get("duration").value).toBe(rapDuration);
        expect(navigated.get("t").value).toBe(10 / 27);
    }, 30000);

    it("are removed when the quick dial is disengaged", async () => {
        const dependencies = await makeDependencies(),
            state = await createStateWithMoreAxis(dependencies),
            draft = state.getDraft();
        draft.getDraftFor("moreAxisTag").value = "";
        const disengaged = draft.metamorphose(dependencies);
        expect(getEntry(disengaged, LAYER_KEYMOMENTS_PATH).size).toBe(0);
    }, 30000);
});
