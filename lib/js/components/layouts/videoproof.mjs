...
import {
    UICLanguageTagCollapsible as UILanguageTag
...
class UIVideoproofArrayLayer extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        ...
        const updateDefaultsDependencies = [
                [`animationProperties@${widgetBus.rootPath}`, 'animationProperties@']
            ]
    ...
    _getDefaults(ppsRecord, modelFieldName, defaultVal=_NOTDEF) {
        ...
        const {fullKey} = ppsRecord
           ...
           , propertyValues = liveProperties.animanion.getPropertiesFromGlobalT(0)
           ;
        ...
        if(defaultVal !== _NOTDEF)
            return defaultVal;
        throw new Error(`KEY ERROR ${this}._getDefaults: not found "${fullKey}" for t=0 in ${liveProperties}`);
    }
}
...
const VideoproofArrayV2Model = _BaseLayoutModel.createClass(
      ...
      , CoherenceFunction.create(
          ...
          , function initAxesMath({axesMath}) {
            ...
            if(items.size === 0) {
                items = items.isDraft
                    ? items
                    : unwrapPotentialWriteProxy(axesMath, 'ensureDraft').getDraftFor('items')
                    ;
      ...
      , CoherenceFunction.create(
          ...
          , function actorsStructure({activeActors: activeActorsProxy, availableVideoproofActorTypes}) {
            ...
            const getActiveActorsDraft = ()=>{
                if(!activeActors.isDraft)
                    ...
                    activeActors = unwrapPotentialWriteProxy(activeActorsProxy, 'ensureDraft');
                return activeActors;
            }
            if(activeActors.size > 1)
            ...
            if(activeActors.size && getEntry(activeActors, '0/actorTypeKey').value !== LAYER_TYPE_KEY)
            ...
            const getLayerActorsDraft = ()=>{
                ...
                if(!layerActors.isDraft)
                    layerActors = getDraftEntry(getActiveActorsDraft(), layerActorsPath);
                return layerActors;
            }
            if(layerActors.size > 1)
                getLayerActorsDraft().splice(1, Infinity);
            ...
            if(layerActors.size) {
                if(!availableVideoproofActorTypes.has(getEntry(layerActors, '0/actorTypeKey').value)){
      ...
      , CoherenceFunction.create(
            ...
            const layerActorsPath = '0/instance/activeActors';
            const videoproofActor = getEntry(unwrapPotentialWriteProxy(activeActors), `${layerActorsPath}/0`)
            ...
            const typeClass = getEntry(videoproofActor, 'actorTypeModel/typeClass').value;
            const CTOR = getEntry(videoproofActor, 'instance').wrapped.constructor;
            if( typeClass !== CTOR ) {
                const immutable = yield * videoproofActor.metamorphoseGen();
                getEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'),  `${layerActorsPath}`).splice(0, 1, immutable);
            }
        })
      ...
      , CoherenceFunction.create(
          ...
          , function initVideoproofActor({activeActors, font}) {
            let videoproofActor = getEntry(unwrapPotentialWriteProxy(activeActors), '0/instance/activeActors/0/instance');
            const getVideoproofActorDraft = (key=null)=>{
                if(!videoproofActor.isDraft)
                    videoproofActor = getDraftEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'), '0/instance/activeActors/0/instance');
                return key===null ? videoproofActor : videoproofActor.getDraftFor(key);
            }
            ...
            if(videoproofActor.get('localActiveFontKey').value !== font.value.fullName) {
                ...
                // the global font key.
                getVideoproofActorDraft('localActiveFontKey').set(font.value.fullName);
            }
      ...
      , CoherenceFunction.create(
          ...
          , function initEmptyActiveActors({activeActors}) {
            const videoproofActorActiveActorsPath = '0/instance/activeActors/0/instance/activeActors'
              , videoproofActorActiveActors = getEntry(unwrapPotentialWriteProxy(activeActors), videoproofActorActiveActorsPath, null)
              ;
            ...
            if(videoproofActorActiveActors.size === 0) {
                const videoproofActorActiveActorsDraft = videoproofActorActiveActors.isDraft
                    ? videoproofActorActiveActors
                    : getDraftEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'), videoproofActorActiveActorsPath);
      ...
      , CoherenceFunction.create(
          ...
          , function* updateRap({activeActors, axesMath, installedFonts, font, duration}) {
            const videoproofActorPath = '0/instance/activeActors/0/instance'
              , videoproofActor = getEntry(unwrapPotentialWriteProxy(activeActors), videoproofActorPath)
            ...
            const childrenPotentiallyRequireKeyMoments = videoproofActor.isDraft &&
              ...
              ;
            if(looksLikeNew || childrenPotentiallyRequireKeyMoments || axesMath.isDraft || fontHasChanged) {
                const videoproofActorDraft = videoproofActor.isDraft
                    ? videoproofActor
                    : getDraftEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'), videoproofActorPath)
                    ;
        ...
        , CoherenceFunction.create(
          ...
          , function actorSpecificOverrides({activeActors}) {
            ...
            let videoproofActor = getEntry(unwrapPotentialWriteProxy(activeActors), videoproofActorPath);
            const getVideoproofActorDraft = ()=>{
                if(!videoproofActor.isDraft)
                    videoproofActor = getDraftEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'), videoproofActorPath);
                return videoproofActor
            }

            if(videoproofActor.get('actorTypeKey').value === 'VideoproofContextualActorModel') {
                const cellAlignmentPath = 'instance/keyMoments/0/cellAlignment'
                  , cellAlignment = getEntry(videoproofActor, cellAlignmentPath)
                  ;
                if(cellAlignment.isEmpty) {
                    ...
                    // workaround.
                    const cellAlignmentDraft = getDraftEntry(getVideoproofActorDraft(), cellAlignmentPath);
                ...
                const presetsPath = 'instance/keyMoments/0/presets'
                  ...
                  ;
                if (presets.isEmpty) {
                    const presetsDraft = getDraftEntry(getVideoproofActorDraft(), presetsPath);
                    presetsDraft.set(presetsDraft.constructor.Model.defaultValue);
                }
                ...
                if (charGroups.size < 1) {
                    ...
                    const charGroupsDraft = getDraftEntry(getVideoproofActorDraft(), charGroupsPath);
...
class UIParameterAxesDisplay extends _BaseComponent {
    ...
    _initTemplate() {
        const element = this.widgetBus.domTool.createElement('div', {class: 'ui-parameters-display_values'});
        this._insertElement(element);
        return [element];
    }
    update(changedMap) {
...
class UIParametersDisplay extends _BaseContainerComponent {
    constructor(widgetBus, _zones, extraClasses=[], customArgs=[]) {
        const classes = ['ui-parameters-display',...extraClasses]
          , localElement = widgetBus.domTool.createElement('div', {class: classes.join(' ')})
          ...
          , widgets = [
              ...
              , [
                    {
                        zone: 'main'
                      , activationTest: ()=>!this.widgetBus.getEntry(this.widgetBus.rootPath).has('activeActors')
                    }
                  , [
...
class VideoproofContainerStyler extends StaticNode {
    update(changedMap) {
        if(changedMap.has('animationProperties@')) {
            ...
            this.node.style.setProperty('--layout-specific-override-actor-renderer-videoproof-array-v2-background-color', 'none');
            actorApplyCSSColors(this.node, propertyValuesMap, getDefault, colorPropertiesMap);
        }
    }
}

class VideoproofArrayV2Controller extends _BaseTypeDrivenContainerComponentMixin(_BaseContainerComponent) {
    constructor(widgetBus, _zones) {
        const generalControlsContainer = widgetBus.domTool.createElement('div', {class: 'sidebar-group'})
          , contextualGlyphsContainer = widgetBus.domTool.createElement('div', {class: 'sidebar-group'})
          , keyMomentsContainer = widgetBus.domTool.createElement('div', {class: 'sidebar-group'})
          ...
          , videoproofContainer = widgetBus.domTool.createElement('div', {class: 'videoproof_layout', style: "height: 100%"})
          , zones = new Map([..._zones,
        ...
        const animationPropertiesRelativePath = Path.fromParts('activeActors', '0', 'instance')
          , animationPropertiesPath = this.widgetBus.rootPath.append(...animationPropertiesRelativePath)
            ...
            // model around.
          , formatAnimationPropertiesKey = animationPropertiesPath=>`animationProperties@${animationPropertiesPath}`
          , animationPropertiesKey = formatAnimationPropertiesKey(animationPropertiesPath)
          , updateDefaultsDependencies = [
                [animationPropertiesKey, 'animationProperties@']
            ]
          , videoProofActorRelativePath = animationPropertiesRelativePath.append('activeActors', '0', 'instance')
          , videoProofActorPath = this.widgetBus.rootPath.append(...videoProofActorRelativePath)
          , videoProofAnimationPropertiesKey = formatAnimationPropertiesKey(videoProofActorPath)
        ...
        const widgets = [
          ...
          , [
                {
                  ...
                  , activationTest: ()=>widgetBus.getEntry(videoProofActorPath.parent.append('actorTypeKey')).value === 'VideoproofArrayV2ActorModel'
                }
              , []
          ...
          , [
                ...
                {
                    zone: 'main'
                  , rootPath: videoProofActorPath.append('keyMoments', '0', 'charGroup')
                  , activationTest: ()=>widgetBus.getEntry(videoProofActorPath.append('keyMoments', '0')).has('charGroup')
                }
              , []
              , UICharGroupContainer
              , zones
              ...
              , {  // injectable
                  ...
                  , getDefaults: this._getDefaults.bind(this, videoProofAnimationPropertiesKey)
                }
              , propertyRoot
              , 'Glyphs'
            ]
         ...
         , ...(()=>{
            const typeKey = 'VideoproofContextualActorModel'
              , VideoproofContextualKeyMomentModel = activatableVideoproofActorTypes
                                .get(typeKey).value
                                .get('typeClass').value
                                .get('keyMoments').Model
              , contextualTypeDrivenFields = new Map([
                    ['presets', {localSettings:{zone: 'general'}}],
                    ['charGroups', {
                        localSettings:{zone: 'glyphs'},
                        resolvers:new Map([...this.constructor._resolvers,
              ...
              , generalSettings = {
                    ...
                    activationTest: ()=>widgetBus.getEntry(videoProofActorPath.parent.append('actorTypeKey')).value === typeKey
                }
              ...
              , ppsMap = ProcessedPropertiesSystemMap.fromPrefix(GENERIC, VideoproofContextualKeyMomentModel.fields.keys())
              , widgets = []
              ;
            for(const [fieldName, {localSettings={}, resolvers=null}] of contextualTypeDrivenFields) {
                const FieldType = VideoproofContextualKeyMomentModel.fields.get(fieldName);
                widgets.push(this._defineGenericWidget(
                        injectable,
                        {...generalSettings, ...localSettings},
                        ppsMap.get(fieldName), fieldName, FieldType, resolvers));
            }
          ...
          , [
                {
                    zone: 'general'
                  , rootPath: videoProofActorPath.append('keyMoments', '0', 'textColor')
                }
              , []
              , UIColorChooser
              , zones
              , 'Text Color'
                // argument = injectable.getDefaults.bind(null, propertyRoot, fieldName, BaseModelType.defaultValue);
              , this._getDefaults.bind(this, videoProofAnimationPropertiesKey, ProcessedPropertiesSystemMap.createSimpleRecord(COLOR, 'textColor'), 'textColor')
              , updateDefaultsDependencies
              , requireUpdateDefaults
            ]
          , [
                {
                    zone: 'general'
                  , rootPath: videoProofActorPath.append('keyMoments', '0', 'stageBackgroundColor')
                }
              , []
              , UIColorChooser
              , zones
              , 'Stage Color'
                // argument = injectable.getDefaults.bind(null, propertyRoot, fieldName, BaseModelType.defaultValue);
              , this._getDefaults.bind(this, videoProofAnimationPropertiesKey, ProcessedPropertiesSystemMap.createSimpleRecord(COLOR, 'stageBackgroundColor'), 'stageBackgroundColor')
              , updateDefaultsDependencies
              , requireUpdateDefaults
            ]
          , [
                {
                    zone: 'general'
                  , rootPath: videoProofActorPath.append('keyMoments', '0', 'languageTag')
                }
              , []
              , UILanguageTag
              , this._zones
          ...
          , [
              ...
              , [
                    // TODO:
                    [videoProofActorRelativePath.append('keyMoments').toString(), 'keyMoments']
    ...
    _getDefaults (animationPropertiesKey, ppsRecord, modelFieldName, defaultVal=_NOTDEF) {
