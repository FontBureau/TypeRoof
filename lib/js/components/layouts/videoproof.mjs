...
const activatableVideoproofActorTypes = (()=>{
    const videoproofActors = ['VideoproofArrayV2ActorModel', 'VideoproofContextualActorModel']
      , availableVideoproofActorTypesDraft = AvailableActorTypesModel.createPrimalDraft({})
      ;
    for(const key of videoproofActors)
        availableVideoproofActorTypesDraft.push([key, activatableActorTypes.get(key)]);
...
class UIVideoproofArrayLayer extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        ...
        , [
                {
                    zone: 'main'
                  , rootPath: Path.fromParts('.', 'keyMoments', '0', 'languageTag')
                }
              , []
              , UILanguageTag
              , this._zones
...
const VideoproofArrayV2Model = _BaseLayoutModel.createClass(
      ...
      , ... StaticDependency.createWithInternalizedDependency(
        ...
        // * FIXME: coherence function order of execution: can I define
        // another CF as an dependency of this one, so the other one will have
        // to be executed before???actorsStructure DONE
        // * DynamicStruct can be force-updated using metamorphose
        // * classical debug of Array/Contextual: does the CTOR run? Does update run ...
        //   we don't have Errors, either stuff doesn't run or somewhere it
        //   defaults to "blank" display...
      , CoherenceFunction.create(
            ['activeActors', 'availableVideoproofActorTypes']
          , function actorsStructure({activeActors, availableVideoproofActorTypes}){
            const activeActorsUnwrapped = unwrapPotentialWriteProxy(activeActors);
        //    console.log('=>>>>>CoherenceFunction actorsStructure');
        //    console.log('CoherenceFunction actorsStructure {activeActors, availableVideoproofActorTypes}:', activeActors, 'activeActorsUnwrapped.isDraft', activeActorsUnwrapped    .isDraft, availableVideoproofActorTypes, 'activeActors.size', activeActors.size);
            // not more than 1 child
            if(activeActors.size > 1) {
                activeActors.splice(1, Infinity);
                console.log('=>>>>actorsStructure: shrunk actors size');
            }

            // the one entry must be a layer
            if(activeActors.size && activeActors.get('0').get('actorTypeKey').value !== LAYER_TYPE_KEY) {
                activeActors.splice(0, Infinity);
                console.log('=>>>>actorsStructure: removed mismatched actor type');
            }

            if(activeActors.size === 0) {
                const layerDraft = createActor(LAYER_TYPE_KEY, activeActors.dependencies);
                activeActors.push(layerDraft);
                console.log('=>>>>actorsStructure: added LAYER');
            }
        //    console.log('activeActors:', activeActors, 'activeActors.keys:',...activeActors.keys());
            const layer = activeActors.get('0').get('instance');
        //    console.log('layer', layer , `${layer}`, layer.keys(), 'layer.wrapped', layer.wrapped);

            const layerActors = layer.wrapped.get('activeActors');

        //    console.log('layerActors', layerActors, `${layerActors}`, layerActors.size);
            // same in the layer, not more than 1 child ...
            if(layerActors.size > 1){
                layerActors.splice(1, Infinity);
                console.log('=>>>>actorsStructure: shrunk layerActors size');
            }
            // we don't have another place that records the actual actor
            // instead, we need to make sure, the actual actor is from
            // the valid set!
            //if(layerActors.size){
            //    console.log(`=>has an actor actorTypeKey:${layerActors.get('0').get('actorTypeKey').value}`);
            //}
            if(layerActors.size) {

                if(!availableVideoproofActorTypes.has(layerActors.get('0').get('actorTypeKey').value)){
            //        console.log(`=>deleting ${layerActors.get('0').get('actorTypeKey').value} it's not in`, availableVideoproofActorTypes.keys());
                    layerActors.splice(0, Infinity);
                    console.log('=>>>>actorsStructure: layerActors removed mis-typed actor');
                }
            //    else
            //        console.log(`Got a valid Type: ${layerActors.get('0').get('actorTypeKey').value} ... ${layerActors.get(0).get('instance').wrapped}`)
            }


            if(layerActors.size === 0) {
                // Pick the first by default.
                const typeKey = availableVideoproofActorTypes.keyOfIndex(1)
                  , videoproofActorDraft = createActor(typeKey, layerActors.dependencies)
                  ;
                layerActors.push(videoproofActorDraft);
                console.log('=>>>>actorsStructure: layerActors Added default actor:',typeKey);
            }

            // actually, the issue appears when the select changrs the type
            // we may not see it at this level of coherence functions, the
            // below will report another type than the final result will be...
            // .metamorphose({}) should help with that ..

            // TODO IMPORTANT: the actor struct might not yet have caught
            // up changing the actual wrapped instance, it's a CoherenceFunction
            // as well. We may have to metamorphose, especially if the goal
            // is to change the keyMoments of the thing.
        //    console.log(`layerActors[0] ${layerActors.get(0).get('instance').wrapped}`, layerActors, `${layerActors}`, layerActors.size);
        })
      , CoherenceFunction.create(
            ['activeActors', 'actorsStructure'],
            function* settleDynamicInstance({activeActors}){
            const layerActors = getEntry(activeActors, '0/instance/activeActors')
              , videoproofActor = layerActors.get('0')
              , draft = unwrapPotentialWriteProxy(videoproofActor, 'draft')
              ;
            if(!draft) {
            //    console.log(`settleDynamicInstance NOTHING TO DO`);
                return;
            }
            // console.log(`settleDynamicInstance checking ...`);
            // console.log(`draft ${draft} instance.wrapped: ${draft.get('instance').wrapped} actorTypeKey: ${draft.get('actorTypeKey').value} WrappedType:`,draft.get('instance').WrappedType, `\n`,
            //     `actorTypeModel:`, draft.get('actorTypeModel').value.get('typeClass').value, ` wrapped.constructor:`, draft.get('instance').wrapped.constructor);
            if( draft.get('actorTypeModel').value.get('typeClass').value !== draft.get('instance').wrapped.constructor) {
                // console.log(`Type missmatch detected, metamorphosing ...`);
                const immutable = yield * draft.metamorphoseGen();
                // I'm not sure if that immutable must be a draft, but I
                // go on with this for now, let's see if it works...
                layerActors.splice(0, 1, immutable);
                // if that is a proxy, we should be good to go!
                console.log('===>settleDynamicInstance metamorphosed unfinished dynamic struct ' + layerActors.get('0').wrapped);
            }
        })
      , ['showParameters', BooleanDefaultTrueModel]
      , CoherenceFunction.create(
            ['settleDynamicInstance', 'activeActors' , 'font', /*'duration', 'availableActorTypes', 'activeActors', 'font', 'installedFonts'*/]
          , function initVideoproofActor({activeActors, font}) {
            // always a loop

            const videoproofActor = getEntry(activeActors, '0/instance/activeActors/0/instance');

            if(videoproofActor.get('isLoop').value !== true){
                videoproofActor.get('isLoop').value = true;
                console.log(`=>>>>>initVideoproofActor is now a loop`);
            }
            ...
            const keyMoments = videoproofActor.get('keyMoments')
              ...
              , needInitialKeymoment = keyMoments.size === 0
                ...
                // The videoproofActor.dependencies could also be set
              ...
              ;
            // console.log('KeyMomentModel is:', KeyMomentModel, `via keyMoments: ${keyMoments}`, keyMoments);

            if(videoproofActor.get('localActiveFontKey').value !== font.value.fullName) {
                ...
                videoproofActor.get('localActiveFontKey').set(font.value.fullName);
                console.log(`=>>>>>initVideoproofActor localActiveFontKey is now `, font.value.fullName);
            ...
            if(needInitialKeymoment) {
                ...
                if(KeyMomentModel.fields.has('charGroup')) {
                    const defaultCharGroup = KeyMomentModel.fields.get('charGroup').fields.get('options').Model.defaultValue;
                    newKeyMoment.get('charGroup').get('options').set(defaultCharGroup);
                }
                keyMoments.push(newKeyMoment);

                console.log(`=>>>>>initVideoproofActor now has initial KeyMoment`);
            }
        })
      , CoherenceFunction.create(
            ['initVideoproofActor', 'activeActors']
          , function initEmptyActiveActors({activeActors}) {
            const videoproofActorActiveActors = getEntry(activeActors, '0/instance/activeActors/0/instance/activeActors', null);
            //console.log(`>>>> initEmptyActiveActors videoproofActorActiveActors ${videoproofActorActiveActors}:`, videoproofActorActiveActors);
            if(videoproofActorActiveActors === null) {
                // Nothing to do
            //    console.log(`>>>> initEmptyActiveActors: NOTHING TO DO ${videoproofActorActiveActors}`, videoproofActorActiveActors);
                return
            }
            //  , availableActorTypes = videoproofArrayV2.get('availableActorTypes')
              ;
            if(videoproofActorActiveActors.size === 0) {
            //    console.log(`VideoproofArrayV2Model initEmptyActiveActors activeActors is empty. adding VideoproofArrayV2CellActorModel ${videoproofActorActiveActors}`, videoproofActorActiveActors);
                insertNewCellActorModel(videoproofActorActiveActors, 'ensureDraft');
                console.log(`=>>>>>initEmptyActiveActors inserted new cell actor`);
            }
            //else
            //    console.log('>>>> initEmptyActiveActors DID NOTHING!')
        })
      , CoherenceFunction.create(
            ['initEmptyActiveActors', // so we know we are writing the keyMoments of the correct instance
            'activeActors', 'axesMath', 'installedFonts', 'font', 'duration']
          , function* updateRap({activeActors, axesMath, installedFonts, font, duration}) {
              // console.log('===>updateRap (depends on initEmptyActiveActors!');
            ...
            const videoproofActor = getEntry(activeActors, '0/instance/activeActors/0/instance')
              // , videoProofActors = videoproofActor.get('activeActors')
            ...
            //     // global font changed (is a dependency)
            //     //       isProxy: true isDraft: false
            //   , '\videoProofActors:', videoProofActors, 'isProxy:', _PotentialWriteProxy.isProxy(videoProofActors), 'isDraft:',  videoProofActors.isDraft
            //     // when only t changed:
            //     //      isProxy: true isDraft: false
            //     // added an actor:
            //     //       isProxy: true isDraft: false
            ...
            // );
            // if(videoProofActors.isDraft) {
            //     for(const [key, actor] of videoProofActors) {
            ...
            //console.log('videoproofActor.isDraft', videoproofActor.isDraft, 'axesMath.isDraft', axesMath.isDraft, 'fontHasChanged', fontHasChanged);

            // FIXME: CONSIDERED BROKEN
            if(videoproofActor.isDraft || axesMath.isDraft || fontHasChanged) {
                // Not a bomb proof heuristic but hopefully good enough
                // for this use case. << BAD heuristic... need updates!
                // also, activeActors is immutable in here!
                //console.log('activeActors:', activeActors, `${activeActors}`, activeActors.size);
                const videoproofActorDraft = unwrapPotentialWriteProxy(videoproofActor, 'ensureDraft');
                //console.log('videoproofActorDraft', videoproofActorDraft);
                // Right, so, videoproofActorDraft may be undefined, especially,
                // as we just metamorphosed it!
                ...
                console.log(`=>>>>>updateRap applied applyAxesMathLocations`);
            }
        })
    )
  ;
...
class UIParameterDisplay extends _BaseComponent {
    constructor(widgetBus, classes=[]) {
        super(widgetBus);
        this.element = this._domTool.createElement('div', {class: classes.join(', ')});
        this._insertElement(this.element);
    }
    update(changedMap) {
...
class UIParametersDisplay extends _BaseDynamicCollectionContainerComponent {
    constructor(widgetBus, _zones, extraClasses=[], customArgs=[]) {
        const classes = ['ui-parameters-display',...extraClasses]
          , parametersElement = widgetBus.domTool.createElement('div', {class: 'ui-parameters-display_values'})
          , localElement = widgetBus.domTool.createElement('div', {class: classes.join(' ')}, parametersElement)
    ...
    _localUpdate(changedMap) {
        if(changedMap.has('animationProperties@')) {
            const animationProperties = changedMap.get('animationProperties@')
              ...
              , propertyValuesMap = animationProperties.animanion.getPropertiesFromGlobalT(globalT)
              ;
            renderAxesParameterDisplay(this._parameters, propertyValuesMap);


            const backgroundColorPropertyName = `${COLOR}stageBackgroundColor`
              , getDefault = property => {
                    return [true, getRegisteredPropertySetup(property).default];
                }
              , [backgroundColor, ] = getColorFromPropertyValuesMap(backgroundColorPropertyName, propertyValuesMap, [getDefault(backgroundColorPropertyName)[1]])
              ...
              , blendedBackgroundColor = blend([{mode: 'rgb', r:1, g:1, b:1}, backgroundColor].filter(c=>c!==null), 'normal')
              , computedStyle = this._domTool.constructor.getComputedStyle(this._element)
              , cssParametersLabelColor = computedStyle.getPropertyValue('--parameters-label-color')
              , fallbackTextColor = {mode: 'oklch', l:0.669, c:0.18368, h: 248.8066} // oklch(0.669 0.18368 248.8066) // #09f
              , labelTextColor = cssParametersLabelColor !== '' && culoriConverter('oklch')(cssParametersLabelColor) || fallbackTextColor
                // TODO: check if background has changed before calculating etc.
                // but that requires caching the last value...
              , enhancedContrastColor = (backgroundColor !== null)
                    ? enhanceContrast(labelTextColor, blendedBackgroundColor)
                    : null
              ;
            if(enhancedContrastColor)
                this._element.style.setProperty('--parameters-label-high-contrast-text-color', culoriFormatCss(enhancedContrastColor));
            else
                // unset, will fall-back to whatever CSS defines
                this._element.style.removeProperty('--parameters-label-high-contrast-text-color');
            ...
            const colorPropertiesMap = [
              ...
              ;
            actorApplyCSSColors(this._element, propertyValuesMap, getDefault, colorPropertiesMap);
        }
    ...
    _createWrapper(rootPath) {
        const settings = {
          ...
          , dependencyMappings = [
              ...
              , [`animationProperties@${rootPath.append('instance')}`, 'animationProperties@']
            ]
          , Constructor = UIParameterDisplay
          , args = [['ui-parameters-display_layer'], ...this._customArgs]
          , childWidgetBus = this._childrenWidgetBus
          ;
        return this._initWrapper(childWidgetBus, settings, dependencyMappings, Constructor, ...args);
    }
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        const widgetWrapper = this.widgetBus.wrapper
          , _compareResult = isInitialUpdate
          ...
          , changeLocaldMap = widgetWrapper.getChangedMapFromCompareResult(isInitialUpdate, _compareResult)
          ;
        if(changeLocaldMap.size)
            this._localUpdate(changeLocaldMap);
...
class VideoproofArrayV2Controller extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        const generalControlsContainer = widgetBus.domTool.createElement('div', {class: 'sidebar-group'})
          ...
          , videoproofContainer = widgetBus.domTool.createElement('div', {class: 'videoproof_layout', style: "height: 100vh"})
          , zones = new Map([..._zones, ['general', generalControlsContainer], ['videoproof', videoproofContainer]])
          ;
        super(widgetBus, zones);
        ...
        const animationPropertiesRelativePath = Path.fromParts('activeActors', '0', 'instance')
        ...
        const widgets = [
          ...
          , [
              ...
              , [
                    [this.widgetBus.rootPath.append('t').toString(), 'globalT'], 'keyMoments', 'isLoop'
                ]
              , ContainerMeta
              , zones
              , initAnimanion
          ...
          , [
                ...
                [
                    ...
                    ["./activeActors/0/instance/activeActors/0/actorTypeKey", "value"],
                ],
                GenericSelect,
          ...
          , (()=>{
                const layerActorType = activatableActorTypes.get(LAYER_TYPE_KEY)
                return getActorWidgetSetup({
                      ...
                      , widgetRootPath: widgetBus.rootPath.append('activeActors', '0', 'instance')
                      , zones: new Map([...zones, ['layer', zones.get('videoproof')]])
...
//                {
//                    zone: 'general'
//                  , rootPath: widgetBus.rootPath.append('videoproofArrayV2', 'keyMoments', '0', 'languageTag')
//                }
//              , []
//              , UILanguageTag
//              , this._zones
...
//                {
//                    zone: 'before-layout'
//                  , rootPath: widgetBus.rootPath.append('videoproofArrayV2')
//                  , activationTest: ()=>{
//                        const showParameters = widgetBus.getEntry('./showParameters');
//                        return showParameters.value;
//                    }
//                }
//              , [
//                    // we'll use this to determine a high contrast text color.
//                    ['./keyMoments/0/stageBackgroundColor', 'stageBackgroundColor']
//                    // not sure if this is required here, as deeper, we'll
//                    // have the @properties of the layers to draw
...
//            ]
          , [
              ...
              , [
                    // TODO:
                    ['activeActors/0/instance/activeActors/0/instance/keyMoments', 'keyMoments']
    ...
    initialUpdate(...args) {
        this.widgetBus.wrapper.getProtocolHandlerImplementation('animationProperties@').resetUpdatedLog();
        console.log(`${this}.initialUpdate args:`, args, `${args[0]}`, args[0].keys());
