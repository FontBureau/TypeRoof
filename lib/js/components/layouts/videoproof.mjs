        ...
        , CoherenceFunction.create(
          ...
          , function actorSpecificOverrides({activeActors}) {
            const videoproofActorPath = '0/instance/activeActors/0'
              , videoproofActor = getEntry(unwrapPotentialWriteProxy(activeActors), videoproofActorPath, null)
              ;
            if(videoproofActor.get('actorTypeKey').value === 'VideoproofContextualActorModel') {
                ...
                if(cellAlignment.isEmpty) {
                    ...
                    // workaround.
                    const cellAlignmentDraft = getDraftEntry(unwrapPotentialWriteProxy(activeActors, 'ensureDraft'),
                            `${videoproofActorPath}/${cellAlignmentPath}`);
