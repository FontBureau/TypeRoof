...
export const AxesMathItemTypeModel = _AbstractGenericModel.createClass('AxesMathItemTypeModel')// => .value will be a concrete _BaseColorModel
  ...
  , AxesMathAxisLocationValuesModel = _AbstractListModel.createClass('AxesMathAxisLocationValuesModel', AxesMathAxisLocationValueModel)
    // An ordered map of axisTag: [list of LocationValue]
  , AxesMathLocationSetsModel = _AbstractOrderedMapModel.createClass('AxesMathLocationSetsModel', AxesMathAxisLocationValuesModel)
  /**
     * Multiplication
     *
  ...
  , AxesMathLocationsProductModel = _BaseAxesMathItemModel.createClass(
      ...
      , ['axesLocationSets', AxesMathLocationSetsModel]
        // options could include:
  ...
  , [availableAxesMathItemTypes, AXES_MATH_ITEM_TYPE_TO_KEY] = (()=>{
        const axesMathItemTypes = [
                ['LocationsSum', '+ locations sum', AxesMathLocationsSumModel]
              , ['LocationsProduct', '× locations product', AxesMathLocationsProductModel]
...
class UIAxesMathLocation extends _BaseContainerComponent {
    constructor(widgetBus, _zones, label) {
        const localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-location'})
...
class UIAxesMathLocationValues extends _BaseComponent {
    constructor(widgetBus) {
        super(widgetBus);
        [this.element] = this._initTemplate();
    }
    // jshint ignore: start
    static TEMPLATE = `<span>(UNINITIALIZED)</span>`;
    // jshint ignore: end
    _initTemplate() {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
        this._insertElement(element);
    ...
    update(changedMap) {
        if(changedMap.has('collection')) {
            ...
            const items = [];
            for(const [k, v] of changedMap.get('collection')){
                const logiVal = v.get('logicalValue').value
                      , value = logiVal === 'number'
                            ? v.get('numericValue').value
                            : `¡${logiVal}!`
                      ;
                    items.push(`${k}_   ${value}; `);
            }
 ...
 *     index = axesLocationSets.indexOfKey(key)
 *     value = axesLocationSets.get(key)
 *     axesLocationSets.splice(index, 1, [newKey, value]);
...
function validateOpenTypeTagString(tag) {
    ...
    if(currentCharCode < 0x41
            || currentCharCode > 0x5A && currentCharCode < 0x62
...
class UIAxesMathLocationsProductTag extends _BaseComponent {
    constructor(widgetBus, eventHandlers, label) {
        super(widgetBus);
        [this.element, this._input] = this._initTemplate(eventHandlers, label);
        this._lastValue = null;
    }
    // jshint ignore: start
    static TEMPLATE = `<label><!-- insert: label --><input
        type="text"
        minlength="1"
        maxlength="4"
        pattern="[A-Za-z]{1}[A-Za-z0-9]{0,3}"
        class="ui_axes_math-locations_product-tag" /></label>`;
    // jshint ignore: end
    _initTemplate(eventHandlers, label) {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
...
class UIAxesMathLocationsProduct extends _BaseDynamicCollectionContainerComponent {
    ROWTEMPLATE
    // jshint ignore: start
    static ITEM_TEMPLATE = `<div class="ui_axes_math-locations_product-item"></div>`;
    // jshint ignore: end
    constructor(widgetBus, _zones) {
        const localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-locations_product'}, 'UIAxesMathLocationsProduct')
          , childrensMainZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-locations_product-items'})
          , zones = new Map([..._zones, ['local', localZoneElement], ['main', childrensMainZoneElement]])
          ;
    ...
    _createWrapperValue(keyId, key) {
        const childWidgetBus = this._childrenWidgetBus
          , settings = {
                rootPath: this.widgetBus.rootPath.append('axesLocationSets')
    ...
    async _keyChangeHandler(index, event) {
        ...
        if(!valid) {
            ...
            console.warn(`FIXME: ${this}._keyChangeHandler invalid "${tag}":`, message);
            return;
        }
        // valid
        return await this._changeState(()=>{
            const axesLocationSets = this.getEntry('axesLocationSets');
            if(axesLocationSets.has(tag)) {
            // TODO: show a message to the user
                console.warn(`FIXME: ${this}._keyChangeHandler key exists "${tag}":`, message);
                return;
            }
            const value = axesLocationSets.getIndex(index);
            axesLocationSets.arraySplice(index, 1, [tag, value]);
        });
    }

    _createWrapperKey(keyId, index, label) {
        const childWidgetBus = this._childrenWidgetBus
          , settings = {
                zone: keyId
            }
          , dependencyMappings = [[keyId, 'key@']]
          , Constructor = UIAxesMathLocationsProductTag
          , eventHandlers = [
                ['input', this._keyChangeHandler.bind(this, index)]
            ]
          , args = [eventHandlers, label]
          ;
        return this._initWrapper(childWidgetBus, settings, dependencyMappings, Constructor, ...args);
    }
    ...
    _provisionWidgets() {
        ...
        const requiresFullInitialUpdate = new Set()
          , currentWidgets = []
          , axesLocationSets = this.getEntry('axesLocationSets')
        ...
        for( const {keyId, widgetWrapper, unregister} of
                this._keySlots.splice(axesLocationSets.size, Infinity)) {
            unregister();
        ...
        for(const [index, [key, /*axesLocationSet*/]] of axesLocationSets.indexedEntries()) {
            // keys
            ...
            if(!keyProtocolHandler.hasRegistered(keyId)) {
                const keyComponent = {value: null}
                    ...
                    , container = this._domTool.createFragmentFromHTML(this.constructor.ITEM_TEMPLATE).firstElementChild
                    ;
                this._zones.set(keyId, container);
                this._zones.get('main').append(container);
                const widgetWrapper = this._createWrapperKey(keyId, index, `${key}:${index}`);
                currentWidgets.push(widgetWrapper);
                requiresFullInitialUpdate.add(widgetWrapper);
                this._createWidget(widgetWrapper);
                // assert new index === index
                this._keySlots.push({keyId, widgetWrapper, unregister});
            }
            else {
...
class UIAxesMathLocationsSum extends _BaseDynamicCollectionContainerComponent {
    constructor(widgetBus, _zones, childrenSettings) {
        const localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-locations_sum'}, 'UIAxesMathLocationsSum')
          , childrensMainZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-locations_sum-items'})
          , zones = new Map([..._zones, ['local', localZoneElement], ['main', childrensMainZoneElement]])
          ;
        localZoneElement.append(childrensMainZoneElement);
        super(widgetBus, zones);
    ...
    _getWidgetSetup(rootPath) {
        ...
        switch (typeKey) {
            case 'LocationsSum':
                ...
                args = [this._zones, this._childrenSettings];
                break;
            case 'LocationsProduct':
                Constructor = UIAxesMathLocationsProduct;
                dependencyMappings.push(['axesLocationSets']);
                args = [this._zones];
                break;
            case 'Location':
                Constructor = UIAxesMathLocation;
                args = [this._zones, `typeLabel: ${typeLabel}`];
                break;
            default:
 ...
 * TODO:
 *
 * This needs an edit mode, when entered:
 *      add/remove/move locations
 *      edit locations:
 *          add/remove/move(?) key/value pairs
 *          the order here is also relevant!
 */

/**
 * This is the summary of consecutive locations. It's not for editing.
 */
let globI = 0
class UIAxesMathConsecutiveLocationsSummary extends _BaseComponent {
    // jshint ignore: start
    static TEMPLATE = `<div class="ui_axes_math-consecutive_locations_summary">
    UIAxesMathConsecutiveLocationsSummary
<ol class="ui_axes_math-consecutive_locations_summary-list"></ol>
</div>`;
    static ITEM_TEMPLATE = `<li class="ui_axes_math-consecutive_locations_summary-list_item"
    ></li>`;
    // jshint ignore: end
    constructor(widgetBus, clickHandler) {
        super(widgetBus);
        [this.element, this._list] = this._initTemplate(clickHandler);
    }
    _initTemplate(clickHandler) {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
          , list = element.querySelector('.ui_axes_math-consecutive_locations_summary-list')
          ;
        element.append(`${globI++}`);
        this._insertElement(element);
        if(clickHandler)
    ...
    update(changedMap) {
        if(changedMap.has('locations@')) {
            const locationKeys = changedMap.get('locations@').data
               , collect = new Map()
               ;
            // console.log('keys', ...locationKeys);
            for(const key of locationKeys) {
                const locations  = this.getEntry(this.widgetBus.rootPath.append('items', key, 'instance', 'axesLocations'));
                for(const [k, v] of locations) {
                    if(!collect.has(k))
                        // FIXME: It could be treated as a set! The case
                        // of repeated values may be rather undesired.
                        collect.set(k, []);
                    const logicalValue = v.get('logicalValue').value
                      , numericOrEmpty = v.get('numericValue')
                      ;
                    collect.get(k).push(logicalValue === 'number' ? numericOrEmpty.value : logicalValue);
                }
            }
            this._domTool.clear(this._list);
            for(const [k, values] of collect) {
                const item = this._domTool.createFragmentFromHTML(this.constructor.ITEM_TEMPLATE).firstElementChild;
                item.append(
                      this._domTool.createElement('strong',{}, k)
                    , ' '
                    , this._domTool.createElement('span',{}, values.join(', '))
                );
...
class UIAxesMathLocationEdit extends _BaseComponent {
    //jshint ignore:start
    static TEMPLATE = `<div class="ui_axes_math-location_edit">NOT INITIALIZED</div>`
    //jshint ignore:end
    constructor(widgetBus) {
        super(widgetBus);
        [this.element] = this._initTemplate();
    ...
    update(changedMap) {
        ...
        if(changedMap.has('axesLocations')) {
            this._domTool.clear(this.element);
            const axesLocations = changedMap.get('axesLocations');
            for(const [axisTag, value] of axesLocations) {
                console.log(axisTag, 'value:', value);
                const logicalValue = value.get('logicalValue').value
                  , numericOrEmpty = value.get('numericValue')
                  , entry = this._domTool.createElement('li', {}, [
                        `${axisTag}: `, this._domTool.createElement('strong', {},
                            logicalValue === 'number' ? numericOrEmpty.value : logicalValue)
                    ]);
...
// CAUTION this is a goner BUT the trick calling
//      _BaseContainerComponent.prototype._provisionWidgets.call(this):
//  in a _BaseDynamicCollectionContainerComponent is a keeper! FIXME: how to preserve?
// Maybe! it is not better than implementing it via two elements, which would
// create a better separation of concerns.
class UIAxesMathConsecutiveLocationsEditing extends _BaseDynamicCollectionContainerComponent {
    constructor(widgetBus, _zones) {
        const localZoneElement = widgetBus.domTool.createElement('div'
                , {'class': 'ui_axes_math-consecutive_locations_editing'}
                , ` ${globI++} UIAxesMathConsecutiveLocationsEditing`)
            // not sure we need this, items can go right into localZoneElement
            // which then should simply replace the main zone.
          , childrensMainZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_axes_math-consecutive_locations_editing-items'})
          , zones = new Map([..._zones, ['local', localZoneElement], ['main', childrensMainZoneElement]])
          ;
        super(widgetBus, zones);
        this._insertElement(localZoneElement);
        this._locationWidgets = new Map();
        const widgets = [
            [
                {zone: 'local'}
              , []
        ...
        this._initWidgets(widgets);
        this._firstCollectionWidgetIndex = this._widgets.length;
    }

    _createWrapper(key) {
        const childWidgetBus = this._childrenWidgetBus
          , settings = {
                rootPath: this.widgetBus.rootPath.append('items', key, 'instance')
              , zone: 'main'
            }
          , dependencyMappings = ['axesLocations']
          , Constructor = UIAxesMathLocationEdit
          , args = [/*this._zones*/]
          ;
        return this._initWrapper(childWidgetBus, settings, dependencyMappings, Constructor, ...args);
    }
    ...
    _provisionWidgets() {
        // we don't need the removed widgets here, as we have them also in
        // this._locationWidgets. However, it is possible to not use
        // this._locationWidgets as we know the order of the keys is fixed
        // and we can identify the keys by looking at the rootPath of the widget.
        this._widgets.splice(this._firstCollectionWidgetIndex, Infinity)
        ...
        const requiresFullInitialUpdate = _BaseContainerComponent.prototype._provisionWidgets.call(this)
          , locationKeys = this.getEntry('locations@').data
          , currentWidgets = []
          ;
        // The data type of the children is all the same, and so are
        // their widgets. However, we don't update widgets with changed
        // paths, we just build new ones or discard the ones that are no
        // longer in the locations.
        {
            const locationKeysSet = new Set(locationKeys);
            for(const key of this._locationWidgets.keys()) {
                if(locationKeysSet.has(key))
                    continue;
                const widgetWrapper = this._locationWidgets.get(key);
                this._destroyWidget(widgetWrapper);
                this._locationWidgets.delete(key);

            }
        }
        for(const key of locationKeys) {
            let widgetWrapper;
            if(this._locationWidgets.has(key)) {
                // keeper, reinsert
                widgetWrapper = this._locationWidgets.get(key)
                widgetWrapper.reinsert();
            }
            else {
                // create new, insert
                widgetWrapper = this._createWrapper(key);
                this._createWidget(widgetWrapper);
                this._locationWidgets.set(key, widgetWrapper);
                requiresFullInitialUpdate.add(widgetWrapper);
            }
            currentWidgets.push(widgetWrapper);
        }
...
/**
 * This is for either editing of consecutive locations or displaying
 * UIAxesMathConsecutiveLocationsSummary.
 *
 * nicely executed but probably doomed...
 */
class UIAxesMathConsecutiveLocations extends _BaseContainerComponent {
    constructor(widgetBus, zones) {
        super(widgetBus, zones);
        this._isEditing = false;
        const locationsId = widgetBus.getExternalName('locations@')
          , widgets = [
            [
                {
                    zone: 'main'
              ...
              , UIAxesMathConsecutiveLocationsSummary
              , event=>{ // so called "clickHandler"
                    event.preventDefault();
                    // so far a toggle
                    this._isEditing = !this._isEditing;
                    // We don't store the isEditing flag in the global
                    // application state, it's only ephemeral local state
                    // that way, we have to update the widget manually.
                    // update requires a comparison object and initial
                    // update creates just that. I'm thinking it may be
                    // better to store that flag on the model. I.e.
                    // switching between widgets in edit mode would persist
                    // editing state in sub-widgets, so one would not have
                    // to re-open (for editing) all sub widgets. It's not
                    // trivial to save those flags though, because the
                    // consecutive locations are rather informal.
                    const rootState = this.getEntryRaw('/');
          ...
          , [
              ...
              , UIAxesMathConsecutiveLocationsEditing
              , this._zones
            ]
        ];
        this._initWidgets(widgets);
...
/* grayZee*/
// FIXME: Remove...
export class UIAxesMathLocationsProductItems extends _BaseDynamicCollectionContainerComponent {
    constructor(widgetBus, zones, childrenSettings) {
        widgetBus.wrapper.setProtocolHandlerImplementation(
            ...SimpleProtocolHandler.create('locations@', {treatAdressAsRootPath: false}));
        super(widgetBus, zones);
        this._childrenSettings = childrenSettings;
        this._locationsUnregisters = [];
        this._widgetsState = [];
    }

    _createWrapper(type, data) {
        const childWidgetBus = this._childrenWidgetBus
          , dependencyMappings = []
          , settings = {zone: 'main'}
          , args = []
        ...
        let Constructor;
        if(type === 'Locations') {
            const locationsProtocolHandler = this.widgetBus.wrapper.getProtocolHandlerImplementation('locations@')
              , {locationsIndex} = data
              , locationsId = locationsProtocolHandler.getId(locationsIndex)
              ;
            settings.rootPath = this.widgetBus.rootPath;
            dependencyMappings.push([locationsId, 'locations@']);
            Constructor = UIAxesMathConsecutiveLocations;
            args.push(this._zones);
        }
        else {
            settings.rootPath = this.widgetBus.rootPath.append(data.key);
            settings.relativeRootPath = Path.fromParts('.', 'instance');
            // Constructor = UIAxesMathList;
            args.push(this._zones, data.label);
        }
        return this._initWrapper(childWidgetBus, settings, dependencyMappings, Constructor, ...args);
    }
    ...
    _provisionWidgets(/*compareResult*/) {
        const requiresFullInitialUpdate = new Set()
          , deletedWidgets = []
          , currentWidgets = []
          , locationsProtocolHandler = this.widgetBus.wrapper.getProtocolHandlerImplementation('locations@')
          ;
        // consecutive Locations go into a UIAxesMathCombinedLocations
        // Lists go to UIAxesMathList which decides
        // There's a complication with updates in here: it would be nice
        // if we could keep UI items alive, but: paths will change!
        // When Paths have changed, it's likely better to recreate the
        // items. Except for the UIAxesMathCombinedLocations which won't
        // have fixed path, it's more like a slice. And Each item of the
        // slice could change.
        // startKey, endKey OR startKey, length? I thinkt the former...
        //
        // We may get away with only looking at the collection, comparing
        // the current state, not looking at compareResult...
        ...
        const collection = this.getEntry('collection')
          ...
          ;
        {
            const consecutiveLocations = []
              , flushConsecutiveLocations = ()=>{
                  if(consecutiveLocations.length) {
                      const locationsIndex = locationsCounter++
                        , data = {
                              locationsIndex
                              // This data is required at the locationsProtocolHandler
                            , keys: consecutiveLocations.splice(0, Infinity)
                          }
                        ;
                      structure.push(['Locations', data]);
                  }
                }
              ;
            for(const [key, item] of collection) {
                const typeKey = item.get('axesMathItemTypeKey').value
                  , typeModel = item.get('axesMathItemTypeModel')
                  , typeLabel = typeModel.get('label').value
                  ;
                if(typeKey === 'Location')
                    consecutiveLocations.push(key);
                else {
                    flushConsecutiveLocations();
                    structure.push([typeKey, {key, label:typeLabel}]);
                }
            }
        ...
        // remove the buckets that are not required anymore.
        this._locationsUnregisters.splice(locationsCounter, Infinity)
                                  .map(unregister=>unregister())
                                  ;
        for(const [i, [type, data]] of structure.entries()) {
            // register the locations
            if(type === 'Locations') {
                // Make sure the consecutive locations are available as a dependency
                const {locationsIndex, keys} = data
                   , locationsId = locationsProtocolHandler.getId(locationsIndex)
                   ;
                if(!locationsProtocolHandler.hasRegistered(locationsId)) {
                    const component = {data: []}
                      , unregister = locationsProtocolHandler.register(locationsId, component)
                      ;
                    // assert this._locationsUnregisters.length === locationsIndex
                    //       such that the new index will be === locationsIndex
                    this._locationsUnregisters.push(unregister);
                }
                const component = locationsProtocolHandler.getRegistered(locationsId);
                // the type is the same, so a regular update should be
                // sufficient.
                // CAUTION however, for locations, the locations items
                // may have changed and must be updated...
                if(component.data[0] !== data.keys[0]
                        || component.data.at(-1) !== data.keys.at(-1)) {
                    component.data = keys;
                    locationsProtocolHandler.setUpdated(locationsId);
                }
            }
            // build the widgets
            const {type: currentType, widgetWrapper, locationsIndex
                      // all is undefined if there's no this._widgetsState[i]
                  } = this._widgetsState[i] || {}
              , typeChanged = currentType !== type
              , locationsIndexChanged = type === 'Locations' && data.locationsIndex !== locationsIndex
              , requireNew = typeChanged || locationsIndexChanged
              ;
            if(!requireNew && widgetWrapper) {
                // just keep, it will get updated.
                currentWidgets.push(widgetWrapper);
                widgetWrapper.reinsert();
                continue
            }
            if(widgetWrapper !== undefined)
                // delete the widget and create a new one of the new type
                deletedWidgets.push(widgetWrapper);

            const newWrapper = this._createWrapper(type, data);
            this._widgetsState[i] = {type, widgetWrapper:newWrapper, ...data};
            currentWidgets.push(newWrapper);
            requiresFullInitialUpdate.add(newWrapper);
            this._createWidget(newWrapper);
        }

        for(const {widgetWrapper} of this._widgetsState.splice(structure.length, Infinity))
            deletedWidgets.push(widgetWrapper);
    ...
    update(...args) {
        this.widgetBus.wrapper.getProtocolHandlerImplementation('locations@').resetUpdatedLog();
        super.update(...args);
    }
    initialUpdate(...args) {
        this.widgetBus.wrapper.getProtocolHandlerImplementation('locations@').resetUpdatedLog();
          ...
          , function initAxesMath({axesMath}) {
            ...
            if(items.size === 0) {
                ...
                const locationsProductItem = createAxesMathItem('LocationsProduct', items.dependencies)
                  , locationSets = getEntry(locationsProductItem, './instance/axesLocationSets')
                  ;
                ...
                for(const axisTag of ['opsz', 'wdth', 'wght']) {
                    ...
                    for(const locationRawValue of ['default', 'min', 'max', 400, 100, 1000]) {
                ...
                for( const locationData of  [
                    ...
                    const locationItem = createAxesMathItem('Location', items.dependencies)
                      , axesLocations = getEntry(locationItem, './instance/axesLocations')
                      ;
                    for(const [axisTag, locationRawValue] of Object.entries(locationData)) {
                        const axisLocationValue = axesLocations.constructor.Model.createPrimalDraft(axesLocations.dependencies);
                        setAxisLocationValue(axisLocationValue, locationRawValue);
                        axesLocations.set(axisTag, axisLocationValue);
                    }
