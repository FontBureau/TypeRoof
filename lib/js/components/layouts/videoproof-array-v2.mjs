...
const VideoproofArrayActorModel = activatableActorTypes.get('VideoproofArrayActorModel').get('typeClass').value
  , VideoproofArrayV2ActorModel = activatableActorTypes.get('VideoproofArrayV2ActorModel').get('typeClass').value
  ;
...
<ol class="ui_key_moments_link_navigation-list"></ol>
    ...
    _clickHandler(event) {
        ...
        const videoproofArrayPath =  Path.fromString(this.parentAPI.getExternalName('keyMoments')).append('..')
...
class VideoproofArrayController extends _BaseContainerComponent {
    constructor(parentAPI, zones) {
        ...
        const animationPropertiesKeyV2 = `animationProperties@${parentAPI.rootPath.append('videoproofArrayV2')}`
          , updateDefaultsDependenciesV2 = [
                [animationPropertiesKeyV2, '@animationProperties']
            ]
          , _updateDefaultsNamesV2 = new Set(Array.from(zip(...updateDefaultsDependencies))[1])
          , requireUpdateDefaultsV2 = changedMap=>Array.from(changedMap.keys())
                                        .some(name=>_updateDefaultsNames.has(name))
         ;
        ...
        const widgets = [
          ...
          , [
                {
                    'animationProperties@': animationPropertiesKeyV2
                }
              , [
                    //...parentAPI.wrapper.getDependencyMapping(parentAPI.wrapper.constructor.DEPENDECIES_ALL)
                    ['t', 'globalT'], ['videoproofArrayV2/keyMoments', 'keyMoments'], ['videoproofArrayV2/isLoop', 'isLoop'] // same as in ContainerMeta!
                    // changedMap.has('keyMoments') || changedMap.has('isLoop') || changedMap.has('@parentProperties')
                ]
              , AnimationLiveProperties
              , initAnimanion // This usage instance won't receive parentProperties.
          ...
          , getActorWidgetSetup({
              ...
              , widgetRootPath: parentAPI.rootPath.append('videoproofArrayV2')
              , zones: new Map([...zones, ['layer', zones.get('layout')]])
              , get layerBaseClass() {throw new Error('NOT IMPLEMENTED get layerBaseClass');}
              , getActorWidgetSetup() {throw new Error('NOT IMPLEMENTED getActorWidgetSetup');}
            })
