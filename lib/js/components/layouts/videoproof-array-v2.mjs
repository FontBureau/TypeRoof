...
class UIAxesMathLocationValues extends _BaseDynamicCollectionContainerComponent {
    ...
    reinsertChildren() {
        // For new children:
        // NOTE: this also strongly implies that this widget has a host!
        this._zones.set('main', this.widgetBus.wrapper.host);
        // FIXME: this method shows a lot that can go wrong with the zones
        // concept! For one, having a zone key that is not stored along
        // with the wrapper! it's resolved in _initWrapper, even before
        // the ComponenWrapper constructor.
        // So, in that way, settings.zone is not even preserved after
        // _initWrapper. Maybe hostElement could directly be a choice
        // for settings.
        // It's also enormous that zones is an argument for all of these
        // containers.
        for(const wrapper of this._widgets)
            wrapper.reinsert(this.widgetBus.wrapper.host);
    }
    ...
    _dropHandler(dropTargetItem, event) {
        ...
        if(type === DATA_TRANSFER_TYPES.AXESMATH_LOCATION_VALUE_PATH) {
            const sourcePathString = event.dataTransfer.getData(DATA_TRANSFER_TYPES.AXESMATH_LOCATION_VALUE_PATH)
    ...
    _initTemplate(eventHandlers) {
        ...
        for(const [event, fn, ...args] of eventHandlers) {
            if(event.startsWith('drag'))
                this._dragHandlers.set(event, fn)
            else
...
class _BaseUIAxesMap extends _BaseDynamicCollectionContainerComponent {
    ...
    // Display alphabetic order: FIXME: is a bit broken
    ...
    *_alphaOrderingGenerator (itemsGen) {
            ...
            // values
            {
                ...
                if(this._locationSetWidgets.has(key)) {
                    ...
                    if(widgetWrapper.host !== host) {
                        ...
                        if(widgetWrapper.widget.reinsertChildren)
                            // widgetWrapper reinsert doesn't take effect
                            // deeply.
                            widgetWrapper.widget.reinsertChildren();
                    }
                }
                else {
                    // create new, insert
    ...
    _dropHandler(dropTargetItem, event) {
        ...
        this._setDropTargetIndicator(event.currentTarget);
        const {rootPath: targetRootPath, keyId} = dropTargetItem
          , keyProtocolHandler = this.widgetBus.wrapper.getProtocolHandlerImplementation('key@')
          , key = keyProtocolHandler.getRegistered(keyId).value
...
class UIAxesMathLocationsSum extends _BaseDynamicCollectionContainerComponent {
    ...
    _getWidgetSetup(rootPath) {
        ...
        switch (typeKey) {
            ...
            case 'Location':
                ...
                args = [this._zones, eventHandlers, typeLabel];
                break;
            default:
    ...
    _dropHandler(dropTargetItem, event) {
        ...
        if(type === DATA_TRANSFER_TYPES.AXESMATH_ITEM_PATH) {
            const sourcePathString = event.dataTransfer.getData(DATA_TRANSFER_TYPES.AXESMATH_ITEM_PATH)
              , sourcePath = Path.fromString(sourcePathString)
              ;
            return this._move(sourcePath, targetPath, insertPosition);
        }
    ...
    _create(typeKey, targetPath, insertPosition) {
        console.log(`${this}._create >>>> typeKey: ${typeKey} AT targetPath: ${targetPath} insertPosition ${insertPosition}`);
        console.error(`NOT IMPLEMENTED: ${this}._create`);
    }
}
      ...
      , CoherenceFunction.create(
          ...
          , function initAxesMath({axesMath}) {
            ...
            if(items.size === 0) {
                ...
                const locationsSumItem = createAxesMathItem('LocationsSum', items.dependencies)
                  , sumItems = getEntry(locationsSumItem, './instance/items')
                  ;
                for(let i=items.length-2,l=items.length;i<l;i++)
                    sumItems.push(items.get(i));
                items.push(locationsSumItem);
            }
        })
