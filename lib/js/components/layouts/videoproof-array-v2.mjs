...
class UIVideoproofArrayLayersOLD extends _BaseComponent {
     // jshint ignore: start
    static TEMPLATE = `<div class="ui_videoproof_array_layers">
<label><!-- insert: label --></label>
<ol class="ui_videoproof_array_layers-list"></ol>
<a class="ui_videoproof_array_layers-add">+</a>
</div>`;
    static ITEM_TEMPLATE = `<li class="ui_videoproof_array_layers-list_item"
    ><a class="ui_videoproof_array_layers-list_item-input"
        ><!-- insert: label --></a></li>`;
    // jshint ignore: end
    constructor(widgetBus, label) {
        super(widgetBus);
    ...
    _initTemplate(label) {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
          , list = element.querySelector('.ui_videoproof_array_layers-list')
          , add = element.querySelector('.ui_videoproof_array_layers-add')
          ;
        this._domTool.insertAtMarkerComment(element, 'insert: label', label);


        // Comparison is not always what we're after here:
        //    it would be nice to have a none-compare version that
        //    is more minimalistic than what is required for the compare
        //    version.
        // TODO: it should be possible to
        //  - reorder items -> drag and drop
        //  - change font of an item
        //  - change colors of an item (foreground/background)
        //          however, we do something smartish here.

        add.addEventListener('click', this._changeStateHandler((event)=>{
            // FIXME: should get the font to use from a <select>
            event.preventDefault();
            const activeActors = this.widgetBus.getEntry('layers') // activeActors
              , cellActorModel = createActor('VideoproofArrayV2CellActorModel', activeActors.dependencies)
              ;
            activeActors.push(cellActorModel);
        }));
        this._insertElement(element);
        return [element, list, add];
    }
    // controls to add, delete
    // don't delete the last item though
    // select a font! => default should be an option, to inherit ...

    update(changedMap) {
        if(changedMap.has('layers')) {
            const layers = changedMap.get('layers')
              , items = []
              ;
            for(const [key, actor] of layers) {
                const layer = actor.get('instance')
                   , listItem = this._domTool.createFragmentFromHTML(this.constructor.ITEM_TEMPLATE).firstElementChild
                   , label = `${key}: ${layer.wrapped} ${layer.get('font').value.nameVersion}`
                   ;
                this._domTool.insertAtMarkerComment(listItem, 'insert: label', label);
                items.push(listItem);
            }
...
class UIVideoproofArrayLayer extends _BaseComponent {
    // jshint ignore: start
    static TEMPLATE = `<div class="ui-videoproof_array-layer">(uninitialized)</div>`;
    // jshint ignore: end
    ...
    update(changedMap) {
        if(changedMap.has('font'))
            this.element.textContent = changedMap.get('font').value.nameVersion;
    }
}
