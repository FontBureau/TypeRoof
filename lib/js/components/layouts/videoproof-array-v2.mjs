...
class UIAxesMathLocationValue extends _BaseComponent {
    ...
    _dragstartHandler(event) {
        ...
        event.dataTransfer.setData('text/plain', `[TypeRoof ${DATA_TRANSFER_TYPES.AXESMATH_LOCATION_VALUE_PATH}: ${path}]`);
...
class _BaseUIMap extends _BaseDynamicCollectionContainerComponent {
    // jshint ignore: start
    ...
    static get ORDER_STRATEGY() {
        throw new Error(`NOT IMPLEMENTED ${this.name} static ORDER_STRATEGY`);
    }
    static get KEY_DATA_TRANSFER_TYPE() {
        throw new Error(`NOT IMPLEMENTED ${this.name} static KEY_DATA_TRANSFER_TYPE`);
    }
    static get KEY_PLAIN_DATA_TRANSFER_TYPE() {
        throw new Error(`NOT IMPLEMENTED ${this.name} static KEY_PLAIN_DATA_TRANSFER_TYPE`);
    }
    ...
    _createWrapperKey(keyId, index) {
        throw new Error(`NOT IMPLEMENTED ${this}._createWrapperKey (for keyId: "${keyId}" index: "${index}").`);
    }
    get _initialWidgets() {
    ...
    constructor(widgetBus, _zones, eventHandlers, label, dragAndDrop=false) {
        ...
        const orderStrategy = dragAndDrop
                  ...
                  // by dragAndDrop UI.
                ? _BaseUIMap.ORDER_STRATEGY_NATURAL
                : this.constructor.ORDER_STRATEGY || _BaseUIMap.ORDER_STRATEGY_NATURAL
                ;
        console.log(`new ${this} ORDER_STRATEGY: ${orderStrategy.toString()} DRAG_AND_DROP: ${!!dragAndDrop}`);
        Object.defineProperties(this,{
                BASE_CLASS: {
                    value: `${this.constructor.ROOT_CLASS}_${this.constructor.TYPE_CLASS_PART}`
                }
    ...
    get initialWidgets() {
        // This way a sub-class can extend initialWidgets
        // within the constructor `super(...)` and fully use `this`.
        const widgets = []
        return widgets;
    }
    ...
    *_alphaOrderingGenerator (itemsGen) {
        ...
        console.log('_alphaOrderingGenerator keys:', ...keys);
    ...
    async _keyCreateSubmitHandler(event) {
        ...
        return await this._changeState(()=>{
            ...
            if(this._onCreateTagPrimeValue)
                this._onCreateTagPrimeValue(value, key);
...
class _BaseUIAxesMap extends _BaseUIMap {
    // jshint ignore: start
    static ROOT_CLASS = `ui_axes_math-map`
    // jshint ignore: end
    constructor(...args) {
        // this is a stub, so far not required.
        super(...args);
    }
...
class UIAxesMathLocationsProduct extends _BaseUIAxesMap {
    ...
    static ORDER_STRATEGY = _BaseUIMap.ORDER_STRATEGY_NATURAL;
    static KEY_DATA_TRANSFER_TYPE = DATA_TRANSFER_TYPES.AXESMATH_LOCATION_VALUES_KEY_PATH;
    static ITEM_DATA_TRANSFER_TYPE = DATA_TRANSFER_TYPES.AXESMATH_ITEM_PATH;
...
class UIAxesMathLocationsSum extends _BaseDynamicCollectionContainerComponent {
    ...
    _dragstartHandler(event) {
        ...
        event.dataTransfer.setData('text/plain', `[TypeRoof ${DATA_TRANSFER_TYPES.AXESMATH_ITEM_PATH}: ${path}]`);
        this.element.classList.add('dragging');
