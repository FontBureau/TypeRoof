...
class UIAxesMathLocationValue extends _BaseComponent {
    constructor(widgetBus) {
        super(widgetBus);
        [this.element, this._input] = this._initTemplate();
    }
    // jshint ignore: start
    static TEMPLATE = `<label><!-- insert: label --><input type="text" /></label>`;
    // jshint ignore: end
    _initTemplate() {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
            , input = element.querySelector('input')
            ;
        this._domTool.insertAtMarkerComment(element, 'insert: label', `${this}`);
        this._insertElement(element);
    ...
    update(changedMap) {
        if(changedMap.has('value')) {
           const value = changedMap.get('value')
             ...
             , strVal = logiVal === 'number'
                            ? value.get('numericValue').value
                            : `¡${logiVal}!`
              ;
...
class UIAxesMathLocationValues extends _BaseDynamicCollectionContainerComponent {
    ...
    _createWrapper(rootPath) {
        console.log(`${this}._createWrapper rootPath: ${rootPath}`,
            '\n    getEntry:', this.getEntry(rootPath));
...
class UIAxesMathLocationsProductTag extends _BaseComponent {
    ...
    _initTemplate(eventHandlers, label) {
        const element = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
            , input = element.querySelector('input')
            ;
        this._domTool.insertAtMarkerComment(element, 'insert: label', `${this} ${label}`);
        for(const args of eventHandlers)
