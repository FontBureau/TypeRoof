...
const // We don't do  prosemirror SchemaSpec yet, but we may need it to also
  ...
  JSONModel = _AbstractStructModel.createClass(
    ...
    [
      ...
      (JSONModel) =>
        _AbstractOrderedMapModel.createClass("JSONMapModel", JSONModel, {
    ...
    [
      "array",
      _AbstractStructModel.WITH_SELF_REFERENCE,
      (JSONModel) => _AbstractListModel.createClass("JSONListModel", JSONModel),
    ],
    CoherenceFunction.create(
      ...
      function checkValues({ type, ...data }) {
        ...
        for (const [typeKey, item] of Object.entries(data)) {
          ...
          if (typeKey === "string" || typeKey === "number") item.clear();
          else if (typeKey === "object") item.arraySplice(0, Infinity);
  ...
  NodeModel = _AbstractStructModel.createClass(
    ...
    [
      "content",
      _AbstractStructModel.WITH_SELF_REFERENCE,
      (NodeModel) => _AbstractListModel.createClass("FragmentModel", NodeModel),
    ],
...
function fromMetaModelJSON(value) {
  ...
  else if (type === "string" || type === "number") return value.get(type).value;
  else if (type === "array")
  ...
  update(changedMap) {
    ...
    if (changedMap.has("activeActors")) {
      ...
      this._actorsElement.append(
        ...this._renderLayer(basePath, activeActors, { counter: 0, depth: 0 }),
      );
      if (!editingActor.isEmpty) this._markActiveItems(editingActor.value);
...
function _getFallback(fullKey, modelDefaultValue = _NOTDEF) {
  ...
  return fallback === modelDefaultValue ? modelDefaultValue : fallback.default;
}
...
class NodeSpecPropertiesManager extends _BaseByPathContainerComponent {
  ...
  _createEmptyWrappers() {
    const widgets = [
      [{ zone: "local" }, [], StaticTag, "span", {}, "(Select a NodeSpec)"],
    ];
    // this._initWrapper(this._childrenWidgetBus, settings, dependencyMappings, Constructor, ...args);
    return widgets.map((widgetArgs) =>
      this._initWrapper(this._childrenWidgetBus, ...widgetArgs),
...
class UIStylePatchesMap extends _UIBaseMap {
  ...
  _onClickHandler(key) {
    this._changeState(() => {
      ...
      // this is a toggle
      if (!selected.isEmpty && selected.value.equals(path)) selected.clear();
  ...
  constructor() {
    Object.defineProperties(this, {
      _localPropertyValuesMap: {
        get: () => {
          throw new Error(`NOT IMPLEMENTED {this}._localPropertyValuesMap.`);
        },
        set: (value) => {
          Object.defineProperty(this, "_localPropertyValuesMap", { value });
        },
        configurable: true,
        writtable: true,
      },
      _propertyValuesMap: {
        get: () => {
          throw new Error(`NOT IMPLEMENTED {this}._propertyValuesMap.`);
        },
        set: (value) => {
          Object.defineProperty(this, "_propertyValuesMap", { value });
        },
        configurable: true,
        writtable: true,
      },
...
export class HierarchicalScopeTypeSpecnion extends _BaseTypeSpecnion {
  ...
  _initPropertyValuesMaps() {
    ...
    const localPropertyValuesMap = LocalScopeTypeSpecnion.initPropertyValuesMap(
        rawProperties,
        parentPropertyValuesMap,
      ),
      // All properties in local override properties in parent
  ...
  update(changedMap) {
    ...
    if (typeSpecnionChanged) {
      ...
      const [identifier, protocolHandlerImplementation] =
        this.widgetBus.getProtocolHandlerRegistration(`typeSpecProperties@`);
  ...
  *_propertiesGenerator(simpleStylePatch) {
  ...
  update(changedMap) {
    ...
    if (requireUpdate) {
      ...
      const [identifier, protocolHandlerImplementation] =
        this.widgetBus.getProtocolHandlerRegistration(`stylePatchProperties@`);
      protocolHandlerImplementation.setUpdated(identifier);
    }
  }
}
...
export class StyleLinkLiveProperties extends _BaseComponent {
  ...
  update(changedMap) {
    ...
    if (this._typeSpecnion !== newTypeSpecnion) {
      ...
      // This should update subscribers that need to re-initialize
      const [identifier, protocolHandlerImplementation] =
        this.widgetBus.getProtocolHandlerRegistration(`styleLinkProperties@`);
      protocolHandlerImplementation.setUpdated(identifier);
    }
  }
...
class UIDocumentTextRun extends _BaseContainerComponent {
  ...
  _setNodeToElement(styleName) {
    ...
    if (styleName !== null) element.setAttribute("data-style-name", styleName);
  ...
  _createStylerWrapper(styleLinkProperties, styleName = null) {
    ...
    if (styleLinkProperties === null) {
      // node to textNode
      if (this.node.nodeType !== Node.TEXT_NODE) this._setNodeToTextNode();
      return null;
    }
  ...
  _provisionWidgets(...args /* compareResult */) {
    ...
    if (oldId === -1) {
      ...
      if (
        oldWrapper.dependencyReverseMapping.get("styleLinkProperties@") !==
        styleLinkPropertiesId
      ) {
...
class UIDocumentTypeSpecStyler extends _BaseComponent {
  ...
  update(changedMap) {
    const propertiesData = [
        ...
        [
          "generic/columnWidth",
          (element, value, propertiesValueMap, getDefault /*, useUnit*/) => {
...
class UIDocumentElement extends _BaseContainerComponent {
  ...
  _provisionWidgets(/* compareResult */) {
    ...
    if (oldId === -1) {
      ...
      const oldWrapper = this._widgets[oldId];
      if (
        oldWrapper.dependencyReverseMapping.get("typeSpecProperties@") !==
        typeSpecProperties
      ) {
        const newWrapper =
          this._createTypeSpecStylerWrapper(typeSpecProperties);
 ...
 * where asPath===true;
 */
function _getTypeSpecPropertiesIdMethod(
  pathOfTypes,
  asPath = false,
...
class ProseMirror extends _BaseComponent {
  ...
  _rawCreateMetamodelNode(cacheMap /* null or a map*/, pmNode, dependencies) {
    ...
    if (pmNode.type.name === "text") {
      ...
      for (let i = 0, l = pmNode.content.childCount; i < l; i++) {
        ...
        contentDraft.push(
          this._createMetamodelNode(cacheMap, pmChildNode, dependencies),
        );
      }
    }
  ...
  _createMetamodelNode(cacheMap /* null or a map*/, pmNode, dependencies) {
    if (cacheMap !== null && cacheMap.has(pmNode)) return cacheMap.get(pmNode);
    ...
    if (cacheMap !== null) mapSetBiDirectional(cacheMap, pmNode, immutableNode);
    return immutableNode;
  }
...
class UIProseMirrorMenu extends _BaseComponent {
  ...
  updateView(view /*, prevState = null*/) {
    ...
    for (const [typeSpec, path] of typeSpecs) {
      ...
      for (const style of stylePatches.keys()) allStylesSuperSet.add(style);
    }
    ...
    const widgets = [
      ...
      [
        ...
        [
          [".", "typeSpec"],
          [
            widgetBus.rootPath.append("stylePatchesSource").toString(),
            "stylePatchesSource",
          ],
