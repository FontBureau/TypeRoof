...
// a configuration thing eventually.
...
class UIStylePatch extends _BaseContainerComponent {
    ...
    _createWrapperForType(typeKey) {
        ...
        if(typeKey === 'SimpleStylePatch') {
            ...
            // Constructor = UISimpleStylePatch;
        }
        ...
        Constructor = StaticTag;

        const args = ['h3'
              , {}, `UI for ${typeKey}`] //this._zones, this._originTypeSpecPath]
...
class StylePatchPropertiesManager extends _CommonContainerComponent {
    ...
    _createStylePatchWrappers(stylePatchPath) {
        const TypeClass =  this.widgetBus.getEntry(stylePatchPath).constructor;
        ...
        const stylePatchPropertiesKey = `stylePatchProperties@${stylePatchPath}`//.append('..', '..')}`
          , requireUpdateDefaults = (/*changedMap*/)=>{
                // FIXME: required?
                return true;
            }
          , getLiveProperties = ()=>{
                ...
                // typeSpecPropertiesKey no longer tries to reference
                // the parent.
                return stylePatchPropertiesKey === 'stylePatchPropertiesKey@'
                    ? null
                    : this.getEntry(stylePatchPropertiesKey)
                    ;
            }
          , getDefaults = typeSpecGetDefaults.bind(null, getLiveProperties)
          ;
        const widgets = [
            [
              ...
              , 'h3'
              , {}
              , `Style-Patch path: ${stylePatchPath}`
            ]
          , [
                {
    ...
    _provisionWidgets(compareResult) {
        const changedMap = this._getChangedMapFromCompareResult(compareResult)
           ...
           , path = !pathOrEmpty.isEmpty
                ...
                : null
           , rebuild = changedMap.has('stylePatchPath')
           ;
        ...
        if(rebuild) {
            ...
            // with this link currently!
            if(path !== null)
                widgetWrappers.push(...this._createStylePatchWrappers(path));
            else
...
class UIStylePatchesMap extends _UIBaseMap {
    ...
    get _initialWidgets() {
        const items = Array.from(availableStylePatchTypes).map(([typeKey, availableType])=>{
                const longLabel = availableType.get('label').value
                    , shortLabel = availableStylePatchesShortLabel.get(typeKey)
                    , label = shortLabel ? `${shortLabel} ${longLabel}` : longLabel
...
function* genericPropertiesBroomWagonGen(outerTypespecnionAPI, hostInstance) {
