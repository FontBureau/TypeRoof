  ...
  , DocumentModel = _AbstractStructModel.createClass(
      ...
      , CoherenceFunction.create(
          ...
          , function initNodes({nodes}) {
                ...
                for(const [i, [/*type*/, text]] of nodesData.entries()) {
                    ...
                    elementNode.get('instance')/* .wrapped */.get('nodes').set(`0`, textRunNode);
                    nodes.set(`${i}`, elementNode);
                }
            }
        )
    )
...
function* axisMathLocationsGen(outerTypespecnionAPI, hostInstance/* here a TypeSpecModel */) {
    ...
    function calculateAxisValue(axisTag, logiVal, font) {
        ...
        console.log('calculateAxisValue axisTag:', axisTag, 'logiVal:', logiVal, 'axisRanges[axisTag]:', axisRanges[axisTag], 'font:', font);
    ...
    for(const [axisTag, axisValue] of axesLocations) {
        ...
        if(logiVal === 'number') {
            ...
            const args = [`${SPECIFIC}font`];
            yield [`axesLocations/${axisTag}`, new SyntethicValue(calculateAxisValue.bind(null, axisTag, logiVal), args)];
        }
    }
}
...
export class StyleLinkLiveProperties extends _BaseComponent {
    ...
    update(changedMap) {
        //console.log(`${this}.update`, ...changedMap);
...
class UIDocumentNodes extends _BaseDynamicMapContainerComponent {
    ...
    _insertIntoSlot(collection, nodeKey, node) {
        ...
        if(keyIndex <= 0)
            ...
            // This would break if the node would calls _insertNodeIntoSlot
            ...
            const beforeKey = collection.keyOfIndex(keyIndex-1)
              , beforeNodeWidgetWrapper = this._keyToWidget.get(beforeKey)
              , beforeNode = beforeNodeWidgetWrapper.widget.getWidgetById('contentWidget').node
              ;
            this._domTool.insertAfter(node, beforeNode);
        }
    }
