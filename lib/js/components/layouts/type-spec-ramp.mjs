...
class TypeSpecPropertiesManager extends _CommonContainerComponent {
    ...
    _createTypeSpecWrappers(typeSpecPath, rootTypeSpecPath/*, typeSpec*/) {
        const TypeClass =  this.widgetBus.getEntry(typeSpecPath).constructor
          , exclude = new Set(['children', 'label'])
          , ppsMap = new ProcessedPropertiesSystemMap(
            Array.from(TypeClass.fields.keys()).map(modelFieldName=>{
                ...
                if(prefix === null)
                    ...
                    return [modelFieldName, null];
                return [modelFieldName, ProcessedPropertiesSystemMap.createSimpleRecord(prefix, modelFieldName)];
            }).filter(([k, item])=>!!item)
        );
        ...
        const widgets = [
          ...
          , [
              ...
              , this._zones
              , {
                    getDefaults: (ppsRecord, fieldName, /*BaseModelType.*/modelDefaultValue, ...rest)=>{
                        ...
                        console.log(`${this}.getDefaults(${ppsRecord}, `, fieldName
                            , /*BaseModelType.*/modelDefaultValue,'...rest', ...rest, `) ppsRecord:`, ppsRecord);
                        let result = null;
                        ...
                        // These requests come via UIManualAxisLocations:
                            ...
                            result = getRegisteredPropertySetup(ppsRecord.fullKey).default/* propertyName, defaultVal=_NOTDEF)*/
                        }
                        console.log(`${this} injectable.getDefaults(${ppsRecord}`, fieldName, `) =>`, result);
                        return result;
                    }
