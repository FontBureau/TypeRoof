...
export class LocalScopeTypeSpecnion {
    ...
    constructor(propertiesGenerators, typeSpec) {
        ...
        this._propertyValuesMap = null;
    }

    *_propertiesGenerator(typeSpec) {
        for(const gen of this._propertiesGenerators.values()) {
            yield *  gen(
                    {} // outerTypespecnionAPI is abandoned so fat, may get removed for good.
                  , typeSpec
            );
        }
    }
    ...
    initPropertyValuesMap(parentPropertyValuesMap) {
        const syntheticProperties = new Set()
            ...
            ;
        for(const [propertyName, propertyValue] of this._propertiesGenerator(this.typeSpec)) {
