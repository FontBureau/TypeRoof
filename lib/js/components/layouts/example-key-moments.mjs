        ...
        const myParentAPI = {
          ...
          , getEntry: (externalName) => {
                ...
                if(this.parentAPI.rootPath.isRootOf(externalName)) {
                    // FIXME: this is a hack: resolving the link to edit
        ...
        for(const [status, data, path] of compareResult) {
            ...
            for(const [i, [itemStatus, oldIndex]] of data.entries()) {
                ...
                if(itemStatus === EQUALS) {
                    ...
                    // point to activeKeyMoment/${key}/keyMoment dynamcally.
                    // The ValueLink, when properly mapped would have a similar
                    // effect, but might be more convoluted ...
                    //
                    // OK works so far, but requires the getEntry workaround.
                    const settings = {
                           rootPath: rootPath
                        }
                      , dependencyMappings = [
                      ...
                      , args = [this._zones]
                      ;
                    newWidgets[i] = this._initWrapper(this._childrenParentAPI, settings, dependencyMappings, Constructor, ...args);
                }
            }
