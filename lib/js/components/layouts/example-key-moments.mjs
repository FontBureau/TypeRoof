...
class DefaultsUpdateTrigger extends _BaseComponent {
    constructor(parentAPI, onUpdate) {
        super(parentAPI);
        this._onUpdate = onUpdate;
    }
    update(changedMap) {
        console.log(`${this.constructor.name} changedMap.keys:`, ...changedMap.keys());
        this._onUpdate();
    }
}
    ...
    constructor(parentAPI, zones) {
        ...
        const widgets = [
          ...
          , [
              ...
              , DefaultsUpdateTrigger
              , ()=>{ // onUpdate
                    // FIXME: would be much better to include this
                    // in the normal update call, as it will spare us from
                    // calling the update functions double when they also
                    // have actual updates themselves.
                    console.log('...DefaultsUpdateTrigger>', this.getEntry(this.parentAPI.rootPath.append('label')).value);
                    const uiManualAxesLocations = this.getWidgetById('UIManualAxesLocations')
                      , uiFontSize =  this.getWidgetById('UIFontSize')
                      , changedMap = new Map([['defaults', true]])
                      ;
                    uiManualAxesLocations.update(changedMap);
                    uiFontSize.update(changedMap);
                }
            ]
        ];
