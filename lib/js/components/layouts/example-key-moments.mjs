...
class KeyMomentsTimeline extends _BaseComponent {
    ...
    _activeKeyMomentsSortedKeys(activeKeyMoments) {
    ...
    initTemplate() {
        ...
        addButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            const keyMoments = this.getEntry('keyMoments')
              ...
              , activeKey = activeKeyMoments.size
                    ...
                    ? this._activeKeyMomentsSortedKeys(activeKeyMoments).at(-1)
                    : null
              ;
        ...
        removeButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            const keyMoments = this.getEntry('keyMoments')
              , activeKeyMoments = this.getEntry('activeKeyMoments')
              , activeKeys = this._activeKeyMomentsSortedKeys(activeKeyMoments)
        ...
        const _changeActiveMoment = changeAmount=>{
            ...
            if(activeKeyMoments.size === 0) {
                ...
                // Use the logically/absolute first if changeAmount is negative
                // and last if changeAmount is positive.
                const activeKeys = this._activeKeyMomentsSortedKeys(activeKeyMoments)
                  , current = parseInt(changeAmount < 0
                                            ? activeKeys.at(0)
                                            : activeKeys.at(-1)
                                      , 10)
                  ;
            ...
            const activeKeyMoment = activeKeyMoments.constructor.Model
                        ...
                        .getDraft()
                        ;
            activeKeyMoment.get('activeKey').value = typeof newIndex === 'number'
                                            ? `${newIndex}`
                                            : newIndex
                                            ;
            activeKeyMoments.splice(0, Infinity, activeKeyMoment);
        };
