...
class KeyMomentController extends _BaseContainerComponent {
    constructor(parentAPI, zones) {
        ...
        // const myParentAPI = Object.assign(Object.create(parentAPI), {});
...
class KeyMomentsController extends _BaseContainerComponent {
    ...
    _provisionWidgets(compareResult) {
        ...
        for(const [status, data, path] of compareResult) {
            ...
            if(!pathEquals || !isListOrder) {
                ...
                // console.log((equals ? '>>>' : '!!!'), (isListOrder ? 'HEUREKA' : '_______'), `${path}:::`, status, path, 'data:', data);
                /**
                ...
                **/
                ...
                //if()
                //    console.log(`${rootPath} ${localPath}`, status, data, path)
                continue;
            }
            // console.log('ListOrder data:\n    ', data.map(([status, ...rest])=> `${status} ${rest.join(', ')}`).join('\n    '));
            ...
            const newWidgets = []
                  // these must be decomissioned
                , deletedWidgets = this._widgets.slice(data.length)
                , skipUpdates = new Set()
                ;
            for(const [i, [itemStatus, oldIndex]] of data.entries()) {
                ...
                if(itemStatus === EQUALS) {
                    ...
                    newWidgets[i] = this._widgets[i];
                    skipUpdates.add(i);
                }
                else if(itemStatus === MOVED) {
                    // when mapping the moved data to this, it doesn't
                    // require an update either.
                    // BUT how to move it?
                    newWidgets[i] = this._widgets[oldIndex];
                    // quite a hack so far!
                    // Ok, so this moves the internal "adresss" of the widget
                    // but to get a zero-diff, we'd still need to map the
                    // old state and the new state. Simpler and more
                    // straight forward would be to just explicitly skip
                    // the uppdate. It's not the full, complete solution
                    // but in the end quuicker as well and likely also
                    // an optimization.
                    newWidgets[i].parentAPI.rootPath = rootPath;
                    // This will be interesting, as it only does not
                    // change if it receives the correct diff
                    skipUpdates.add(i);
                }
                    ...
                    // needs an initial update

                    // hmm, so this mapping would have to change!
                    // const _path = this.parentAPI.rootPath.append('keyMoments', activeKey.value);
                    //
                    // We can try to go via the ValueLink['keyMoment', new ValueLink('activeKey')]
                    // activeKeyMoment/${key}/keyMoment
                    // and remap with getEntry (as done before).
                    //
                    // possibly, we still want to keep the linked list, for link-specific data,
                    // but still link directly to keyMoments as I see it could be simpler
                    // I think the interesting idea in here could be to
                    // rewrite the dependencyMapping of the widget to
                    // point to keyMoments/${key} dynamcally.
                    // The ValueLink, when properly mapped would have a similar
                    // effect, but might be more convoluted ...
                    //
                    ...
                    const childParentAPI = Object.assign(Object.create(this._childrenParentAPI), {
                            getEntry: (externalName) => {
                                ...
                                // HERE:
                                var activeKeyMomentsPath = this.parentAPI.rootPath.append('activeKeyMoments');
                                // Seems like I don't know the I in here without parsing the path...

                                // if it starts with activeKeyMomentsPath
                                // then an integer index
                                // then "keyMoment"

                                const externalPath = externalName instanceof Path
                                    ? externalName
                                ...
                                if(activeKeyMomentsPath.isRootOf(externalPath)) {
                                    ...
                                    const rootElement = this.parentAPI.getEntry(this.parentAPI.rootPath)
                                      // , activeKeyPath = activeKeyMomentsPath.append(index, 'activeKey')
                                      // , activeKey = this.parentAPI.getEntry(activeKeyPath).value
                                      , activeKey = getEntry(rootElement, Path.fromParts('activeKeyMoments', index, 'activeKey')).value
                                      // , redirectedPath = this.parentAPI.rootPath.append('keyMoments', activeKey, ...rest)
                                      ;
                                      // return this.parentAPI.getEntry(redirectedPath);
                      ...
                      , widgetWrapper = this._initWrapper(childParentAPI, settings, dependencyMappings, Constructor, ...args)
                    ;
    ...
    update(compareResult) {
        ...
        // console.log(`${this.constructor.name}.update(compareResult):`, compareResult);
        return super.update(compareResult);
    }

    // in initialUpdate:
    // StateComparison.createInitial(rootState, this.parentAPI.wrapper.dependencyMapping);
    // in update:
    // compareResult.getChangedMap(this.parentAPI.wrapper.dependencyMapping);
    //
    // I don't think it's super interesting to implement this!
    _update(...args/* compareResult, requiresFullInitialUpdateSet, isInitialUpdate */) {
        // console.warn(this.constructor.name + '._update');
        const [compareResult, requiresFullInitialUpdateSet, isInitialUpdate, ...rest] = args;

        // get the /activeState/activeKeyMoments by the name "helloWorld"
        // Map([["/activeState/activeKeyMoments", "helloWorld"]])
        // this.parentAPI.wrapper.dependencyMapping

        // get the comparison of it
        // create/destroy widgets dynamically according to compare
        // run updates as required

        // console.log('compareResult:', compareResult);
        // console.log('requiresFullInitialUpdateSet:', requiresFullInitialUpdateSet);
        // console.log('isInitialUpdate:', isInitialUpdate);
        // console.log('...rest:', rest);
        //
        // // this.dependencies only includes childrens dependencies!
        // console.log('this.dependencies:', this.dependencies);
        // console.log('this.parentAPI.wrapper.dependencyMapping', this.parentAPI.wrapper.dependencyMapping);
        // console.warn(this.constructor.name + '._update END ______________');
