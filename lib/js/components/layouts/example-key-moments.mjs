...
function getKeyMomentsAnalysis(keyMoments, isLoop, keyMomentPropertyGenerator) {
    ...
    return {propertyToKeyMoments, tToKeyMoments, isLoop, fullDuration};
}
...
class Animanion {
    constructor(outerAnimanion, propertiesGenerator, keyMoments, isLoop) {
        ...
        this._keyMoments = keyMoments, isLoop;
        this._isLoop = isLoop;

        this._propertyValuesForTCache = new Map();
        this._propertyToKeyMoments = null;
        this._tToKeyMoments = null;
    }
    get childrenAPI() {
        return {
            getPropertyAtMomentT: (propertyName, momentT)=>{
    ...
    // this._generatorArgs => [[momentT, keyMoment]] ???
    // for(const args of this._generatorArgs)
    //     // would yield many properties w/o keyMoment, momentT relation => worthless?
    //     // [keyMoment, momentT] = args;
    //     yield *this._propertiesGenerator(this._outerAnimanion.childrenAPI, ...args);


    for(const [momentT, keyMoments] of momentTTokeyMoments)
        for( const keyMoment of keyMoments)
            // would yield many properties w/o keyMoment, momentT relation => worthless?
            yield *this._propertiesGenerator(this._outerAnimanion.childrenAPI, keyMoment, momentT);



//    *ownProperties() {
//        yield *this._propertiesGenerator(this._outerAnimanion.childrenAPI, ...this.generatorArgs);
//    }
//    // This would so far only generate the properties, or whatever it does
//    // of the inner/curent Animanion, but it would be nice to have a way
//    // to access the complete consstruct, i.e. here, yield also the _outerAnimanion
//    *[Symbol.iterator]() {
//        if(this._outerAnimanion !== null)
//            yield *this._outerAnimanion;
//        yield *ownProperties();
//    }



    _getKeyMomentsAnalysis(from=null) {
        if(this._keyMomentsAnalysis === null) {
            const keyMomentPropertyGenerator = this._propertiesGenerator.bind(null, this._outerAnimanion.childrenAPI);// => keyMomentPropertyGenerator(keyMoment, momentT)
            // this should be cached
            // => {propertyToKeyMoments, tToKeyMoments, isLoop, fullDuration}
            this._keyMomentsAnalysis = getKeyMomentsAnalysis(this._keyMoments, this._isLoop, keyMomentPropertyGenerator);
        }
        if(from === null)
        ...
        return this._keyMomentsAnalysis[from];
    }

    get tToKeyMoments() {
        return this._getKeyMomentsAnalysis('tToKeyMoments');
    }
    get propertyToKeyMoments() {
        return this._getKeyMomentsAnalysis('propertyToKeyMoments');
    }

    getPropertyValuesMapForAbsoluteT(absoluteT, noCache=false) {
        ...
        if(noCache)
            return getPropertyValues(this.propertyToKeyMoments, absoluteT /* between 0 and fullDuration*/));

        if(!this._propertyValuesForTCache.has(absoluteT)) {
            // where, on change of keyMoments or isLoop:
            //      [this.propertyToKeyMoments, this.tToKeyMoments] = getPropertyToKeyMoments(keyMoments, isLoop)
            this._propertyValuesForTCache.set(absoluteT,
                getPropertyValues(this.propertyToKeyMoments, absoluteT /* between 0 and fullDuration*/));
        }
        return this._propertyValuesForTCache.get(t);
    }

    // These are the interfaces to AnimationLiveProperties which we
    // likely want to implement here.
    getPropertyToKeyMoments(keyMoments, isLoop) => propertyToKeyMoments

    getPropertyValuesMapForT
    getPropertyValuesMapForKeyMoment
    // propertyValuesMap =>
    ...
    getPropertyValuesMapForKeyMoment(keyMomentsKey) {
        ...
        const momentT = this.keyMomentsToT.get(keyMomentsKey);
    ...
    if(autoOPSZ) {
        const fontSize = keyMoment.get('fontSize')
          , fontSizeValue = fontSize.isEmpty
                ...
                ? outerAnimanionAPI.getPropertyAtMomentT('fontSize', momentT)
                : fontSize.value
          ;
...
// fontSizeGen(/*animanionAPI*/, fontSizeKeyMomentData);
outerAnimanion = Animanion(null, fontSizeGen, keyMoment);
innerAnimanion = Animanion(outerAnimanion, axisLocationsGen, keyMoment, momentT);
...
class AnimationLiveProperties extends _BaseComponent {
    constructor(parentAPI) {
        ...
        this._propertyValuesForTCache = new Map();
        this.keyMomentsToT = new Map();
    }

    update(changedMap) {
        if(changedMap.has('keyMoments') || changedMap.has('isLoop')) {
            this._propertyValuesForTCache.clear();
            ...
            this.keyMomentsToT.clear();
            const fullDuration = tToKeyMoments.fullDuration;
            for(const [momentT, keyMoments] of this.tToKeyMoments) {
                for(const [key/* , keyMoment*/] of keyMoments)
                    this.keyMomentsToT.set(key, momentT/fullDuration);
            }
        }
        ...
        if(changedMap.has('t') || changedMap.has('keyMoments') || changedMap.has('isLoop')) {
            const t = (changedMap.has('t')
              ...
              , propertyValuesMap = this.getPropertyValuesMapForT(t /* between 0 and 1*/)
              ;
            this.t = t;
    ...
    getPropertyValuesMapForT(t) {
        ...
        // It could be wise to limit the precision of t, considering the
        // caching, e.g. to 4 digits below 0 or so.
        if(!this._propertyValuesForTCache.has(t)) {
            this._propertyValuesForTCache.set(t,
                getPropertyValues(this.propertyToKeyMoments, t /* between 0 and 1*/));
        }
        return this._propertyValuesForTCache.get(t);
    }
    ...
    getPropertyValuesMapForKeyMoment(keyMomentsKey) {
        ...
        // and especially the UIManualAxesLocations
        // maybe we got to inject a function into these!
        const t = this.keyMomentsToT.get(keyMomentsKey);
        return this.getPropertyValuesMapForT(t);
    }
}
