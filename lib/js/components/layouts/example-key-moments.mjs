...
function _createLabelForKeyMoment(keyMoments, index) {
    ...
    initTemplate() {
        ...
        insertButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            ...
            const keyMoments = this.getEntry('keyMoments')
              ...
              , label = _createLabelForKeyMoment(keyMoments, Math.max(0, parseFloat(insertIndex, 10) - 1))
        ...
        addButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            ...
            const label = _createLabelForKeyMoment(keyMoments, index);
 ...
 */
class KeyMomentsController extends _BaseDynamicContainerComponent {
    /**
    ...
    _redirectedGetEntry(externalName) {
        ...
        // this._itemsCollectionName === 'activeKeyMoments'
        ...
        // then "keyMoment"
        const activeKeyMomentsPath = this.parentAPI.rootPath.append(this._itemsCollectionName)
        ...
        if(activeKeyMomentsPath.isRootOf(externalPath)) {
            ...
            if(keyMoment !== this._itemEntryPath) // this._itemEntryPath === 'keyMoment'
                break redirection;
            const rootElement = this.parentAPI.getEntry(this.parentAPI.rootPath)
              , activeKey = getEntry(rootElement, Path.fromParts(this._itemsCollectionName, index, 'activeKey')).value
              ;
        ...
        const widgets = [
          ...
          , [
            ...
            , [
              ...
              , 'activeKeyMoments' // itemsCollectionName under rootPath
