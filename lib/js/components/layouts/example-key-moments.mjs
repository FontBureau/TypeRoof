    ...
    initTemplate() {
        ...
        for(const key of this._basicKeys) {
            const entry = this.getEntry(key)
              , container = this._createBasicField(key, entry.value)
              ;
            if(key === 'duration')
...
class KeyMomentsController extends _BaseDynamicContainerComponent {
    ...
    _redirectedGetEntry(externalName) {
        ...
        // this.parentAPI.getExternalName('collection') === /activeState/activeKeyMoments
        //
        ...
        // then "keyMoment"
        const itemsCollectionPath = Path.fromString(this.parentAPI.getExternalName('collection'))
          , itemsCollectionName = itemsCollectionPath.parts.slice(this.parentAPI.rootPath.parts.length)
