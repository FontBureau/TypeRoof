 ...
 *
 * @param {Object} harfbuzz - harfbuzzjs v1.x module namespace
...
export function measureWordWidths(harfbuzz, font, words, features = "+liga") {
    ...
    const hbFace = font.hbFace,
        upem = hbFace.upem,
        hbFont = new harfbuzz.Font(hbFace),
        hbFeatures = [harfbuzz.Feature.fromString(features)],
        hbVariations = Object.entries(getWidestVariations(font)).map(
            ([tag, value]) => new harfbuzz.Variation(tag, value),
        );
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        harfbuzz.shape(hbFont, buffer, hbFeatures);
        const shaped = buffer.getGlyphInfosAndPositions();
        let advanceX = 0;
        for (const item of shaped) advanceX += item.xAdvance;
