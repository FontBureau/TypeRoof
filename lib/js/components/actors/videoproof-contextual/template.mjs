...
export function fill(parts, args) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        out +=
            typeof p === "number"
                ? // this way \9 if we have arity 2 will not produce an "undefined" but "9"
                  args.length > p
                    ? args[p]
                    : `${p + 1}`
                : p;
...
export function resolveKeyToCharSet(
    charGroupsData,
    key,
    extended,
    fontModel = null,
) {
    const [chars, extendedCharsMap] = !_FONT_DEPENDENT_OPTIONS.has(key)
            ? getCharsFromCharGroups(charGroupsData, key)
            : (() => {
                  const { chars, extendedChars } = getCharsForSelectUI(
                      charGroupsData,
...
function compileSelector(
    ...
    const stateTokens = [typeKey === ForeignKey.NULL ? "NULL" : typeKey];
    if (typeKey === ForeignKey.NULL) return [null, stateTokens];

    const instance = selector.get("instance");

    if (typeKey === "Simple") {
        const argIndex = instance.get("argIndex").value;
        stateTokens.push(`${typeKey}\\${argIndex}`);
        ...
        if (optionsModel.isEmpty)
            // Allowed, but since we don't inherit here so far, this can't
            // select anything.
            return [result, stateTokens];
        ...
        if (options === "custom") {
            ...
            // we could produce the same set of chars with a different
            // separator, so it's more true to the result if we store
            // the actual chars instead of the text + separator, however,
            // the eventual order would also be insignificant in this case
            // and sorting the result for a clean token is overkill.
            stateTokens.push(
                `${customText.length}:`,
                customText,
                `${customSeparator.length}:`,
                customSeparator,
            );
            const chars =
                customSeparator !== ""
            ...
            fontModel !== null ||
            !_FONT_DEPENDENT_OPTIONS.has(options)
        ) {
    ...
    if (typeKey === "CombinatorAnd" || typeKey === "CombinatorOr") {
        const childrenList = instance.get("children");
        const children = [];
        stateTokens.push(`${typeKey}:${childrenList.size}`);
        for (const [i, childSelector] of childrenList) {
            const [child, childStateTokens] = compileSelector(
                childSelector,
                charGroupsData,
            );
            stateTokens.push(`s#${i}`, ...childStateTokens);
            if (child !== null) children.push(child);
        }
        if (children.length === 0) return [null, stateTokens];
        const test =
            typeKey === "CombinatorOr"
                ? (args) => children.some((c) => c.test(args))
                : (args) => children.every((c) => c.test(args));
        return [{ test }, stateTokens];
    }
...
export function compileTemplate(
    ...
    const stateTokens = [];
    let defaultPattern = templateModel.get("defaultPattern").value || "\\1";
    stateTokens.push(defaultPattern);

    const { parts } = compilePattern(defaultPattern),
        rulesList = templateModel.get("rules"),
        rules = [];
    stateTokens.push(`rules:${rulesList.size}`);
    for (const [k, rule] of rulesList) {
        const pattern = rule.get("pattern").value;
        stateTokens.push(`r#${k}`, pattern);
        const { parts } = compilePattern(pattern),
            [selector, selectorStateTokens] = compileSelector(
                rule.get("selector"),
                charGroupsData,
                fontModel,
            );
        stateTokens.push(...selectorStateTokens);
        if (selector !== null) rules.push({ test: selector.test, parts });
    }
    let arity = _getMaxArity(parts, 0);
    ...
    return {
        ...
        arity,
        stateTokens,
        templateModel /* it's immutable, thus a good cache validation indicator*/,
        fontModel /* also immutable or null*/,
    };
}
 ...
 *
 * @param {{ rules: Array<{ test: function, parts: Array }>, parts: Array, arity: number }} compiledTemplate
...
export function generateWords(compiledTemplate, innerChars, outerChars = null) {
    const { rules, parts, arity } = compiledTemplate,
        words = [],
        _fillWords = (rules, args) => {
            let matched = false;
            ...
            if (!matched) words.push(fill(parts, args));
        };
    if (arity <= 1 || outerChars === null) {
        ...
        for (const args /* === [i, o] */ of pairProductGen(
            innerChars,
            outerChars,
        )) {
            _fillWords(rules, args);
