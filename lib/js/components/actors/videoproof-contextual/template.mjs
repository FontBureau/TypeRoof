...
function compileSelector(
    ...
    stateTokens = [typeKey === ForeignKey.NULL ? "NULL" : typeKey];
    ...
    if (typeKey === "Simple") {
        ...
        if (options === "custom") {
            ...
            stateTokens.push(`${customText.length}:`, customText, `${customSeparator.length}:`, customSeparator);
            const chars =
                customSeparator !== ""
...
export function compileTemplate(templateModel /*TemplateModel*/, charGroupsData) {
    const stateTokens = [];
    ...
    return { rules, parts, arity, stateTokens, templateModel/* it's immutable, thus a good cache validation indicator*/ };
}
...
export function generateWords(compiledTemplate, innerChars, outerChars = null) {
    ...
    if (arity <= 1 || outerChars === null) {
        ...
        for (const args /* === [i, o] */ of pairProductGen(innerChars, outerChars)) {
            _fillWords(rules, args);
        }
    }
    return words;
