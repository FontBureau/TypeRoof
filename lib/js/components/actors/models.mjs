...
import { validateOpenTypeUIFeatureTag } from "../../ot-feature-info.mjs";
...
export const NumericPropertiesModel = _AbstractOrderedMapModel.createClass('NumericPropertiesModel', NumberModel)
    // The 3d-transform related properties (perspective, rotationX/Y/Z),
    // see REGISTERED_PROPERTIES[THREE_D]. Kept apart from
    // NumericPropertiesModel, because these are applied together, as one
    // CSS transform, not as individual CSS properties.
  , ThreeDPropertiesModel = _AbstractOrderedMapModel.createClass('ThreeDPropertiesModel', NumberModel)
  ...
  , propertiesModelMixin = Object.freeze([
      ...
      , ['3dProperties', ThreeDPropertiesModel]
  ...
  , typographyKeyMomentPureModelMixin = [
      ...
      , ['letterSpacing', NumberOrEmptyModel] // inherited from upper layers
      , ['wordSpacing', NumberOrEmptyModel] // inherited from upper layers
  ]
  , OpenTypeFeaturesModel = _AbstractOrderedMapModel.createClass('OpenTypeFeaturesModel', BooleanModel, {
        // Use a fixed ordering as the ordering itself doesn't carry
        // information in here.
        ordering: _AbstractOrderedMapModel.ORDER.KEYS_ALPHA,
        // could probably be validateOpenTypeTagString as well, but
        // we know all valid keys already
        validateKeyFn: validateOpenTypeUIFeatureTag
    })
  , openTypeFeaturesModelMixin = [
        ['openTypeFeatures', OpenTypeFeaturesModel]
    ]
  ...
  , stageKeyMomentModelMixin = [
      ...
      , ...manualAxesLocationsModelMixin // requires font, fontSize: NOT ANYMORE!
      , ['fontSize', FontSizeModel]
      , ['letterSpacing', NumberOrEmptyModel]
      , ['wordSpacing', NumberOrEmptyModel]
      , ['textColor', ColorModel] // inherited from upper layers
