...
import {
  ...
  , THREE_D_PERSPECTIVE_PROPERTY_DATA
...
export class StageDOMNode extends StaticNode {
    constructor(widgetBus, node, layerNode, cssClasses, automaticZoomDefault=false) {
        ...
        this._classesAndStylesManager = this.widgetBus.getWidgetById('classes-and-styles-manager')
    }
    ...
    _stopResizeFitToHost() {
        ...
        if(this._automaticZoomDefault)
            ...
            // we can't use reset here, as it resets the whole session
            // and that is the job of the main-ui
            this._classesAndStylesManager.setStyleProperty('overflow', null) ;
    ...
    update(changedMap) {
        ...
        if(changedMap.has('animationProperties@') || changedMap.has('globalT')) {
            ...
            // The stage has no 3d rotations of its own, but as a container
            // it can set the perspective for its children. It goes to the
            // layer node, as this.node uses `transform` for the zoom level.
            actorApplyCssProperties(this._layerNode, propertyValuesMap, getDefault
                                  , THREE_D_PERSPECTIVE_PROPERTY_DATA);
