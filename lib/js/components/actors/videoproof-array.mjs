...
function _getCellContents(charGroupsData, fonts, propertyValuesMap) {
    ...
    const fonts_ = Array.from(new Set(fonts)).toSorted((fontA, fontB)=>{
        const a = fontA.fullName
          , b = fontB.fullName
          ;
            if (a < b)
                return -1;
      ...
      , cellContents = {chars: [], stateKey: null}
      ;
    if((propertyValuesMap.has('generic/charGroup/options'))) {
        ...
        if(charGroup === 'custom') {
            ...
            const charsSet = new Set()
              , extendedCharsSet = new Set()
              ;
            for(const font of fonts_){
                const [chars, extendedChars] = getCharsForSelectUI(charGroupsData, font, charGroup);
                for(const elem of chars)
                    charsSet.add(elem);
                for(const elem of extendedChars)
                    extendedCharsSet.add(elem);
            }

            data.chars = Array.from(charsSet);
            data.extendedChars = Array.from(extendedCharsSet);
        }
        ...
        // not ideal.
        const sort = charGroup !== 'all-groups'
        if(sort) {
...
export class VideoproofArrayV2ActorRenderer extends _BaseContainerComponent {
    ...
    _provisionWidgets(compareResult) {
        ...
        const cellContents = _getCellContents(this._charGroupsData, uniqueFonts, propertyValuesMap)
          , changed =  this._cellsStateKey !== cellContents.stateKey
          ;
        if(changed)
            this._cellsStateKey = cellContents.stateKey;
    ...
    _setCellsProperties(cellWidths, fontSizePt, minMaxExtents, extents) {
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        const  [,,cellWidthEm] = this._cellWidths
         , {fontSizePt /*, overflows*/} = this._getGridFontSize(
          ...
          ;
        this._setCellsProperties(this._cellWidths, fontSizePt, minMaxExtents, extents);
