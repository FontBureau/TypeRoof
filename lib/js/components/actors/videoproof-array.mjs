...
export class VideoproofArrayV2ActorRenderer extends _BaseContainerComponent {
    ...
    constructor(widgetBus, _zones, charGroupsData) {
        ...
        this._cellWidthEm = null;// set in initial update
        // This is attached to the ModelType, so we don't have to hard
        // code it in here. This way a different VideoproofArrayActorModel
        // can be used with this same UIElement.
        // See also: UISelectCharGroupInput.
    ...
    _getGridFontSize(availableWidthPt, avaialableHeightPt
        ...
        return {fontSizePt:lastFittingFontSizePt, overflows: false};
    }

    _setCellsProperties(widestEM, fontSizePt) {
        ...
        const fontSizePtFloored = Math.floor(fontSizePt * 100) / 100;
        this.element.style.setProperty('--font-size-pt', fontSizePtFloored);
        // may become required i.e. to inform each actor of the update
        // but for now css can do the propagation.
        // for(const { contentId } of this.cells.values()) {
        //    const container = this._zones.get(contentId);
        //    container.style.setProperty('--widest-cell-em', widestEM);
        //    container.style.setProperty('--font-size-pt', fontSizePt);
        // }
    }
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        const {width, height} = this.widgetBus.wrapper.host.getBoundingClientRect()
          ...
          , [gapWidthEm, lineHeightEm] = [
                    ...
                    // properties maybe further up, as configuration
                    // but not
                    ['--gap-widht-em', 0]
                  , ['--line-height-em', 1]
          ...
          ;
        const {fontSizePt, overflows} = this._getGridFontSize(
                    availableWidthPt, avaialableHeightPt
                  , minFontSizePt, maxFontSizePt
                  , this._cellWidthEm, gapWidthEm, lineHeightEm
                  , this.cells.length
                  );
        this._setCellsProperties(this._cellWidthEm, fontSizePt);
