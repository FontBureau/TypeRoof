...
export const VideoproofArrayKeyMomentModel = _AbstractStructModel.createClass(
      ...
      , ...typographyKeyMomentModelMixin
        // FIXME: should be OrEmpty! Otherwise, new Keyframes
        // switch this, because they have to set it.ALSO FIXME:
        // at the moment, "insert"  KeyFrame sets this to the default,
        // but it should be the current value rather. This looks like
        // an oversight in the the actors modulification, as the
        // behavior should be controllable there, I believe.
        //   1. Add a KeyMoment
        //   2. Set Glyphs to anything, e.g.: Latin.Symbols.Parenthetical
        //   3. Set to "is a loop"
        //   4. Add another KeyMoment
        // => the second KeyMoment is set to "all-gid"
        // => The cell content changes when played. The Second KeyMoment
        //    should have the same value as the first.
        //    It is also too slowfor animation of he cells contents,
        //    but that is not an issue here and now.
...
export class VideoproofArrayActorRenderer extends _BaseComponent {
    ...
    update(changedMap) {
        ...
        //      // , getDefault = property => [true, _getRegisteredPropertySetup(property).default]
            ...
            if((propertyValuesMap.has('generic/charGroup'))) {

                const charGroup = propertyValuesMap.get('generic/charGroup')
                   , [chars, /* extendedChars */] = getCharsForSelectUI(this._charGroupsData, font, charGroup)
                   , cellsStateKey = [font.fullName, charGroup].join(';')
                   ;
