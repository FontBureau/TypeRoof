...
import charGroupsData from '../../../assets/glyph-groups.json' with { type: 'json' }
...
// The keyMoments of these actors are separated by role: entry 0 is the
// property-setting keyMoment (user data, serialized), entries 1+ are
// animation-driving keyMoments, generated from axesMath and marked
// GENERATED_DATA (see axes-math.mjs). Generated entries are omitted
// from serialization, they are re-created on load. Lists without
// marked entries, e.g. in motion-stage where keyMoments are
// user-editable, serialize completely.
function _createKeyMomentsListModel(className, KeyMomentModel) {
    const result = {
        [className]: class extends _AbstractListModel.createClass(className, KeyMomentModel) {
            [SERIALIZE](options = SERIALIZE_OPTIONS) {
                const [errors, values] = super[SERIALIZE](options);
                if(values === null)
                    return [errors, values];
                // Indexes in values align 1:1 with the entries of this list.
                const filtered = [];
                let i = 0;
                for(const [, childItem] of this) {
                    const value = values[i++];
                    if(Object.hasOwn(childItem, GENERATED_DATA))
                        continue;
                    filtered.push(value);
                }
                return [errors, filtered.length ? filtered : null];
            }
        },
    };
    return Object.freeze(result[className]);
}
...
export const CharGroupOptionsModel = _AbstractEnumModel.createClass(
  ...
  , VideoproofArrayKeyMomentsModel = _createKeyMomentsListModel('VideoproofArrayKeyMomentModel', VideoproofArrayKeyMomentModel)
    ...
    update(changedMap) {
        ...
        //      // , getDefault = property => [true, _getRegisteredPropertySetup(property).default]
            ...
            setTypographicPropertiesToSample(this.element, propertyValuesMap, true, {
                verboseFontVariationSettings: getVerboseFontVariationSettings(this)
...
export const VideoproofArrayV2CellKeyMomentModel = _AbstractStructModel.createClass(
  ...
  , VideoproofArrayV2CellKeyMomentsModel = _createKeyMomentsListModel('VideoproofArrayV2CellKeyMomentModel', VideoproofArrayV2CellKeyMomentModel)
...
// REMOVED with the harfbuzzjs v1.x upgrade: Pointer32Malloc and
// Pointer32StackAlloc lived here. They are obsolete as v1.x binds
// Font.hExtents() directly and no code requires raw Module memory
// access anymore. (The big comment block above documents the old
// malloc/stackAlloc pitfalls, kept for the record.)

// harfbuzzjs v1.x: Font.setVariations takes Variation[] instead of a
// plain {tag: value} object.
function _toHBVariations(hb, variations) {
    return Object.entries(variations)
        .map(([tag, value]) => new hb.Variation(tag, value));
}

// harfbuzzjs v1.x: shape() takes Feature[] instead of a feature string.
function _toHBFeatures(hb, features) {
    if(!features)
        return [];
    return String(features).split(',')
        .map(str => str.trim())
        .filter(str => !!str)
        .map(str => hb.Feature.fromString(str));
}
...
export class VideoproofArrayV2ActorRenderer extends _BaseContainerComponent {
            ...
            if(actorIsNew || requireNewWidget) {
                const widgetWrapper = this._createWrapper({
                  ...
                  , [
                      ...
                      , 'animationProperties@'
                      , 'font'
                        // see ActorRendererContainer; provided by the
                        // layout, hence via the parents dependency name.
                      , [this.widgetBus.getExternalName('verboseFontVariationSettings')
    ...
    /**
     * This started from the harfbuzzjs v0.x hbjs.example.js
     * (migrated to the v1.x API). I'll keep it for now to help with
    ...
    _hbShapeExample(hb, fontBlob, text, variations, features) {
        ...
        // harfbuzzjs v1.x: setVariations takes Variation[]
        font.setVariations(_toHBVariations(hb, variations));
        ...
        // harfbuzzjs v1.x: shape takes Feature[]
        hb.shape(font, buffer, _toHBFeatures(hb, features));
        const shape = buffer.getGlyphInfosAndPositions();

        // returns glyphs paths, totally optional
        const glyphs = {};
        for(const x of shape) {
            if (glyphs[x.codepoint])
                continue;
            glyphs[x.codepoint] = {
    ...
    // example how to produce and use the results of harfbuzz shape. Will
     ...
     * calculation; with harfbuzzjs v1.x Font.glyphExtents() is available.
    ...
    _getCellsWidths() {
        ...
        const cellContentProtocolHandler = this.widgetBus.wrapper.getProtocolHandlerImplementation('cellContent@')
          , firstCell = this.cells[0]
            ...
            // font model as well.
          , advancesXInEM = {}
          , harfbuzz = this.widgetBus.harfbuzz
          ;
        for(const [actorIndex, [/*type*/, cellActor]] of firstCell.cellActors.entries()) {
            ...
            const hbFont = new harfbuzz.Font(hbFace); // TODO: maybe keep centrally, seems cheap though
            hbFont.setVariations(_toHBVariations(harfbuzz, variations));
            ...
            for(const { contentId /*, unregister, cellActors*/}
                ...
                harfbuzz.shape(hbFont, hbBuffer, _toHBFeatures(harfbuzz, features));
                const shaped = hbBuffer.getGlyphInfosAndPositions();
    ...
    _getExtents() {
        const firstCell = this.cells[0]
         ...
         if(!firstCell)
            return result;
        const harfbuzz = this.widgetBus.harfbuzz
        ...
        for(const [actorIndex, [/*type*/, cellActor]] of firstCell.cellActors.entries()) {
            const font = cellActor.widgetBus.getEntry('font').value
              ...
              , hbFace = font.hbFace
              , hbFont = new harfbuzz.Font(hbFace)// ; // TODO: maybe keep centrally, seems cheap though
            hbFont.setVariations(_toHBVariations(harfbuzz, variations));
            // harfbuzzjs v1.x: bound directly, previously via custom
            // Module._hb_font_get_h_extents export + Pointer32StackAlloc.
            // Scale defaults to upem (font units).
            const {ascender, descender, lineGap} = hbFont.hExtents();
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        // The measurement comes from the 'environment@' protocol
        // (the .typeroof-layout box, css-px, before css-transforms):
        // a viewport-stable reference. Measuring wrapper.host (the
        // layer) instead fed back through content growth (taller host
        // -> more rows -> smaller cells) once the container could grow
        // for the background scroll edge fix.
        const layoutBox = (changedMap.has('environment@layout')
                        ? changedMap.get('environment@layout')
                        : this.getEntry('environment@layout'))
          , availableWidthPt = layoutBox.width / 4 * 3
          , avaialableHeightPt = layoutBox.height / 4 * 3
          , minFontSizePt = 24
        ...
        // I need a flexible way to instruct this actor layout-specific
        // to not apply this property! This is a workaround/hack see the
        // comment in the CSS for the specific implementation.
          , colorPropertiesMap = [
