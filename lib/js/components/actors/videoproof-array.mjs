...
export const VideoproofArrayV2ActorModel = _BaseActorModel.createClass(
  ...
  , CoherenceFunction.create(
        ['activeActors', 'availableActorTypes']
      , function initEmptyActiveActors({activeActors}) {
        if(activeActors.size === 0) {
            console.log('VideoproofArrayV2ActorModel initEmptyActiveActors activeActors is empty.');
...
// FIXME: this is a copy of  AnimationPropertiesProtocolHandler
// this._registeredComponentes is renamed to this._registeredValues
// There should just be one SimpleProtocolHandler
export class SimpleProtocolHandler extends _ProtocolHandler {
    ...
    _unregister(identifier) {
        ...
        console.log(`unregistered: ${identifier}`);
    }
...
// jshint ignore:start
    ...
    // jshint ignore:end
    constructor(parentAPI, _zones, charGroupsData) {
        parentAPI.wrapper.setProtocolHandlerImplementation(
            ...SimpleProtocolHandler.create('cellContent@'));
    ...
    _provisionWidgets(compareResult) {
        console.log(`${this}._provisionWidgets compareResult:`, compareResult);

        const requiresFullInitialUpdate = false // ???
            // calculate own changedMap rather thn for a child.
          , changedMap = this.parentAPI.wrapper
                .getChangedMapFromCompareResult(requiresFullInitialUpdate, compareResult);
        console.log(`${this}._provisionWidgets changedMap:`, changedMap);
        const font = (changedMap.has('font')
                        ? changedMap.get('font')
                        : this.getEntry('font')).value
          ;
        ...
        // FIXME: should rather work without this.parentAPI.rootPath
        // but I can't register "activeActors" directly as dependency in
        // getActorWidgetSetup and I don't currently understand why.
        const activeActors = this.getEntry(this.parentAPI.rootPath.append('activeActors'));
        console.log(`${this}._provisionWidgets activeActors`, activeActors);
        ...
        console.log(`${this}._provisionWidgets changed ${changed}, animationProperties ${animationProperties}`);
        console.log('cellContents', cellContents);

        const cellContentProtocolHandler = this.parentAPI.wrapper.getProtocolHandlerImplementation('cellContent@')
          , contents = cellContents.chars
          , newCells = new Set()
          ;
        ...
        for(const [index, cellContent] of contents.entries()) {
            ...
            if(this.cells[index] === undefined) {
                const container = this._domTool.createElement('div', {'class': `actor_renderer-videoproof_array_v2-zone actor_renderer-videoproof_array_v2-zone_${index}`})
        ...
        // cleanup
        const removed = this.cells.splice(contents.length)
          , removedWidgets = new Set()
          ;
        for(const {contentId, unregister, cellActors} of removed) {
            const container = this._zones.get(contentId);
            container.remove();
            this._zones.delete(contentId);

            // cellContentProtocolHandler._unregister:
            unregister();
            for(const widgetWrapper of cellActors) {
                this._destroyWidget(widgetWrapper)
                removedWidgets.add(widgetWrapper);
            }
        }

        this._widgets.splice(0, Infinity, ...this._widgets.filter(
                    widgetWrapper=>!removedWidgets.has(widgetWrapper)));

        // compile this from changed actors information
        const updateCellCommands = []
          , requiresFullInitialUpdateSet = new Set()
          ;
        ...
        for(const cell of this.cells) {
            ...
            if(newCells.has(contentId)) {
                // all new actors
                for(const [index, actor] of activeActors) {
                    console.log('index, actor:', index, actor, contentId);

                    // look at ActorRendererContainer ...!!!
                    // especially for creating/linking @animationProperties
                    const [settings, dependencyMappings, Constructor, ...args] = [
                        {
                            zone: contentId // !
                          , rootPath: `${index}/instance`
                        }
                      , [
                            [contentId, '@cellContent']
                        ]
                      , VideoproofArrayV2CellActorRenderer
                    ];
                    const widgetWrapper = this._initWrapper(this._childrenParentAPI, settings, dependencyMappings, Constructor, ...args);
                    // -> must be removed again as well.
                    this._widgets.push(widgetWrapper);
                    this._createWidget(widgetWrapper);
                    requiresFullInitialUpdateSet.add(widgetWrapper);
                    // it's also in this._widgets, but there it may needs
                    // to be removed again. In basics.mjs
                    // _BaseDynamicCollectionContainerComponent._provisionWidgets
                    // we have a:
                    // this._widgets.splice(0, Infinity, ...newWidgets);
                    // that method seems a very good model for this method.
                    cellActors.push(widgetWrapper);
                }
            }
            else {
                // update ...
                for(const cmd of updateCellCommands) {
                    // CAUTION must only return widgetWrapper if it is
                    // a new one!
                    const widgetWrapper = cmd(contentId, cellActors);
                    if(widgetWrapper)
                        requiresFullInitialUpdateSet.add(widgetWrapper);
                }
            }
        ...
        // For each [cellIndex, cellContent]
        //     make a div container
        //     for each actor
        //          make an actor, that applies itself to the diff
        //              the settings of the actor are the same per actor
        //              the cell content of the actor is the same per cell
        //      I think I prefer this.
        //
        // eventually update => there need to be the right amount of cells
        //      and the right amount of cellContent@#{cellIndex}
        //      if we need more we need more/less chanfe
        //          cell divs
        //          cellContent@#{cellIndex}
        //
        // cellContent@#{cellIndex} = cellContent
        //
        //
        // or should this create the actors itself and send them to different
        // containers?

        return requiresFullInitialUpdateSet;
...
/*
        // PROS: widget can be subscribed to @cellContent in it's dependncies
        // CONTRAS:
        //      I had a contra indicating thought. I forgot about it.
        //

        // per cell widget
        const cellContentKey = `cellContent@${this.parentAPI.rootPath.append(index)}`;
        const updateDefaultsDependencies = [
                [this._cellContentPropertiesKey, '@cellContent']
            ]

        // used to register a widget here
        {
                    'animationProperties@': animationPropertiesKey
        }

        // but we can probably just do this ourselvesl like:
        protocolHandlerImplementation.register(identifier, cellContent)
        // where cellContent is not  awidget in this case.

        then later in that widgets update method:
...
*/
        ...
        throw new Error(`NOT IMPLEMENTED ${this}._provisionWidgets`);
/*
...
*/
    ...
    // COPY from _BaseContainerComponent
    /*
    initialUpdate(rootState) {
        throw new Error(`NOT IMPLEMENTED ${this}.initialUpdate`);
