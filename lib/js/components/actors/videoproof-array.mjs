...
// jshint ignore:start
     static TEMPLATE = `<div class="actor_renderer-videoproof_array_v2 fixed-line-breaks">(content not initialized)</div>`;
    // jshint ignore:end
    ...
    _provisionWidgets(compareResult) {
        ...
        for(const [index, cellContent] of contents.entries()) {
            ...
            if(this.cells[index] === undefined) {
                const container = this._domTool.createElement('div', {})
                  ...
                  ;
                this.cells[index] = { contentId, container, unregister, actorWidgets: [] };
        ...
        for(const {container, unregister} of removed) {
            container.remove();
            unregister();
        }
        ...
        // Deleted cells should delete actors as well.

        const actors = [], addActor=()=>{};
        for(const {contentId, container} of this.cells) {
            console.log('contentId', contentId);
            if(newCells.has(contentId)) {
                // all new actors
                for(const [key, actor] of activeActors) {
                    console.log('key, actor:', key, actor);
                    const widgetWrapper = addActor(actor, contentId, container);
                ...
                for(const cmd of updateCellCommands) {
                    ...
                    // a new one!
                    const widgetWrapper = cmd(contentId, container);
                    if(widgetWrapper)
                        requiresFullInitialUpdateSet.add(widgetWrapper);
                }
            }
...
/*
        ...
        const updateDefaultsDependencies = [
                [this._animationPropertiesKey, '@cellContent']
            ]
