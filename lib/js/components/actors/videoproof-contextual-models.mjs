...
function ensureAtLeastOneCharGroup({charGroups}) {
    if(charGroups.size < 1) {
        ...
        charGroups.push(draft);
             ...
             // for the items ($1, $2)
export const CharGroupArgumentsListModel = _AbstractListModel.createClass('CharGroupArgumentsListModel', CharGroupModel)
// --- CharsSelector Model (AxesMath-style self-referential dynamic type) ---
// Number model for argIndex: 0 or 1
  , CharsSelectorArgIndexModel = NumberModel.createClass('CharsSelectorArgIndexModel', {
        defaultValue: 0,
        min: 0,
        max: 1,
        toFixedDigits: 0,
  ...
  , [availableCharsSelectorTypes, SELECTOR_TYPE_TO_SELECTOR_TYPE_KEY] =
        createAvailableTypes(AvailableTypesModel, [
              ...
              , ['CombinatorOr', 'Match Some Selector', CombinatorOrCharsSelectorModel]
              , ['CombinatorAnd', 'Match All Selector', CombinatorAndCharsSelectorModel]
        ])
  ;
