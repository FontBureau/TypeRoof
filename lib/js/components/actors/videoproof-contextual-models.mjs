...
const CharsSelectorArgIndexModel = NumberModel.createClass('CharsSelectorArgIndexModel', {
  ...
  , CharsSelectorKeyModel = StringModel.createClass('CharsSelectorKeyModel')
  , CharsSelectorKeysModel = _AbstractListModel.createClass('CharsSelectorKeysModel', CharsSelectorKeyModel)
  ;
...
const CharsSelectorModel = _AbstractStructModel.createClass(
  ...
  , SimpleCharsSelectorModel = _AbstractStructModel.createClass(
      ...
      , ['keys', CharsSelectorKeysModel]
      , ['extended', BooleanDefaultTrueModel]
    )
  ...
  , CoherenceFunction.create(
      ...
      , function ensureCharGroupsCount({charGroups}) {
            if(charGroups.size === 0)
                ...
                // FIXME: push what???
                charGroups.push(CharGroupModel.createPrimalDraft(charGroups.dependencies));  // push default
            // keep max length at 2
            charGroups.splice(0, 2);
        }
    )
)
