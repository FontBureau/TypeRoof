 ...
 */
function compileSelectorFromPath(
    propertyValuesMap,
    prefix,
    charGroupsData,
    stateTokens,
    ...
    if (typeKey === undefined || typeKey === ForeignKey.NULL) return null;
    ...
    if (typeKey === "Simple") {
        ...
        // FIXME: this kind of code must be unified somewhere!!!
        // i.e. READ charGroups from a propertiesMap!
            ...
            const customTextPath = `${cgPrefix}/customText`,
                ...
                customSeparator =
                    propertyValuesMap.get(customSeparatorPath) || "";
            stateTokens.push(
                customTextPath,
                customText,
                customSeparatorPath,
    ...
    if (typeKey === "CombinatorAnd" || typeKey === "CombinatorOr") {
        ...
        for (let i = 0; ; i++) {
            ...
            const childTypeKey = propertyValuesMap.get(
                `${childPrefix}/selectorTypeKey`,
            );
            if (childTypeKey === undefined) break;
...
export function compileTemplateFromPropertyValuesMap(
    ...
    if (!defaultPattern) return _DEFAULT_COMPILED_TEMPLATE;
