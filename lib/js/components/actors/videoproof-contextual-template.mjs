...
function compileSelectorFromPath(propertyValuesMap, prefix, charGroupsData, stateTokens) {
    ...
    if(typeKey === 'Simple') {
        const argIndexPath = `${instancePrefix}/argIndex`
            , extendedPath = `${instancePrefix}/extended`
        ...
        if(argIndex === undefined)
            ...
            ;
        stateTokens.push(argIndexPath, argIndex, extendedPath, extended);
        // Collect all keys from the keys list
        const keys = [];
        for(let i = 0; ; i++) {
            const keyPath = `${instancePrefix}/keys/${i}`
                , key = propertyValuesMap.get(keyPath)
                ;
            if(key === undefined)
                break;
            stateTokens.push(keyPath, key);
            keys.push(key);
        }
        // Build a single union set from all keys
        const charSet = new Set();
        for(const key of keys) {
            const resolved = resolveKeyToCharSet(charGroupsData, key, extended);
...
export function compileTemplateFromPropertyValuesMap(
    ...
    const defaultParts = compilePattern(defaultPattern);
