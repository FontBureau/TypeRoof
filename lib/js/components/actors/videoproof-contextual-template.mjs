...
function compileSelectorFromPath(propertyValuesMap, prefix, charGroupsData, stateTokens) {
    ...
    if(typeKey === 'Simple') {
        ...
        // Build a union set from all charGroups in the selector
        const charSet = new Set()
            , charGroupsPrefix = `${instancePrefix}/charGroups`
            ;
        for(let i = 0; ; i++) {
            const cgPrefix = `${charGroupsPrefix}/${i}`
                , optionsPath = `${cgPrefix}/options`
                , options = propertyValuesMap.get(optionsPath)
                ;
            if(options === undefined)
                break;
            const extendedPath = `${cgPrefix}/extended`
