...
import {
  ...
  , VideoproofContextualActorRenderer
} from './videoproof-contextual/index.typeroof.jsx';
...
export const [/*referencableActorTypes*/, activatableActorTypes] = initAvailableActorTypes([
      ...
      , ['EllipseActorModel', 'Ellipse', EllipseActorModel]
      ...
      , ['VideoproofContextualActorModel', 'Videoproof Contextual', VideoproofContextualActorModel]
    ])
...
export function getActorWidgetSetup(setup) {
    const {typeClass, zones
            ...
            // Dependencies that the layout adds to all of its actor
            // renderers, e.g. `verboseFontVariationSettings`, which is
            // a layout level setting and hence not available in every
            // layout that uses actors.
    ...
    if(typeClass === EllipseActorModel) {
        return [
            {
                rootPath: widgetRootPath
              , relativeRootPath: widgetRelativeRootPath
    ...
    if(typeClass === VideoproofArrayV2ActorModel) {
        ...
        return [
          ...
          , [
              ...
              , 'keyMoments'
                // viewport-stable available space for cell sizing
                // (replacing the wrapper.host measurement, which fed
                // back through content growth)
              , 'environment@layout'
              , ...extraActorDependencies
            ]
          , zones
    ...
    if(typeClass === VideoproofContextualActorModel) {
        // Navigate: keyMoments -> KeyMomentModel -> charGroups (list) -> CharGroupModel -> options
        const KeyMomentModel = VideoproofContextualActorModel.fields.get('keyMoments').Model
          , CharGroupModel = KeyMomentModel.fields.get('charGroups').Model
          , charGroupOptionsModel = CharGroupModel.fields.get('options')
          , isOrEmpty = charGroupOptionsModel.prototype instanceof _AbstractSimpleOrEmptyModel
          , charGroupsData = (isOrEmpty
                                  ? charGroupOptionsModel.Model
                                  : charGroupOptionsModel).charGroupsData
        ...
        return [
          ...
          , [
                ['font', 'font']
                // viewport-stable available space for layout
                // (replacing the wrapper.host measurement, which feeds
                // back through content growth once the container can
                // grow for the background scroll edge fix)
              , 'environment@layout'
              , ...extraActorDependencies
            ]
          , charGroupsData
    ...
    if(typeClass === LayerActorModel) {
        const {layerBaseClass} = setup;
         return [
          ...
          , zones
          , layerBaseClass
          , setup.getActorWidgetSetup || getActorWidgetSetup
        ];
    }
    const {typeLabel='Unknown Label', typeKey='UnknownKey'} = setup;
...
export function getActorTreeNodeType(actor) {
    const typeKey = actor.get('actorTypeKey').value
      ...
      , leafNodeTypes = new Set([CircleActorModel, EllipseActorModel, RectangleActorModel
...
export function isTypographicActorTypeKey(typeKey) {
    const typographicTypeKeys = new Set(['LayerActorModel', 'LineOfTextActorModel'
        ...
        , 'VideoproofArrayV2CellActorModel', 'VideoproofContextualActorModel']);
...
export function getActorTypeKeySpecificWidgets(typeKey) {
    ...
    if(['LayerActorModel', 'LineOfTextActorModel', 'VideoproofArrayActorModel'
      , 'VideoproofArrayV2ActorModel', 'VideoproofArrayV2CellActorModel'
      , 'VideoproofContextualActorModel'].includes(typeKey))
