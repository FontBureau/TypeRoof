...
export class VideoproofContextualActorRenderer extends _BaseComponent {
    // jshint ignore:start
    static TEMPLATE = `<div class="actor_renderer-videoproof_contextual-scroll_container" tabindex="0">`
    ...
    constructor(widgetBus, charGroupsData) {
        ...
        [this._scrollContainer, this._spacer, this.element] = this.initTemplate();
    ...
    _syncScrollTop() {
        const targetScrollTop = this._wordIndexToScrollTop(this._scrollWordIndex);
    ...
    _onScroll(/*event*/) {
        if(this._suppressScrollHandler)
            return;
        const wordIndex = this._scrollTopToWordIndex(this._scrollContainer.scrollTop);
        if(wordIndex === this._scrollWordIndex)
            return;
