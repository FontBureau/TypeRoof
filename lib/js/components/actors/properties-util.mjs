...
function _getTransformValue(propertyValuesMap, getDefault, propertyData) {
    const [fullKey, , , cleanFn] = propertyData
      , [, value] = getPropertyValue(propertyValuesMap, getDefault, fullKey)
      ;
    
    return cleanFn ? cleanFn(value) : value;
}

function _actorApplyTransforms(element, propertyValuesMap, getDefault, propertyDescriptions) {
    const transforms = propertyDescriptions.filter((pd) => pd[1] === 'transform');
    if (transforms.length === 0) return;
    const values = transforms.map((pd) => _getTransformValue(propertyValuesMap, getDefault, pd));
    const description = ['numericProperties/scale', 'transform', '', () => values.join(' ')];
    _actorApplyCssProperty(element, propertyValuesMap, getDefault, description);
}

export function actorApplyCssProperties(element, propertyValuesMap, getDefault, propertyDescriptions) {
    var stack = propertyDescriptions.filter((pd) => pd[1] !== 'transform');
    ...
    _actorApplyTransforms(element, propertyValuesMap, getDefault, propertyDescriptions);
}
