 ...
 * explicitly in font-variation-settings, using their default location when
 * the propertyValuesMap doesn't specify one. This is the equivalent of the
 * legacy tools "applyDefaultsExplicitly" flag (see the legacy
 * layouts/vartools-typespec.mjs `_filterLocations`). Requires `font`, as
 * only the font knows its axes and their default locations; without it,
...
export function setTypographicPropertiesToSample(sample, propertyValuesMap
    ...
    for(const [key, value] of propertyValuesMap) {
        ...
        explicitAxes.add(axisTag);
    }

    if(verboseFontVariationSettings && font !== null) {
        // Add all axes of the font that are not set explicitly, at their
        // default location. The order follows the fvar table, with the
        // explicitly set axes (which may include axes that are not in
        // the font at all) first, as before.
        for(const [axisTag, axisRange] of Object.entries(font.axisRanges)) {
            if(explicitAxes.has(axisTag))
                continue;
            variations.push([axisTag, axisRange['default']]);
        }
    }
    ...
    sample.style.setProperty('font-variation-settings'
            , variations.map(([axisTag, value])=>`"${axisTag}" ${value}`).join(','));
}

/**
 * `verboseFontVariationSettings` is a layout level setting (like
 * `showParameters`), thus not every layout that renders actors provides
 * it — motion-stage e.g. doesn't. Hence it is an optional dependency of
 * the actor renderers, added by the layout via
 * `getActorWidgetSetup`s `extraActorDependencies`.
 */
export function getVerboseFontVariationSettings(component) {
    return component.widgetBus.wrapper.dependencyReverseMapping.has(
                        'verboseFontVariationSettings')
        ? component.getEntry('verboseFontVariationSettings').value
        : false
        ;
}
...
export const DYNAMIC_MARKER = Symbol('DYNAMIC_MARKER')
  ...
  , DIRECT_PROPERTY = Symbol('DIRECT_PROPERTY')
  ;
...
export function actorApplyCssProperties(element, propertyValuesMap, getDefault, propertyDescriptions) {
    ...
    while(stack.length) {
        ...
        else if(propertyDescription[0] === DIRECT_PROPERTY) {
            const [, cssProperty, propertyValue] = propertyDescription;
            element.style.setProperty(cssProperty, propertyValue);
        }
...
function setCSS3DRotation(element, rotationX, propertyValuesMap, getDefault) {
    ...
    if(rotations.length) {
        ...
        // The elements are reused along the timeline, hence both properties
        // must be removed, otherwise `transform-style` would stick around
        // for the key moments that don't rotate.
