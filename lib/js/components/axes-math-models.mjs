...
/**
 * Collect the axis tags that are used by an axes-math item (and its
 * children), without resolving the actual locations. Used to determine
 * which axes of a font are *not* covered by the rap, i.e. the candidates
 * for the "But wait, there's more" quick dial.
 */
export function getAxesMathAxisTags(_item, tags=new Set()) {
    const item = unwrapPotentialWriteProxy(_item)
      , typeKey = AXES_MATH_ITEM_TYPE_TO_KEY.get(item.constructor)
      ;
    switch(typeKey) {
        case 'LocationsSum':
            for(const [/*key*/, entry] of item.get('items'))
                getAxesMathAxisTags(entry.get('instance').wrapped, tags);
            break;
        case 'LocationsProduct':
            for(const [axisTag/*, values*/] of item.get('axesLocationValuesMap'))
                tags.add(axisTag);
            break;
        case 'Location':
            for(const [axisTag/*, value*/] of item.get('axesLocations'))
                tags.add(axisTag);
            break;
        default:
            throw new Error(`NOT IMPLEMENTED getAxesMathAxisTags for typeKey: "${typeKey}" item: ${item}.`);
    }
    return tags;
}
