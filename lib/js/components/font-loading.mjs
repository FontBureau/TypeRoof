...
export class PlainAddFonts {
    //jshint ignore:start
    static TEMPLATE = `<button class="ui_font-select">+ Add local fonts</button>`;
    //jshint ignore:end
...
export class UIDialogManageFonts extends _DialogBase {
    ...
    static hostInit() {
        ...
        const ctorArgs = []
        , showArgs = []
        , availableFonts = this.getEntry('availableFonts')
        , fonts = []
        ;
        for(const [key, {value:font}] of availableFonts) {
        ...
        ctorArgs.push(this.widgetBus, removeFontsFN);
        showArgs.push(fonts);
    ...
    constructor(domTool, widgetBus, removeFonts) {
        ...
        [this.element, this._form, this._fonts, this._actionDescription
         , this._removeButton
         , this._plainAddFonts] = this._initTemplate(widgetBus);
    }
    _initTemplate(widgetBus) {
        const [dialog] = super._initTemplate()
          ...
          , removeButton = dialog.querySelector('button[value="remove"]')
          ;
        fonts.addEventListener('change', this._fontsChangeHandler.bind(this));

        const plainAddFonts = new PlainAddFonts(widgetBus, form
          , '… or drag a font onto the dialog'
          , ()=>{
                // this will close the dialog again, especially as we don't
                // update the dialog font list live (TODO).
                this._resolve('add-fonts');
            }
        );
        this._domTool.insertAtMarkerComment(dialog, 'insert: addFonts', plainAddFonts.element);

        return [dialog, form, fonts, actionDescription, removeButton, plainAddFonts];
    }
    ...
    _fontsChangeHandler(/*event*/) {
        ...
        if (fonts.length > 0) {
            this._actionDescription.textContent = `Remove ${fonts.length} font${fonts.length === 1 ? '' : 's'}.`;
            this._removeButton.removeAttribute('disabled');
        } else {
            this._actionDescription.textContent = 'Nothing selected, nothing to do.';
            this._removeButton.setAttribute('disabled', '');
        }
    }
