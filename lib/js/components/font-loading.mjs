    ...
    initTemplate() {
        ...
        select.addEventListener('change', this._changeStateHandler(async (/*event*/)=>{
            ...
            const fontName = this._allowNoneSelect && this._select.value === ''
                    ? ForeignKey.NULL
                    : this._select.value
              , userIdentifier = this.widgetBus.getExternalName('activeFontKey')
              ;
            // ensure deferred font is installed here
            if(fontName !== ForeignKey.NULL)
                await this.widgetBus.useFont(userIdentifier, fontName);
            else
                await this.widgetBus.unuseFont(userIdentifier);
