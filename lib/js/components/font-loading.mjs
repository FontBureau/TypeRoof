...
export class FontSelect extends _BaseComponent {
    ...
    <span class="ui_font-select_label">Font</span>
    ...
    _updateOptions(availableFonts/* changes */) {
        ...
        const value = this._select.value // keep around to set back later
          ...
          , createOptGroup = name=>{
                ...
                legend.textContent = optgroup.label;
        ...
        if(this._allowNoneSelect) {
            ...
            option.value = '';
            option.textContent = `(inherited: no selection)`;
...
function _getFilesHandler(widgetBus, eventuallyCallback=null) {
    const handleFiles = async (files)=>{
        const fontKeys = await widgetBus.loadFontsFromFiles(...files);
        if(!fontKeys.length)
            return;
        const fontName = fontKeys.at(-1);
        // activate the last entry
        widgetBus.requireReviewResources();
        widgetBus.getEntry('activeFontKey').set(fontName);
        if(eventuallyCallback!=null)
            eventuallyCallback();
    };

    return async (...args)=>widgetBus.changeState(()=>handleFiles(...args));
}

export class PlainAddFonts {
    static TEMPLATE = `<button>Add font...</button>`;
    constructor(widgetBus, dropElement, title=null, eventuallyCallback=null) {
        [this.element, this.dropElement] = this.initTemplate(widgetBus, dropElement, title, eventuallyCallback);
    }

    initTemplate(widgetBus, dropElement, title=null, eventuallyCallback=null) {
        const frag = widgetBus.domTool.createFragmentFromHTML(this.constructor.TEMPLATE)
        ...
        if(title !== null)
            addFonts.setAttribute('title', title);

        const filesHandler = _getFilesHandler(widgetBus, eventuallyCallback);
        makeFileInput(filesHandler, addFonts, dropElement, fileInputDragCallbacks);
...
export class AddFonts extends _BaseComponent {
    constructor(widgetBus, dropElement, title=null) {
        super(widgetBus);
        this._plainAddFonts = new PlainAddFonts(this.widgetBus, dropElement, title);
        this._insertElement(this._plainAddFonts.element);
    }
...
export class UIDialogFontExists extends _DialogBase {
    static TEMPLATE = UI_DIALOG_FONT_EXISTS;
    constructor(domTool) {
        super(domTool);
...
export class UIDialogHandleFontMissing extends _DialogBase {
    static TEMPLATE = UI_DIALOG_HANDLE_FONT_MISSING;
...
/**
 * The fonts that UIDialogManageFonts can remove, as a list of
 * [fontName, labelText], projected from an availableFonts state entry.
 */
export function getRemovableFonts(availableFonts) {
    const fonts = [];
...
export class UIDialogManageFonts extends _DialogBase {
    static TEMPLATE = `<dialog>
      <h4>Manage fonts</h4>
      <!-- insert: addFonts -->
      <div class="dialog-fonts">
        <ul class="dialog-fonts-list"></ul>
    </div>
  </dialog>
`;

    constructor(domTool, widgetBus, removeFonts) {
        super(domTool);
        this._removeFonts = removeFonts;
        this._onClickRemoveFont = this._onClickRemoveFont.bind(this);
        [this.element, this._fonts] = this._initTemplate(widgetBus);
    }

    _initTemplate(widgetBus) {
        const [dialog] = super._initTemplate()
          , fonts = dialog.querySelector('.dialog-fonts-list')
          ;

        // No eventuallyCallback: the dialog stays open, the added font
        // appears in the list via setFonts, called by the host.
        const plainAddFonts = new PlainAddFonts(widgetBus, dialog
          , '… or drag a font onto the dialog'
        );
        this._domTool.insertAtMarkerComment(dialog, 'insert: addFonts', plainAddFonts.element);

        return [dialog, fonts];
    }

    /**
     * Render the list of removable fonts, as a list of [fontName, labelText].
     * The host calls this when availableFonts changed, e.g. after a font
     * was removed or added.
     */
    setFonts(fonts, installedFonts) {
        this._domTool.clear(this._fonts);

        if (fonts.length === 0) {
            this._fonts.append(this._domTool.createElement('li', null, 'No fonts installed.'))
        }

        this._fonts.append(...fonts.map(([fontName, labelText])=>{
            const label = this._domTool.createElement('span', null, labelText)
              , icon = this._domTool.createElement('span', {class: 'material-symbols-outlined'}, 'delete')
              , button = this._domTool.createElement('button', {'data-font': fontName, title: 'Delete font'}, icon)
              , li = this._domTool.createElement('li', null, [button, label])
              ;
            if (installedFonts?.some((font) => font[0] === fontName)) {
                button.disabled = true;
                button.title = 'This font is currently in use and cannot be deleted'
            }
            button.addEventListener('click', this._onClickRemoveFont);
            return li;
        }));
    }

    async show(fonts, installedFonts) {
        const promise = super.show()
          , dialog = this.element
          ;

        this.setFonts(fonts, installedFonts);
    ...
    _onClickRemoveFont(event) {
        const button = event.currentTarget;
        // The list is not updated here: removal can be refused, e.g. for
        // a font that is currently installed. Instead, the host calls
        // setFonts once the new state is applied.
        this._removeFonts([button.getAttribute('data-font')]);
    }
}
