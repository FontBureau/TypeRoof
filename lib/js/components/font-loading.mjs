    ...
    _updateOptions(availableFonts/* changes */) {
        ...
        const value = this._select.value // keep around to set back later
          ...
          , createOptGroup = name=>{
                ...
                switch(name) {
                    case 'from-url':
                        legend.textContent ='Included fonts';
                        break;
                    case 'from-file':
                        legend.textContent = 'Your local fonts';
                        break;
                    default:
                        legend.textContent = `Origin: ${name}`;
                }
...
export class UIDialogManageFonts extends _DialogBase {
    ...
    _initTemplate(widgetBus) {
        ...
        return [dialog, form, fonts, actionDescription, plainAddFonts];
    }
