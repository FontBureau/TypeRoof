...
export function createDynamicModel(name, staticTypes, moreMixin=[], moreDependencies=[]) {
    const UCFirstName = name[0].toUpperCase() + name.slice(1)
      ...
      , result = {}
      ;
    [result[availabaleTypesName] , result[typeToTypeKeyName]] = createAvailableTypes(AvailableTypesModel, staticTypes)
