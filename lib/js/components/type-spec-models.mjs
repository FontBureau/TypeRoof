...
export const StylePatchTypeModel = _AbstractGenericModel.createClass('StylePatchTypeModel')// => .value will be a concrete _BaseStyleModel
  ...
  , MarkLinkApplyTypeEnumModel = _AbstractEnumModel.createClass(
    ...
    //      * null-style: the style is available (a styleLinkProperties@
    //        handler is registered, tag binding applies) but no style
    //        patch is applied. Replaces the former "" = NULL-STYLE encoding.
  ...
  , StylePatchLinkModeEnumModel = _AbstractEnumModel.createClass(
      ...
      , ['link', 'null-style', 'unlinked']
      , 'link'
    )
    // The value of a StylePatchLinksMapModel edge:
    //      * stylePatch: key into the StylePatchesMapModel, meaningful
    //        only in mode 'link' (cleared otherwise by the
    //        CoherenceFunction).
    //      * mode: link | null-style | unlinked (see above).
