...
export class EnvironmentProvider extends _BaseComponent {
    ...
    _readLayout() {
        // Used offsetWidth/offsetHeight before, but in order to fit
        // within the space when scrollbars are present, we must use
        // clientWidth/clientHeight.
        return {
            width: this._layoutElement.clientWidth,
            height: this._layoutElement.clientHeight,
        };
    }
...
export const PATH_SPEC_ENVIRONMENT_PROVIDER = [
        [
    ...
    ENVIRONMENT_PROVIDER_KEYS = PATH_SPEC_ENVIRONMENT_PROVIDER.map(
        (spec) => spec[0],
    ).flat(),
    ENVIRONMENT_PROVIDER_ENTRIES = ENVIRONMENT_PROVIDER_KEYS.map(
        (k) => `environment@${k}`,
    );
