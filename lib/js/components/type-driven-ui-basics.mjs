...
import {
    GutterPointModel
} from './type-spec/horizontal-layout-models.mjs';
...
export const baseResolvers = new FreezableMap([
    ...
    ['raw:zones', simpleArgument(({zones})=>zones)],
    ...
    ['settings:updateDefaultsDependencies', simpleDependencyMapping(({injectable})=>{
        return injectable.updateDefaultsDependencies;
    })],
    ['genericTypeToUIElement', simpleArgument(({injectable})=>{
        return injectable.genericTypeToUIElement;
    })],
    // this is synonym to `injectable` should probably be removed.
    ...
    ['ppsMap', simpleArgument(({injectable, ppsRecord, BaseModelType})=>{
        ...
        if(BaseModelType === LineWidthLeadingModel || BaseModelType === GutterPointModel)
            return getParentRootPPSMap(ppsRecord, BaseModelType);
...
export function _BaseTypeDrivenContainerComponentMixin(Base) {
    ...
    _defineGenericWidget(injectable, generalSettings, ppsRecord, fieldName, FieldType, resolvers=null) {
        const elementTypeConfig = injectable.genericTypeToUIElement(FieldType);
        return this._getWidgetConfig(injectable, generalSettings, ppsRecord, fieldName, elementTypeConfig, resolvers);
    }

    _getResolvers() {
        return this?._resolvers || this.constructor._resolvers;
    }

    _getWidgetConfig(injectable, generalSettings, ppsRecord, fieldName, elementTypeConfig, resolvers=null) {
        const [UIElement, BaseModelType, parameters] = elementTypeConfig
          ...
          , env = {
                ...
                , ppsRecord, fieldName
                , BaseModelType
                , resolvers: resolvers !== null ? resolvers : this._getResolvers()
                , hostName: `${this}`
            }
          ;
...
export class UITypeDrivenListItem extends _BaseTypeDrivenContainerComponentMixin(_UIBaseListContainerItem) {
    ...
    constructor(widgetBus, _zones, ppsRecord, eventHandlers=[], draggable=false, deletable=false, injectable=null, transferTypePath=null) {
        ...
        if(!(ppsRecord  instanceof ProcessedPropertiesSystemRecord))
            throw new Error(`${new.target.name} ${this.constructor.name} ppsRecord is not a ProcessedPropertiesSystemRecord!`);
        ...
        if(this._injectable.genericTypeToUIElement(TypeClass, null) !== null) {
            const config = this._defineGenericWidget(this._injectable
                  // The widget is for the model at rootPath itself; its
                  // base is `rootPath.parent`, _defineGenericWidget figures
                  // the rest.
                , {...generalSettings, rootPath: this.widgetBus.rootPath.parent}
...
export class UITypeDrivenContainer extends _BaseTypeDrivenContainerComponent {
    ...
    constructor(widgetBus, _zones, injectable, ppsMap, label, createLocalZone=true) {
        let localZoneElement;
        if(createLocalZone === true) {
            localZoneElement = widgetBus.domTool.createElement('div', {'class': 'ui_generic_struct_container'})
            widgetBus.insertElement(localZoneElement);
        }
        else if(_zones.has(createLocalZone))
            localZoneElement = _zones.get(createLocalZone);
        else
            localZoneElement = widgetBus.wrapper.host;
