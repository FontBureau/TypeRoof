...
export function uniqueKey(generatedKeySuffix, keys, sourceKey = null) {
...
// This is intended to be used if a new key is generated automatically
// It's at the moment just simpler to mirror the API than to configure
// _UIBaseMap with a different Mode.
export class UINoInputProviderMapKey extends _BaseComponent {
    constructor(widgetBus, getKeyFn) {
        super(widgetBus);
        this._getKeyFn = getKeyFn;
    }
    // copy the API of UIBaseMapKey
    isFocused() {
        return false;
    }
...
export class _UIBaseMap extends _BaseDynamicCollectionContainerComponent {
    ...
    constructor(widgetBus, _zones, eventHandlers, label=null, dragEntries=false, deletableEntries=false) {
        ...
        Object.defineProperties(this,{
              ...
              , DRAG_ENTRIES: {
                ...
                // Adds a per-item "x" delete button that removes the
                // entry from the collection. This is not drag-to-delete.
              , DELETE_ENTRIES: {
                    value: !!deletableEntries
                }
        });
    ...
    // Per-item "x" delete button (DELETE_ENTRIES): remove the entry.
    async _deleteEntry(key) {
        return await this._changeState(()=>{
            this.getEntry('childrenOrderedMap').delete(key);
        });
    }
    ...
    *_alphaOrderingGenerator (itemsGen) {
    ...
    /**
     * Maps a data-transfer type to [handlerMethodName, dropEffect].
     * The handler method is called on drop as:
     *     this[handlerMethodName](dropTargetItem, event, insertPosition, targetPath)
     * Extend in subclasses to accept additional drag types.
     */
    _getDragTypeHandlers() {
        return new Map([
            [this.constructor.KEY_DATA_TRANSFER_TYPE, ['_dropMoveHandler', 'move']],
        ]);
    }

    get _dragTypeHandlers() {
        if(this.__dragTypeHandlers === null)
            this.__dragTypeHandlers = this._getDragTypeHandlers();
        return this.__dragTypeHandlers;
    }

    _takeDragEventOrLeaveIt(event) {
        // order is relevant here
        for(const type of this._dragTypeHandlers.keys()) {
    ...
    // Whether the item at dropTargetItem has an empty nested container
    // that itself acts as a drop target (isEmptyLayerContainer). Resolved
    // at event time, as emptiness changes. Flat maps have no nested
    // containers, override in recursive subclasses.
    ...
    _getDropTargetInsertPosition(dropTargetItem, event) {
        ...
        const {height, top} = event.currentTarget.getBoundingClientRect()
          ...
          , testPosition = this._isEmptyLayerItem(dropTargetItem)
    ...
    _setDropTargetIndicator(element, insertPosition=null) {
        ...
        const classPrefix = 'drop_target_indicator-'
          ...
          , childrensMainZoneElement = this._zones.get('main')
            // The main zone element itself is marked for 'insert' (empty
            // container drop target), it is not its own direct child and
            // thus not covered by the `:scope >` query below.
          , markedElements = [
                ...(childrensMainZoneElement.classList.contains(markedClass)
                    ? [childrensMainZoneElement]
                    : [])
              , ...childrensMainZoneElement.querySelectorAll(`:scope > .${markedClass}`)
            ]
          ;
        for(const elem of markedElements) {
    ...
    _dragoverHandler(dropTargetItem, event) {
        ...
        const [, dropEffect] = this._dragTypeHandlers.get(type);
    ...
    _dropHandler(dropTargetItem, event) {
        ...
        const [handlerMethodName] = this._dragTypeHandlers.get(type);
        return this[handlerMethodName](dropTargetItem, event, insertPosition, targetPath);
    }

    _dropMoveHandler(dropTargetItem, event, insertPosition, targetPath) {
    ...
    // Default marker for generated keys; override in subclasses.
    // See uniqueKey for the format rules.
    _generatedKeySuffix = '_';

    _uniqueKey(keys, sourceKey = null) {
        return uniqueKey(this._generatedKeySuffix, keys, sourceKey);
    }

    // Version to move between instances of _AbstracrOrderedMap.
    _move(sourcePath, targetPath, insertPosition) {
        // Refuse to move an item into itself or its own descendants:
        // deleting the source would delete the target as well, the net
        // effect would be deleting the source item.
        if(sourcePath.isRootOf(targetPath)) {
        ...
        return this._changeState(()=>{
            const targetParent = insertPosition === 'insert'
              ...
              , source = sourceParent.get(sourceKey)
                // We may preserve the old key if it is free, otherwise
                // generate a unique one (preserving the source key's
                // suffix style). Same-parent reorder must keep the key
                // stable, so the check only applies across parents.
              , uniqueKey = targetParent !== sourceParent && targetParent.has(sourceKey)
                    ? this._uniqueKey(targetParent.keys(), sourceKey)
                    : sourceKey
              , sourceEntry = [uniqueKey, source]
              ;
            if(insertPosition === 'insert') {
    ...
    _provisionWidgets() {
        ...
        for(const [index, [key, /*axesLocationSet*/]] of gen) {
            ...
            if(!keyProtocolHandler.hasRegistered(keyId)) {
                ...
                if(this.DELETE_ENTRIES) {
                    deleteButtonElement = this._domTool.createElement(
                        'button',
                        { class: 'ui_button ui_button-remove', title: 'remove' },
                        createIcon('delete'),
                    );
                    // keyId is the stable slot identity; the key it hosts
                    // changes on slot recycling, hence resolve it at event
                    // time (the _dropHandler pattern), never capture it here.
                    deleteButtonElement.addEventListener('click', () => {
                        const key = this.widgetBus.wrapper
                            .getProtocolHandlerImplementation('key@')
                            .getRegistered(keyId).value;
