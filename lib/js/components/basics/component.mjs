...
/* --- Update relevance filtering (pruning, stages 1+2) --------------------
 * UPDATES ONLY WHAT CAN BE AFFECTED.
 *
 * Historically, an update was a "something changed somewhere"
 * notification and every container recursed into (and re-provisioned)
 * every widget on every change. With big documents (e.g. the wikipedia
 * viewer) that is the dominating cost.
 *
 * Stage 1 (update filtering): each widgetWrapper gets a relevance
 * report — relevant when a model dependency intersects the
 * compareResult's changed paths, or a protocol dependency is marked
 * updated, or (for containers) any descendant is relevant. Irrelevant
 * widgets are skipped — they provably can not be affected.
 *
 * Stage 2 (provisioning skip): a container's _provisionWidgets is
 * skipped when no descendant has a model-relevant dependency AND all
 * mapped protocol ids are still registered (membership stable). This
 * covers both events provisioning reacts to: structural changes are
 * visible via model deps, and registry changes (e.g. a typeSpec rename)
 * surface as unregistered mapped ids — membership violation — which
 * forces provisioning so stale mappings self-heal by re-resolution.
 *
 ...
 * (e.g. a collection dep whose parent path contains the change — the
 * container must provision). rawCompare yields CHANGED for every
 * ancestor of a change, which the prefix set exploits; those ancestor
 * entries are excluded from the root index, as they would match
 * everything. Protocol deps are first-class: the viewer's stylers
 * depend on e.g. typeSpecProperties@ via SimpleProtocolHandler
 * updated-logs, outside the compareResult.
 *
 * The relevance memo is per-compareResult, so the aggregated scan of
...
function _getUpdateRelevanceContext(compareResult) {
    ...
    const changedRoots = new Set(
            candidates.filter(p => !candidates.some(q => q !== p && q.startsWith(`${p}/`)))
        )
...
/* For stage 2 the relevance of a wrapper is a triple report:
 *  - updateRelevant: this wrapper (or a descendant) should be updated.
 *  - modelRelevant: own model deps intersected (needed for provisioning
 *    decision of containers: provisioning reacts to model changes).
 *  - membershipOk: all mapped protocol ids of this wrapper are still
 *    registered; if not, the mapping is stale and a container in the
 *    chain must re-provision (self-healing re-resolution, e.g. after a
 *    typeSpec rename; otherwise getUpdated would throw downstream).
 */
function _scanWrapper(widgetWrapper, context) {
    let report = context.memo.get(widgetWrapper);
    if(report !== undefined)
        return report;
    report = { updateRelevant: false, modelRelevant: false, membershipOk: true };
    ...
    for(const [external] of fullDependencyMappings[ComponentWrapper.DEPENDECIES_DEFAULT].entries())
        if(_modelDepIntersectsChangedRoots(external, context)) {
            report.modelRelevant = report.updateRelevant = true;
            break;
        }
    for(const [protocolHandler, implementation] of widgetWrapper._protocolHandlers) {
        ...
        for(const [external] of dependenciesMap.entries()) {
            if(!implementation.hasRegistered(external)) {
                // stale mapping, membership violation
                report.membershipOk = false;
                continue;
            }
            if(implementation.getUpdated(external)[0])
                report.updateRelevant = true;
        }
    }
    ...
    if(widget instanceof _CommonContainerComponent)
        for(const childWrapper of widget.activeWidgets()) {
            const childReport = _scanWrapper(childWrapper, context);
            report.modelRelevant = report.modelRelevant || childReport.modelRelevant;
            report.updateRelevant = report.updateRelevant || childReport.updateRelevant;
            report.membershipOk = report.membershipOk && childReport.membershipOk;
        }
    context.memo.set(widgetWrapper, report);
    return report;
}

function _aggregateChildrenReport(container, context) {
    const aggregate = {
        updateRelevant: false, modelRelevant: false, membershipOk: true,
    };
    for(const childWrapper of container.activeWidgets()) {
        const report = _scanWrapper(childWrapper, context);
        aggregate.updateRelevant = aggregate.updateRelevant || report.updateRelevant;
        aggregate.modelRelevant = aggregate.modelRelevant || report.modelRelevant;
        aggregate.membershipOk = aggregate.membershipOk && report.membershipOk;
    }
    return aggregate;
}
...
export class _CommonContainerComponent extends _BaseComponent {
    ...
    *activeWidgets() {
    ...
    update(compareResult) {
        let requiresFullInitialUpdate;
        if(this.constructor.UPDATE_RELEVANCE_FILTER === false)
            requiresFullInitialUpdate = this._provisionWidgets(compareResult);
        else {
            // Stage 2: skip provisioning of subtrees that can't be
            // affected by the change. Provisioning must run when any
            // descendant's model deps are relevant (structural/collection
            // changes drive provisioning) or when a protocol mapping
            // became stale (unregistered id → re-resolution, self-healing,
            // e.g. after a typeSpec rename). When only protocol deps of
            // descendants are marked updated, provisioning can not
            // change anything, so it is skipped.
            const context = _getUpdateRelevanceContext(compareResult)
              , aggregate = _aggregateChildrenReport(this, context);
            requiresFullInitialUpdate =
                (aggregate.modelRelevant || !aggregate.membershipOk)
                    ? this._provisionWidgets(compareResult)
                    : new Set();
        }
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        for(const widgetWrapper of this.activeWidgets()) {
            ...
            if(!requiresFullInitialUpdate && !isInitialUpdate
                ...
                const report = _scanWrapper(widgetWrapper, _getUpdateRelevanceContext(compareResult))
                  // Containers with a protocol membership violation must
                  // be updated (their provisioning re-resolves stale ids
                  // and re-creates the stale wrappers); simple widgets
                  // with stale ids are skipped — the container re-created
                  // them in its provisioning phase.
                  , mustUpdate = report.updateRelevant
                        || (widgetWrapper.widget instanceof _CommonContainerComponent
                            && !report.membershipOk);
                if(!mustUpdate)
