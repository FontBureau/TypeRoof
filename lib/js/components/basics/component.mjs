...
export class _BaseComponent {
     ...
     */
    update(changedMap) {
        // Default: no-op. Subclasses with model dependencies override
        // this; the parameter documents the interface.
        void changedMap;
    }
...
export class SimpleProtocolHandler extends _ProtocolHandler {
    ...
    *[Symbol.iterator]() {
    ...
    /** Whether any identifier was marked updated since the last reset. */
    hasUpdated() {
        return this._updatedLog.size > 0;
    }
...
class ComponentWrapper {
    ...
    hasProtocolHandlerRegistration(protocolHandler) {
        return this._protocolHandlerRegistrations.has(protocolHandler);
    }
    ...
    /*
     * The activationTest receives getEntry as its first argument: it
     * works like this widget's widgetBus.getEntry, but enforces that the
     * requested entry is a declared dependency of this widget.
     * Rationale: the update-relevance filter (UPDATE_RELEVANCE_FILTER,
     * pruning stage 1) decides from the declared dependencies alone which
     * widgets to update and provision. State that an activationTest reads
     * without declaring it as a dependency is invisible to the filter:
     * when that state changes, provisioning may not run and the
     * activation change is missed (e.g. the showParameters /
     * showNodeTypeSpecLabels toggles had no effect until an unrelated
     * full provisioning). Hence, reading model state in an activationTest
     * must go through this injected getEntry, not through the general
     * (component- or widgetBus-level) getEntry.
     */
    _activationTestGetEntry(internalName) {
        const externalName = this.widgetBus.getExternalName(internalName);
        if(!this._fullDependencyMappings[ComponentWrapper.DEPENDECIES_ALL].has(externalName))
            throw new Error(`ACTIVATION TEST DEPENDENCY ERROR in ${this}: `
                + `getEntry("${internalName}") resolves to "${externalName}", `
                + `which is not a declared dependency. Add it to the widget's `
                + `dependencyMappings so the update-relevance filter keeps `
                + `this widget in provisioning range.`);
        return this.widgetBus.getEntry(internalName);
    }
    ...
    activationTest() {
        if(!this._activationTest)
            ...
            return true;
        return this._activationTest(this._activationTestGetEntry.bind(this));
    }
...
/* --- Update relevance filtering (pruning, stage 1) -----------------------
 * UPDATES ONLY WHAT CAN BE AFFECTED.
 *
 * Historically, an update was a "something changed somewhere"
 * notification and every container recursed into every widget on every
 * change. With big documents (e.g. the wikipedia viewer) that is the
 * dominating cost, as every document node re-provisions every time.
 * Instead of recursing unconditionally, each widgetWrapper is checked
 * for relevance: if neither its model dependencies intersect the
 * compareResult's changed paths, nor any of its protocol handler
 * dependencies are marked updated, nor (for containers) any descendant
 * is relevant, the widget is skipped — it provably can not be affected.
 * Skipping a container also skips its provisioning, which stays correct
 * because the events that provisioning reacts to (structure, activate
 * tests, collection compare entries) are all covered by model deps.
 *
 * Model dep intersection matches in both directions: dep under a
 * changed root (descendant changed) or a changed root under the dep
 * (e.g. a collection dep whose parent path contains the change —
 * the container must provision). rawCompare yields CHANGED for every
 * ancestor of a change, which the prefix set exploit. Protocol deps are
 * first-class: the viewer's stylers depend on e.g. typeSpecProperties@
 * via SimpleProtocolHandler updated-logs, outside the compareResult.
 * Unregistered/missing ids are treated as relevant (conservative),
 * so e.g. a typeSpec rename re-provisions before the consumer throws.
 *
 * The relevance memo is per-compareResult, so the (memoized) scan of
 * descendant relevance runs once per update, not once per container.
 */
const _updateRelevanceContexts = new WeakMap();

/**
 * Keep only the deepest paths of the set: exclude every path that is a
 * proper ancestor of another path in `paths` (duplicates collapse).
 * The naive pairwise filter is quadratic in the number of entries and
 * dominated update time on document-sized comparisons (e.g. swapping
 * the wikipedia document: ~6.5 s for ~1200 entries, growing with the
 * square of the entry count). Instead: with the path separator
 * replaced by "\u0000" — which sorts before any other character —
 * ancestor relations become *plain* string prefixes (the character
 * after a prefix is always the separator; CAUTION: the separator must
 * be *replaced*, not removed, or merely similar path names falsely
 * prefix-match, e.g. "a/b" -> "ab" vs "ab/c" -> "abc"). Then, after
 * sorting by key, all of a path's duplicates and descendants form one
 * contiguous run starting with the path itself (the run's end is the
 * upper bound for the key with "\uFFFF" appended — "\uFFFF" sorts
 * after "\u0000", i.e. after the path itself, and after any other
 * character, i.e. after all descendants). Checking the boundaries of
 * that run requires no further scan: the entry at the run's start is a
 * duplicate iff it equals the key, the entry before the run's end is a
 * descendant iff it starts with the key. Both runs are found via
 * binary search; ties keep the last occurrence, like a Set. => one
 * O(n log n) sort + two binary searches per path.
 *
 * Exported (not for reuse — the unit test must exercise the very
 * implementation that runs in the hot path).
 */
export function _keepDeepestPaths(paths) {
    const entries = paths.map((path) => [
            path.replaceAll(Path.SEPARATOR, "\u0000"),
            path,
        ])
      , deepest = new Set()
      // First index i with entries[i][0] >= key.
      , lowerBound = (key) => {
            let lo = 0,
                hi = entries.length;
            while(lo < hi) {
    ...
    for(const [i, [key, path]] of entries.entries()) {
        // The run of entries with this very key (duplicates, this
        // entry included) ...
        const lastDuplicate = lowerBound(`${key}\u0000`) - 1,
            // ... and the run of descendants: keys prefixed by the key
            // plus the separator (NOT a plain key prefix, which would
            // also capture siblings like "/1" vs "/10"; the root path
            // "/" already ends with the separator).
            descendantPrefix = key.endsWith("\u0000")
                ? key
                : `${key}\u0000`,
            lastDescendant = lowerBound(`${descendantPrefix}\uFFFF`) - 1;
        // Not the last occurrence (duplicates keep the last one, like
        // a Set) — or a descendant exists (its key is within the
        // descendant run, i.e. beyond the duplicates).
        if(lastDuplicate > i || lastDescendant > lastDuplicate)
            continue;
        deepest.add(path);
    }
    return deepest;
...
function _getUpdateRelevanceContext(compareResult) {
    let context = _updateRelevanceContexts.get(compareResult);
    if(context !== undefined)
        return context;
    // Index the changed paths, excluding EQUALS entries and proper
    // ancestors of other changed paths (rawCompare yields CHANGED for
    // every ancestor of a change, down to the root — indexing those
    // would match every dep/rawCompare counts on parents' prefixes).
    // Ancestor-level deps still match: the dep-is-a-prefix check below
    // covers them via changedPrefixes.
    const candidates = [];
    for(const [status, , pathInstance] of compareResult) {
        if(status === StateComparison.COMPARE_STATUSES.EQUALS)
            continue;
        candidates.push(pathInstance.toString());
    }
    const changedRoots = _keepDeepestPaths(candidates)
      , changedPrefixes = new Set()
      , memo = new Map()
    ;
    for(const path of changedRoots) {
        let current = path;
        while(true) {
            const index = current.lastIndexOf(Path.SEPARATOR);
            if(index <= 0)
                break;
            current = current.slice(0, index);
            changedPrefixes.add(current);
        }
    }
    context = { changedRoots, changedPrefixes, memo };
    _updateRelevanceContexts.set(compareResult, context);
    return context;
}

function _modelDepIntersectsChangedRoots(dep, context) {
    // dep is equal to or under a changed root
    if(context.changedRoots.has(dep))
        return true;
    let current = dep;
    while(true) {
        const index = current.lastIndexOf(Path.SEPARATOR);
        if(index <= 0)
            break;
        current = current.slice(0, index);
        if(context.changedRoots.has(current))
            return true;
    }
    // a changed root is under dep (dep is a prefix of it)
    return context.changedPrefixes.has(dep);
}

function _ownDepsRelevant(widgetWrapper, context) {
    const fullDependencyMappings = widgetWrapper._fullDependencyMappings;
    for(const [external] of fullDependencyMappings[ComponentWrapper.DEPENDECIES_DEFAULT].entries())
        if(_modelDepIntersectsChangedRoots(external, context))
            return true;
    for(const [protocolHandler, implementation] of widgetWrapper._protocolHandlers) {
        const dependenciesMap = fullDependencyMappings[protocolHandler];
        if(dependenciesMap === undefined)
            continue;
        for(const [external] of dependenciesMap.entries()) {
            let wasUpdated;
            try {
                [wasUpdated] = implementation.getUpdated(external);
            }
            catch {
                // Unregistered/missing id: conservative, re-provisioning
                // may be required (e.g. after a typeSpec rename).
                return true;
            }
            if(wasUpdated)
                return true;
...
function _widgetWrapperIsRelevant(widgetWrapper, context) {
    let relevant = context.memo.get(widgetWrapper);
    if(relevant !== undefined)
        return relevant;
    relevant = _ownDepsRelevant(widgetWrapper, context);
    if(!relevant) {
        const widget = widgetWrapper.widget;
        if(widget instanceof _CommonContainerComponent)
            for(const childWrapper of widget.activeWidgets())
                if(_widgetWrapperIsRelevant(childWrapper, context)) {
                    relevant = true;
                    break;
                }
    }
    context.memo.set(widgetWrapper, relevant);
    return relevant;
}

export class _CommonContainerComponent extends _BaseComponent {
    [UPDATE_STRATEGY] = UPDATE_STRATEGY_COMPARE;
    // Escape hatch for the relevance filter (see _update); checked via
    // this.constructor so subclasses can opt out explicitly.
    static UPDATE_RELEVANCE_FILTER = true;
    ...
    *activeWidgets() {
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        for(const widgetWrapper of this.activeWidgets()) {
            ...
            if(!requiresFullInitialUpdate && !isInitialUpdate
                    && this.constructor.UPDATE_RELEVANCE_FILTER !== false
                    && !_widgetWrapperIsRelevant(widgetWrapper, _getUpdateRelevanceContext(compareResult)))
                // provably unaffected, skip entirely
                continue;
