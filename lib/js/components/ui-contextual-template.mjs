...
export class UIContextualTemplateContainer extends _BaseTypeDrivenContainerComponentMixin(
    ...
    constructor(widgetBus, _zones, injectable, propertyRoot, label) {
        ...
        // FIXME: rather as widget eventually into local, at the end ... but
        // it should be created by the interface for rules anyways!
        // "Add rule" button
        const addButton = widgetBus.domTool.createElement(
            "button",
        ...
        addButton.addEventListener("click", () => {
            this._changeState(() => {
                const rules = this.getEntry(
                    this.widgetBus.rootPath.append("rules"),
                );
                rules.push(
                    TemplateRulesModel.Model.createPrimalDraft(
                        rules.dependencies,
                    ),
                );
            });
    ...
    _defineWidgets(
        ...
        console.log(`${this}._defineWidgets ppsMap:`, ppsMap);
        return [
...
class UICharsSelectorContainerItem extends _UIBaseListContainerItem {
    ...
    constructor(
        ...
        console.log(
            ...
            propertyRoot,
            `rootPath:`,
            widgetBus.rootPath.toString(),
        );
        if (typeof propertyRoot !== "string")
            throw new Error(
                `${this.constructor.name} propertyRoot is not a string!`,
            );
