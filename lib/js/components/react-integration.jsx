...
WidgetBus.propTypes = {
    ...
    widgetBus: PropTypes.instanceOf(Object).isRequired,
};
...
export class ReactRoot extends _BaseComponent {
    ...
    addListener(listener, rawDependencyMappings) {
        const fullDependencyMappings =
            this.widgetBus.wrapper.constructor.absPathDependencies(
        ...
        const fullDependencyMappings =
            ...
            modelDependencies =
                fullDependencyMappings[
                    this.widgetBus.wrapper.constructor.DEPENDECIES_ALL
                ], // .keys()
            compareResult = StateComparison.createInitial(
                rootState,
                modelDependencies,
            ),
            initialChangedMap =
                this.widgetBus.wrapper.constructor.getChangedMapFromCompareResult(
                    fullDependencyMappings,
                    this.widgetBus.protocolHandlers,
                    true,
                    compareResult,
    ...
    update(compareResult) {
        for (const {
            ...
            if (changedMapForListener.size) listener(changedMapForListener);
        }
    }
}
...
export function useMetamodel(dependencies = []) {
    ...
    if (!widgetBus) {
        throw new Error(
            "useMetamodel must be used within a WidgetBus component",
        );
    }
    ...
    useEffect(() => {
        const listener = (changedMap) => {
            setEntries((prevEntries) =>
                Object.assign({}, prevEntries, Object.fromEntries(changedMap)),
            );
        };
...
export function useMetamodelSimple(path) {
    ...
    useEffect(() => {
        ...
        // must be in there!
        const listener = (changedMap) =>
            setStateValue(changedMap.get(path).value);
