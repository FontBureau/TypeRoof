...
export class UINumberAndRangeOrEmptyInput extends _BaseComponent {
    constructor(parentAPI, baseID, label, unit, minMaxValueStep, getDefault=null) {
        super(parentAPI);
        this._minMaxValueStep = minMaxValueStep;
    ...
    update(changedMap) {
        if(changedMap.has('value') || changedMap.has('defaults')) {
            ...
            console.log(`${this.constructor.name}.update`
                , this.getEntry(this.parentAPI.rootPath.append('../keyMoment/label')).value
                ,...changedMap.keys()
                , 'isEmpty:', valueOrEmpty.isEmpty
                , valueOrEmpty
            )
            let active, value;
            if(!valueOrEmpty.isEmpty) {
                ...
                value = this._getDefault
                            ...
                            : this._defaultValue
                            ;
                console.log(this.constructor.name,'_getDefault', active, value);
            }
