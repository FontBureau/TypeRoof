...
export class GenericSelect extends _BaseComponent {
    ...
    // Default is to select the first item on reset. -1 will
    // select nothing. Change the getter method RESET_OPTION_INDEX
    // to return a calculated index (current, first non-disabled ...).
    static RESET_OPTION_INDEX = 0;

    constructor(widgetBus, baseClass, labelContent, optionGetLabel=null, allowNull=[]
            , onChangeFn=null, optionGetGroup=null, optionsGen=null) {
        super(widgetBus);
        if(optionGetLabel)
        ...
        if(this._allowNull) {
            // FIXME: this behavior makes inheritance problematic!
    ...
    get RESET_OPTION_INDEX() {
        return this.constructor.RESET_OPTION_INDEX;
    }
    ...
    *_optionsGen(availableOptionsMap) {
        yield * availableOptionsMap; // like a maps Symbol.iterator  => [[key, value], ...]
    }

    _createOption(value, label) {
    ...
    _populateSelect(options /* [[key, value, label],...] */) {
        const rootItems = []
          , groups = new Map()
          , createOption=(optionElements, value, label)=>{
                optionElements.push(this._createOption(value, label));
            }
          ...
          ;
        if(this._allowNull) {
            const [key, value, label] = [this.constructor.NULL_SELECT_VALUE, this._nullModelValue, this._allowNullLabel]
              , optionElements = getGroupItems(value)
              ;
            createOption(optionElements, key, label);
        }

        for(const [key, value, label] of options) {
            const optionElements = getGroupItems(value);
            createOption(optionElements, key, label);
        }
    ...
    _updateValue(activeValue) {
        this._select.value = this._allowNull && activeValue.value === this._nullModelValue
                ? this.constructor.NULL_SELECT_VALUE
                : activeValue.value.toString()
                ;
    }
    ...
    get isNullValue() {
        return this._allowNull && this.constructor.NULL_SELECT_VALUE === this._select.value;
    }
    ...
    setCustomValidity(message="") {
        this._select.setCustomValidity(message);
...
export class PlainSelectInput {
    static TEMPLATE = `<div class="ui_plain_select">
    <label>
    ...
    _initTemplate(label, items) {
        ...
        // results.
        for(const keyLabel of items) {
             const [key, label] = typeof keyLabel === 'string' ? [keyLabel, keyLabel] : keyLabel
               , option = this._domTool.createElement('option',{}, label)
               ;
             option.value = key;
             options.push(option);
        }
        ...
        if(label)
            ...
            this._domTool.removeNode(container.querySelector('label'));
...
export class PlainNumberAndRangeInput {
    ...
    updateTemplateVars(label, unit, minMaxValueStep) {
        ...
        if (label instanceof HTMLString) {
            const fragment = this._domTool.createFragmentFromHTML(label);
            this._domTool.clear(this._labelElement);
            this._labelElement.append(fragment);
        } else {
            this._labelElement.textContent = `${label}`;
        }

        const labelContainer = this._labelElement.closest('label')
                                    ?? this._labelElement;
        labelContainer.style.display = (label === null) ? "none" : null;

        if(unit) {
            this._domTool.clear(this._unitElement);
            this._unitElement.append(`${unit}`);
            this._unitElement.hidden = false;
        } else {
            this._unitElement.textContent = '';
            this._unitElement.hidden = true;
        }
    }
...
export class PlainLineOfTextInput {
    ...
    constructor(domTool, changeHandler, label=null) {
        this._domTool = domTool;
        this._passive = false;
    ...
    _initTemplate(label=null) {
        const container = this._domTool.createFragmentFromHTML(this.constructor.TEMPLATE).firstElementChild
          ...
          ;
        if(label !== null)
            this._domTool.insertAtMarkerComment(container, 'insert: label',
                this._domTool.createElement('strong', {}, label));
        return [container, input];
    }
...
export class PlainToggleButton {
    ...
    constructor(domTool, changeStateHandler, classToken, labelIsOn, labelIsOff, title) {
        ...
        this.element = this._domTool.createElement('button', {
                'class': `ui_button ${this.constructor.baseClass} ${this.constructor.baseClass}-${classToken}`
              , title: title
            }, '(initializing)');
    ...
    update(booleanValue) {
        ...
        if (this._booleanValue === newValue)
            return;
        this._booleanValue = newValue;
        const label = this._booleanValue
            ? this._labelIsOn
            : this._labelIsOff
            ;
        if ([this._labelIsOn, this._labelIsOff].some((label) => label instanceof HTMLString)) {
            this._domTool.clear(this.element);
            this._domTool.insert(this.element, 'append', label);
        } else {
...
export class StaticNode extends _BaseComponent {
    ...
    update(/*changedMap*/){
        // pass
        // This is present as when we use an activationTest, we may have
        // to add a dependency, and that causes update to be called.
    }
}
...
export class DynamicTag extends _BaseComponent {
    ...
    update(changedMap) {
        if(changedMap.has('data')) {
            const data = changedMap.get('data'),
                value = data instanceof _BaseSimpleModel
                    ? data.value
                    : data;
            this.element.textContent = this._formatter(value);
        }
    }
}
...
export class MoveItemsInListButton extends _BaseComponent {
    ...
    static setings = {
        [this.BACKWARD]: {
          ...
          , classToken: 'backward'
          , label: createIconAndLabel('keyboard_double_arrow_left', 'move backward')
        }
      , [this.FORWARD]: {
           title: 'Move item(s) one position forward.'
         , classToken: 'forward'
         , label: createLabelAndIcon('move forward', 'keyboard_double_arrow_right')
        }
    };
        ...
        this.element.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            ...
            for(const key of keys) {
                if(action === this.constructor.FORWARD)
                    ...
                    throw new Error(`TYPE ERROR action unknown ${action.toString()}.`);
            }
        }));
        this._insertElement(this.element);
    }
...
export class Collapsible extends _BaseComponent {
    static BASE_CLASS = 'ui_collapsible';
    static OPENED_CLASS = `${Collapsible.BASE_CLASS}-opened`;
    static CLOSED_CLASS = `${Collapsible.BASE_CLASS}-closed`;
    static FLAVORS = {
        minimal: 'minimal-collapsible',
        separating: 'separating-collapsible'
    }

    constructor(widgetBus, togglerLabel, contentsElement, isOpened = false, flavor='separating') {
        const initialClasses = [
            Collapsible.BASE_CLASS
          , isOpened ? Collapsible.OPENED_CLASS : Collapsible.CLOSED_CLASS
          , ... ((flavor in new.target.FLAVORS) ? [new.target.FLAVORS[flavor]]: [])
        ];
        const element = widgetBus.domTool.createElement('div', {class: initialClasses.join(' ')});
        const togglerElement = widgetBus.domTool.createElement('div', {class: `${Collapsible.BASE_CLASS}_toggle`, role: 'button', 'aria-expanded': isOpened});
    ...
    _onClickToggler() {
        const wasOpen = this._togglerElement.getAttribute('aria-expanded') === 'true';
        const isOpen = !wasOpen;
        this._togglerElement.setAttribute('aria-expanded', isOpen);
        this._contentsElement.setAttribute('aria-hidden', !isOpen);
        if (isOpen) {
            this._element.classList.add(Collapsible.OPENED_CLASS);
            this._element.classList.remove(Collapsible.CLOSED_CLASS);
...
export class CollapsibleContainer extends _BaseContainerComponent {
    ...
    static FLAVORS = Collapsible.FLAVORS;
    constructor(widgetBus, _zones, togglerLabel, flavor, classNameParticle,
                                _widgets=[], isOpened = false, scroll = true) {
        const initialClasses = [
          ...
          , togglerElement = widgetBus.domTool.createElement('div', {
                    class: `${new.target.BASE_CLASS}_toggle`
                  , role: 'button'
          ...
          , zones = new Map([..._zones, ['main', contentsElement], ['label', togglerElement]])
          , widgets = Array.from(_widgets)
          ;
        if(togglerLabel !== null && togglerLabel !== false)
            widgets.unshift([
                {zone: 'label'},
                [],
                StaticNode,
                widgetBus.domTool.createElement('span', {class: 'typeroof-ui-label'}, togglerLabel)
            ]);
    ...
    _shouldBeActive(widgetWrapper) {
        if(!this.isOpen) {
            if(widgetWrapper.zone === 'label')
                // label widgets may be open, but they can run their own activationTest
                return super._shouldBeActive(widgetWrapper);
            // none-label widgets are closed
            return false;
        }
        return super._shouldBeActive(widgetWrapper);
    }
...
export class WasteBasketDropTarget extends _BaseDropTarget {
    ...
    _dropHandlerImplementation(event) {
        ...
        // These are absolute paths!
                ...
                || type === DATA_TRANSFER_TYPES.PROSEMIRROR_NODE_SPEC_PATH
                || type === DATA_TRANSFER_TYPES.PROSEMIRROR_MARK_SPEC_PATH) {
            return this._changeState(()=>{
