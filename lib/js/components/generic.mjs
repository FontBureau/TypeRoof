...
export class GenericSelect extends _BaseComponent {
    ...
    // jshint ignore:end
    constructor(parentAPI, baseClass, labelContent, optionGetLabel, allowNull=[]) {
        super(parentAPI);
        if(optionGetLabel)
...
export class PlainNumberAndRangeInput {
    ...
    <input id='number'  size="3" /><!-- insert: unit -->
    <input id='range' />
</div>`;
    //jshint ignore:end
    /**
    ...
    _initTemplate() {
        ...
        for(const typeId of ['number', 'range']) {
            const elem = fragment.getElementById(typeId);
            if(!elem)
                continue;
            elem.id = '';
            elem.type = typeId;
            elementsById.set(typeId, elem);
        }
        for(const label of container.querySelectorAll('label')) {
            const htmlFor = label.htmlFor;
            label.htmlFor = '';
            label.addEventListener('click', ()=>{
                let target = elementsById.get(htmlFor);
                if(!target || target.style.getPropertyValue('display') === 'none')
                    target = elementsById.get('number');
                if(target)
                    target.focus();
            });
        }
        const inputs = Array.from(container.getElementsByTagName('input'));
        return [container, inputs, labelElement, unitElement, elementsById.get('range')];
    }
...
export class UINumberAndRangeInput extends _BaseComponent {
    constructor(parentAPI, label, unit, minMaxValueStep) {
        super(parentAPI);
        const changeHandler = this._changeStateHandler(
            value=>this.getEntry('value').set(value));
        this._ui = new PlainNumberAndRangeInput(this._domTool, changeHandler, label, unit, minMaxValueStep);
...
export class PlainNumberAndRangeOrEmptyInput {
    constructor(domTool, numberChangeHandler, toggleChangeHandler, label, unit, minMaxValueStep) {
        this._domTool = domTool;
        ...
        this._defaultValue = 'default' in minMaxValueStep
                ...
                : 36
                ;
        this._uiNumber = new PlainNumberAndRangeInput(this._domTool, numberChangeHandler, label, unit, minMaxValueStep);
...
export class UINumberAndRangeOrEmptyInput extends _BaseComponent {
    constructor(parentAPI, label, unit, minMaxValueStep, getDefault=null, requireUpdateDefaults=()=>false) {
        super(parentAPI);
        this._minMaxValueStep = minMaxValueStep;
