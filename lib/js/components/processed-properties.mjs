...
export class UIshowProcessedProperties extends _BaseComponent {
    ...
    _createTextValueElement(key, value) {
        ...
        const valueContainer = this._domTool.createElement(
                              'span'
    ...
    update(changedMap) {
        ...
        if(changedMap.has('properties@')) {
            ...
            for(const [property, value] of propertyValuesMap) {
                if(property.startsWith(INTENT_STYLE_LINKS) || property.startsWith(MARK_STYLE_LINKS))
                    // Style-link edge structs are not displayable properties.
                    continue;
...
export class UIshowProcessedPropertiesCollapsible extends CollapsibleContainer {
    constructor(
        widgetBus,
        _zones,
        togglerLabel = "Inspect Processed Properties",
        isOpened = false,
        scroll = false,
) {
        const classNameParticle = "ui-show_processed_properties",
            flavor = "minimal";
        const widgets = [
            [
                {
                ...
                [
                    ...
                    [
                        widgetBus.getExternalName("referenceItem"),
                        "referenceItem"],
                    [
                        widgetBus.getExternalName("properties@"),
                        "properties@",
                    ],
                ],
