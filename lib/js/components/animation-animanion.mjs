...
import {
    easingFunctions
} from '../easingFunctions.ts';

export const EasingFunctionModel = _AbstractEnumModel.createClass('EasingFunctionModel',
        [...easingFunctions.keys()],
        'linear'),
    keyMomentBaseModelMixin = Object.freeze([
        CoherenceFunction.create(['duration'],
        function setDefaults({duration}) {
...
// exported because it may be useful somewhere, but it also is not used
// in here anymore. TODO: rather move to color.mjs.
export function interpolateCuloriColors(t, a, b) {
...
function interpolate(t, a, b) {
    if(isCuloriColor(a) && isCuloriColor(b)){
        // FIXME: I don't think this case ever happens anymore, as we're
        // getting the flattened processed properties in here.
        throw new Error(`DEPRECATED: we don't interpolate culori color objects in here anymore.`);
...
function getEasingFunction(keyMoment/*, propertyName*/) {
    // FIXME: 'easing' key is not implemented in keyMoment
    //        Also, it may not directly contain a function
    // identity equals linear easing
    //
    // Currently, I rather think that a "metadata" structure will be
    // attached to each keyMoment, and that can contain optional easing
    // function instructions. a catch-all for all axesLocations/ is
    // nice, maybe we can work with e.g. '*' wildcards.
    const easingFunctionKey = keyMoment.get('easing').value
    return easingFunctions.get(easingFunctionKey);
}

function getPropertyValue(fullDuration, isLoop, propertyData
                        , absoluteT, propertyName) {
    ...
    if(left === null) {
        ...
        const toMomentData = propertyData.get(toMomentTKey).at(0) // => {key, keyMoment, propertyValue}
          ...
          , easingFunction = getEasingFunction(toMomentData.keyMoment, propertyName)
...
function getPropertyValues(propertyToKeyMoments, absoluteT /* between 0 and fullDuration */) {
    ...
    for(const [propertyName, propertyData] of propertyToKeyMoments) {
        const value = getPropertyValue(fullDuration, isLoop, propertyData, absoluteT, propertyName);
...
export class LocalScopeAnimanion {
    ...
    *_propertiesGenerator(propertiesWithDependeciesMap, keyMoment, momentT) {
    ...
    _resolveLocalPropertyDependency(dependantPropertyName, wipPropertyToKeyMoments, momentT, dependentPropertyValue) {
        ...
        if(wipPropertyToKeyMoments.has(propertyName)) {
            const {fullDuration, isLoop} = wipPropertyToKeyMoments
              ...
              ;
            return getPropertyValue(fullDuration, isLoop, propertyData, momentT, propertyName);
        }
        // TODO:
    ...
    getPropertyValueForLocalT(propertyName, localRelativeT, defaultVal=super._NOTDEF) {
        ...
        const propertyData = propertyToKeyMoments.get(propertyName);
        return getPropertyValue(fullDuration, isLoop, propertyData, localAbsoluteT, propertyName);
    }
...
export class AnimationLiveProperties extends _BaseComponent {
    ...
    update(changedMap) {
        ...
        // Even when the local t (and hence the local propertyValuesMap)
        // didn't change, the *inherited* properties may have: subscribers
        // read them live, via getPropertiesFromGlobalT, from the whole
        // animanion hierarchy. Usually the local t changes along with the
        // parent, so this is already covered by propertiesChanged, but it
        // is not when this scope has its time set explicitly, e.g. the
        // "Children Time" (numericProperties/t) that the videoproof
        // layout's "wait there's more" layer uses to pause the videoproof
        // actor while it animates an axis of its own.
        const parentPropertiesChanged =
                hasParentProperties && changedMap.has('@parentProperties');
         if(animanionChanged || propertiesChanged || parentPropertiesChanged) {
