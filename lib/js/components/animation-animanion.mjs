...
function interpolateColors(t, a, b) {
        ...
        if(a.mode !== b.mode) {
            ...
            // be removed again.
            const missing = [];
            for(const component of culoriGetMode(a.mode).channels) {
                if(Object.hasOwn(a, component))
                    // Not missing.
                    continue;
                if(component === 'alpha')
                    // Missing alpha is not an issue.
                    continue;
                if(component === 'L' && a.mode.startsWith('ok')
                                     && b.mode.startsWith('ok'))
            ...
            if(missing.length)
                ...
                a = {...a, ...Object.fromEntries(missing)};
        }
...
function interpolate(t, a, b) {
    if(isCuloriColor(a) && isCuloriColor(b))
        return interpolateColors(t, a, b);
