...
function interpolate(t, a, b) {
    ...
    if(typeof a === 'string' && typeof b === 'string') {
        if(a === b)
            // This is always true, as no change can be expected between
            // two equal values. Could be a general rule, also a little
            // optimization, but I keep it with this special case to
            // not open up for potentially unkown errors.
            //
            // This special case so far is used for: `colors/${name}/mode`.
            // The color interpolation mode between two colors must be
            // the same, in so far, this behavior and the error below
            // `if a !== b` reflect that assumption, but it is not
            // explicitly worded for the color mode case, so if here are
            // any changes about that behavior, the color-mode use case must
            // be checked, if it still works as expected.
            return a;
        throw new Error(`VALUE ERROR don't know how to interpolte two different `
                      + `strings  a: "${a}" with b: "${b}" at t: ${t}.`);
    }
    if(typeof a !== 'number' || typeof b !== 'number')
        throw new Error(`VALUE ERROR don't know how to interpolate `
            + `"a" ${a} (${typeof a}) with "b" ${b} (${typeof b}).`);
...
export class HierarchicalScopeAnimanion {
    ...
    getLocalTs(globalT) {
        ...
        const ts = this.animanions.reduce(
            (ts, animanion) => {
                const rawT = animanion.getOwnPropertyAtLocalT('numericProperties/t', ts.at(-1), ts.at(-1))
                  , modT = rawT % 1
...
export class AnimationLiveProperties extends _BaseComponent {
    constructor(parentAPI, initAnimanion, isInheritingPropertyFn=null) {
        super(parentAPI);
    ...
    update(changedMap) {
        const hasParentProperties = this.parentAPI.wrapper.dependencyReverseMapping.has('@parentProperties');
        ...
        if(changedMap.has('keyMoments') || changedMap.has('isLoop') || changedMap.has('@parentProperties')) {
            ...
            if(hasParentProperties) {
                const parentProperties = changedMap.has('@parentProperties')
                    ...
                    // animanions, it is be smart to only update that when the
