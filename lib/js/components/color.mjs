...
export const ColorTypeModel = _AbstractGenericModel.createClass('ColorTypeModel')// => .value will be a concrete _BaseColorModel
  ...
  , AngleNumberModel = _AbstractNumberModel.createClass('AngleNumberModel', {min:0, max:360, defaultValue: 0, toFixedDigits: 5})
  , AngleNumberOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(AngleNumberModel)
  , HexadecimalNumberModel = _AbstractNumberModel.createClass('HexNumberModel', {min:0, max:255, defaultValue: 0, toFixedDigits: 0})
  , HexadecimalNumberOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(HexadecimalNumberModel)
  , OklchChromaNumberModel = _AbstractNumberModel.createClass('OklchChromaNumberModel', {min: 0, max:0.4, defaultValue: 0, toFixedDigits: 7})
  , OklchChromaNumberOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(OklchChromaNumberModel)
  , OklabABNumberModel = _AbstractNumberModel.createClass('OklabABNumberModel', {min: -0.4, max:0.4, defaultValue: 0, toFixedDigits: 7})
  , OklabABNumberOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(OklabABNumberModel)
  ...
  , OKLCHColorModel = _BaseColorModel.createClass(
      ...
      , ['L', PercentNumberOrEmptyModel]
        ...
        // Using percentages for this, as users understand that well.
      , ['C', OklchChromaNumberOrEmptyModel]
        // *H*ue
  ...
  , RGBColorModel = _BaseColorModel.createClass(
      ...
      , ['R', HexadecimalNumberOrEmptyModel]
      , ['G', HexadecimalNumberOrEmptyModel]
      , ['B', HexadecimalNumberOrEmptyModel]
      , ... alphaColorMixin
    )
...
export function* colorsGen(_outerAPI, hostInstance /*keyMoment or TypeSpecModel*/ /*, in motion-stage: momentT*/) {
 ...
 * Body text
 *      AA 4.5:1, AAA 7:1
 * Large-scale text (120-150% larger than body text)
 *      AA 3:1, AAA 4.5:1
 * Active user interface components and graphical objects such as icons and graphs
 *      AA 3:1 AAA Not defined
 */
const _CONTRAST_SETTINGS = {
        SMALL_TEXT: {AA: 4.5, AAA: 7}
      , LARGE_TEXT: {AA: 3, AAA: 4.5}
      , GRAPHICAL: {AA: 3, AAA: 3}// AAA is not defined so we fall back to AA for now
    }
    // for experimentation purposes workingColorSpaceSetup is an abstraction
    // the color spaces share a component for "Lightness", which is mostly 'l'
    // but can be different, e.f. Jzazbz uses 'j'.
    // We could also use an entirely different channel, ideally it has an
    // impact on the contrast ratio.
    // Perceptually uniform color spaces are best for this purpose oklch
    // and Jzazbz should be fine.
  , _WORKING_COLORSPACES = {
        'oklch': ['oklch', 0, 1]
      , 'lch': ['lch', 0, 100]
        //Jzazbz; worked out nicely, looks smooth! And is also perceptionally
        // uniform such as oklch
      , 'jch': ['jch', 0 , 0.222, 'j']
      , 'cubehelix':  ['cubehelix', 0, 1]
      // It would be interesting to play with CIECAM02 / CIECAM16
      // but culori does not implement it yet: https://github.com/Evercoder/culori/issues/41
    }
  ;

export function enhanceContrast(textColor, backgroundColor, contrastTypeSetting='SMALL_TEXT') {
    const lrgbConverter = culoriConverter('lrgb')
      // for experimentation purposes the color space we do the work in is abstracted
      , [lchColoSpace, lMin, lMax, L='l'] = _WORKING_COLORSPACES['oklch']
      , lchConverter = culoriConverter(lchColoSpace)
      , resultConverter = culoriConverter('oklch')
      , backgroundColorLRGB = lrgbConverter(backgroundColor)
      , textColorLCH = lchConverter(textColor)
      ;
    // Don't consider alpha at all, we got to calculate the actual,
    // mixed background color for this. That is beyond the scope
    // for this method.
    // FIXME: mmixing textColor if it has an alpha into backgroundColor
    // could be considered.
    delete backgroundColorLRGB.alpha;
    delete textColorLCH.alpha;

    const hasContrastTypeSetting = contrastTypeSetting in _CONTRAST_SETTINGS
      , {AA, AAA} = hasContrastTypeSetting
        ? _CONTRAST_SETTINGS[contrastTypeSetting]
        : _CONTRAST_SETTINGS.SMALL_TEXT
    let targetMinContrast = AAA; // use AA or AAA

    if(!hasContrastTypeSetting)
        console.warn(`Function enhanceContrast: UNKNOWN contrastTypeSetting "${contrastTypeSetting}". `
                 + `Known settings are: ${Object.keys(_CONTRAST_SETTINGS).join(', ')}.`);

    const currentRatio = wcagContrast(lrgbConverter(textColorLCH), backgroundColorLRGB);
    if (currentRatio >= targetMinContrast)
        return resultConverter(textColorLCH);
    const backgroundColorLCH = lchConverter(backgroundColorLRGB)
      , textIsLighterThanBg = textColorLCH[L] > backgroundColorLCH[L]
      , maxLighterCandidate = { ...textColorLCH, [L]: lMax }
      , maxDarkerCandidate = { ...textColorLCH, [L]: lMin }
      , maxLighterRatio = wcagContrast(lrgbConverter(maxLighterCandidate), backgroundColorLRGB)
      , maxDarkerRatio = wcagContrast(lrgbConverter(maxDarkerCandidate), backgroundColorLRGB)
      ;
    {
        // Analyze and maybe shortcut.
        if(maxDarkerRatio < targetMinContrast && maxLighterRatio < targetMinContrast) {
            // We won't reach AAA
            // FIXME: if we go with a fallback, one option could be
            // to tell the caller, who could then e.g. make the text-font
            // bolder or turn the grad-axis up.
            if(maxLighterRatio > maxDarkerRatio && maxLighterRatio >= AA) {
                // going lighter we reach AA and it's the better choice
                // than going darker.
                return resultConverter(maxLighterCandidate);
            }
            if(maxDarkerRatio >= AA) {
                // going darker we reach AA
                return resultConverter(maxDarkerCandidate);
            }
            // This is the last reserve.
            const blackWhiteCandidates = [lrgbConverter('#000'), lrgbConverter('#fff')]
              , fallBackBlackWhite = blackWhiteCandidates.sort((a, b) => wcagContrast(b, backgroundColorLRGB) - wcagContrast(a, backgroundColorLRGB))[0];
              ;
            return resultConverter(fallBackBlackWhite);
        }
    }
    // We know the result will be acceptable as we already tested the
    // extreme cases already ...
    const goingLighterHasSufficientPotential = maxLighterRatio >= targetMinContrast
        // goLighter === true: would stay in dark-mode configuration, i.e.
        // text is lighter than back ground
      , goLighter = textIsLighterThanBg && goingLighterHasSufficientPotential
      ;


     // Define binary search bounds
    let low = goLighter ? textColorLCH[L] : lMin
      , high = !goLighter ?  textColorLCH[L] : lMax
      , candidate = textColorLCH
      ;

    // Increase number of iterations to make l fit closer to targetMinContrast
    // e.g. 30 for a very good precision. looks like 5 is sufficient for
    // our UI use case, where speed is also an important consideration.
    for (let i=0; i<5; i++) {
        const middle = (low + high) / 2;
        candidate = { ...textColorLCH, [L]: middle }
        const ratio = wcagContrast(lrgbConverter(candidate), backgroundColorLRGB);
        if (ratio >= targetMinContrast) {
            // this basically means we wet too far...
            // if we’re making text darker, go darker (decrease L)
            // if we’re making text lighter, go lighter (increase L)
            [low, high] = goLighter
                ? [low, middle] // go darker
                : [middle, high] // go lighter
                ;
        }
        else {
    ...
    return resultConverter(candidate);
}
