...
/**
 * Logical↔storage conversion for typeSpec links and their resolution.
 *
 ...
 * - The *storage* form is the path in the typeSpec tree model: key
 *   names interleaved with the "children" map field of every level
 *   ("children/de_DE/children/quote/children/paragraph").
 *
 * Trap: one logical level is TWO storage segments ("children" + key),
 * so naive append/slice operations on storage paths (Path.sanitize
...
/**
 * Logical typeSpec levels → storage path segments.
 ...
 */
export function logicalLevelsToStorageParts(logicalLevels) {
    const storageParts = [];
    for (const level of logicalLevels)
        storageParts.push(CHILDREN_SEGMENT, level);
    return storageParts;
}

/**
 * Storage path segments → logical typeSpec levels, or null when the
 ...
 */
export function storagePartsToLogicalLevels(storageParts) {
    const logicalLevels = [];
    for (let i = 0; i < storageParts.length; i += 2) {
        if (
            storageParts[i] !== CHILDREN_SEGMENT ||
            storageParts[i + 1] === undefined
        )
            // Rejects malformed shape once, here and nowhere else.
            return null;
        logicalLevels.push(storageParts[i + 1]);
    }
 ...
 * storage candidate for the resolver's fallback walk. anchorPath is
 ...
 * origin, has malformed storage shape, or the link is out of bounds
...
export function resolveTypeSpecLinkFromAnchor(
    ...
    const belowOrigin = anchorPath.parts.slice(originPath.parts.length),
        baseLevels = storagePartsToLogicalLevels(belowOrigin);
    ...
    return originPath.append(...logicalLevelsToStorageParts(resolvedLevels));
}
