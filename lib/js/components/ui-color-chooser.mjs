...
class PlainNumberAndRangeColorInput extends PlainNumberAndRangeInput {
    static TEMPLATE = `<div class="number-and-range-color-input">
    ...
    constructor(...args) {
        super(...args);
        this._colorBarContainer = this.element.querySelector('.number-and-range-color-input_color-bar');
...
export class PlainColorComponentOrEmptyInput {
    constructor(domTool, baseClass='ui_color_component') {
        ...
        this._uiNumber = new PlainNumberAndRangeColorInput(this._domTool
                , (...args)=>this._changeHandler(this._componentKey, ...args)
                , '', '', {});

        this._uiToggle = new PlainToggleButton(this._domTool
          , (...args)=>this._toggleHandler(this._componentKey, ...args)
          , 'toggle', createIcon('toggle_off') , createIcon('toggle_on')
    ...
    _renderColorStrip (componentKey, culoriColor) {
        ...
        if(hueInterpolationMethod) {
            if(!HUE_INTERPOLATION_METHODS.has(hueInterpolationMethod))
                console.error(`KEY ERROR unknown hueInterpolationMethod "${hueInterpolationMethod}".`);
    ...
    update(componentKey, color, changeHandler, toggleHandler, culoriColor, defaultValue=0) {
        ...
        const currentClass = `${this._baseClass}-${this.getComponentClassFragment(componentKey)}`;
        for(const className of [...this.element.classList]) {
            if(className !== currentClass && className.startsWith(`${this._baseClass}-`))
        ...
        if(this._changeHandler !== changeHandler)
            this._changeHandler = changeHandler;
        if(this._toggleHandler !== toggleHandler)
...
class UIColorComponentRanges extends _BaseComponent {
     ...
     constructor (widgetBus, getDefaults=null, requireUpdateDefaults=()=>false) {
        super(widgetBus);
        this.__componentChangeHandler = this._componentChangeHandler.bind(this)
        this.__componentToggleHandler = this._componentToggleHandler.bind(this)
    ...
    async _componentToggleHandler(componentKey) {
    ...
    async _componentChangeHandler(componentKey, newValue/*, ...args*/) {
        await this._changeState(()=>{
            const color = this.getEntry('color')
              , component = color.get(componentKey)
              ;
    ...
    _updateComponent(i, componentKey, color, culoriColor, defaultValue) {
        ...
        component.update(
              ...
              , color
               // set change handlers
              , this.__componentChangeHandler
              , this.__componentToggleHandler
              , culoriColor
              , defaultValue
        );
    }
...
class UIColorPatch extends _BaseComponent{
     constructor (widgetBus, getDefaults, requireUpdateDefaults, elementTag='div', addNoColorLabel=true) {
        super(widgetBus);
        this._getDefaults = getDefaults;
        this._requireUpdateDefaults = requireUpdateDefaults;
        [this.element] = this.initTemplate(elementTag, addNoColorLabel);
    }

    initTemplate(elementTag, addNoColorLabel=true) {
        const injectNoColorLabel = addNoColorLabel
...
export class UIColorChooser extends _BaseContainerComponent {
    constructor(widgetBus, _zones, label, getDefaults, updateDefaultsDependencies, requireUpdateDefaults) {
        ...
        // declared dependencies ('colorTypeKey' is declared on the
        // widget below).
        const activationTest = (getEntry)=>{
                if(!this._displaySettings)
                    return false;
                const testResult = getEntry('colorTypeKey').value !== ForeignKey.NULL;
                return testResult;
            }
          , widgets = [
...
export class UIColorChooserTwoColorsWithSwap extends _BaseContainerComponent {
    constructor(widgetBus, _zones, labels, getDefaults, updateDefaultsDependencies, requireUpdateDefaults) {
        const baseClass = 'ui_color_chooser_two_colors'
          , localZoneElement = widgetBus.domTool.createElement(
                                    'div', {'class': baseClass})
          , labelZoneElement = widgetBus.domTool.createElement('label', {'class': `${baseClass}-label_toggle`})
          , zones = new Map([..._zones
        ...
        this._color1 = this._getColorFieldData('color1', labels[0]);
        this._color2 = this._getColorFieldData('color2', labels[1]);
        ...
        const widgets = [
          ...
          , [
            ...
            ...this._getColorPatchWidgets(this._color1),
            [
                {zone: 'label'},
                [],
                UIButton,
                createIcon('swap_horiz'),//label
                [['click', this._swapColorsHandler.bind(this, this._color1, this._color2)]],//eventHandlers
                {title: "swap colors", classPart: "swap"}//options
            ],
            ...this._getColorPatchWidgets(this._color2),
            ...this._getColorSettingsWidgets(this._color1),
            ...this._getColorSettingsWidgets(this._color2),
        ];
        this._initWidgets(widgets);
    }

    _getColorFieldData(colorKey, label) {
        const rootPath = Path.fromString(this.widgetBus.getExternalName(colorKey))
          , fieldName = rootPath.parts.at(-1)
          ;
        return {
            colorKey,
            rootPath,
            fieldName,
            // This handling will have to be changed at some point
            ppsRecord: ProcessedPropertiesSystemMap.createSimpleRecord(COLOR, fieldName),
            label: (label === undefined || label === null ? null : label)
        };
    }

    /**
     * The color of a field that has no explicit color set is inherited/
     * defaulted, swapping two of these (or one of these with an explicit
     * color) would appear as if nothing happened. Hence, the default color
     * is materialized into an explicit color before swapping.
     */
    _getExplicitColor(containerDraft, {ppsRecord, fieldName}) {
        const color = containerDraft.get(fieldName);
        if(color.get('instance').hasWrapped)
            return color;
        const defaultCuloriColor = this._getDefaults(ppsRecord, fieldName, null);
        if(defaultCuloriColor === null) {
            console.error(`_getExplicitColor: can't get a default value for fieldName ${fieldName}`);
            return color;
        }
        return culoriToColor(defaultCuloriColor, containerDraft.dependencies);
    }

    async _swapColorsHandler(color1Data, color2Data, evt) {
        evt.preventDefault();
        return await this._changeState(()=>{
            const container1Draft = this.getEntry(color1Data.rootPath.parent)
                , container2Draft =  this.getEntry(color2Data.rootPath.parent)
                , color1 = this._getExplicitColor(container1Draft, color1Data)
                , color2 = this._getExplicitColor(container2Draft, color2Data)
                ;
            container1Draft.set(color1Data.fieldName, color2);
            container2Draft.set(color2Data.fieldName, color1);
        });
    }

    _getColorPatchWidgets({rootPath, colorKey, ppsRecord, fieldName, label}) {
        const labelInject = label === null
              ? []
              : [[
                    {
                        zone: 'label'
                      , onInit: widget=>widget.node.addEventListener('click', this._toggleOptionsHandler.bind(this, colorKey))
                    }
                  , []
                  , StaticNode
                  , this._domTool.createElement('span', null, `${label} `)
            ...
            [
                {
                    zone: 'label',
                    rootPath
                  , onInit: widget=>widget.element.addEventListener('click', this._toggleOptionsHandler.bind(this, colorKey))
                }
              , [
                    ['instance', 'color']
              ...
              , UIColorPatch
              , this._getDefaults.bind(this, ppsRecord, fieldName), this._requireUpdateDefaults
              , 'span'
              , false
            ]
        ];
    ...
    _getColorSettingsWidgets(colorData) {
        const {rootPath, colorKey, ppsRecord, fieldName, label} = colorData
            // getEntry is injected by ComponentWrapper and only serves
            // declared dependencies ('colorTypeKey' is declared on the
            // widgets below).
          , activationTest = (getEntry)=>{
                if(this._displaySettings !== colorKey)
                    return false;
                const testResult = getEntry('colorTypeKey').value !== ForeignKey.NULL;
                return testResult;
            }
          , isInherited = (getEntry)=>{
                if(this._displaySettings !== colorKey)
                    return false;
                const testResult = getEntry('colorTypeKey').value === ForeignKey.NULL;
                return testResult;
        ...
        return [
            [
                    {
                        zone: 'local',
                        activationTest: ()=>this._displaySettings === colorKey,
                        // this will always only toggle off, as it is only
                        // visible when the colorKey is already on.
                        onInit: widget=>widget.node.addEventListener('click', this._toggleOptionsHandler.bind(this, colorKey))
                    }
                  , []
                  , StaticNode
                  , this._domTool.createElement('span', {class: "typeroof-ui-label ui_color_chooser-label_toggle"},
                        (label !== null ? `${label}: ` : '')
            ...
            [
                    {
                        zone: 'local',
                        activationTest: isInherited
                    }
                    // colorTypeKey: read in the activationTest (isInherited).
                  , [[rootPath.append('colorTypeKey').toString(), 'colorTypeKey']]
                  , StaticNode
                  , this._domTool.createElement('span', null, '(inherited)')
            ],
            [
                {
                    zone: 'local'
              ...
              , [true, '(no color)', ForeignKey.NULL] // [allowNull, allowNullLabel, nullModelValue]
                // changing this should convert the previous color mode to the
                // new color mode, using colori!
              , this._changeColorModeHandler.bind(this, colorData) // onChangeFn(newValue)
            ]
          , [
                  {
                      zone: 'local',
    ...
    _toggleSettingsVisibilityClasses() {
        const [addClass, removeClass] = this._displaySettings !== null
                    ? [this._visibleClass, this._hiddenClass]
                    : [this._hiddenClass, this._visibleClass]
                    ;
        this._container.classList.add(addClass);
    ...
    _toggleOptionsHandler(colorKey, evt) {
        // ideally:
        evt.preventDefault();
        const turnOff = this._displaySettings === colorKey;
        this._displaySettings = turnOff ? null : colorKey;
        this._toggleSettingsVisibilityClasses();
        // re-run ActivationTest ...
        const rootState = this.getEntry('/');
        this.initialUpdate(rootState);
    }

    _getDefaultColor({ppsRecord, fieldName}, color) {
        return _getDefaultColor(this._getDefaults.bind(this, ppsRecord, fieldName), color);
    }

    _changeColorModeHandler({rootPath, fieldName, ppsRecord}, newValue) {
        // new Value is either a string, e.g. "OKLCH" or Symbol('NULL') === ForeignKey.NULL
        if(newValue === ForeignKey.NULL)
            return;
        const containerDraft = this.getEntry(rootPath.parent)
          , colorWrapper = containerDraft.get(fieldName).get('instance')
          ;
        let sourceCuloriColor;
        if(colorWrapper.hasWrapped) {
            const color = colorWrapper.wrapped
              , [, defaultCuloriColor] = this._getDefaultColor({ppsRecord, fieldName}, color)
              , explicitCuloriColor = colorToCulori(color)
              ;
            // We might have some undefined components, using defaultCuloriColor
            // to populate these.
            ...
            sourceCuloriColor = this._getDefaults(ppsRecord, fieldName, null);
            if(sourceCuloriColor === null) {
                // Maybe it's even better when we call this._getDefaults()
                // without the defaultVal argument and produce the KeyError
                // as it would show where a default should be defined.
                console.error(`_changeColorModeHandler: can't get a default value for colorFieldName ${fieldName}`);
                return;
            }
        }
        const ColorType = availableColorTypes.get(newValue).value.get('typeClass').value
          , culoriTargetMode = COLOR_TYPE_TO_CULORI_MODE.get(ColorType)
          , targetCuloriColor = convertCuloriColorMode(sourceCuloriColor, culoriTargetMode)
          , color = culoriToColor(targetCuloriColor, containerDraft.dependencies)
          ;
        containerDraft.set(fieldName, color);
    }
}
