    ...
    constructor(rawData) {
        ...
        for(const [index, [tag]] of rawData.data.entries()) {
            if(tag.indexOf('..') !== -1) {
                ...
                for(const generatedTag of this.constructor._generateTags(startTag, endTag))
    ...
    get(tag, ...requiredKeys) {
        const tagIndex = this._indexes.get(tag)
          ...
          , keyEntries = requiredKeys.length
                ? requiredKeys.map(k=>[keys.indexOf(k), k]).filter(([i])=>i>=0)
                : keys.entries()
          ;
...
export class UILanguageInput extends _BaseComponent {
    ...
    constructor(widgetBus, data/* a language subtagRegistryMap*/, label, classes) {
        super(widgetBus);
        this._data = data;
    ...
    _getDataListId() {
        return  this.widgetBus.getWidgetById('dom-global-id-registry')
            .getSharedID(this._data, ()=>_createDataList(this._domTool, this._data));
    }
    ...
    _initTemplate(label, classes=[]) {
        ...
        input.setAttribute('size', Math.max(this._data.uiHints.minlength, Math.min(10, this._data.uiHints.maxlength)) + 1);
        input.addEventListener('input', (/*event*/)=>{
            if(!this._data.has(input.value) && input.value !== '')
                return;
...
class UILanguageTagInfo extends _BaseComponent {
    ...
    _makeInfo(tag) {
        const elements = [];
        for(const [key, value] of [['Tag', tag], ...Object.entries(this._data.get(tag))]) {
            elements.push(
                this._domTool.createElement('dt', {}, key)
              , ...(Array.isArray(value)? value : [value])
                    .map(v=>this._domTool.createElement('dd', {}, v))
            );
        }
        return elements;
    }
...
export class UILanguageTag extends _BaseContainerComponent {
    constructor(widgetBus, _zones) {
        const info = widgetBus.domTool.createElement('div', {class:'ui-language_tag-info'}, [
                widgetBus.domTool.createElement('h4', {}, 'Language Tag Info'),
                widgetBus.domTool.createElement('a', {
                        class:'external-link'
          ...
          , element = widgetBus.domTool.createElement('div', {class:'ui-language_tag'},
                [
        ...
        super(widgetBus, zones, [
          ...
          , ...Array.from(LanguageTagModel.fields).map(([subTag, {Model: SubTagModel}])=>{
                return [
                    {
                        zone: 'info'
                      , activationTest: ()=>!widgetBus.getEntry(widgetBus.rootPath.append(subTag)).isEmpty
                    }
                  , [
                        [subTag, 'value'] //  require('settings:internalPropertyName', 'value')
                    ]
