...
export class UIManualAxesLocations extends _BaseComponent {
    ...
    constructor (parentAPI, getDefaults=null) {
        super(parentAPI);
            ...
            const input = new PlainNumberAndRangeOrEmptyInput(
              ...
              , ()=>{
                    const value = this._axesLocations.has(axisTag)
                        ...
                        : (this._getDefaults
                                    ? this._getDefaults('axesLocations/', axisTag, defaultVal)
                                    : defaultVal
                          )
                        ;
    ...
    *axesEntries() {
    ...
    _getValueForAxis(axisTag) {
        return this._axesLocations.has(axisTag)
                    ...
                    : [false, (this._getDefaults !== null
                            ? this._getDefaults('axesLocations/', axisTag, this.axesGet(axisTag)['default'])
    ...
    _updateOPSZAxisInterface(changedMap) {
        const autoOPSZ = this._autoOPSZInput.checked
          ...
          , requireUpdate = changedMap.has('autoOPSZ') // always
                      ...
                      || changedMap.has('defaults') // always
                      || (autoOPSZ
    ...
    _update (changedMap) {
        ...
        if(changedMap.has('fontSize') || changedMap.has('defaults')){
            ...
            if(!fontSize.isEmpty)
                ...
                this._fontSize = this._getDefaults('', 'fontSize', this.axesGet('opsz')['default']);
        }
        ...
        if(changedMap.has('axesLocations') || changedMap.has('font') || changedMap.has('defaults')) {
        ...
        if(this.axesHas('opsz') && changedMap.has('defaults'))
