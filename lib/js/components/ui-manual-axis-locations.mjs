...
export class UIManualAxesLocations extends _BaseComponent {
    ...
    constructor (widgetBus, getDefaults=null, ppsRecord=null, requireUpdateDefaults=()=>false, label='Manual Axes Locations') {
        super(widgetBus);
        ...
        if(label!== null) {
            ...
            if(typeof label === 'string')
                labelElement =this._domTool.createElement('h3', {}, label);
    ...
    static CUSTOM_STYLE_VALUE = 'custom';

    _newStyleSelect({insertElement, changeHandler},  instances) {
        const container = this._domTool.createElementfromHTML('label', {class: 'ui_generic_select'}
                        , `<span class="ui_generic_select-label">Style</span><select></select>`)
          , widget = {
                container: container
    ...
    _newCheckbox({insertElement, changeHandler}, label) {
        const container = this._domTool.createElementfromHTML('label', {class:'ui_checkbox'}
...
export class UIManualAxesLocationsCollapsible extends CollapsibleContainer {
    constructor(
        widgetBus,
        _zones,
        getDefaults=null,
        ppsRecord=null,
        requireUpdateDefaults=()=>false,
        togglerLabel = "Variable Font Axes",
        isOpened = false,
        scroll = false,
    ) {
        const classNameParticle = "manual_axes_locations",
            flavor = "minimal",
            conditionalDependencies = [];

        if(widgetBus.wrapper.dependencyReverseMapping.has('properties@')) {
            conditionalDependencies.push([widgetBus.getExternalName("properties@"), "properties@"]);
            conditionalDependencies.push([widgetBus.getExternalName("rootFont"), "rootFont"])
        }
        else {
            conditionalDependencies.push([widgetBus.getExternalName("font"), "font"]);
            // Grid doesn't have this, its instance doesn't use inheritance
            // at all so far.
            if(widgetBus.wrapper.dependencyReverseMapping.has('animationProperties@'))
                conditionalDependencies.push([widgetBus.getExternalName("animationProperties@"), "animationProperties@"]);
        }
        ...
        const widgets = [
            [
                ...
                [
                    [widgetBus.getExternalName("axesLocations"), "axesLocations"],
                    [widgetBus.getExternalName("fontSize"), "fontSize"],
                    [widgetBus.getExternalName("autoOPSZ"), "autoOPSZ"],
                    ...conditionalDependencies
                ],
                ...
                null // label='Manual Axes Locations
            ],
        ];
        super(
            widgetBus,
