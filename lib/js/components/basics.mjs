...
export class _BaseDynamicMapContainerComponent extends _BaseDynamicCollectionContainerComponent {
    ...
    _provisionWidgets(compareResult) {
        const {/*LIST_NEW_ORDER, MOVED,*/ EQUALS, NEW, CHANGED, DELETED} = StateComparison.COMPARE_STATUSES
          ...
          ;
        if(!resultsMap.size) {
            // the sanity check below catches this. Maybe an item is not really
            // required in this case, I'm unsure. I think this is not yet in the
            // changeResult, as the parent is new, and thus all of the parents
            // children are not considered.
            // I'm uncertain about this, but it looks so far like a fix.
            resultsMap.set(NEW);
        }
        for(const [status/*, _payload*/] of resultsMap) {
            if(status === EQUALS)
        ...
        for(const [key, itemStatus] of itemChangeStatuses.entries()) {
            ...
            if(itemStatus === EQUALS) {
                ...
                // This would eventually do something like:
                //    const widgetWrapper = this._keyToWidget.get(key);
                //    widgetWrapper.updateRootPath(rootPath);
                //    deletedWidgets.delete(widgetWrapper);
                // CAUTION there's more to it, see the comments in
                // _provisionWidgets of _BaseDynamicCollectionContainerComponent.
                throw new Error('NOT IMPLEMEMENTED ERROR only HANDLE_CHANGED_AS_NEW is implemented');
            }
            else if(itemStatus === NEW || itemStatus === CHANGED) {
                // Treating CHANGED like DELETE + NEW is more resilient
                const widgetWrapper = this._createWrapper(rootPath);
                // The entry that is overridden here is already be in
        ...
        this._widgets.splice(0, Infinity, ...orderedWidgets);
...
export class UIBaseMapKey extends _BaseComponent {
    ...
    constructor(widgetBus, eventHandlers, options={/*rootClass:null, inputAttributes:{}*/}) {
        super(widgetBus);
...
export class _UIBaseMap extends _BaseDynamicCollectionContainerComponent {
    ...
    _setClassesHelper(requireClasses) {
        const baseClasses =  [this.constructor.ROOT_CLASS, this.BASE_CLASS
                                        , ...this.constructor.BASE_CLASSES];
        for(const baseClass of baseClasses) {
            for(const [element, ...classParts] of requireClasses)
                element.classList.add([baseClass, ...classParts].join('-'));
        }
