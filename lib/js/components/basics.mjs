...
class ComponentWrapper {
    ...
    constructor(parentAPI, rawDependencyMapping
        ...
        for(const [restKey, restValue] of Object.entries(restOptions)) {
            ...
            if(this._protocolHandlers.has(restKey)) {
                console.warn(`protocol handler registration detected for "${restKey}" with value:`, restValue);
        ...
        this._fullDependencyMappings = null;
    ...
    _absPathDependencies(dependencyMappings) {
        ...
        for(const entry_ of dependencyMappings) {
            ...
            if(protocolHandler.length && this._protocolHandlers.has(protocolHandler)) {
                console.warn(`got protocolHandler ${protocolHandler}`, external, internal);
    ...
    _updateRootPath(rootPath) {
        ...
        this._fullDependencyMappings = this._absPathDependencies(this._rawDependencyMapping);

        // this are only model dependencies, keeping the name so far for
        // historical reasons
        this.dependencyMapping = this._fullDependencyMappings[this.constructor.DEPENDECIES_DEFAULT];
...
export class _BaseDynamicCollectionContainerComponent extends _CommonContainerComponent {
    ...
    _provisionWidgets(compareResult) {
        const {LIST_NEW_ORDER, EQUALS, MOVED, NEW, CHANGED} = StateComparison.COMPARE_STATUSES
          ...
          , itemsPath = Path.fromString(this.parentAPI.getExternalName('collection'))
          ;

        for(const [status, _payload, path] of compareResult) {
            let data;
            const pathEquals = path.equals(itemsPath);
            ...
            else if(status === NEW /* and not empty */ ) {
