    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        ...
        for(const widgetWrapper of this.activeWidgets()) {
            ...
            if(widget[UPDATE_STRATEGY] === UPDATE_STRATEGY_SIMPLE) {
                ...
                console.log(`${this}._update widgetWrapper:`, widgetWrapper, '\nthis', this);
                throw new Error(`UPDATE_STRATEGY unkown for ${widget}: ${widget?.[UPDATE_STRATEGY]?.toString()} in ${this}`);
            }
        }
    }
}
    ...
    _provisionWidgets(compareResult) {
        ...
        for(const [status, _payload] of resultsMap) {
            ...
            if(status === NEW /* and not empty */ ) {
                ...
                data = _payload;
            else if(items instanceof _AbstractOrderedMapModel && status === CHANGED ) {
                // _AbstractOrderedMapModel has no equivalent of LIST_NEW_ORDER
                data = new Map();
                // Extracting the compareMap entries for all direct children.
                for(const key of items.keys()) {
                    const childPathAsString = itemsPath.append(key).toString();
                    if(compareMap.has(childPathAsString)) {
                        for(const childStatus of compareMap.get(childPathAsString)) {
                            // for _AbstractOrderedMapModel the first status should be sufficient/all there is
                            data.set(key, childStatus);
                            break;
                        }
                    }
                }
