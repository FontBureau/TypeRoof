...
class ComponentWrapper {
    ...
    insertElement(element) {
        this.constructor.insertElement(this.insertedElements, this._placeholder || this.host, element);
    }
...
export class _BaseContainerComponent extends _BaseComponent {
    ...
    constructor(parentAPI, zones, widgets) {
        ...
        // this._childrenParentAPI = {
        //       ...parentAPI
        //     , getWidgetById: this.getWidgetById.bind(this)
        // };
        this._widgetsPlaceholder = new Map();
    ...
    destroy() {
        ...
        for(const placeholder of  this._widgetsPlaceholder.values())
            DOMTool.removeNode(placeholder);
        this._widgetsPlaceholder.clear();
    }
    ...
    _provisionWidgets(/* compareResult */) {
        ...
        for(const widgetWrapper of this._widgets) {
            ...
            if(widgetWrapper.host // no host: not managed in here
                    ...
                    && !this._widgetsPlaceholder.has(widgetWrapper) // this is not the initial run
                    ) {
                const placeholder = this.parentAPI.domTool.createComment('widget placeholder');
                this._widgetsPlaceholder.set(widgetWrapper, placeholder);
                widgetWrapper.host.append(placeholder);
                widgetWrapper.setPlaceholder(placeholder);
            }
