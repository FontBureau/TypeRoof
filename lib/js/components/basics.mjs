...
class ComponentWrapper {
    ...
    getChangedMapFromCompareResult(isInitialUpdate, compareResult, toLocal=true) {
        let modelChangedMap;
        try {
            modelChangedMap = compareResult.getChangedMap(this.dependencyMapping, toLocal)
        }
        catch(error) {
            // NOTE: the position where this error occurs, the widget that
            // cayches it, is not the origin of the error. It would be nice
            // to capture that information. So far error contains the
            // model path that could not be found.
            throw new Error(`${this} Can't get changedMap, see cause.`, { cause: error });
        }
