...
function createProseMirrorSchemaModel() {
    ...
    setAttributeSpec(paragraphDraft.get("attrs"), "htmlAttrs", "string", "");
    ...
    setAttributeSpec(linkDraft.get("attrs"), "htmlAttrs", "string", "");
...
describe("getPathOfContentIndexes", () => {
    // The document-node-path segments of a PM resolved path: the sibling
    // indexes getPathOfTypes walks past. Behavior asserted on the
    // returned segments, not the walk.
    const schema = createProseMirrorSchemaFromMetaModel(
        schemaSpec,
        createProseMirrorSchemaModel(),
    );
    ...
    // Resolve to the first child of `node` (a block position), i.e. the
    // path a NodeView's getPos() resolves for that child.
    const firstChildPos = (doc, from) => {
        let seen = null;
        doc.nodesBetween(from, doc.content.size, (node, pos) => {
            if (seen === null && node.isBlock && pos > from) seen = pos;
            return seen === null;
        });
        return seen;
    };

    it("collects the sibling index of each ancestor, one per triple", () => {
        // Behavior contract: one index per resolved ancestor, sourced
        // from path[i+1]; a top-level block resolves to a single (doc)
        // ancestor at index 0.
        const doc = parse("<p>a</p><p>b</p>");
        const pos = firstChildPos(doc, -1); // first top-level block
        const resolved = doc.resolve(pos);
        const indexes = getPathOfContentIndexes(resolved.path);
        expect(indexes).toEqual([0]);
        // One per ancestor, and from the i+1 (index) slot of each triple.
        expect(indexes.length).toBe(resolved.path.length / 3);
        expect(indexes).toEqual(
            Array.from(
                { length: resolved.path.length / 3 },
                (_, k) => resolved.path[k * 3 + 1],
            ),
        );
    });

    it("yields the sibling index of each ancestor for a nested block", () => {
        // doc > [p, unknown_block > [p, p]] : the inner second <p> has
        // sibling indexes [1 (unknown_block in doc), 1 (p in unknown_block)].
        const doc = parse(
            "<p>a</p>" +
                '<div data-unknown-block-type="x">' +
                "<p>b</p><p>c</p>" +
                "</div>",
        );
        // find the deepest block ("c")
        let target = null;
        doc.descendants((node, pos) => {
            if (node.type.name === "paragraph" && node.textContent === "c") {
                target = pos;
                return false;
            }
        });
        ...
        expect(getPathOfContentIndexes(resolved.path)).toEqual([1, 1]);
    });

    it("aligns positionally with getPathOfTypes on the same resolved path", () => {
        const doc = parse(
            "<p>a</p>" +
                '<div data-unknown-block-type="x">' +
                "<p>b</p><p>c</p>" +
        ...
        const resolved = doc.resolve(target);
        const types = getPathOfTypes(resolved.path, resolved.parent.type.name);
        const indexes = getPathOfContentIndexes(resolved.path);
        // Same walk, different element of each triple: one type-name per
        // ancestor, one sibling index per ancestor; the leaf type is the
        // extra currentType entry in getPathOfTypes.
        expect(indexes.length).toBe(types.length - 1);
    });
});
...
describe("generic-style htmlAttrs", () => {
...
describe("editable attr replay schema generation", () => {
    it("node with declared htmlAttrs: getAttrs collects the foreign bag, toDOM replays it", () => {
        const schema = createProseMirrorSchemaFromMetaModel(
                schemaSpec,
                createProseMirrorSchemaModel(),
            ),
        ...
        dom.setAttribute("id", "mw-1");
        dom.setAttribute("class", "intro");
        dom.setAttribute("style", "color:red");
        dom.setAttribute("htmlAttrs", "junk");
        const attrs = spec.parseDOM[0].getAttrs(dom);
        expect(JSON.parse(attrs.htmlAttrs)).toEqual([
            ["id", "mw-1"],
            ["class", "intro"],
        ...
        const node = schema.nodes.paragraph.create({
            ...
            [tag, outAttrs] = spec.toDOM(node);
        expect(tag).toBe("p");
        expect("htmlAttrs" in outAttrs).toBe(false);
        expect(outAttrs.id).toBe("mw-1");
        expect(outAttrs.class).toBe("intro");
        expect("data-node-type" in outAttrs).toBe(false);
    });

    it("mark with declared htmlAttrs: declared attrs stay typed, bag collects the tail", () => {
        const schema = createProseMirrorSchemaFromMetaModel(
                schemaSpec,
                createProseMirrorSchemaModel(),
            ),
            spec = schema.marks.link.spec,
            dom = document.createElement("a");
        dom.setAttribute("href", "/wiki/X");
        dom.setAttribute("rel", "mw:WikiLink");
        ...
        const mark = schema.marks.link.create({
                href: "/wiki/X",
                htmlAttrs: JSON.stringify([["rel", "mw:WikiLink"]]),
            }),
            [tag, outAttrs] = spec.toDOM(mark);
        expect(tag).toBe("a");
        expect(outAttrs.href).toBe("/wiki/X");
        expect("htmlAttrs" in outAttrs).toBe(false);
        expect(outAttrs.rel).toBe("mw:WikiLink");
    });
});

describe("view-side bag replay", () => {
    it("generic-style MarkView replays the bag at construction and on update", () => {
        const schema = createProseMirrorSchemaFromMetaModel(
                schemaSpec,
                createProseMirrorSchemaModel(),
            ),
            boldMark = schema.marks["generic-style"].create({
                "data-style-name": "bold",
                htmlAttrs: JSON.stringify([
                    ["id", "mw-1"],
                    ["class", "x"],
            ...
            markEl = element.querySelector('[data-mark-type="generic-style"]');
        expect(markEl.getAttribute("id")).toBe("mw-1");
        expect(markEl.getAttribute("class")).toBe("x");

        let tr = view.state.tr.removeMark(
            0,
            view.state.doc.content.size,
            schema.marks["generic-style"],
        );
        tr = tr.addMark(
            1,
            7,
            schema.marks["generic-style"].create({
                "data-style-name": "bold",
                htmlAttrs: JSON.stringify([
                    ["id", "mw-2"],
                    ["title", "t"],
        ...
        view.dispatch(tr);
        const markEl2 = element.querySelector(
            '[data-mark-type="generic-style"]',
        );
        // style name unchanged: view reused, bag re-applied in place
        expect(markEl2).toBe(markEl);
        expect(markEl2.getAttribute("id")).toBe("mw-2");
        expect(markEl2.getAttribute("title")).toBe("t");
        view.destroy();
    });

    it("editable NodeView replays the bag on the outer element", () => {
        const schema = createProseMirrorSchemaFromMetaModel(
                schemaSpec,
                createProseMirrorSchemaModel(),
            ),
            ...
            nodeWidgetBus = {
                domTool: stubWidgetBus.domTool,
                getWidgetById: () => fakeSubscriptions,
                getLinked: () => mmSchema,
            },
            makeDoc = (bag) =>
        ...
        expect(p.getAttribute("id")).toBe("mw-1");
        ...
        expect(p2).toBe(p);
        expect(p2.getAttribute("id")).toBe("mw-2");
        expect(p2.getAttribute("lang")).toBe("en");
        view.destroy();
    });
});
