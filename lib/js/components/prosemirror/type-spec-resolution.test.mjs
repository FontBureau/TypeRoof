...
describe("typeSpec resolution: relative-by-default discriminator", () => {
    ...
    it("an absolute link in storage form ('/children/h3') resolves", () => {
        const harness = createResolutionHarness(
    ...
    it("a bare 'children/x' link now anchors 'children' as a literal level name", () => {
        // Before the discriminator change, bare 'children/…' was a
        // storage-form traversal from the origin. Now it is parent-
        // anchored like any bare name; 'children' resolves only when
        // a real typeSpec is named 'children'. Here it isn't →
        // broken → fallback to root.
        const harness = createResolutionHarness(
