...
// Thinking: columnCount may be triggering column-layout, if set, and thus,
// it must be not inheriting. Otherwise we should use a boolean flag.
//
// column mode also sets:
//    display: block (display: table; doesn't do columns, and we have that in the viewer)
//    child: width becomes equal to lineLength OR "auto" but we don't like auto! can't use it for leading
//
//    The inherited width should become smarter anyways! We'll now add
//    inline margins. Those eat away from the child-width. So what's inherited
//    is always without those margins!
//    One questions, currently unanswered, what does the EN actually mean?
//    I think it has to be, as in html, defined in the context that owns
//    the EM (interesting implication: when we set a new font-size, do we
//    transfer ownwership of the EN-based width? it would be reasonable.
//    Also interesting how calculatenions are based on the baseFontSize
//
//export const ColumnCountModel
//    , ColumnGapModel
//    ;
//
// AutoVerticalLayoutModel
// vertical layout is Margin Left column-width
...
export const LeadingNumberModel = _AbstractNumberModel.createClass('LeadingNumberModel', {defaultValue: 1.3/*, toFixedDigits: 5*/})
  ...
  , {
   ...
   , BlockMarginUnitModel = _AbstractEnumModel.createClass(
        'BlockMarginUnitModel'
      , [
          ...ABSOLUTE_UNITS,
          'lineHeight', 'em', 'baseEm', 'lineHeightAfter', 'emAfter'
        ]
      , 'lineHeight')
   , BlockMarginUnitOrEmptyModel = _AbstractSimpleOrEmptyModel.createClass(BlockMarginUnitModel)
   , BlockMarginValueModel = _AbstractStructModel.createClass(
        'BlockMarginValueModel'
      ...
      , ['unit', BlockMarginUnitOrEmptyModel]
        // Both set or both empty: a value without a unit is meaningless.
      , unitValuePairCoherenceFn
    )
  , ManualBlockMarginsModel = _AbstractStructModel.createClass(
        'ManualBlockMarginsModel'
      , ['start', BlockMarginValueModel]
      , ['end', BlockMarginValueModel]
    )
    ;
...
const typographyKeyMomentPureModelMixin = [
  ...
  , typographyParagraphMixin = [
    ...
    */
        ...
        // if the overall approach is fully manual, lineLength will be
        // configured as part of that structure. Similar also line-heigth.
        // will be within a sub-struct.
        // ['verticalLayout', VerticalLayoutRunionModel]
        // We'll keep lineLength, but we'll have to set it from a runion
        // that will be extracting {lineLength} = verticalLayout
        // ['inlineMargins', ManualMarginsModel] similar, should be extracted
        // from verticalLayout
        // then also, of course lineLength, coulmnCount, columnGap.
        // One important distinction we need to work on: width vs. lineLength
        // OR, also "lineLength". I would argue, depending on the nesting
        // context lineLength === lineLength and width === inlineMargin.start + lineLength + inlineMargin.start
        // This is important, as we need to feed linLength into the LeadingRunion (AutoLinearLeadingSyntheticValue)
        // The LeadingRunion is very smart and compensates the ambiguity
        // caused by lineLength being EN in baseFontSize:
        //        actuallineLength = lineLengthEn / relativeFontSize;
        //
        // I think one fundamental new question is: how do we decide who
        // gets to write a (general, shallow?) property. And the answer
        // might be: if it's not set, we can put it in and shadow the
        // inherited value, not-set is already the mechanism we need.
        // A generator should be able to distinguish that: it's set locally
        //    ? don't yield extracting code
        //    : yield extracting code
        // We could have an extracting generator, maybe just inline
        // into the actual value generating generator or even next to
        // it. the generator reads the bucket and yields just the value
        // that is required.
        //
        // No, how to re-route: line-width/column-width -> child.width
        // we have a re-routing of t in the animanion ...
        ['leading', LeadingAlgorithmModel]
      , ['blockMargins', ManualBlockMarginsModel] // direction block i.e. start === top end === bottom
      , ['textAlign', TextAlignmentOrEmptyModel]
      , ['horizontalLayout', HorizontalLayoutAlgorithmModel]
      /* pageGutter: binding-side margin for facing pages in, not yet modeled */
...
export const StylePatchTypeModel = _AbstractGenericModel.createClass('StylePatchTypeModel')// => .value will be a concrete _BaseStyleModel
  ...
  , StylePatchKeysModel = _AbstractListModel.createClass('StylePatchKeysModel', StylePatchKeyModel)
    // These link the StylePatches from the StylePatchesMapModel into the
    // TypeSpecs. Two maps, two link targets:
    //      * intentStyleLinks: keys are intent names, matched against the
    //        data-style-name of generic-style marks.
    //      * markStyleLinks: keys are schema-defined ProseMirror mark
    //        type names.
    //      * Values are the edge structs (see below).
    ...
    // NOTE: mode is a plain enum and always serialized.
  ...
  , StylePatchLinkModeEnumModel = _AbstractEnumModel.createClass(
    ...
    // Shared fields of both style-link edge structs (mixin convention):
  ...
  , stylePatchLinkModelMixin = [
    ...
    // intentStyleLinks edge: links an intent (generic-style mark,
    // matched by data-style-name === edge key) to a StylePatch.
    //      * tag: names an HTML element applied to the intent at render
    //        time; OrEmpty and hence omitted when empty, the intent
    //        renders as the default span. Schema-defined marks get their
    //        tag from their MarkSpec, not from here.
  , IntentStylePatchLinkModel = _AbstractStructModel.createClass(
        'IntentStylePatchLinkModel'
      , ...stylePatchLinkModelMixin
      , ['tag', StringOrEmptyModel]
    )
    // markStyleLinks edge: links a schema-defined ProseMirror mark type
    // (matched by mark type name === edge key) to a StylePatch.
  , MarkStylePatchLinkModel = _AbstractStructModel.createClass(
        'MarkStylePatchLinkModel'
      , ...stylePatchLinkModelMixin
    )
  , IntentStyleLinksMapModel = _AbstractOrderedMapModel.createClass('IntentStyleLinksMapModel'
            , IntentStylePatchLinkModel
            , { validateKeyFn: validateStyleName }
    )
  , MarkStyleLinksMapModel = _AbstractOrderedMapModel.createClass('MarkStyleLinksMapModel'
            , MarkStylePatchLinkModel
            , { validateKeyFn: validateStyleName }
    )
  , CompositeStylePatchModel = _BaseStylePatchModel.createClass(
        ...
        // FIXME: could we define a list of keys into
           // the dependency "stylePatchesSource" of the parent TypeStageModel // a StylePatchesMapModel
  ...
  , TypeSpecModel = _AbstractStructModel.createClass(
      ...
      , ...manualAxesLocationsModelMixin
      , ...typographyFontMixin
        // Resolution/provisioning flags (shim semantics), not part of
        // the inheriting processed-properties stream:
        // excludeFromFallback: the nodeSpec→typeSpec fallback walk passes
        //   through this typeSpec; an explicit link still resolves to it.
        // noStyler: a resolved node never provisions its own styler (the
        //   node renders inherit-only); the resolved spec still speaks
        //   for marks/margins and the anchor of relative links.
      , ["excludeFromFallback", BooleanModel]
      , ["noStyler", BooleanModel]
      ...
      , ['stylePatchesSource', new InternalizedDependency('stylePatchesSource', StylePatchesMapModel)]
        ...
        // and the values are keys in TypeStageModel/stylePatchesSource
      ...
      , ['intentStyleLinks', IntentStyleLinksMapModel]
      , ['markStyleLinks', MarkStyleLinksMapModel]
    )
  ;
...
// Resolve which intentStyleLinks edge key applies to an intent
// (generic-style mark): the edge keyed by data-style-name, or null when
// there is no such edge (the unknown-style fallback applies).
export function getStylePatchLinkForIntent(intentStyleLinks, styleName) {
    return intentStyleLinks.has(styleName) ? styleName : null;
}

// Resolve which markStyleLinks edge key applies to a schema-defined
// ProseMirror mark: the edge keyed by the mark type name, or null when
// there is no such edge (the unknown-style fallback applies).
export function getStylePatchLinkForMark(markStyleLinks, mark) {
    return markStyleLinks.has(mark.type.name) ? mark.type.name : null;
}

// Resolve the HTML tag an intent (generic-style) mark should be rendered
// as: the tag of the intentStyleLinks edge keyed by the style name;
// otherwise null, the mark renders as the default span. Schema-defined
// marks get their tag from their MarkSpec, not from here.
export function getStylePatchTagForIntent(intentStyleLinks, styleName) {
    if (!intentStyleLinks.has(styleName)) return null;
    const tag = intentStyleLinks.get(styleName).get("tag");
...
// Used to be called getLineWidthLeadingPPSMap (for LineWidthLeadingModel)
// but when I was copying it to create getGutterPointPPSMap
// (for GutterPointModel) I decided to join the implementations The
// fullKey is constructed from with `parentPPSRecord.propertyRoot` as the
// root instead of prefix, so the property is embedded in a "deep" name-space.
export function getParentRootPPSMap(parentPPSRecord, FieldType) {
    const entries = [];
...
export const deserializeManualBlockMarginsModel = _deserializeModel.bind(null, ManualBlockMarginsModel)
  , deserializeManualInlineMarginsModel = _deserializeModel.bind(null, ManualInlineMarginsModel)
  , deserializeInlineLengthValueModel = _deserializeModel.bind(null, InlineLengthValueModel)
  ;
