...
import { logicalLevelSegmentsToModelTreeSegments } from './paths.mjs';
...
/**
 * Truncate to `digits` decimals by string manipulation — toward zero,
 * sufficient for geometry values (widths, always ≥ 0). Rounding
 * (toFixed) could round UP, breaking the "horizontal layout must fit
 * into the available width" contract; floor-by-multiplication has FP
 * edge cases. The horizontal-layout pt conversions (enToPt/enToPtNumeric)
 * truncate to 2 decimals to give the multi-column box wiggle-room.
 */
export function truncateDecimals(value, digits = 2) {
    const [integer, fraction = ""] = String(value).split(".");
    return fraction ? `${integer}.${fraction.slice(0, digits)}` : integer;
}

// EN → pt conversions, shared between the style channel
// (properties-generators.mjs) and the node channel
// (node-properties-generators.mjs). enToPtNumeric returns a NUMBER
// (arithmetic consumers); enToPt returns the CSS-ready "pt" string.
const _enToPtFactor = (value, fontSize) => (value / 2) * fontSize;
export function enToPtNumeric(value, fontSize) {
    return Number(truncateDecimals(_enToPtFactor(value, fontSize), 2));
}
export function enToPt(value, fontSize) {
    return `${truncateDecimals(_enToPtFactor(value, fontSize), 2)}pt`;
}
...
// Adds the (UNLINK) tombstone option to the special options group of the
// style-link selects. NULL-STYLE (stylePatch "") keeps the style available
// without styling; (UNLINK) removes the edge key from the effective,
// inherited style-links (mode 'unlinked').
function _injectUnlinkOption(select) {
    const nullOption = Array.from(select._select.options).find(
            (option) => option.value === select.constructor.NULL_SELECT_VALUE,
        ),
        unlinkOption = select._createOption(
            select.constructor.UNLINK_SELECT_VALUE,
            UNLINKED_STYLE_LINK_LABEL,
        );
    if (nullOption !== undefined) nullOption.after(unlinkOption);
    else select._select.prepend(unlinkOption);
}

class StyleLinksMapKeyCreateSelect extends LinksMapKeyCreateSelect {
    static UNLINK_SELECT_VALUE = "[_UNLINK_]";
    _populateSelect(options) {
        super._populateSelect(options);
        _injectUnlinkOption(this);
    }
}

class StyleLinksMapKeyChangeSelect extends LinksMapKeyChangeSelect {
    static UNLINK_SELECT_VALUE = StyleLinksMapKeyCreateSelect.UNLINK_SELECT_VALUE;
    _populateSelect(options) {
        super._populateSelect(options);
        _injectUnlinkOption(this);
    }
    update(changed) {
        ...
        if (changed.has("linkMode")) this.checkValue();
    }
    _updateSelectValue(modelValue, availableOptions) {
        if (this.getEntry("linkMode").value === "unlinked") {
            this._select.value = this.constructor.UNLINK_SELECT_VALUE;
            this._onChangeFn(this._select.value);
            return;
        }
        super._updateSelectValue(modelValue, availableOptions);
    }
    async _changeSelectedValueHandler(/*event*/) {
        if (this._select.value === this.constructor.UNLINK_SELECT_VALUE) {
            // the coherence function clears stylePatch when mode is 'unlinked'
            await this._changeState(() =>
                this.getEntry("linkMode").set("unlinked"),
            );
            this._onChangeFn(this._select.value);
            return;
        }
        // any other selection is a link (NULL-STYLE "" included)
        if (this.getEntry("linkMode").value !== "link")
            await this._changeState(() => this.getEntry("linkMode").set("link"));
        return super._changeSelectedValueHandler();
    }
}

const EMPTY_STYLE_LINK_LABEL = '(NULL-STYLE)'
  , UNLINKED_STYLE_LINK_LABEL = '(UNLINK)'
  , EMPTY_TYPESPEC_LINK_LABEL = '(NULL-TYPESPEC)'
  ;
class UIStylePatchLinksValueLabel extends DynamicTag {
    update(changedMap) {
        if(changedMap.has('data') || changedMap.has('sourceMap') || changedMap.has('linkMode')) {
            const mode = (changedMap.has('linkMode')
                    ? changedMap.get('linkMode')
                    : this.getEntry('linkMode')).value;
            if(mode === 'unlinked') {
                this.element.textContent = UNLINKED_STYLE_LINK_LABEL;
                return;
            }
...
// immediately.
class _UIStyleLinksMapValue extends _BaseContainerComponent {
    constructor(widgetBus, _zones, optionGetLabel=null, optionsGen=null) {
        ...
        this._initWidgets(this._getWidgets(optionGetLabel, optionsGen));
    }

    _getWidgets(optionGetLabel=null, optionsGen=null) {
        const sourceKeyPath = this.widgetBus.getExternalName('sourceKey')
           , sourceMapPath = this.widgetBus.getExternalName('sourceMap')
           , modePath = Path.fromString(sourceKeyPath).append('..', 'mode').toString()
           ;
        return [
            [
                {zone: 'local'}
...
// An existence check for select options: accepts logical absolute
// paths (e.g. "/quote/bq"), converts to the model tree form.
function typeSpecHas(typeSpec, path) {
    const instance = Path.fromString(path);
    // Only unambiguous absolute paths are options we accept.
    // '/' (or empty root link) anchors at the root (origin). Anything
    // else is custom. The empty link resolves to the origin.
    const isAbsolute = path === '' || instance.isExplicitlyAbsolute;
    if (!isAbsolute) return false;
    const modelTreePath =
        path === ''
            ? ''
            : `/${logicalLevelSegmentsToModelTreeSegments(instance.parts).join('/')}`;
    return !!getEntry(typeSpec, modelTreePath, false);
}
...
class UIStyleNodeToTypeSpecValueLabel extends DynamicTag {
    update(changedMap) {
        if(changedMap.has('data') || changedMap.has('sourceMap')) {
            const edge = (changedMap.has('data')
                    ? changedMap.get('data')
                    : this.getEntry('data'))

              , path = edge.get('link').value
              , rootTypeSpec = (changedMap.has('sourceMap')
                    ? changedMap.get('sourceMap')
                    : this.getEntry('sourceMap'))
              , edgeLabel = edge.get('label').value
              // A stored link is logically absolute ("/…") when it is
              // an option key; relative forms are unambiguously custom
              // at this level. Absolute links resolve against the root;
              // relative ones render as (relative), because this label
              // has no parent-node context. The model tree conversion is
              // the same as in typeSpecHas.
              , instance = Path.fromString(path)
                // The empty link is the absolute root ("") — treat as
                // absolute here, like typeSpecHas.
              , isAbsolutePath = instance.isExplicitlyAbsolute || path === ''
              , modelTreePath = isAbsolutePath
                    ? (path === ''
                        ? ''
                        : `/${logicalLevelSegmentsToModelTreeSegments(instance.parts).join('/')}`)
                    : null
              , typeSpec = isAbsolutePath
                        ? getEntry(rootTypeSpec, modelTreePath, null)
                        : null
              , typeSpecLabel = typeSpec === null
                        ? (isAbsolutePath
            ...
            this.element.textContent = _typeSpecFullLabel(path, typeSpecLabel, edgeLabel);
        }
    }
}
    ...
    constructor(widgetBus, _zones, optionGetLabel=null, optionsGen=null) {
        ...
        const widgets = [
            [
                {zone: 'local'}
              , [
                    [Path.fromString(sourceKeyPath).append('..').toString(), 'data']
                  , [sourceMapPath, 'sourceMap']
                ]
          ...
          , [
              ...
              , [
                    [Path.fromString(sourceKeyPath).toString(), 'value']
                ]
              , UILinksMapValueInput
              , [ // event handlers
                    ['input', this._valueInputChangeHandler.bind(this)]
          ...
          , [
                { zone: 'local' }
              , [
                    [Path.fromString(sourceKeyPath).append('..', 'label').toString(), 'value']
                ]
              , UILineOfTextInput
              , 'Label'
            ]
...
export class _UIStyleLinksMap extends _UIBaseMap {
    static CREATE_SELECT_OPTIONS_SOURCE = 'sourceMap';
    static CREATE_SELECT_LABEL = 'Link a Style';
    static CREATE_SELECT_OPTION_GET_LABEL = null;
    static VALUE_CONSTRUCTOR = null; // null: _UIStyleLinksMapValue
    ...
    // static UIKeyCreate => implement in sub-class
    ...
    constructor(...args) {
        ...
        if(!this.constructor.VALUE_CONSTRUCTOR)
            throw new Error(`NOT IMPLEMENTED ERRROR ${this} must define a VALUE_CONSTRUCTOR`)
    }
    ...
    get _initialWidgets() {
        const selectState = {lastSelectedValue: null}
          , select = [
              ...
              , [
                    [this.widgetBus.getExternalName(this.constructor.CREATE_SELECT_OPTIONS_SOURCE) , 'options']
                  // , ['activeLayoutKey', 'value'] => it doesn't track a value
                ]
              , StyleLinksMapKeyCreateSelect
              , 'ui_links_map-create_select'// baseClass
              , this.constructor.CREATE_SELECT_LABEL// labelContent
              , this.constructor.CREATE_SELECT_OPTION_GET_LABEL// optionGetLabel=null || optionGetLabel(key, value)=>label
              , [true, EMPTY_STYLE_LINK_LABEL, '']// allowNull: [allowNull, allowNullLabel, nullModelValue]
                // As a suggestion, it makes sense to re-use the
                // targetKey name, e.g. "italic" => "italic".
                // The following code tries to differentiate between a
              ...
              , (targetKey) => { // onChangeFn
                    ...
                    if(targetKey !== null && targetKey !== StyleLinksMapKeyCreateSelect.UNLINK_SELECT_VALUE) {
                        if(allowAutoValue) {
    ...
    _createWrapperValue(keyId, key) {
        const childWidgetBus = this._childrenWidgetBus
          ...
          , Constructor = this.constructor.VALUE_CONSTRUCTOR
          , args = [
    ...
    _createKeyValue(childrenOrderedMap) {
        const linkSelect = this.getWidgetById('key-create-link-select')
          ...
          ;
        if(linkSelect.value === StyleLinksMapKeyCreateSelect.UNLINK_SELECT_VALUE)
            keyItem.get('mode').value = 'unlinked';
        else
            keyItem.get('stylePatch').value = linkSelect.value;
        return keyItem;
    }
}


class UIStylePatchLinksValue extends _UIStyleLinksMapValue {
    constructor(widgetBus, zones) {
        super(widgetBus, zones);
    }

    _getWidgets(optionGetLabel=null, optionsGen=null) {
        const widgets = super._getWidgets(optionGetLabel, optionsGen),
            sourceKeyPath = this.widgetBus.getExternalName('sourceKey');

        // The intent edge's optional tag binding (sibling of
        // stylePatch): an HTML element applied to the intent at
        // render time. markStyleLinks edges have no tag binding.
        widgets.push([
            { zone: 'local' }
          , [
                [Path.fromString(sourceKeyPath).append('..', 'tag').toString(), 'value']
            ]
          , UILineOfTextOrEmptyInput
          , () => '' // getDefault
          , null // requireUpdateDefaults
          , 'Tag'
        ]);
        return widgets;
    }
...
// markStyleLinks map: keys are schema mark type names (the create
// select offers the available styles, like the intent map; the key is
// adjusted to the mark name). No tag binding.
class UIMarkStyleLinksValue extends _UIStyleLinksMapValue {
    constructor(widgetBus, zones) {
        super(widgetBus, zones);
    }
}
...
class UIStylePatchesLinksMap extends _UIStyleLinksMap{
    static VALUE_CONSTRUCTOR = UIStylePatchLinksValue;
    static UIKeyCreate = UIStylePatchesLinksMapCreate;
}


class UIMarkStyleLinksMapCreate extends _UILinksMapCreate {
    static labelText = ' to a MarkSpec ';
}

class UIMarkStyleLinksMap extends _UIStyleLinksMap {
    static VALUE_CONSTRUCTOR = UIMarkStyleLinksValue;
    static UIKeyCreate = UIMarkStyleLinksMapCreate;
}


// Displays the style-link edges a TypeSpec inherits from its ancestors:
// the effective edges (via the typeSpecnion) minus the locally defined
// keys; tombstoned edges are already excluded by getStyleLinks.
// Read-only: to change an inherited edge, create a local edge with the
// same key (whole-edge override) resp. set its mode to unlinked.
class _UIInheritedStyleLinksListBase extends _BaseComponent {
    constructor(widgetBus, prefix, getSuffix, label=null) {
        super(widgetBus);
        this._prefix = prefix;
        this._getSuffix = getSuffix;
        [this.element, this.list] = this._initTemplate(label);
    }

    _initTemplate(label=null) {
        const list = this._domTool.createElement("ul", {
                class: "ui_links_map-inherited-list",
            }),
            element = this._domTool.createElement(
                            "div", {class: "ui_links_map-inherited"});
        if(label !== null)
            element.append(this._domTool.createElement(
                            "span", {class: "typeroof-ui-label"}, label));
    ...
    update(changedMap) {
        const typeSpecProperties = changedMap.has("typeSpecProperties@")
                ? changedMap.get("typeSpecProperties@")
                : this.getEntry("typeSpecProperties@"),
            localStyleLinks = changedMap.has("localStyleLinks")
                ? changedMap.get("localStyleLinks")
                : this.getEntry("localStyleLinks"),
            propertyValuesMap =
                typeSpecProperties === null
                    ? new Map()
                    : typeSpecProperties.typeSpecnion.getProperties(),
            effectiveLinks = getStyleLinks(propertyValuesMap, this._prefix),
            items = [];
        for (const [key, edge] of effectiveLinks) {
            // locally defined edges are managed in the map below
            if (localStyleLinks.has(key)) continue;
            // NULL-STYLE is "" in link mode; unlinked edges are excluded
            const description =
                edge.get("stylePatch").value || EMPTY_STYLE_LINK_LABEL;
            items.push(
                this.widgetBus.domTool.createElement(
                    "li",
                    {},
                    `${key}: ${description}${this._getSuffix(edge)}`,
                ),
            );
        }
        this.list.replaceChildren(...items);
        this.element.style.display = items.length !== 0 ? '' : 'none';
    }
}

class UIInheritedStyleLinksList extends _UIInheritedStyleLinksListBase {
    constructor(widgetBus, label=null) {
        super(widgetBus, INTENT_STYLE_LINKS, (edge) => {
            const tag = edge.get("tag");
            return !tag.isEmpty && tag.value !== "" ? ` → ${tag.value}` : "";
        }, label);
    }
}

class UIInheritedMarkStyleLinksList extends _UIInheritedStyleLinksListBase {
    constructor(widgetBus, label=null) {
        super(widgetBus, MARK_STYLE_LINKS, () => "", label);
    }
}

// The style-links UI of one link family: the inherited-edges list above
// the editable map. Both families are applied in
// TypeSpecPropertiesManager inside one CollapsibleContainer; the fields
// are excluded from the type-driven UI there (see sections), following
// the color/UIColorChooserTwoColorsWithSwap precedent.
export class UIStylePatchesLinksContainer extends _BaseContainerComponent {
    constructor(widgetBus, zones) {
        super(widgetBus, zones);
        const typeSpecPath = widgetBus.getExternalName("typeSpecPath"),
            localStyleLinksPath = widgetBus.getExternalName("localStyleLinks"),
            stylesSourceMapPath = widgetBus.getExternalName("stylesSourceMap"),
            widgets = [
                [
                    { zone: "main" },
                    [
                        [localStyleLinksPath, "childrenOrderedMap"],
                        [stylesSourceMapPath, "sourceMap"],
                    ],
                    UIStylePatchesLinksMap,
                    zones,
                    [], // eventHandlers
                    "Soft Style Links", // label
                    true, // dragEntries (dragAndDrop)
                    true, // deletableEntries (per-item "x" delete button)
                ],
                [
                    { zone: "main" },
                    [
                        [localStyleLinksPath, "localStyleLinks"],
                        [
                            `typeSpecProperties@${typeSpecPath}`,
                            "typeSpecProperties@",
                        ],
                    ],
                    UIInheritedStyleLinksList,
                    "Inherited Soft Style Links",
                ],
            ];
        this._initWidgets(widgets);
    }
...
export class UIMarkStyleLinksContainer extends _BaseContainerComponent {
    constructor(widgetBus, zones) {
        super(widgetBus, zones);
        const typeSpecPath = widgetBus.getExternalName("typeSpecPath"),
            localStyleLinksPath = widgetBus.getExternalName("localStyleLinks"),
            ...
            widgets = [
                ...
                [
                    ...
                    UIInheritedMarkStyleLinksList,
                    "Inherited Hard Style Links",
                ],
            ];
        this._initWidgets(widgets);
    }
...
function _typeSpecFullLabel(pathStr, typeSpecLabel, edgeLabel='') {
    const isRoot = pathStr === '';
    return `${edgeLabel !== '' ? edgeLabel + ' :: '  : ''}${isRoot ? '[root]' : pathStr} :: ${typeSpecLabel}`;
}

function typeSpecGetOptionLabel(pathStr, typeSpec) {
    const typeSpecLabel = typeSpecGetRawLabel(pathStr, typeSpec);
    return _typeSpecFullLabel(pathStr, typeSpecLabel);
}

// Select options for typeSpec links use logical absolute paths
// exclusively — the root-anchored form is the only unambiguous option
// in this global mapping select. Relative encodings like "./…" here
// would be ambiguous with other written forms. Stored links are
// matched via typeSpecHas. Anything not /^\/$/ renders as 'custom'.
function* typeSpecOptionsGen(typeSpec, path=Path.fromParts(Path.ROOT)) {
    // Depth-first is the natural representation of the tree data.
    const key = `${path}`;
    yield [key, typeSpec];
    const children = typeSpec.get('children');
    for(const [childKey, value] of children)
        yield * typeSpecOptionsGen(value, path.append(childKey));
}
...
export class UINodeSpecToTypeSpecLinksMap extends _UIBaseMap {
    ...
    _createKeyValue(childrenOrderedMap) {
        const linkSelect = this.getWidgetById('key-create-link-select')
          ,  keyItem = childrenOrderedMap.constructor.Model.createPrimalDraft(childrenOrderedMap.dependencies)
          ;
        keyItem.get('link').value = linkSelect.value;
        return keyItem;
    }
}
...
export class UILeadingAlgorithm extends _BaseTypeDrivenContainerComponentMixin(
                                            _BaseDynamicCollectionContainerComponent) {
    ...
    _changeTypeHandler(getDefaults, ppsRecord, newValue) {
        ...
        if(newValue === 'ManualLeading') {
            ...
            const minRecord = ProcessedPropertiesSystemMap.createSimpleRecord(ppsRecord.propertyRoot, 'minLeading')
              , maxRecord = ProcessedPropertiesSystemMap.createSimpleRecord(ppsRecord.propertyRoot, 'maxLeading')
              , minLeading = getDefaults(minRecord, null/*fieldName not required here*/, null)
              , maxLeading = getDefaults(maxRecord, null/*fieldName not required here*/, null)
              ;
            if(minLeading !== null && maxLeading !== null) {
                const {hostDraft, fieldName, wrapper} = getStateInstances()
                  ...
                  ;
                newInstance.get('instance').get('leading').value = (minLeading + maxLeading) / 2;
