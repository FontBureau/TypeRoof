  ...
  , ApplicationModel = _AbstractStructModel.createClass(
      ...
      , ['activeFontKey', new ForeignKey('installedFonts', ForeignKey.NOT_NULL, ForeignKey.CUSTOM,
        function* (targetContainer, currentKeyValue) {
          ...
          // ...SOOO here it is becoming complicated!
          if(currentKeyValue !== undefined) {
              // if(currentKeyValue === 'from-url Roboto Flex Regular Version_3-200 gftools_0-9-32_')
              // throw new Error('show me where this happens exactly!');
              // => or anything else
              // => this means the key is not in installedFonts
              // => we should detect this and try to install the font!
              // => ideally we install dependencies before metamorphose, so they
              //     are already there and we don't have

              // SO, the way I see this, a generator/coroutine style is
              ...
              console.warn(`activeFontKey ${this} via yield ResourceRequirement, _firstKey:`, key);
              if(!targetContainer.has(key))
                  ...
                  throw keyConstraintError(new Error(`CONSTRAINT ERROR ${this} Can't set key from requested `
                      + `ResourceRequirement ${key} is not in targetContainer "${this.targetName}".`));
              return key;
          }
          // else if(currentKeyValue === undefined)
          return this[ForeignKey.SET_DEFAULT_FIRST](targetContainer, currentKeyValue);
        })]
