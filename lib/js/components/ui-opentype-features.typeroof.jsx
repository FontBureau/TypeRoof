...
class UIOTFeaturesSetSelect extends GenericSelect {
    ...
    _optionGetGroup(value) {
        ...
        if (!group)
            throw new Error(`KEY ERROR group "${value.group}" not found`);
    ...
    *_optionsGen(/*font*/) {
    ...
    _getIsHiddenFn() {
        if (this._optionsFilter === this.constructor.OPTION_FILTERS.IN_FONT) {
            return (value) =>
                value !== "[_NULL_]" && !(value in this._font.features);
        } else if (
            this._optionsFilter === this.constructor.OPTION_FILTERS.ALL
        ) {
            return null;
        }
        throw new Error(
    ...
    update(changedMap) {
        ...
        if (changedMap.has("rootFont") || requireUpdateDefaults) {
            ...
            if (requireUpdateDefaults) {
                const maybeFont = this._getDefaults(
                    this._ppsRecord,
                    "font",
                    false,
                );
    ...
    _updateRestrictions() {
        const groups = new Map(
            Array.from(this._select.getElementsByTagName("optgroup")).map(
                (group) => [group, group.getElementsByTagName("option").length],
            ),
        );
        for (const option of this._select.options) {
            ...
            option.style.setProperty(
                "display",
                isHidden === true ? "none" : null,
            );
            if (isHidden && groups.has(option.parentElement)) {
                groups.set(
                    option.parentElement,
                    groups.get(option.parentElement) - 1,
                );
            }
        }
...
export class UIBooleanOTFeature extends _BaseComponent {
    ...
    _getTemplate(h) {
        return (
            <div class="ui_boolean_ot_feature">
                <label>
                    <input
                        class="ui_boolean_ot_feature-input"
                        type="checkbox"
                    />{" "}
    ...
    _initTemplate(tagName) {
        const element = this._getTemplate(this._domTool.h),
            label = element.querySelector(
                ".ui_boolean_ot_feature-label-content",
            ),
    ...
    _setFont(font = null) {
        const tag = this._tagName,
            feature =
                font !== null && tag in font.features
                    ? font.features[tag]
                    : null,
        ...
        if (!this._zones.has(zone)) {
            ...
            for (const group_ of this._groups.values()) {
                ...
                if (group_.info.index <= group.info.index) {
                    ...
                    if (!after || after.info.index > group_.info.index)
                        after = group_;
                }
            }
            ...
            if (before) this._domTool.insertAfter(group.outer, before.outer);
                ...
                this._domTool.insertBefore(group.outer, after.outer);
        ...
        super(widgetBus, zones, [
            ...
            [
                ...
                >
                    <option
                        selected
                        value={UIOTFeaturesSetSelect.OPTION_FILTERS.IN_FONT}
                    >
                        Show tags in font
                    </option>
