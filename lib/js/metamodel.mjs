...
export async function deserialize(asyncResolver, Model, dependencies, serializedString, options=SERIALIZE_OPTIONS) {
    let serializedValue;
    ...
    const gen =  Model.createPrimalStateGen(dependencies, serializedValue, options)
    return await driveResolveGenAsync(asyncResolver, gen);
}
...
export function failingResourceResolve (resourceRequirement) {
    throw new Error(`FAILING DELIBERATELY failingResourceResolve with resource requirement: ${resourceRequirement}`);
}
