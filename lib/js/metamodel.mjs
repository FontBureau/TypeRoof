...
function _safeDeserialize(draft, serializedValue, options) {
    try {
        return draft[DESERIALIZE](serializedValue, options);
    }
    catch(error) {
        const errors = [];
        error.message = `${error.message} ${draft}`;
        errors.push([[/*path*/], error]);
        return errors
    }
}
...
function deserializeItem(draft, serializedValue, options=SERIALIZE_OPTIONS) {
    const isContainer = draft instanceof _BaseContainerModel
      , resultErrors = []
      ;
    if(isContainer) {
        if(!Array.isArray(serializedValue))
            resultErrors.push([[], new Error(`VALUE ERROR NOT ARRAY model `
                +`${draft} is a container but serializedValue is not an array`)
                , typeof serializedValue, serializedValue]);
    }
    else if(typeof serializedValue !== 'string') {
        resultErrors.push([[], new Error(`VALUE ERROR NOT SIMPLE model ${draft} `
            +`is simple but serializedValue is not null or a string`)
            , typeof serializedValue, serializedValue]);
    }
    if(resultErrors.length && options.earlyExitOnError)
        return resultErrors;
    const errors = _safeDeserialize(draft, serializedValue, options);
    resultErrors.push(...errors);
    return resultErrors;
}

export function deserialize(Model, dependencies, serializedString, options=SERIALIZE_OPTIONS) {
    // TODO/WIP

    if(options.format === SERIALIZE_FORMAT_JSON) {
        const draft  = Model.createPrimalDraft(dependencies)
          , serializedValue = JSON.parse(serializedString)
          , errors = deserializeItem(draft, serializedValue, options)
          ;
        return [errors, draft];
    }
...
export class _BaseModel {
    ...
    static deserialize(/*value, serializeOptions=SERIALIZE_OPTIONS*/) {
...
function _serializeContainer(containerItem, keepKeys, options=SERIALIZE_OPTIONS) {
    ...
    for(const [key, childItem] of containerItem) {
        ...
        if(serializedValue === null || Array.isArray(serializedValue) && serializedValue.length === 0) {
            if(!keepKeys) {
    ...
    return [resultErrors, resultValues];
}

function _deserializeStruct(draft, serializedValues, keepKeys, options=SERIALIZE_OPTIONS) {
    const resultErrors = []
      , keys = draft.ownKeys()
      , valuesMap = new Map()
      ;
    ...
    // owned (hasOwn) by Model.
    console.log(`${draft}._deserializeStruct valuesMap:`, valuesMap);
    const seen = new Set()

    // It sets e.g. foreignKeys first, therefore making sure that i.e. the type
    // of a _AbstractDynamicStruct can be known before it is initialized.
    for(const key of [draft.constructor.initOrder, ...valuesMap.keys()]) {
        console.log(`${draft}._deserializeStruct key ${key}`);
        if(!valuesMap.has(key)) continue;
        if(seen.has(key)) continue;
        seen.add(key);
        const serializedValue = valuesMap.get(key);
        console.log(`${draft}._deserializeStruct key ${key} ${serializedValue}`, serializedValue);
        // FIXME: seems like there's an issue with _AbstractDynamicStruct,
        // as it does not get the full dependencies at this instance.

        // ??? draft._lockItem(

        // Hmmm, it would be really good if the values could be put right
        // into the constructor/the initial metamorphose ...
        // i.e. for the ValueLinks to be populated correctly.
        const childDraft = draft.getDraftFor(key)
          , errors = deserializeItem(childDraft, serializedValue, options)
          ;
        resultErrors.push(..._serializeErrorsPrependKey(key, errors));
        if(resultErrors.length && options.earlyExitOnError)
            return resultErrors;
...
// _AbstractOrderedMapModel and _AbstractListModel
function _deserializeContainer(draft, serializedValues, keepKeys, options=SERIALIZE_OPTIONS) {
    const resultErrors = []
      , entries = []
      ;
    for(const [i, entry] of serializedValues.entries()) {
        const [key, serializedValue] = (keepKeys
                ? entry
                : [`${i}`, entry] // no key;
            )
          , ChildModel = draft.constructor.Model
          // FIXME: what is the getDraftFor interface in these even for
          // if it can't create a new item?
          , childDraft = ChildModel.createPrimalDraft(draft.dependencies)
          , errors = deserializeItem(childDraft, serializedValue, options)
          ;
        resultErrors.push(..._serializeErrorsPrependKey(key, errors));
        if(resultErrors.length && options.earlyExitOnError)
            return resultErrors;
        const draftEntry = keepKeys
            ? [key, childDraft]
            : childDraft
            ;
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    static createClass(className, ...definitions) {
        ...
        const staticHas = this.has.bind({fields, foreignKeys, links, internalizedDependencies, fallBackValues})
          ...
          , ownExternalDependencies = new FreezableSet(iterFilter(_ownAllDependencies, dependency=>!staticDependencies.has(dependency)))
        ...
        const result = {[className]: class extends this {
            ...
            static fallBackValues =  Object.freeze(fallBackValues);

            static staticDependencies = Object.freeze(staticDependencies);
    ...
    _lockItem(localScope, locked, changedDependencyNames, name) {
        ...
        this._lockDependencies(localScope, locked, changedDependencyNames, descriptor.dependencies);
        let immutable;
    ...
    _lockDependencies(localScope, locked, changedDependencyNames, dependencies) {
        for(const name of dependencies){
            this._lockItem(localScope, locked, changedDependencyNames, name);
        }
    }
    ...
    metamorphose(dependencies={}) {
        ...
        for(const [k, v ] of Object.entries(dependenciesData)) {
            if(_PotentialWriteProxy.isProxy(v) && this[OLD_STATE] !== null)
                throw new Error(`VALUE ERROR dependency "${k}" is a _PotentialWriteProxy `
        ...
        for(const name of this.constructor.initOrder) {
            ...
            if(this.constructor.childrenExternalDependencies.has(name)) {
                ...
                this._lockDependencies(localScope, locked, changedDependencyNames, key.dependencies); // is { key.targetName }
                if(this[OLD_STATE] === null && !this._value.has(name)) {
                    // this is a primal state
                ...
                try {
                    this._lockDependencies(localScope, locked, changedDependencyNames, link.dependencies); // is { link.keyName }
                } catch(error) {
                ...
                if(value.constructor !== fallBackValue.Model)
                    throw new Error(`TYPE ERROR fall back value is not a ${fallBackValue.Model} but a ${value.constructor.name}.`);
                localScope.set(name, value);
            }
            else
        ...
        this.ownKeys().map(this._lockItem.bind(this, localScope, locked, changedDependencyNames));
    ...
    *allEntries() {
    ...
    [SERIALIZE](options=SERIALIZE_OPTIONS) {
        return _serializeContainer(this, /*keepKeys*/SERIALIZE_OPTIONS.structStoreKeys, options);
    }
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        return _deserializeStruct(this, serializedValue, /*keepKeys*/SERIALIZE_OPTIONS.structStoreKeys, options);
    }
}
...
export class _AbstractListModel extends _BaseContainerModel {
    ...
    *[Symbol.iterator]() {
    ...
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        return _deserializeContainer(this, serializedValue, /*keepKeys*/false, options);
    }
}
...
export class _AbstractOrderedMapModel extends _BaseContainerModel {
    ...
    *indexedEntries() {
    ...
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        return _deserializeContainer(this, serializedValue, /*keepKeys*/true, options);
    }
}
...
export class _AbstractDynamicStructModel extends _BaseContainerModel {
        ...
        if(this._value === null // only in primal state creation
            ...
            Object.defineProperty(this, '_value', {
                value: this.WrappedType
                            ? this.WrappedType.createPrimalState(childDependencies)
                            : null
              , configureable: true
            });
        }
    ...
    *allEntries() {
    ...
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        ...
        console.log(`${this}[DESERIALIZE]`, this, 'serializedValue'
            , serializedValue, typeof serializedValue, 'this.WrappedType', this.WrappedType);
        // As this is a draft this.wrapped is supposed to be a draft already.
        const errors = deserializeItem(this.wrapped, serializedValue, options);
        if(errors.length) {
            resultErrors.push(...errors);
...
export  class _AbstractGenericModel extends _BaseSimpleModel {
    ...
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        if(this.constructor.deserializeFN !== null) {
            console.log(this+' this.constructor.deserializeFN', this.constructor.deserializeFN, this);
...
export class _AbstractSimpleOrEmptyModel extends _BaseSimpleModel {
    ...
    [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        if(serializedValue === null)
            ...
            return [];
        const valueDraft = this.Model.createPrimalDraft(this.dependencies)
          , errors = deserializeItem(valueDraft, serializedValue, options)
          ;
        if(errors.length && options.earlyExitOnError)
            return errors;
        this.set(valueDraft);
...
export  class _AbstractNumberModel extends _BaseSimpleModel {
    ...
    static validate(value) {
        ...
        if(this.toFixedDigits !== null && parseFloat(value.toFixed(this.toFixedDigits)) !== value)
            ...
            return [null, `value (${value}) is not equal to fixed-point notation "${value.toFixed(this.toFixedDigits)}"`];
    ...
    [SERIALIZE](/*options=SERIALIZE_OPTIONS*/) {
        return [
          ...
          , this.constructor.toFixedDigits !== null
                ? this.value.toFixedDigits(this.constructor.toFixedDigits)
                : this.value.toString()
        ];
    }
    [DESERIALIZE](serializedValue /*, options=SERIALIZE_OPTIONS*/) {
...
export const AnyModel = _AbstractGenericModel.createClass('AnyModel')
  , IntegerModel = _AbstractNumberModel.createClass('IntegerModel', {min: Number.MIN_SAFE_INTEGER, max: Number.MAX_SAFE_INTEGER, toFixedDigits: 0})
    // Beautiful
  ...
  , KeyValueModel = _AbstractGenericModel.createClass('KeyValueModel',  {
        validateFN: function(value) {
          ...
          if(typeof value !== 'string' && value !== ForeignKey.NULL)
                return [false, 'NOT A VAILID KEY must be string or ForeignKey.NULL`'];
            return [true, null];
        }
...
export class Path {
    ...
    *[Symbol.iterator]() {
    ...
    get parent() {
        ...
        return this.fromParts(this.explicitAnchoring || '', ...this.parts.slice(0, -1));
    }
    startsWith(rootPath) {
        const parts = [...this];
