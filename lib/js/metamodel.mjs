...
const SERIALIZE = Symbol.for('SERIALIZE')
  , DESERIALIZE = Symbol.for('SERIALIZE')
  ;
export const SERIALIZE_FORMAT_JSON = Symbol('SERIALIZE_FORMAT_JSON')
...
function _serializeErrorsPrependKey(key, errors) {
    ...
    for(const [path, message] of errors) {
        ...
        newErrors.push([newPath, message]);
    }
    return newErrors;
}
...
function _safeDeserialize(Model, serializedValue, options) {
    try {
        return Model[DESERIALIZE](serializedValue, options);
    }
    catch(error) {
        const errors = [];
        errors.push([[/*path*/], error]);
    ...
    if(options.checkResults) {
        if(isContainer) {
            if(!Array.isArray(value))
                resultErrors.push([[], `VALUE ERROR NOT ARRAY ${modelInstance} is a container but does not produce an array`]);
        }
        else if(value !== null && value !== 'string') {
            resultErrors.push([[], `VALUE ERROR NOT SIMPLE ${modelInstance} is simple but does not null or a string`]);
        }
    }
    return [resultErrors, value];
}

function deserializeItem(Model, serializedValue, options=SERIALIZE_OPTIONS) {
    const isContainer = Model.prototype instanceof _BaseContainerModel
      , resultErrors = []
      ;
    if(isContainer) {
        if(!Array.isArray(serializedValue))
            resultErrors.push([[], `VALUE ERROR NOT ARRAY Model ${Model.name} is a container but serializedValue is not an array`]);
    }
    else if(serializedValue !== 'string') {
        resultErrors.push([[], `VALUE ERROR NOT SIMPLE Model ${Model.name} is simple but serializedValue is not null or a string`]);
    }
    if(errors.length && options.earlyExitOnError)
        return [resultErrors, null];
    const [errors, draft] = _safeDeserialize(Model, serializedValue, options);
    resultErrors.push(...errors);
    return [resultErrors, draft];
}

export function deserialize(Model, serializedValue, options=SERIALIZE_OPTIONS) {
    // TODO/WIP
    // const [errors, draft] = deserializeItem(Model, serializedValue, option);
    // return draft;
}
...
export function serialize(/*_BaseModel:*/modelInstance, options=SERIALIZE_OPTIONS) {
    ...
    if(options.format === SERIALIZE_FORMAT_JSON)
        return [
            resultErrors.length === 0 ? null : resultErrors
...
export class _BaseModel {
    ...
    static [DESERIALIZE](/*serializedValue, SerializeOptions: options*/){
        throw new Error(`NOT IMPLEMENTED [DESERIALIZE] in ${this}.`);
    }
}
...
function _deserializeStruct(Model, serializedValues, keepKeys, options=SERIALIZE_OPTIONS) {
    const resultErrors = []
      , draft = Model.createPrimalDraft({})
      , keys = Model.ownKeys()
      , valuesMap = new Map()
      ;
    ...
    // owned (hasOwn) by Model.
    for(const [key, serializedValue] of valuesMap) {
        const ChildModel = Model.foreignKeys.has(key)
                ? KeyValueModel
                : Model.fields.get(key)
          , [errors, childDraft] = deserializeItem(ChildModel, serializedValue, options)
          ;
        resultErrors.push(..._serializeErrorsPrependKey(key, errors));
        if(resultErrors.length && options.earlyExitOnError)
            return [resultErrors, draft];
        draft.set(key, childDraft);
    }
    return [resultErrors, draft];
}

// _AbstractOrderedMapModel and _AbstractListModel
function _deserializeContainer(Model, serializedValues, keepKeys, options=SERIALIZE_OPTIONS) {
    const resultErrors = []
      , draft = Model.createPrimalDraft({})
      , entries = []
      ;
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    metamorphose(dependencies={}) {
        ...
        const localScope = new Map()
          ...
          , collectChildDependencies = child=>Object.fromEntries(
                iterMap(child.dependencies, (key)=>{
                    const value = localScope.get(key);
                    if(value === undefined)
                        throw new Error(`VALUE ERROR in ${this} dependency "${key}" for child "${child}" is undefined.`);
                    return [key, value];
                }))
          , locked = new Set()
          , lockItem = (name)=>{
                ...
                if(descriptor instanceof ForeignKey) {
                    ...
                    const childDependencies = collectChildDependencies(descriptor);
        ...
        for(const name of this.constructor.initOrder) {
            ...
            if(this.constructor.childrenExternalDependencies.has(name)) {
                ...
                if(this[OLD_STATE] === null && !this._value.has(name)) {
                    ...
                    // for a new instance.
                    const childDependencies = collectChildDependencies(Model);
                ...
                const coherenceFunction = this.constructor.coherenceFunctions.get(name)
                  ...
                  , childDependencies = collectChildDependencies(coherenceFunction)
                  ;
    ...
    *allEntries() {
    ...
    static [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
...
export class _AbstractDynamicStructModel extends _BaseContainerModel {
    ...
    *allEntries() {
    ...
    static [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        // FIXME: This is going to be a major concern as the type is
        // defined by the parent and located in the dependencies.
        // See implementation of get WrappedType().
        throw new Error(`NOT IMPLEMENTED _AbstractDynamicStructModel.[DESERIALIZE]`)

        const draft = this.createPrimalDraft({}) // this requires proper dependencies!
          , resultErrors = []
          ;
        ...
        const [errors, wrappedDraft] = deserializeItem(draft.WrappedType, serializedValue, options);
        if(errors.length) {
            resultErrors.push(...errors);
            if(options.earlyExitOnError)
                return [resultErrors, draft];
        }
        draft.wrapped = wrappedDraft;
        return [resultErrors, draft];
    }
}
...
export  class _AbstractGenericModel extends _BaseSimpleModel {
    ...
    static [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        if(this.constructor.deserializeFN !== null){
            const draft = this.createPrimalDraft({});
            draft.value = this.constructor.deserializeFN(serializedValue, options);
            return [[], draft];
        }
        return super[SERIALIZE](serializedValue, options);
    }
}
...
export class _AbstractSimpleOrEmptyModel extends _BaseSimpleModel {
    ...
    static [DESERIALIZE](serializedValue, options=SERIALIZE_OPTIONS) {
        const draft = this.createPrimalDraft({})
          ...
          ;
        if(serializedValue === null)
            // serializedValue should not be null, as the parent wouldn't call
            // this directly. I'm not sure for all cases though.
            return[errors, draft];
        const [childErrors, valueDraft] = deserializeItem(this.Model, serializedValue, options);
        if(childErrors.length) {
            errors.push(childErrors);
            if(options.earlyExitOnError)
                return [errors, draft];
        }
        draft.set(valueDraft);
        return [errors, draft];
    }
}
...
export  class _AbstractNumberModel extends _BaseSimpleModel {
    ...
    static [DESERIALIZE](serializedValue /*, options=SERIALIZE_OPTIONS*/) {
        const draft = this.createPrimalDraft({});
        draft.value = parseFloat(serializedValue);
        return [[], draft];
    }
}
