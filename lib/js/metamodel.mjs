...
function _serializeContainer(containerItem, presenceIsInformation, keepKeys, options=SERIALIZE_OPTIONS) {
    const resultErrors = []
      , resultValues = []
      ;
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    static createClass(className, ...definitions) {
        ...
        for(const definition of definitions) {
            ...
            if(seen.has(name))
                throw new Error(`VALUE ERROR ${className} multiple definitions for name "${name}" in "${className}".`);
            seen.add(name);
    ...
    *allEntries() {
    ...
    set(key, entry) {
        ...
        if(!this.hasOwn(key))
            throw new Error(`KEY ERROR trying to set not owned (or unknown) "${key}" in ${this}.`);
...
export class _AbstractListModel extends _BaseContainerModel {
    ...
    *[Symbol.iterator]() {
    ...
    [SERIALIZE](options=SERIALIZE_OPTIONS) {
        return _serializeContainer(this, /*presenceIsInformation*/true,/*keepKeys*/false, options);
    }
}
...
export class _AbstractOrderedMapModel extends _BaseContainerModel {
    ...
    *indexedEntries() {
    ...
    static validateKey(key) {
        ...
        if((allowedTypes.has(keyType) || key === null) && (keyType !== 'number' || isFinite(key))){
            ...
            if(key.indexOf(Path.SEPARATOR) != -1)
                return [false, `key can't contain the path separator "${Path.SEPARATOR}" `
                    + `as path parts are not reliably escaped so far. Key is "${key}". (FIXME)`
                    ];
            if(this.validateKeyFn !== null)
                return this.validateKeyFn(key);
            return [true, null];
        }
    ...
    [SERIALIZE](options=SERIALIZE_OPTIONS) {
        return _serializeContainer(this, /*presenceIsInformation*/true,/*keepKeys*/true, options);
    }
}
...
export class _AbstractEnumModel extends _BaseSimpleModel {
    static createClass(className, enumItems, defaultValue, attachStaticProperties) {
        const enumItems_ = Object.freeze(Array.from(enumItems))
        ...
        if(enumItemsSet.size !== enumItems_.length)
            throw new Error(`VALUE ERROR ${this.name} enumItems must not have duplicates ${enumItems_.join(', ')}.`);
        if(!enumItemsSet.has(defaultValue))
             throw new Error(`VALUE ERROR ${this.name} defaultValue "${defaultValue}" must be in enumItems (${enumItems_.join(', ')} but is not.`);
        // this way name will naturally become class.name.
        const result = {[className]: class extends this {
            // jshint ignore: start
            static enumItems =enumItems_;
            static _enumItemsReverseMap = Object.freeze(new FreezableMap(enumItems_.map((value, i)=>[value, i])))
...
export  class _AbstractNumberModel extends _BaseSimpleModel {
    static createClass(className, setup={}) {
        // numeric or _NOTDEF
        setup = {min:_NOTDEF, max:_NOTDEF, toFixedDigits:_NOTDEF, defaultValue:_NOTDEF
            , validateFN:_NOTDEF, sanitizeFN:_NOTDEF, sanitzeByDefault:true, ...setup};
