    ...
    keyToIndex(key) {
        ...
        if(index < 0 || index >= this._value.length)
           return [null, `KEY ERROR NOT FOUND key "${stringKey}" is not an index (= ${index})`
                       + ` (index > 0 && index < ${this._value.length}. in ${this}`];
        return [index, null];
    }
...
export  class _AbstractGenericModel extends _BaseSimpleModel {
    static createClass(className, setup={}) {
        setup = {sanitizeFN:_NOTDEF, validateFN:_NOTDEF, defaultValue:_NOTDEF, ...setup};
        for(const fnName of ['sanitizeFN', 'validateFN']) {
        ...
        const result = {[className]: class extends this {
            // jshint ignore: start
            static _sanitizeFN = setup.sanitizeFN === _NOTDEF ? null : setup.sanitizeFN;
            static _validateFN = setup.validateFN === _NOTDEF ? null : setup.validateFN;
    ...
    static sanitize(rawValue) {
        if(this._sanitizeFN === null)
            return [rawValue, null];
        return this._sanitizeFN(rawValue);
    }

    static validate(value) {
        if(this._validateFN === null)
            return [true, null]; // valid
        return this._validateFN(value);
    }
    ...
    set(rawValue, sanitize=_NOTDEF) {
        ...
        if(sanitize !== _NOTDEF && sanitize || sanitize === _NOTDEF && this.constructor._sanitizeFN !== null) {
...
export class _AbstractEnumModel extends _BaseSimpleModel {
    static createClass(className, enumItems, defaultValue) {
