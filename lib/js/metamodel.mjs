...
const SERIALIZE = Symbol.for('SERIALIZE')
  , SERIALIZE_OPTIONS=Object.freeze({depth: 0})
  ;
...
export function serialize(/*_BaseModel:* /modelInstance, options=SERIALIZE_OPTIONS) {
    const Type = modelInstance.Constructor
      , serialization = modelInstance[SERIALIZE](options)
      ;
    for(const [pathParts, isContainer, error, value] of iterate(modelInstance)) {
        if(error !== null) {
            continue;
        }
        if(isContainer) {
            const [hasKeys, key, raw] = value;
        }
        else {
            const raw = value
        }
...
// We should walk down all the way, to serialize a struct:
result = []
for(const [key, child] of my struct) {
    // depth first
    const [errors, serialization] = serialize(child)
    if(useKeys)
        result.push([key, serialization]);
    else
        result.push(serialization);
}
// deserialize a struct:
for(const [i, data] of dataList) {
    resultDraft =
    const [key, serialization] = useKeys
        ? data
        : [Type.keys.at(i), data]
        ;
    const ChildType = Type.ChildTypes.get(key)
        // depth first!
      , [errors, item] = deserialize(ChildType, serialization, dependencies???)
      ;
    resultDraft.set(key, item)
}
*/
...
export class _BaseModel {
    ...
    [SERIALIZE](/*SerializeOptions: options*/) {
        ...
        // console.error(`NOT IMPLEMENTED serialize in ${this}.`, this);
        // return `NOT IMPLEMENTED serialize in ${this}`;
    }
}
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    *allEntries() {
    ...
    /**
     * return a string(?), that can be de-serialized when using compatible
     * options.
     * deserialize should probably be a static function living of the class.
     */
    [SERIALIZE](serializeOptions=SERIALIZE_OPTIONS) {
        // some of the next lines should be shared code.
        const depth = serializeOptions.depth || 0
          , childSerializeOptions = Object.assign({}, serializeOptions, {depth: depth + 1})
          , indent = '    '
          , ownIndent = indent.repeat(depth)
          , lines = []
          ;
        for(const [key, value] of this) {
            lines.push(`${indent}"${key}": ${value[SERIALIZE](childSerializeOptions)}`);
        }
        return [`{\n`, ...lines.map(line=>`${ownIndent}${line}\n`), ownIndent, `}`].join('');
    }
}
...
export class _AbstractDynamicStructModel extends _BaseContainerModel {
    ...
    *allEntries() {
    ...
    [SERIALIZE](serializeOptions=SERIALIZE_OPTIONS) {
        // some of the next lines should be shared code.
        const depth = serializeOptions.depths || 0
          , childSerializeOptions = {...serializeOptions, depth: depth + 1}
          // , indent = '  '.repeat(depth + 1)
          ;
        // NOTE: to deserialize this WrappedType must be available by its name!!!
        if(this.WrappedType !== null)
            return `(${this.WrappedType.name}:${this.wrapped[SERIALIZE](childSerializeOptions)})`;
        else
            return `(NULL)`;
    }
}
...
export  class _AbstractGenericModel extends _BaseSimpleModel {
    ...
    [SERIALIZE](...args) {
        if(this.constructor.serializeFN !== null)
            return this.constructor.serializeFN(this.value, ...args);
...
export class _AbstractEnumModel extends _BaseSimpleModel {
    ...
    [SERIALIZE](/*serializeOptions=SERIALIZE_OPTIONS*/) {
        return `${this.constructor.defaultValue === this.value ? 'DEF' : 'CUS'}:${this.value}`;
    }
}
...
export class _AbstractSimpleOrEmptyModel extends _BaseSimpleModel {
    ...
    [SERIALIZE](...options) {
        if(this.isEmpty)
            return `(EMPTY, )`;
        return `(,${this._value[SERIALIZE](...options)})`;
    }
}
...
export  class _AbstractNumberModel extends _BaseSimpleModel {
    ...
    [SERIALIZE](/*options*/) {
        const value = this.constructor.toFixedDigits !== null
  ...
  , KeyValueModel = _AbstractGenericModel.createClass('KeyValueModel',  {
      ...
      , serializeFN: function(value/*, serializeOptions=SERIALIZE_OPTIONS*/) {
            if(typeof value === 'string')
                return `S:${value}`;
            if(value === ForeignKey.NULL)
                return 'N:ULL'; // could be empty string, doesn't start with "S:"!
