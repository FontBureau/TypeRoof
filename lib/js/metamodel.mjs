...
export class _BaseModel {
    ...
    * metamorphoseGen() {
    ...
    metamorphose(...dependencies) {
        ...
        return driveResolverGenFailing(gen);
    }
...
export class _BaseContainerModel extends _BaseModel {
     ...
     * await driveResolveGenAsync(asyncResolver, gen)
     */
...
export class ResourceRequirement {
    ...
    toString(){
        return `[${this.constructor.name} with description: ${this.description.join(', ')}]`
    }
}

export class _BaseResourceResolver {
    resolve(resourceRequirement) {
        throw new Error(`NOT IMPLEMENTED ${this}.resolve with resource requirement: ${resourceRequirement}`);
    }
    toString() {
        return `[${this.constructor.name}]`;
    }
...
export class FailingResourceResolver extends _BaseResourceResolver{
    resolve(resourceRequirement) {
        throw new Error(`FAILING DELIBERATELY ${this}.resolve with resource requirement: ${resourceRequirement}`);
    }
}

export function driveResolverGenSync(syncResolver, gen) {
    let result
    , sendInto=undefined
    ;
    do {
        ...
        if(result.value instanceof ResourceRequirement)
            sendInto = syncResolver.resolve(result.value);
        else if(!result.done)
...
export function driveResolverGenFailing(gen) {
    const failingResourcesResolver = new FailingResourceResolver();
    return driveResolverGenSync(failingResourcesResolver, gen);
}

export async function driveResolveGenAsync(asyncResolver, gen) {
    ...
    do {
        ...
        sendInto = undefined;// don't send same value again
        if(result.value instanceof ResourceRequirement)
            sendInto = await asyncResolver.resolve(result.value);
        else if(!result.done)
            throw new Error(`VALUE ERROR Don't know how to handle genereator result with value {result.value}`);
    } while(!result.done);
    return result.value;
