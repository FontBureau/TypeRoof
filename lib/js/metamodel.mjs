...
export  class _AbstractNumberModel extends _BaseSimpleModel {
    static createClass(className, setup={}) {
        ...
        const result = {[className]: class extends this {
            // jshint ignore: start
            static minVal = setup.min;
            static maxVal = setup.max;
            static toFixedDigits = setup.toFixedDigits;
            static defaultValue = setup.defaultValue;
    ...
    static sanitize(rawValue) {
        ...
        if(this.minVal !== _NOTDEF && cleanValue < this.minVal)
            cleanValue = this.minVal;

        if(this.maxVal !== _NOTDEF && cleanValue > this.maxVal)
            cleanValue = this.maxVal;

        if(this.toFixedDigits !== _NOTDEF)
    ...
    validate(value) {
        ...
        if(this.minVal !== _NOTDEF && value < this.minVal)
            return [false, `value (${value}) is smaller than min value (${this.minVal})`];

        if(this.maxVal !== _NOTDEF && value > this.maxVal)
            return [null, `value (${value}) is bigger than max value (${this.maxVal})`];

        if(this.toFixedDigits !== _NOTDEF && parseFloat(value.toFixed(this.toFixedDigits)) !== value)
