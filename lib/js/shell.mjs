...
class FontManager {
    ...
    async _rawInstallFont(availableFonts, fontName) {
    ...
    async _installFont(fontName) {
        ...
        if(!installedFontsDraft.has(fontName)) {
            ...
            const fontState = await this._rawInstallFont(availableFonts, fontName)
        ...
        const releaseFontFn = ()=>{
            console.warn('releaseFontFn for',fontName);
    ...
    /**
     * This reqquires a context where availableFontsDraft and installedFontsDraft are already prepared.
     */
    async _registerFont(deferredFont) {
        ...
        if(availableFontsDraft.has(fontName)) {
            if(!this.appReady)
                throw new Error(`APP NOT READY _registerFont: ${fontName}`);
    ...
    _unregisterFont(fullName) {
        ...
        if(installedFontsDraft.has(fullName)) {
            const installedFont = installedFontsDraft.get(fullName)
            console.log('installedFont', fullName,installedFont);
            // installedFont.destroy();
        }
        installedFontsDraft.delete(fullName);
        this._installedFontsReferenceCounter.delete(fullName);
        // CAUTION: the existing releaseFontFn can be called, but should not
        // cause any harm when called. But I did not check.
        ...
        if(availableFontsDraft.has(fullName)) {
            const availableFont = availableFontsDraft.get(fullName);
            console.log('availableFont', fullName,availableFont);
            // availableFont.destroy();
        }
...
export class VideoproofController {
    ...
    async _allInitialResourcesLoaded(resources) {
        ...
        // This are the matching returns to the `this._useStateDependencyDraft(key)`
    ...
    _initUI(/*{ ... remote resources }*/) {
        ...
        this._ui = new MainUIController({ // widgetBus
          ...
          , removeFontsFromFiles: this._uiStateDependencyDraftContextWrapper(true
    ...
    async _updateState(draft=null) {
        ...
        const draftState = draft === null
                ...
                ;
          // NOTE I tried:
          //   await newState = draftState.metamorphose(dependencies)
          // and it should be possible at this point! Maybe in the future
          // it'll be require to introduce async to metamorphose and
          // here it could be acceptable. HOWEVER: there are many cases
          // where metamorphose is called within the code and it's
          // expected to be sync, so that would require a big rewrite!
          // The paradigm is rather to resolve all async requirements
          // before calling metamorphose.
          // SO, got a font loading/unloading issue. If the only
          // "installed font"
          // is deleted, e.g. the "Manage fonts" dialog, and no other font
          // is installed, the foreign key constraint fails, as it can't
          // find another font within installedFonts. It should rather fallback
          // to available fonts.
          // Despite that, the reference counting is broken as well. e.g.
          // A layer/family in the array, when deleted, does not call
          // unload. It should however. This could maybe be fixed, but it
          // would be nicer if we would not have to do this explicitly.
          // So, maybe here, we detect all referenced fonts and make sure
          // the are loaded and the rest is unloaded. I already made this
          // method async, so there should be no issue about it.
          // The release function get not called appropriately either.

          // ACTUALLY one more issue is, that metamorphose will find out
          // that the constraint can't be resolved. It would be nice if
          // we could handle that in place instead of creating another
          // mechanism that handles that.
          // AND the UI code that changes state may be interested in reading
          // from the new font instantly, so we should probably have a way
          // to load a font in place. It will just be directly unloaded
          // again if it is not used in the model.
          // await manageFonts();
          const newState = draftState.metamorphose(dependencies);
