...
export class ShellController {
    ...
    async _loadInitialStateFromLocationHash(Model) {
        const {fetch, location} = this._contentWindow
          ...
          , hash = (_rawHash.startsWith('#') // it's supposed to start with #
                ? _rawHash.slice(1)
                : _rawHash).trim()
          , HASH_FROM_URL = 'from-url:'
          ;
        if(!hash.length)
            return;

        if(hash.startsWith(HASH_FROM_URL)) {
            const url = hash.slice(HASH_FROM_URL.length)
            ...
            return deserializeStateString(Model, serializedValue);
        }
        throw new Error(`STATE LOAD ERROR don't know how to handle location hash: "${hash}"`);
    }
    ...
    async _allInitialResourcesLoaded(resources) {
        ...
        try {
            const likeADraft = await this._loadInitialStateFromLocationHash(ApplicationModel);
            if(likeADraft)
