...
import opentype from './vendor/opentype.js/dist/opentype.module.js';
import woff2decompress from './vendor/wawoff2/decompress.mjs';
...
export class VideoproofController {
    ...
    constructor(contentWindow) {
        ...
        for(let key of ['ready', 'localFontStorage'])
     ...
     * at 0.
     * In a non-async method, this can savely be combined directly with
     * _returnStateDependencyDraft e.g. using the method _useImmediateStateDependencyDraft
     * to obtain the draft and return it immediately without having to call
     * multiple methods.
     */
    ...
    /**
     * See comment at _prepareStateDependencyDraft.
     */
    _useImmediateStateDependencyDraft(key) {
        const draft = this._prepareStateDependencyDraft(key);
    ...
    _useStateDependencyDraft(key) {
        if(!this._stateDependenciesDrafts.has(key))
            // There's a reference counter to resolve this, shouldn't happen.
            throw new Error(`Call to _useStateDependencyDraft but there is no draft for "${key}". `
    ...
    _useStateDependency(key) {
        ...
        return this._useStateDependencyDraft(key);
    }
    ...
    async activateFont(fontName) {
        ...
        const availableFonts = this._useStateDependency('availableFonts')
          , installedFontsDraft = this._useImmediateStateDependencyDraft('installedFonts')
          ;

        // (deferred) font must be in availableFonts!
        if(!availableFonts.has(fontName))
            throw new Error(`KeyError font not available "${fontName}".`);
        ...
        const releaseFontFn = ()=>{
            const installedFontsDraft = this._useImmediateStateDependencyDraft('installedFonts');
     ...
     * (See activateFont) on a "per user" basis, because a user can be
     ...
     */
    async _useFont(userIdentifier, fontName) {
        ...
        const releaseFontFn = await this.activateFont(fontName);
    ...
    async _allInitialResourcesLoaded(resources) {
        ...
        const availableLayoutsDraft = this._useStateDependencyDraft('availableLayouts');
        for(const [label, LayoutModule] of Layouts) {
            ...
            availableLayoutsDraft.push([LayoutModule.Model.name, availableLayout]);
        }

        const availableFonts = this._useStateDependencyDraft('availableFonts')
        ...
        await this._useFont(fontUserPath, fontName);
        ...
        const dependencies = this._collectReadyStateDependencies(); // => {availableFonts, installedFonts, availableLayouts}
    ...
    async _registerFont(deferredFont) {
        ...
        const availableFontsDraft = this._useStateDependencyDraft('availableFonts')
        ...
        if(availableFontsDraft.has(fontName)) {
            ...
            if(action === 'replace') {
                const installedFontsDraft = this._prepareStateDependencyDraft('installedFonts');
                ...
                this._returnStateDependencyDraft('installedFonts');
            }
            else {
    ...
    _initUI({charGroups}) {
        ...
        this._ui = new MainUIController({
          ...
          , uiLoadFontsFromFiles: this.uiLoadFontsFromFiles.bind(this)
          , useFont: this._useFont.bind(this)
          , unuseFont: this._unuseFont.bind(this)
        });
    ...
    _collectReadyStateDependencies() {
        const dependencies = {};
        // Using an Array.from copy of this._stateDependenciesDrafts to
        // iterate, because it's going to be altered and this is the
        // sane way without knowing about the implementation of the
        // this._stateDependenciesDrafts entries genrator.
        for(const [key, [referenceCounter, draft]] of Array.from(this._stateDependenciesDrafts)) {
            if(referenceCounter <= 0) {
                dependencies[key] = draft.metamorphose({});
                this._stateDependenciesDrafts.delete(key);
            }
        }
        return dependencies;
    }
    ...
    _updateState(draft=null) {
        ...
        const dependencies = this._collectReadyStateDependencies()
    ...
    async _uiLoadFontsFromLoaderFn(sourceLoaderFn, ...sources) {
        // NOTE: this API could be exported so that it is already wrapped into withChangeState.
        if(!this.draftState)
            throw new Error(`CONTEXT ERROR _uiLoadFontsFromLoaderFn is expected to be called from within withChangeState`);
        this._prepareStateDependencyDraft('availableFonts');
        try {
            const fontsAndNulls = await Promise.all(sources.map(sourceLoaderFn))
              , fontKeys = fontsAndNulls.filter(font=>!!font).map(font=>font.fullName)
              ;
            return fontKeys;
        }
        finally {
            this._returnStateDependencyDraft('availableFonts');
        }
        return [];
    }
     ...
     */
    async uiLoadFontsFromFiles(...files) {
        return this._uiLoadFontsFromLoaderFn(this._loadFontFromFile.bind(this), ...files)
            .catch(error => this.uiReportError('uiLoadFontsFromFiles', error));
    }

    /**
     * So far unused, just for completeness.
     * Should be very similar to uiLoadFontsFromFiles
     */
    async uiLoadFontsFromUrls(...urls) {
        return this._uiLoadFontsFromLoaderFn(this._loadFontFromUrl.bind(this), ...urls)
            .catch(error => this.uiReportError('uiLoadFontsFromUrls', error));
    }
    ...
    async _loadInitialFontsFromLocalFontStorage(/* localFontStorage */) {
            ...
            switch(type) {
                ...
                case 'localFontStorage':
                    promise = byType.get(type)[0]
                        .then(localFontStorage=>this._loadInitialFontsFromLocalFontStorage(localFontStorage));
                    break;
                case 'ready':
