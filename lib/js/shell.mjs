...
export class VideoproofController {
    ...
    // FIXME: This and everything that leads to it should be called from
    // UI-land wraped into withChangeState.
    async _availableFontsDraftToState() {
        const readyToUse = this._returnStateDependencyDraft('availableFonts');
        if(!readyToUse)
            return;
        // activate the last entry
        // FIXME: With a growing number of diverse and more complex layouts,
        // this seems to be too much for the shell to decide, It would
        // be better to put this into UI land.
        const draft = this.state.getDraft()
          , availableFonts = this._useStateDependencyDraft('availableFonts')
          , fontName = availableFonts.indexToKey(-1)[0]
          , fontUserPath = '/activeFontKey'
          ;
        await this._useFont(fontUserPath, fontName);
        getEntry(draft, fontUserPath).value = fontName;
    ...
    _updateState(draft=null) {
        if(this.draftState)
            // This is within an withChangeState transaction and
            // withChangeState will call _updateState eventually.
            // Maybe it's enough to just return at this point...
            // Though, of a draft was passed and that draft is not
