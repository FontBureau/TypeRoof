...
export class VideoproofController {
     ...
     * is at zero.
     */
    async _activateFont(fontName) {
        // Run only when all dependencies are loaded.
        if(!this.appReady) {
            console.warn(`_activateFont: App not yet available for activating ${fontName}.`);
            return false;
        }
     ...
     * (See _activateFont) on a "per user" basis, because a user can be
    ...
    async _useFont(userIdentifier, fontName) {
        ...
        const releaseFontFn = await this._activateFont(fontName);
    ...
    _initUI({charGroups}) {
        ...
        this._ui = new MainUIController({
          ...
          , loadFontsFromFiles: this._uiStateDependencyDraftContextWrapper(true
                    , this._loadFontsFromLoaderFn.bind(this, this._loadFontFromFile.bind(this))
          ...
          , loadFontsFromUrls: this._uiStateDependencyDraftContextWrapper(true
                    ...
                    , 'availableFonts')
          , useFont: this._uiStateDependencyDraftContextWrapper(true, this._useFont.bind(this), 'installedFonts')
          , unuseFont: this._uiStateDependencyDraftContextWrapper(false, this._unuseFont.bind(this), 'installedFonts')
