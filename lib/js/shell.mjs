...
// harfbuzzjs v1.x module namespace, WASM initialized at import time
// (top-level await).
...
async function parseFont(fontBuffer) {
    // decompressFontBuffer: woff/woff2 → SFNT (woff via the opentype.js
    // fork's woffToOTF, woff2 via wawoff2).
    const fontBuffer_ = decompressFontBuffer(fontBuffer)
      // createFontObject: harfbuzzjs-based opentype.js facade
      // (font-info.mjs). opentype.parse is gone: it crashed on valid
      // fonts ("STAT axis count must be greater than or equal to fvar
      // axis count") and upstream is unmaintained.
      , fontObject = createFontObject(harfbuzz, fontBuffer_)
      ;
...
class FontManager {
    ...
    async _deleteFontsFromStorage(fontNames) {
        ...
        for(const fontName of fontNames) {
            if (installedFontsDraft.has(fontName)) {
                results.push(['_localFontStorage.delete', fontName, `SKIP currently installed`]);
                continue;
            }
        ...
        console.log('_deleteFontsFromStorage results:', results);
        return results;
    }
...
export class ShellController {
    ...
    _requireReviewResourcesFlag = true; // initially true
    _ui = null;// TODO: improve these apis!_
    _changeStateQueue = [];
    _changeStateWorkerBusy = false;
    _harfbuzz = null;
    /**
     ...
     */
    constructor(contentWindow, APP/* { MainUIController, Layouts, LAYOUT_GROUPS } */) {
        ...
        // harfbuzzjs v1.x: initialized at import time, no async loader
        // required anymore. Keeping the 'harfbuzz' initial-dependency
        // flow unchanged.
        this.setInitialDependency('harfbuzz', harfbuzz);
    ...
    async _loadInitialStateFromLocationHash(Model) {
        const {fetch, location} = this._contentWindow
          , _rawHash = location.hash
          , hash = (_rawHash.startsWith('#') // it's supposed to start with #
                ? _rawHash.slice(1)
                : _rawHash).trim()
          ;
        if(!hash.length)
            return [null, new Set()];
        /*
         * flags: optional, but must be enclosed within []
         */
        const parsedHash = /^(\[(?<flags>.*)\])?((?<target>[a-z-]+)(:(?<payload>.*))?)?/.exec(hash);
        if(parsedHash === null)
            return [null, new Set()];
        const flags = new Set(
            (parsedHash.groups.flags || '')
                                .split(',')
                                .map(f=>f.trim())
                                .filter(f=>f.length)
            );
        if(parsedHash.groups.target === 'from-url') {
            const url = parsedHash.groups.payload?.trim();
            if(url === undefined || url.length === 0)
                return [null, flags];
            const response = await fetch(url, {method: 'GET'});
            if (!response.ok)
                throw new Error(`STATE LOAD ERROR failed to load from location hash `
                    + `by url, response status: ${response.status} for URL "${url}"`);
    ...
    async _allInitialResourcesLoaded(resources) {
        ...
        for(const [key, label, LayoutModule, layoutGroupKey_] of this._APP.Layouts) {
            const layoutGroupKey = Object.hasOwn(this._APP.LAYOUT_GROUPS, layoutGroupKey_)
                        ? layoutGroupKey_
                        : this._APP.LAYOUT_GROUPS.default
        ...
        const _initState = async(likeADraft) => {
        ...
        // TODO: we should listen on window.hashchange and update the
        // state accordingly. similarly flags should get updated, as
        // people will start using the address bar as an UI. Therefore,
        // outside of the state a flags set should be available and
        // usable as a dependency (e.g. like fonts) and everything
        ...
        try {
            ...
            this._initUI(uiFlags);
        }
        finally {
            this._lockChangeState = null;
        }

        if(uiFlags.has('autoplay') || uiFlags.has('autopause')){
            // no need to await as withChangeState is queued
            this.withChangeState(()=>{
                getEntry(this.draftState, 'activeState/playing', {value: undefined}).value = uiFlags.has('autoplay');
            })
        }

        this._domTool.window.shell = this._ui.widgetBus;
        ...
        if(adapterWindow !== null) {
            const handlePostMessage = this._useStateDependencyDraftContextWrapper(
                    true, this._handlePostMessage.bind(this), 'availableFonts', 'installedFonts')
              ;
    ...
    async _handlePostMessageFontUpdate(fontBuffer, metaData) {
        // FIXME: we should do sanitation on this property
    ...
    async _handlePostMessage(event) {
        ...
        if(type === 'font-update') {
            ...
            const {fontBuffer, metaData} = event.data;
            return await this._handlePostMessageFontUpdate(fontBuffer, metaData);
        }
        else {
    ...
    _initUI(uiFlags) {
        ...
        this._ui = new this._APP.MainUIController({ // widgetBus
            domTool: this._domTool
          , rootPath: Path.fromParts('/')
            // Used by the top-level UI-Controllers
          ...
          , harfbuzz: this.harfbuzz // harfbuzzjs v1.x module namespace
          ...
          , toString() {
            ...
            // FIXME: see remark in _allInitialResourcesLoaded
          , uiFlags
        });
     ...
     * UI.
     * It's allowed to call withChangeState at any time even if another
     * change is being processed. It uses a queue to update state only
     * once, after all calls have finished.
     */
    // returns a promise that resolves to the return value of fn,
    // hence shuld be treated as async where it matters.
    withChangeState(fn) {
        const {promise , resolve, reject} = Promise.withResolvers();
        this._changeStateQueue.push([fn, {resolve, reject}]);
        this._changeStateWorkerActivate();// is async, but we don't wait for it
        return promise;
    }

    async _changeStateWorkerActivate() {
        if(!this.appReady) {
        ...
        if(this._changeStateWorkerBusy)
            // is locked
            return;
        // lock it
        this._changeStateWorkerBusy = true;
        const draft = this.state.getDraft();
        if(this.draftState !== null)
            throw new Error(`ASSERTION FAILED this.draftState must be null at this point.`);
        this.draftState = draft;
        try {
            while(this._changeStateQueue.length) {
                const [fn, { resolve, reject }] = this._changeStateQueue.shift();
                try {
                    ...
                    const maybePromise = fn()
                        // only async if there's a "then". Don't bother
                        // to check if the "then" is a function, it's not
                        // exact anyways and await will resolve if it's
                        // not a promise as well. Thus, this is just an
                        // attempt of a minor optimization.
                      , result = maybePromise && maybePromise.then !== undefined
                            ? await maybePromise
                            : maybePromise
                        ;
                    resolve(result);
            ...
            // got to release the lock and resources
            this._changeStateWorkerBusy = false;
        }
        if(this._changeStateQueue.length)
            // this happens when during await this._updateState(draft);
            // another call got scheduled
            this._changeStateWorkerActivate();
    }
    ...
    async _updateState(draft=null) {
        if(this.draftState) {
            // This is during a withChangeState transaction and
    ...
    // Dispatch an update when the model did not change but a root
    // protocol handler (environment@) has pending updates.
    _protocolUpdateRequired() {
        const protocolHandlers = this._ui?.widgetBus.protocolHandlers;
        if(!protocolHandlers)
            return false;
        let protocolHandlerHasUpdates = false;
        for(const handler of protocolHandlers.values()) {
            if(handler.hasUpdated && handler.hasUpdated()) {
                protocolHandlerHasUpdates = true;
                break;
            }
        }
        if(!protocolHandlerHasUpdates)
            return false;
        return true;
    }
    ...
    _replaceState(newState) {
        ...
        let _protocolUpdateRequired = null;
        const checkProtocolUpdateRequired = ()=>{
            if(_protocolUpdateRequired === null)
                _protocolUpdateRequired = this._protocolUpdateRequired();
            return _protocolUpdateRequired;
        }

        if(newState === this.state) {
            // No model change at all (e.g. a no-op changeState). Protocol
            // handlers registered on the root widgetBus (environment@)
            // may still carry updates that must be dispatched.
            if(checkProtocolUpdateRequired() === false)
                return;
        }
        ...
        if(statuses.size === 1 && statuses.has(COMPARE_STATUSES.EQUALS)) {
            // Different reference, equal content (rare for a no-op):
            // same protocol-only dispatch as the identical-state case.
            if(checkProtocolUpdateRequired() === false)
                return;
        }
        ...
        for(let type of byType.keys()) {
            ...
            switch(type) {
                ...
                case 'ready':
                    ...
                    break;
                default:
                    console.warn(`ATTEMPT TO LOAD  UNKNOWN TYPE: ${type}`);
            }
            types.push(type);
...
// Deprecated
export const VideoProofController = ShellController;
