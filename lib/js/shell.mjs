...
class FontManager {
    ...
    async _installFont(fontName) {
        ...
        console.log(`${this}._installFont installedFontsDraft.keys:`, ...installedFontsDraft.keys());
...
export class VideoproofController {
    ...
    async _allInitialResourcesLoaded(resources) {
        ...
        // THIS
        // FIXME
        // THIS what happens if we don't use it? Does there have to be
        // a first font in installedFonts?
        ...
        //      CONSTRAINT ERROR [ForeignKey:installedFonts NOT NULL]
        //      Can't set first key, there is no first entry. (in [model ApplicationModel:draft])
        // FIXME: It would be good to handle this differently, e.g. use
        // the first key of availableFonts explicitly!
        await this._fontManager._installFont(fontName);
    ...
    _initUI(/*{ ... remote resources }*/) {
        ...
        this._ui = new MainUIController({ // widgetBus
          ...
          , loadFontsFromUrls: this._uiStateDependencyDraftContextWrapper(true
                    ...
                    , 'availableFonts')
            // FIXME: potentially there's a use case for this:
    ...
    async _asyncResolve(sharedState, resourceRequirement) {
        ...
        if(indicator instanceof ForeignKey) {
            ...
            if(targetContainer.constructor === InstalledFontsModel) {
                ...
                const installedFonts = this._useStateDependency('installedFonts')
                  , availableFonts = this._useStateDependency('availableFonts')
                  ;
                ...
                if(availableFonts.has(requiredKey)) {
                    ...
                    // This requires a context where installedFontsDraft is already prepared.
                    await this._fontManager._installFont(requiredKey);
                    console.log(`${this}._asyncResolve installedFonts.keys:`, ...installedFonts.keys());
                    targetContainer.set(requiredKey, installedFonts.get(requiredKey));
                    return requiredKey;
                }
    ...
    async _asyncMetamorphoseState(draft=null) {
        ...
        const primedDependencies = {}
          , requiredStateDependecyDrafts = ['installedFonts', 'availableFonts']
          ;


        for(const key of requiredStateDependecyDrafts)
            this._useStateDependencyDraft(key);
        ...
        for(const key of ['availableLayouts'])
            ...
            primedDependencies[key] = this._useStateDependency(key)
        ...
        const appInstalledFontNames = [...this._useStateDependency('installedFonts').keys()]
        ...
        for(const fontName of appInstalledFontNames) {
            ...
            // no longer used
            // this requires a context where  installedFontsDraft is already prepared.
            console.log(`${this}._asyncMetamorphoseState no longer used font -- uninstalling: ${fontName}`);
            this._fontManager._uninstallFont(fontName);
        }

        // TODO: wrap this into a finally { }
        for(const key of requiredStateDependecyDrafts)
            this._unuseStateDependencyDraft(key);
