...
async function parseFont(fontBuffer) {
    let fontBuffer_
    , signature = opentype._parse.getTag(new DataView(fontBuffer, 0), 0)
    ;
    if(signature === 'wOFF')
        ...
        fontBuffer_ = opentype.woff_to_otf(fontBuffer);
    else if(signature === 'wOF2') {
        const ttfFontBufferView = await woff2decompress(fontBuffer);
...
class FontManager {
    ...
    async _hydrateFont(deferredFont) {
        const contentDocument = this._contentWindow.document
          ...
          , font = new VideoProofFont(fontObject, origin, fontBuffer_, fontFace, contentDocument, deferredFont.fullName)
          ;
