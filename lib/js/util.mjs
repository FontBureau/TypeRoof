...
/**
 * return a curried getter for a specific key of a plain object or map e.g.:
 * ```
 * const getName = pullFrom("name");
 * const name = getName({name: "Hans"}); // name === "Hans"
 * ```
 */
export function pullFrom(key) {
    function pullFrom(dict) {
        return typeof dict.get === 'function' ? dict.get(key) : dict[key];
    }
    return pullFrom;
}
