...
function _getFirstNameValue(entry) {
    for (const lang of Object.keys(entry)) return entry[lang];
}
...
function _collectNames(hbFace) {
    ...
    for (const { nameId, language } of hbFace.listNames()) {
        ...
        if (!(language in names[key])) names[key][language] = value;
    }
    return names;
}
...
function _parseFvarInstances(fvarBytes, axesArrayEnd) {
    const view = new DataView(
            fvarBytes.buffer,
            fvarBytes.byteOffset,
            fvarBytes.byteLength,
        ),
    ...
    for (let i = 0; i < instanceCount; i++) {
        ...
        if (offset + 4 + 4 * axisCount > fvarBytes.byteLength) break; // defensive: malformed table
...
function _getFvar(hbFace) {
    ...
    if (!Object.keys(axisInfos).length) return undefined; // not a variable font
    ...
    if (!fvarBytes) return undefined; // not a variable font
    const fvarView = new DataView(
            fvarBytes.buffer,
            fvarBytes.byteOffset,
            fvarBytes.byteLength,
        ),
        axesArrayEnd =
            fvarView.getUint16(4) +
            fvarView.getUint16(8) * fvarView.getUint16(10);
    for (const [subfamilyNameID, coordinates] of _parseFvarInstances(
        fvarBytes,
        axesArrayEnd,
    )) {
        ...
        instances.push({
            name: {
                en:
                    hbFace.getName(subfamilyNameID, "en") ||
...
function _getFeatureUIName(hbFace, tableTag, featureIndex) {
    ...
    if (!nameIds || nameIds.uiLabelNameId === undefined) return undefined;
    ...
    const featureTags = hbFace.getTableFeatureTags(tableTag),
        features = featureTags.map((tag, featureIndex) => {
            ...
            if (tag.startsWith("ss") || tag.startsWith("cv")) {
                const uiName = _getFeatureUIName(
                    hbFace,
                    tableTag,
                    featureIndex,
                );
                if (uiName) entry.feature.uiName = uiName;
            }
            return entry;
        }),
        scripts = [],
        toIndexes = (tags) => tags.map((tag) => featureTags.indexOf(tag));
    for (const [scriptIndex, scriptTag] of hbFace
        .getTableScriptTags(tableTag)
        .entries()) {
        const script = { defaultLangSys: null, langSysRecords: [] },
            defaultFeatureTags = hbFace.getLanguageFeatureTags(
                tableTag,
                scriptIndex,
                DEFAULT_LANGSYS_INDEX,
            );
        if (defaultFeatureTags.length)
            script.defaultLangSys = {
                featureIndexes: toIndexes(defaultFeatureTags),
            };
        for (const [langIndex, langTag] of hbFace
            .getScriptLanguageTags(tableTag, scriptIndex)
            .entries()) {
            script.langSysRecords.push({
                tag: langTag,
                langSys: {
                    featureIndexes: toIndexes(
                        hbFace.getLanguageFeatureTags(
                            tableTag,
                            scriptIndex,
                            langIndex,
                        ),
