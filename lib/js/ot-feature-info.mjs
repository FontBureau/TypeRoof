import { FreezableMap } from "./metamodel.ts";
...
export const featureData = Object.create(null)
    // Order is important for the UI!
  , GROUPS = Object.freeze(new FreezableMap([
        ['LIGA', {
          label: 'Ligation and Alternate Forms'
        }]
      , ['CASED', {
          label: 'Caps etc.'//'Bicameral Alphabets (cased)'
        }]
      , ['DIGITS', {
          label: 'Digits and Math'
        }]
      , ['POS', {
          label: 'Positioning'
        }]
      ...
      , ['CURSIVE', {
          label: 'Cursive Scripts or Fonts'
        }]
      , ['DIR', {
          label: 'Writing Direction'
        }]
      , ['NO_UI', {
            label: '(not accessible via the UI)'
        }]
    ].map(([name, data], index)=>{
      // add members array
      data.members = [];
...
featureData.dist = {
    ...
    //  This feature could be made active or inactive by default, at the user's preference.
    // Control of the feature should not generally be exposed to the user.
    // Wikipedia: Adjusts horizontal positioning between glyphs. (Always enabled, as opposed to 'kern'.)
  , uiBoolean: null
  , slSensitivity: ['Indic scripts']
};
featureData.dlig = {
...
featureData.rtbd = {
    ...
    //
    // https://simoncozens.github.io/feature-tags/?#rtla
...
for(const [tag, data] of Object.entries(featureData)) {
    let groupName;
    if(!('group' in data)) {
        if(data.uiBoolean === null)
            groupName = 'NO_UI';
        else
          throw new Error(`VALUE ERROR tag "${tag}" has a UI (${data.uiBoolean}) but doesn't define a group.`);
    }
    else {
        if(data.uiBoolean === null)
          // No need to error, but we don't put it into that group either
          // Maybe it belongs into that original group, but we turned off the UI
          groupName = 'NO_UI';
        else
          groupName = data.group;
    }
    if(!GROUPS.has(groupName))
      throw new Error(`KEY ERROR group name "${groupName}" in tag "${tag}" is unkown in GROUPS: ${Array.from(GROUPS.keys()).join(', ')}.`)
...
export const OTFeatureInfo = {
  ...
  , ui: _getSubset(item=>item.uiBoolean !== null)
  , groups: GROUPS
  , getFeature(tag) {
...
export function validateOpenTypeUIFeatureTag(tag) {
    if(!Object.hasOwn(OTFeatureInfo.all, tag))
        return [false, `Unkown OpenType feature tag: "${tag}"`];
    if(!Object.hasOwn(OTFeatureInfo.ui, tag))
        return [false, `No Effect: OpenType feature tag must be user controllable (default or discretionary). Tag: "${tag}".`];
     return [true, null];
}
