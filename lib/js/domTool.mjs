...
// This class tells DOMTool to render HTML contents passed as a string.
// It was created to allow passing labels containing HTML elements, such as
// `span`s for icons loaded from the Material Symbols font.
...
export default class DOMTool {
  ...
  static appendChildren(elem, contents, cloneChildNodes) {
      ...
      for(let child of _contents) {
          if(!child || typeof child.nodeType !== 'number')
              child = (child instanceof HTMLString)
                  ? DOMTool.createFragmentFromHTML(elem.ownerDocument, child)
                  : DOMTool.createTextNode(elem.ownerDocument, child)
                  ;
  ...
  static createElement(document, tagname, attr, contents, cloneChildNodes) {
      ...
      if(attr) for(let k in attr) {
          if(k === 'xmlns')
              continue;
          if(k.startsWith('on') && typeof attr[k] === 'function') {
            // event handler (e.g., onChange → change)
            const event = k.slice(2).toLowerCase();
            elem.addEventListener(event, attr[k]);
            continue;
          }
  ...
  _h (tagname, attr = {}, ...children) {
        return this.createElement(tagname, attr, children);
  }

  get h() {
      return this._h.bind(this);
  }
  ...
  static insert(element, position, child) {
      if (child instanceof HTMLString) {
          child = DOMTool.createFragmentFromHTML(element.ownerDocument, child);
      }
...
export const getComputedStyle = DOMTool.getComputedStyle;
