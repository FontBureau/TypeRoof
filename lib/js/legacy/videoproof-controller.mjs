/*eslint no-prototype-builtins: "off", no-self-assign: "off"*/
...
// (top-level await). Used by createFontObject.
...
class _UIBase{
    ...
    static SERIALIZATION_TYPE_FLAT = Symbol('SERIALIZATION_TYPE_FLAT');
...
class UINumberAndRangeInput extends _UIBase {
    ...
    [_UIBase.SERIALIZATION_TYPE_FLAT];
    ...
    _makeOptions(fontFeatures) {
        ...
        for(const tag of tags) {
            const feature = fontFeatures.get(tag)
                // NOTE: custom tags (uppercase) will be skipped by setting uiBoolean === null
              , {uiBoolean, friendlyName} = feature?.info || {uiBoolean: null, friendlyName: tag}
              ;
            if(uiBoolean === null)
...
class UIDialogManageFonts extends UIDialogBase {
    static template = UI_DIALOG_MANAGE_FONTS;
...
export class VideoproofController {
    ...
    async _parseFont(fontBuffer) {
        // decompressFontBuffer: woff/woff2 → SFNT (woff via the
        // opentype.js fork's woffToOTF, woff2 via wawoff2).
        // createFontObject: harfbuzzjs-based opentype.js facade
        // (../font-info.mjs, shared with the main app). opentype.parse
        // is gone: it crashed on valid fonts ("STAT axis count must be
        // greater than or equal to fvar axis count").
    ...
    _videoPoofLegacyStates = new Set(['grid', 'type-your-own', 'contextual']);
