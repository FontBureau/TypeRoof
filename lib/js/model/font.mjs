...
function _getFontFeaturesInfo(font) {
  const featuresInfo = OTFeatureInfo.all
    , fontFeatures = {},
    _getNewFeature = (otFeatureTag) => {
      ...
      fontFeatures[otFeatureTag] = data
      return data;
    },
    _setFeaturesFromFont = (
      ...
      for (const idx of featureIndexes) {
        ...
        // NOTE: There are uppercase tags, i.e. we have seen `BASE` and
        // `SCAL` (both GSUB) in the wild so far. These are custom and not
        // in featuresInfo i.e. `featureData.info` will be undefined.
        const featureData = Object.hasOwn(fontFeatures, otFeatureTag)
            ? fontFeatures[otFeatureTag]
            : _getNewFeature(otFeatureTag)
            ;
        ...
        featureData.bestName = featureData.uiName || featureData.info?.friendlyName || otFeatureTag;
      }
    };
  ...
  for (const tableTag of ["GSUB", "GPOS"]) {
    ...
    if (
      !(openTypeJSTableTag in font.tables) ||
      !font.tables[openTypeJSTableTag].scripts
    )
      continue;
...
export class VideoProofFont {
    ...
    get features() {
        if(!this._featuresCache) {
            const features = _getFontFeaturesInfo(this.fontObject);
            Object.defineProperty(this, '_featuresCache', {
                value: deepFreeze(features)
            });
        }
    ...
    destroy() {
        ...
        // harfbuzzjs v1.x: no .destroy(), objects are reclaimed
        // automatically via FinalizationRegistry.
        this._hbCache = null;
