...
import type { _BaseModel, _BaseContainerModel, FreezableMap } from './base-model.ts';
...
export function createAvailableTypes(
    AvailableTypesModel: typeof _BaseContainerModel & { Model: typeof _BaseContainerModel },
    types: Iterable<[string, string, typeof _BaseModel]>,
): [_BaseModel, Map<typeof _BaseModel, string>] {
    const availableTypesDraft = AvailableTypesModel.createPrimalDraft({}) as _BaseContainerModel,
        TYPE_TO_KEY = new Map<typeof _BaseModel, string>(),
        TypeModel = AvailableTypesModel.Model;
    for (const [key, label, Model] of types) {
        const availableType = TypeModel.createPrimalDraft({}) as _BaseContainerModel;
        (availableType.get("typeClass") as { value: unknown }).value = Model;
        (availableType.get("label") as { value: unknown }).value = label;
        (availableTypesDraft as unknown as { push(entry: [string, _BaseModel]): void }).push([key, availableType as unknown as _BaseModel]);
...
export function createDynamicType(
    ...
    const availableTypesKey =
        ...
        availableTypes = DynamicModel.staticDependencies.has(availableTypesKey)
            ? DynamicModel.staticDependencies.get(availableTypesKey)!.state as _BaseContainerModel
            : dependencies[availableTypesKey] as _BaseContainerModel,
        getTypeFor = (key: string) =>
            ((availableTypes.get(key) as _BaseContainerModel).value as _BaseContainerModel).get("typeClass").value as typeof _BaseContainerModel,
        getDraftFor = (name: string, deps: DependenciesMap) => getTypeFor(name).createPrimalDraft(deps) as _BaseContainerModel,
        draft = getDraftFor(typeKeyValue, dependencies),
        resultDraft = DynamicModel.createPrimalDraft(dependencies) as _BaseContainerModel;
    (resultDraft.get(typeKeyName) as { value: unknown }).value = typeKeyValue;
    (resultDraft.get("instance") as unknown as { wrapped: unknown }).wrapped = draft;
    return resultDraft;
}

export function getMinMaxRangeFromType(Type: typeof _BaseModel & { Model?: typeof _BaseModel; minVal?: number; maxVal?: number }): [number | undefined, number | undefined] {
    const UnwrappedType = (
...
export function* getFieldsByType(
    FromType: typeof _BaseModel & { fields: FreezableMap<string, typeof _BaseModel> },
