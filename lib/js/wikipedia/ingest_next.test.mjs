...
// Hard-coded engine test rule tables (do NOT import from ingest_next —
...
const PHRASING_RULES = [
    { selector: "br", rule: { kind: "void", typeKey: "hard_break" } },
    {
        selector: [
            ...
            "strong", "sub", "sup", "time", "u", "var", "wbr", "a",
            "ins", "del",
        ].join(", "),
...
describe("ingestDOM", () => {
    ...
    it("patches mw-empty-elt elements through as atoms, preserving metadata", () => {
        const { document, report } = ingestTest(
            ...
            { pre: [{ selector: ".mw-empty-elt, meta", rule: { kind: "raw" } }] },
        );
        const [section] = kids(document);
...
describe("figcontent end-to-end (wikipedia initial state)", () => {
    ...
    it("does not set htmlTag on atoms that do not declare it (cite-link)", () => {
        const { document } = ingestWikipediaDocument(
            parse(
                `<p><sup class="reference"><a href="#c">[1]</a></sup></p>`,
            ),
            loadStateSchema(),
        );
