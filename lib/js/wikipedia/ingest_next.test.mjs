...
describe("ul/li list ingestion", () => {
    ...
    it("a table child does not make an li-block (current behavior)", () => {
        // <table> is not in KNOWN_BLOCK_TAGS, so it joins the inline
        // run: li-inline with a raw_html_inline atom.
        // Phase 5 changes this: positive inline resolution will break
        // the run -> li-block with paragraph + raw_html_block.
        const { document, report } = ingestDOM(
        ...
        const [li] = kids(ul);
        expect(typeKey(li)).toBe("li-inline");
        expect(kids(li).map(typeKey)).toEqual(["text", "raw_html_inline"]);
        expect(report.catchAllInline).toEqual({ TABLE: 1 });
        expect(report.catchAllBlocks).toEqual({});
    });
...
describe("emissionRules option", () => {
    it("an explicit rule beats the default table", () => {
        // default: <p> -> paragraph; the explicit rule reroutes it
        const { document } = ingestDOM(parse(`<p>x</p>`), {
            emissionRules: [
                {
                    selector: "p",
    ...
    it("a context-misfit entry is skipped (falls through)", () => {
        ...
        // the paragraph falls to the default inline-node handling
        const { document, report } = ingestDOM(
            parse(`<p>a<span>b</span></p>`),
            {
                emissionRules: [
                    {
                        selector: "span",
    ...
    it("the context field narrows a both-contexts kind", () => {
        ...
        // element falls through to the block catch-all instead of
        // emitting an inline atom where PM would reject it
        const rules = [
            {
                selector: "sup",
        ...
        const inlineCase = ingestDOM(parse(`<p><sup>1</sup></p>`), {
            emissionRules: rules,
        });
        ...
        expect(kids(section).map(typeKey)).toEqual(["raw_html_block"]);
        expect(blockCase.report.catchAllBlocks).toEqual({ SUP: 1 });
    });

    it("skip emits nothing, no descent", () => {
        const { document, report } = ingestDOM(
            parse(`<p>a<span>gone</span>b</p>`),
            { emissionRules: [{ selector: "span", rule: { kind: "skip" } }] },
        );
        const [p] = kids(document);
    ...
    it("a markEmission-style rule fires for any matched element", () => {
        // the legacy engine gated mark rules on a fixed tag set; an
        // emissionRules generic entry claims e.g. <span class="x">
        const { document } = ingestDOM(
