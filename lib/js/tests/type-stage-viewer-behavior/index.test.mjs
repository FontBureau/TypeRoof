...
describe.skipIf(!HAVE_FIXTURE)("type-stage viewer behavior", () => {
    ...
    it("editor-only mode: no viewer article, but per-node nodeProperties@ registrations are live", () => {
        ...
        for (const id of documentNodeIds) {
            ...
            // layout/width is computed per node from its own style
            // facts (width-semantics handoff): a node with an explicit
            // lineLength shrinks to its measure instead of inheriting
            // the pane width.
            expect(effective.get("layout/width")).toBeDefined();
        }
    ...
    it("G8: a child's node-properties scope observes its parent's settled map", () => {
        ...
        const parentPayload = nodePropertiesHandler.getRegistered(
            ...
            // The child's parent layer is the parent's OUTBOUND
            // projection (inheritance controls applied — plain Map),
            // not the parent's own effective cascade. The observable
            // contract: the child reads the parent's settled values
            // through its cascade's parent layer. NOTE: the projection
            // is a deliberate SUBSET of the effective map — inheritance
            // controls (tombstones, e.g. the horizontal-layout
            // consumption policy) withhold keys by design — so the
            // comparison runs against getInheritableProperties(), not
            // getProperties().
            parentProjection =
                parentPayload.nodeProperties.inheritableSettledProperties,
            childScope = childPayload.nodeProperties,
            childParentLayer = childScope.getProperties().getLayer("parent");
        expect(childParentLayer).toBeDefined();
        // Content equality with the parent's projection (identity is
        // an implementation detail of the meta layer's scope building;
        // the contract is the child's view of the parent's settled
        // values).
        expect([...childParentLayer.keys()]).toEqual(
            expect.arrayContaining([...parentProjection.keys()]),
        );
        for (const [key, value] of parentProjection)
            expect(childParentLayer.get(key)).toEqual(value);

        // After a no-op state pass (the update cycle settles), the
        // observation still holds — the child's cascade keeps tracking
        // the parent's current settled map.
        applyChange(() => {});
        const childLayerAfter = nodePropertiesHandler
                .getRegistered(`nodeProperties@${childPath}`)
                .nodeProperties.getProperties()
                .getLayer("parent"),
            parentProjectionAfter = nodePropertiesHandler.getRegistered(
                `nodeProperties@${parentPath}`,
            ).nodeProperties.inheritableSettledProperties;
        expect(childLayerAfter).toBeDefined();
        for (const [key, value] of parentProjectionAfter)
            expect(childLayerAfter.get(key)).toEqual(value);
    });

    it("G9: node scopes compute per-node geometry from the typeSpec style facts", () => {
        // Node-properties geometry proof: a node's scope computes its
        // own layout/width from its resolved typeSpecnion's style facts
        // (fontSize — the en factor) and the inherited availableWidth,
        // via the horizontalLayoutNodePropertiesGen synthetics. With no
        // explicit lineLength (the fill case), a node's width equals
        // its inherited available width — asserted against the
        // environment-published layout size, the observable end of the
        // channel, no fixture positions hard-coded.
        const controller = world.root.getWidgetById("typeStageController"),
            nodePropertiesHandler =
                controller.widgetBus.getProtocolHandlerImplementation(
                    "nodeProperties@",
            ...
            documentNodeId = [...nodePropertiesHandler]
                .map(([id]) => id)
                .find((id) =>
                    id.startsWith(
            ...
            nodeScope = nodePropertiesHandler
                .getRegistered(documentNodeId)
                .nodeProperties.getProperties(),
            hostProperties = nodePropertiesHandler
                .getRegistered(documentNodeId)
                .nodeProperties._typeSpecnion.getProperties(),
            fontSizePT = hostProperties.get("generic/fontSize");
        expect(fontSizePT).toBeDefined();
        // Fill semantics: width equals the inherited available width
        // (the environment layout width in pt; the harness publishes
        // 0 by default — 0 is a resolved width, not a dropped one).
        const availableWidth = nodeScope.get("layout/availableWidth");
        expect(availableWidth).toBeDefined();
        expect(nodeScope.get("layout/width")).toBe(availableWidth);
        // Per-node computation proof: change the environment, the
        // node's width follows (the channel re-resolves).
        world.environmentHandler._unregister("layout");
        world.environmentHandler.register("layout", {
            width: 400,
            height: 300,
        });
        world.environmentHandler.setUpdated("layout");
        applyChange(() => {});
        const nodeScopeAfter = nodePropertiesHandler
            .getRegistered(documentNodeId)
            .nodeProperties.getProperties();
        // 400 css-px = 300 pt
        expect(nodeScopeAfter.get("layout/availableWidth")).toBe(300);
        expect(nodeScopeAfter.get("layout/width")).toBe(300);
    });

    it("compare mode: editor and viewer apply identical per-node styling (shared nodeProperties@ channel)", () => {
        // Phase 3 (editor nodeProperties@ wiring): before it, the editor
        // styled from the typeSpecnion alone while the viewer consumed the
        // per-node nodeProperties@ channel — the same node rendered with
        // different property universes. Now both renderers map
        // nodeProperties@<documentNodePath> and consume the identical
        // per-node map, so their geometry-ish output for the same node
        // matches. Asserted on the observable surfaces: the channel maps
        // (public protocol handler) and the rendered CSS width both
        // renderers derive from it — whatever that width currently is
        // (the width-semantics takeover owns its value; this test pins
        ...
        // Collect the per-node maps the channel publishes, keyed by
        // document-node path (viewer blocks carry data-node-type; both
        // renderers address the same ids).
        const channelMaps = new Map(); // documentNodePath -> properties map
        for (const [id, payload] of nodePropertiesHandler) {
            if (!id.startsWith("nodeProperties@/activeState/document/content/"))
                continue;
            const path = id.slice("nodeProperties@".length),
                map = payload?.nodeProperties?.getProperties?.();
            if (map) channelMaps.set(path, map);
        }
        expect(channelMaps.size).toBeGreaterThan(0);

        // Both renderers mark rendered block elements with
        // data-node-type — EXCEPT reproducing atoms, which the editor
        // renders as the bare reproduced element (e.g. <table>) with no
        // data-node-type wrapper while the viewer wraps them. So the
        // symmetric, aligned surface is the set of NON-atom styled nodes;
        // reproducing atoms are excluded from the sequence comparison
        // (both renderers still render and style them, just with
        // different DOM marking).
        const REPRODUCING_ATOM_TYPES = new Set(["raw_html_block"]);
        applyChange((activeState) => {
            activeState.get("documentRendererMode").value = "viewer";
        });
        const viewerBlocks = [
            ...
            .filter((b) => !REPRODUCING_ATOM_TYPES.has(b.type));

        applyChange((activeState) => {
            activeState.get("documentRendererMode").value = "editor";
        });
        // The editor's width goes to the styled CONTENT element (inner,
        // data-node-content; for reproducing atoms that IS the block
        // element) — the analogue of the viewer's single element. Read
        // the width off that styled element, not the block wrapper.
        const editorBlocks = [
            ...queryHost(world).querySelectorAll("[data-node-type]"),
        ]
            .map((el) => {
                const styled =
                    el.querySelector(":scope > [data-node-content]") ?? el;
                return {
                    type: el.getAttribute("data-node-type"),
                    width: styled.style.width || null,
                };
            })
            .filter((b) => !REPRODUCING_ATOM_TYPES.has(b.type));

        // Parity: the same (non-atom) styled nodes render on both sides
        // (type sequence identical), and the width each renderer derived
        // from the shared per-node channel agrees node-for-node.
        expect(editorBlocks.map((b) => b.type)).toEqual(
            viewerBlocks.map((b) => b.type),
        );
        for (let i = 0; i < viewerBlocks.length; i++)
            expect(editorBlocks[i].width).toBe(viewerBlocks[i].width);

        // The channel actually feeds the styling: at least one node's
        // map carries the geometry the renderers read (availableWidth),
        // proving the shared per-node source rather than an empty fallback.
        const feedsGeometry = [...channelMaps.values()].some((map) =>
            map.has("layout/availableWidth"),
        );
        expect(feedsGeometry).toBe(true);
    });
});
