...
describe.skipIf(!HAVE_FIXTURE)("type-stage viewer behavior", () => {
    ...
    it("G8: a child's node-properties scope observes its parent's settled map", () => {
        ...
        const parentPayload = nodePropertiesHandler.getRegistered(
            ...
            // through its cascade's parent layer.
        ...
        expect([...childParentLayer.keys()]).toEqual(
            expect.arrayContaining([...parentEffective.keys()]),
        );
        for (const [key, value] of parentEffective)
        ...
        const childLayerAfter = nodePropertiesHandler
                ...
                .nodeProperties.getProperties()
                .getLayer("parent"),
            parentEffectiveAfter = nodePropertiesHandler
                .getRegistered(`nodeProperties@${parentPath}`)
                .nodeProperties.getProperties();
        expect(childLayerAfter).toBeDefined();
        for (const [key, value] of parentEffectiveAfter)
