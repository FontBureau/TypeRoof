...
describe.skipIf(!HAVE_FIXTURE)("type-stage viewer behavior", () => {
    ...
    it("editor-only mode: no viewer article, but per-node nodeProperties@ registrations are live", () => {
        ...
        // Delegation baseline (Phase 5b): every per-node payload
        // currently delegates to the root scope — its resolved
        // layout/availableWidth equals the root registration's. Phase 2
        // replaces the delegation with real per-node scopes and must
        // keep this observation valid at the root level. The root
        // registration is the one id NOT keyed by document-node path.
        const documentNodeIds = expectedIds,
            rootId = [...registeredIds].find(
                (id) => !documentNodeIds.includes(id),
            );
        expect(rootId).toBeDefined();
        const rootAvailableWidth = nodePropertiesHandler
            .getRegistered(rootId)
            .nodeProperties.getProperties()
            .get("layout/availableWidth");
    ...
    it("G8: a child's node-properties scope observes its parent's settled map", () => {
        ...
        // Depth-first search for the first document node whose parent
        // is itself a content child (i.e. depth ≥ 2 in meta terms).
        let parentPath = null,
            childPath = null;
        (function findNested(node, path, parentContentPath) {
            ...
            for (const [key, child] of node.get("content")) {
                ...
                if (parentContentPath !== null && childPath === null) {
