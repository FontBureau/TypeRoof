var culori = (() => {
	...
	var fn2 = (c2 = 0) => {
		...
		if (abs3 > 31308e-7) {
			return (
				(Math.sign(c2) || 1) * (1.055 * Math.pow(abs3, 1 / 2.4) - 0.055)
			);
		}
		return c2 * 12.92;
	};
	...
	var definition16 = {
		...
		toMode: {
			...
			rgb: c2 =>
				convertLab65ToRgb_default(convertLchToLab_default(c2, 'lab65'))
		},
		fromMode: {
			rgb: c2 =>
				convertLabToLch_default(convertRgbToLab65_default(c2), 'lch65'),
	...
	var definition21 = {
		...
		toMode: {
			...
			rgb: c2 =>
				convertOklabToRgb_default(convertLchToLab_default(c2, 'oklab'))
		},
		fromMode: {
			rgb: c2 =>
				convertLabToLch_default(convertRgbToOklab_default(c2), 'oklch'),
	...
	var matrix = (values, mode, preserve_mode = false) => {
		...
		return color => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				res[ch] =
					(res[ch] || 0) + values[i] * (c2[channels[i % count]] || 0);
			}
			if (!preserve_mode) {
				return res;
			}
