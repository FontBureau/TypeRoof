	...
	var culori = (() => {
		...
		function parseColorSyntax(tokens) {
			...
			for (let ii = 0, c2, ch; ii < channels.length; ii++) {
				...
				if (c2.type !== Tok.None) {
					res[ch] =
						c2.type === Tok.Number ? c2.value : c2.value / 100;
					if (ch === 'alpha') {
		...
		var linearize = (v = 0) =>
			Math.pow(Math.abs(v), 563 / 256) * Math.sign(v);
			...
			return (std, smp) => {
				...
				let T =
					...
					hStd <= (345 / 180) * Math.PI
						? 0.56 +
							Math.abs(
								0.2 * Math.cos(hStd + (168 / 180) * Math.PI)
							)
						: 0.36 +
							Math.abs(
								0.4 * Math.cos(hStd + (35 / 180) * Math.PI)
							);
				let Sl =
					lStd < 16
						? 0.511
		...
		var convertDlabToLab65 = c2 =>
			convertDlchToLab65_default(convertLabToLch_default(c2, 'dlch'));
		var convertLab65ToDlab = c2 =>
			convertLchToLab_default(convertLab65ToDlch_default(c2), 'dlab');
		var definition4 = {
			mode: 'dlab',
			parse: ['--din99o-lab'],
		...
		var definition5 = {
			...
			toMode: {
				...
				rgb: c2 =>
					convertLab65ToRgb_default(convertDlchToLab65_default(c2))
			},
			fromMode: {
				lab65: convertLab65ToDlch_default,
				dlab: c2 => convertLabToLch_default(c2, 'dlch'),
				rgb: c2 =>
					convertLab65ToDlch_default(convertRgbToLab65_default(c2))
			},
		...
		function transferPqDecode(v) {
			...
			return (
				1e4 * Math.pow(Math.max(0, c2 - C1) / (C2 - C3 * c2), 1 / M1)
			);
		}
		...
		var convertXyz65ToItp = ({ x, y, z, alpha }) => {
			...
			const i = 0.5 * l + 0.5 * m;
			const t =
				1.61376953125 * l - 3.323486328125 * m + 1.709716796875 * s;
			const p4 =
				4.378173828125 * l - 4.24560546875 * m - 0.132568359375 * s;
		...
		var definition10 = {
			...
			toMode: {
				...
				rgb: color =>
					convertXyz65ToRgb_default(convertItpToXyz65_default(color))
			},
			fromMode: {
				xyz65: convertXyz65ToItp_default,
				rgb: color =>
					convertXyz65ToItp_default(convertRgbToXyz65_default(color))
			},
			ranges: {
				i: [0, 0.581],
				t: [-0.369, 0.272],
			...
			toMode: {
				...
				rgb: c2 =>
					convertLab65ToRgb_default(
						convertLchToLab_default(c2, 'lab65')
					)
			},
			fromMode: {
				rgb: c2 =>
					convertLabToLch_default(
						convertRgbToLab65_default(c2),
						'lch65'
					),
		...
		var convertOklabToLrgb = ({ l, a, b, alpha }) => {
			...
			let res = {
				...
				r:
					4.076741661347994 * L -
					3.307711590408193 * M3 +
					0.230969928729428 * S,
				g:
					-1.2684380040921763 * L +
					2.6097574006633715 * M3 -
					0.3413193963102197 * S,
				b:
					-0.004196086541837188 * L -
					0.7034186144594493 * M3 +
		...
		var convertOklabToRgb = c2 =>
			convertLrgbToRgb_default(convertOklabToLrgb_default(c2));
		...
		function parseOklab(color, parsed) {
			...
			if (l.type !== Tok.None) {
				res.l = Math.min(
					Math.max(
						0,
						l.type === Tok.Number ? l.value : l.value / 100
					),
					1
				);
			}
			...
			toMode: {
				...
				rgb: c2 =>
					convertOklabToRgb_default(
						convertLchToLab_default(c2, 'oklab')
					)
			},
			fromMode: {
				rgb: c2 =>
					convertLabToLch_default(
						convertRgbToOklab_default(c2),
						'oklch'
					),
		...
		var interpolatorSplineMonotoneClosed = arr => {
			...
			yp[n] =
				(sgn(s[n - 1]) + sgn(s_n)) *
				min(abs2(s[n - 1]), abs2(s_n), 0.5 * abs2(p4[n]));
			return interpolator(arr, yp, s);
		};
		...
		var inrange_rgb = c2 => {
			return (
				c2 !== void 0 &&
				(c2.r === void 0 || (c2.r >= 0 && c2.r <= 1)) &&
				(c2.g === void 0 || (c2.g >= 0 && c2.g <= 1)) &&
				(c2.b === void 0 || (c2.b >= 0 && c2.b <= 1))
			);
		};
		...
		var matrix = (values, mode, preserve_mode = false) => {
			...
			return color => {
				...
				for (let i = 0; i < values.length; i++) {
					...
					res[ch] =
						(res[ch] || 0) +
						values[i] * (c2[channels[i % count]] || 0);
				}
				if (!preserve_mode) {
					return res;
				}
