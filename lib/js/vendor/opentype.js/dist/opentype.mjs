...
function parseGsubTable(data, start) {
  ...
  for (const fi in table.features) {
    ...
    if (f.tag.match(/ss(?:0[1-9]|1\d|20)/)) {
      const p2 = new Parser(data, f.feature.tableOffset);
      f.feature.featureParamsTable = p2.parseStylisticSetFeatureParams();
    } else if (f.tag.match(/cv(?:0[1-9]|[1-9]\d)/)) {
      const p2 = new Parser(data, f.feature.tableOffset);
      f.feature.featureParamsTable = p2.parseCharacterVariantFeatureParams();
    }
