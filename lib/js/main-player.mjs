...
import {
    ClassesAndStylesManager
} from './components/classes-and-styles-manager.mjs';

import {
    EnvironmentProvider
} from './components/environment-provider.mjs';

import {
    FullscreenWakeLock
} from './components/screen-wake-lock.mjs';

import { createAndGetDefaultZones } from './zones.typeroof.jsx';

import {PlayerLayout as MotionStageLayout} from './components/layouts/motion-stage.mjs';
import * as VideoproofArrayLayout from './components/dev-layouts/videoproof-array.mjs';
import * as VideoproofLayout from './components/layouts/videoproof.typeroof.jsx';
import TypeStage from './components/layouts/type-stage/index.typeroof.jsx';
...
export const Layouts = Object.freeze([
  ...
  , ['Videoproof', 'Video Proof', VideoproofLayout, LAYOUT_GROUP_DEFAULT]
...
class ToggleFullscreen extends _BaseComponent {
    ...
    constructor(widgetBus, fullScreenElement) {
        ...
        // Prevent the screen from dimming or locking while the player
        // is in full screen mode.
        this._wakeLock = new FullscreenWakeLock(this._domTool.document);
    }
...
export class MainUIController extends _BaseContainerComponent {
    ...
    constructor(widgetBus) {
        const zones = createAndGetDefaultZones(
           ...
           , layoutAndFontContainer = widgetBus.domTool.createElement('div', {'class': 'layout-and-font'})
          ;
        zones.set('layout-and-font' , layoutAndFontContainer);
        // Shell-level marker: CSS can distinguish the player shell
        // (e.g. .wrapper.player .typeroof-layout--motion-stage) from
        // the editor shell for layout styling.
        zones.get('wrapper').classList.add('player');
        // [zoneName, dependecyMappings, Constructor, ...args] = widgets[0]
        const rootPath = Path.fromParts('.', 'activeState');

        // The root controller has no ComponentWrapper, so
        // widgetBus.wrapper.setProtocolHandlerImplementation is not
        // available here. Instead, environment@ is installed on the
        // root widgetBus directly; children inherit it via
        // Object.create(widgetBus).
        const [environmentProtocolName, environmentHandler] =
                SimpleProtocolHandler.create('environment@', {treatAdressAsRootPath: false});
        widgetBus.protocolHandlers.set(environmentProtocolName, environmentHandler);
          ...
          , [
                {
                  ...
                  , activationTest:(getEntry)=>{
                       const activeState = getEntry('activeState');
              ...
              , [
                    'playing'
                    // '.' (the rootPath itself) is read in the activationTest.
                  , ['.', 'activeState']
                ]
              , MonitorPlayingState
              , function(value) {
                    ...
                    return [
                        {
                            rootPath: rootPath
                          , zone: 'main'
                            // getEntry is injected by ComponentWrapper and
                            // only serves declared dependencies.
                          , activationTest:(getEntry)=>{
                                const activeState = getEntry('activeState');
        ...
        // Kept for update/initialUpdate (reset of the protocol update
        // log); the root controller has no wrapper, so the handler is
        // reached via this direct reference.
        this._environmentProtocolHandler = environmentHandler;
    }
    get dependencies() {
    ...
    update(...args) {
        // The updated-log is the relevance signal for environment-only
        // updates (no model change): it must survive until consumers
        // have been asked; reset after the update pass.
        const result = super.update(...args);
        this._environmentProtocolHandler.resetUpdatedLog();
        return result;
    }
    initialUpdate(...args) {
        const result = super.initialUpdate(...args);
        this._environmentProtocolHandler.resetUpdatedLog();
        return result;
    }
}
