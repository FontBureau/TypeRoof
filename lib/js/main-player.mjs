...
import * as VideoproofArrayLayout from './components/layouts/videoproof-array.mjs';
...
class UIVisibilityManager extends _BaseComponent {
    ...
    _addVisibilityAndTimeout(event, disappearTime=1500) {
        this._addVisibility();
        this._cancelTimeout();
    ...
    setVisibilityAutomatic(automatic, visible=true) {
        if(!automatic) {
            ...
            if(visible)
                ...
                this._addVisibilityAndTimeout(null, 0);
            else
                this._removeVisibilty();
        }
    }
